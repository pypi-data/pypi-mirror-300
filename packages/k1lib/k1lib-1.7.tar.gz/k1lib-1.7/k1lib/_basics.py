...
import logging, warnings, os, time, re, gc, json, k1lib, importlib, urllib.parse, math, base64, dill, inspect, threading, datetime, traceback, functools, collections, numpy as np
from typing import Any, List, Union, Tuple, Iterator, Dict
try: import torch; hasTorch = True
except: hasTorch = False
__all__ = ["_docsUrl", "dep", "depCli", "isNumeric",
           "patch", "cache", "cron", "wrapMod", "wraps", "squeeze", "raiseEx",
...
"""                                                                              # patch
    ...
    return inner                                                                 # patch
class cache:                                                                     # cache
    def __init__(self, maxsize=128, timeout=60):                                 # cache
...
:param timeout: timeout in seconds"""                                            # cache
        self.maxsize = maxsize; self.timeout = timeout; self.cache = collections.OrderedDict() # cache
    def __call__(self, func):                                                    # cache
        @functools.wraps(func)                                                   # cache
        def wrapper(*args):                                                      # cache
            if self.maxsize == 0: return func(*args)                             # cache
            current_time = time.time()                                           # cache
            key = tuple((tuple(x) if isinstance(x, list) else x) for x in args) # can accept lists of hashable items! # cache
            if key in self.cache:                                                # cache
                result, timestamp = self.cache[key]                              # cache
                if current_time - timestamp < self.timeout: self.cache.move_to_end(key); return result # cache
                else: del self.cache[key]                                        # cache
            result = func(*args)                                                 # cache
            if len(self.cache) >= self.maxsize: self.cache.popitem(last=False)   # cache
            self.cache[key] = (result, current_time); return result              # cache
        return wrapper                                                           # cache
def cron(f=None, delay=None):                                                    # cron
...
"""                                                                              # cron
    def inner(func):                                                             # cron
        if delay:                                                                # cron
            ...
            threading.Thread(target=startLoop).start(); return func              # cron
        else:                                                                    # cron
