...
__all__ = ["Object", "Range", "Domain", "AutoIncrement", "Wrapper", "Every",
           ...
           "Settings", "settings", "_settings", "UValue", "ConstantPad", "AutoUpdateValue", "Aggregate"]
class Object:                                                                    # Object
...
"""                                                                              # Aggregate
        ...
        def inner():                                                             # Aggregate
            while True:                                                          # Aggregate
                with self.lock: arr = self.arr; self.arr = []; res = []          # Aggregate
                if len(arr) > 0:                                                 # Aggregate
                    try: res = processF(arr)                                     # Aggregate
                    except Exception as e:                                       # Aggregate
                        if verbose: print(f"{type(e)}\ue004{e}"); print(traceback.format_exc()) # Aggregate
                    if res:                                                      # Aggregate
                        with self.lock: self.arr.extend(res)                     # Aggregate
                time.sleep(refreshPeriod)                                        # Aggregate
        threading.Thread(target=inner, daemon=True).start()                      # Aggregate
