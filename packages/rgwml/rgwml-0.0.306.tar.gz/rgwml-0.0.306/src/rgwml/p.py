    ...
    def oaiba(self, job_name, model, column_to_analyse, prompt, new_column_name, is_image_url_analysis=False):
        ...
        for idx, row in self.df.iterrows():
            ...
            if is_image_url_analysis:
                if isinstance(value_to_analyze, str) and "drive.google.com" in value_to_analyze:
                    image_url = convert_google_drive_url(value_to_analyze)
                elif isinstance(value_to_analyze, str):
                    image_url = value_to_analyze
                else:
                    print(f"Invalid data for image URL at index {idx}: {value_to_analyze}")
                    continue  # Skip this row if the value is not a valid string URL
