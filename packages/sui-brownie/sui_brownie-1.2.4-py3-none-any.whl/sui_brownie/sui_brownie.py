...
FLAG_TO_SIGNATURE_SCHEME = {
    0x00: "ED25519",
    0x01: "Secp256k1",
}


def decodeSuiPrivateKey(encoded_key: str):
    hrp, data = bech32.bech32_decode(encoded_key)

    if hrp != SUI_PRIVATE_KEY_PREFIX:
        raise ValueError("Invalid Sui private key prefix")

    decoded_bytes = bytes(bech32.convertbits(data, 5, 8, False))

    if len(decoded_bytes) != PRIVATE_KEY_SIZE + 1:
        raise ValueError("Invalid decoded bytes length")

    flag = decoded_bytes[0]
    private_key_bytes = decoded_bytes[1:]

    signature_scheme = FLAG_TO_SIGNATURE_SCHEME.get(flag)
    if signature_scheme is None:
        raise ValueError(f"Unknown signature scheme flag: {flag}")
...
class SuiProject:
    ...
    def load_config(self):
        ...
        for account_name, env_name in self.config["sui_wallets"]["from_mnemonic"].items():
            ...
            elif SUI_PRIVATE_KEY_PREFIX in env[env_name]:
                self.accounts[account_name] = Account(private_key=decodeSuiPrivateKey(env[env_name]))
            else:
