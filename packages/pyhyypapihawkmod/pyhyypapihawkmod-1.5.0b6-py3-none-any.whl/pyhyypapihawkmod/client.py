...
class HyypClient:
    ...
    def initialize_fcm_notification_listener(self, callback, restart = False):
    ...
    def fcm_notification_thread(self, callback, restart):
        if not restart:
            if not self.tools.internet_connectivity():
                return
        if self.fcm_credentials is None:
            self.fcm_credentials = self.get_initial_fcm_credentials()
        if self.fcm_credentials is None:
    ...
    def get_initial_fcm_credentials(self):
        # if not self.tools.internet_connectivity():
            # return False
        res = self.fcm_register.register()
        _LOGGER.warning("From Get init ^^^")
        _LOGGER.warning(res)
        return res
    ...
    def get_sync_info(self, json_key: str | None = None) -> Any:
        ...
        _params = STD_PARAMS.copy()
        _params["_appPkg"] = _params["pkg"]
        if self.fcm_credentials is not None and "fcm" in self.fcm_credentials:
            _params["gcmId"] = self.fcm_credentials["fcm"]["token"]
            _LOGGER.warning("^^^ In sync info")
            _LOGGER.warning(_params["gcmId"])
        try:
            req = self._session.get(
