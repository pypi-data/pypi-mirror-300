...
from .constants import GCF_SENDER_ID, GCM_APP_ID
from .mcs_pb2 import (
    Close,
    DataMessageStanza,
    HeartbeatAck,
...
# start of new FCM functions
    ...
    def fcm_subscribe(self, credentials):
        ...
        refresh_token = credentials["fcm"]["refreshToken"]
...
class FCMListener:
    ...
    def debug_threaded_runner(self, callback, credentials = None):
        thread.Thread(target=self.runner, args=(callback,
                                                    credentials)).start()
        
    def runner(self, callback, credentials = None):
        ...
        def on_notification(obj, notification, data_message):
            ...
            if idstr in self.received_persistent_ids:
                _LOGGER.warning("Found pers ID, skipping")
                return
            self.received_persistent_ids.append(idstr)
            _LOGGER.warning("ID STR: " + idstr) #test
            _LOGGER.warning("^^ ID STR") #test
            _LOGGER.warning("ID STR array: " + self.received_persistent_ids) #test
            _LOGGER.warning("^^ ID STR array") #test
