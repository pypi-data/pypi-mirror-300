...
process_and_project_data <- function(output_prefix, ref_file) {
  ...
  required_packages <- c("Seurat", "SeuratDisk", "ProjecTILs", "Matrix", "sctransform")
  ...
  seurat_data <- SCTransform(seurat_data, do.correct.umi = TRUE, verbose = FALSE)
  
  sct_counts <- GetAssayData(seurat_data, slot = "counts", assay = "SCT")
  sct_data <- GetAssayData(seurat_data, slot = "data", assay = "SCT")
  
  seurat_data <- SetAssayData(seurat_data, slot = "counts", new.data = sct_counts, assay = "RNA")
  seurat_data <- SetAssayData(seurat_data, slot = "data", new.data = sct_data, assay = "RNA")
  
  DefaultAssay(seurat_data) <- "RNA"
  seurat_data[["SCT"]] <- NULL
  ...
  query_projected <- Run.ProjecTILs(seurat_data, ref = reference, filter.cell = FALSE)
