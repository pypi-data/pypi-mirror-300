...
from pararamio.exceptions import PararamioRequestException, PararamNoNextPost, PararamNoPrevPost
from pararamio.file import File
from pararamio.utils.helpers import encode_digit, get_formatted_attr_or_load, parse_iso_datetime, rand_id

if TYPE_CHECKING:
    from pararamio.chat import Chat
    from pararamio.client import Pararamio
    from pararamio._types import PostMention, PostMetaT, TextParsedT, FormatterT
...
class Post:
    __slots__ = ('_data', '_chat', 'post_no', '_load_on_key_error')
    _data: Dict[str, Any]
    _chat: 'Chat'
    post_no: int
    is_bot: bool
    ...
    def is_event(self) -> bool:
        """
        is this message is event
        """
        return bool(self.event)

    @property
    def mentions(self) -> List['PostMention']:
        text_parsed = getattr(self, 'text_parsed', None)
        if not text_parsed:
            return []
        mentions_ = []
    ...
    def is_mention(self) -> bool:
        ...
        if not text_parsed:
            return False
        for item in text_parsed:
            if item.get('type', '') == 'mention' and item.get('id', None) == self.client.profile.get('id', -1):
                return True
        return False
