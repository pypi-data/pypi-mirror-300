...
class User:
    __slots__ = ('_data', '_client', 'id', '_load_on_key_error',)
    id: int
    info: Optional[str]
    ...
    def _load_users_request(cls, client: 'Pararamio', ids: List[int]) -> list:
        ...
        return [
            ...
            for u in client.api_get(url).get('users', [])
        ]
    ...
    def post(self, text: str, quote_range: Dict[str, Union[str, int]] = None, reply_no: int = None) -> Post:
    ...
    def search(cls, client: 'Pararamio', search_string: str) -> List['User']:
        ...
        return [cls(client, **unescape_dict(user, keys=['name'])) for user in client.api_get(url).get('users', [])]
    ...
    def get_activity(self, start: 'datetime', end: 'datetime', actions: List[ActivityAction] = None) -> List[Activity]:
        """get user activity
