...
__all__ = ('TeamMember', 'Team', 'TeamMemberStatus', )
...
class Team:
    __slots__ = ('_data', 'client', '_load_on_key_error',)
    _data: Dict[str, Any]
    client: 'Pararamio'
    admins: List[int]
    ...
    def __getattr__(self, key: str) -> Any:
        return get_formatted_attr_or_load(self, key, ATTR_FORMATTERS, )
    ...
    def create_role(self, name: str, description: Optional[str] = None) -> 'Group':
        return Group.create(self.client, organization_id=self.id, name=name, description=description)
