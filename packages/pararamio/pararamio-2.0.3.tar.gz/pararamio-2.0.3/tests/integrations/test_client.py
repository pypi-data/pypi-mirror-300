...
from pararamio.exceptions import PararamioException, PararamioHTTPRequestException, PararamioValidationException
...
class PararamioClientTest(BasePararamioTest):
    ...
    def test_022_activity_loading(self):
        ...
        for act in acts:
            self.assertTrue(act.action == ActivityAction.POST, 'post activity list contains another action')
        self.assertTrue(int(post.time_created.timestamp()) == int(acts[-1].time.timestamp()), 'last post action time not match last post time')
    ...
    def test_024_is_bot_meta(self):
        bot_post = self.bot.post_private_message_by_user_id(self.client.profile['id'], 'test message from bot')
        chat = Chat(self.client, bot_post['chat_id'])
        post = chat.posts(bot_post['post_no'], bot_post['post_no'])[0]
        self.assertTrue('is_bot' in post.meta.get('user', {}), 'meta.user.is_bot not found in received post')
        self.assertTrue(post.meta.get('user', {}).get('is_bot') is True, 'meta.user.is_bot is not True for post from bot')
