...
from mypy_boto3_iam.type_defs import PolicyDocumentTypeDef, PolicyDocumentDictTypeDef
    ...
    async with aws.S3ClientContext(request, credentials=credentials) as s3_client:
        ...
        async with DesktopObjectActionLifecycle(request=request,
            ...
            async with aws.IAMClientContext(request=request, credentials=elevated_credentials) as iam_client:
                old_collaborator_ids = await _get_collaborators(b.name, iam_client)
                new_collaborator_ids = b.collaborator_ids
                coros: list[Coroutine[Any, Any, None]] = []
                for collaborator_id in set(old_collaborator_ids).difference(new_collaborator_ids):
                    coros.append(_delete_collaborator(request, volume_id, b.name, collaborator_id, iam_client))
                for collaborator_id in set(new_collaborator_ids).difference(old_collaborator_ids):
...
async def delete_bucket(request: web.Request) -> web.Response:
    ...
    async with DesktopObjectActionLifecycle(request=request,
        ...
        async with aws.IAMClientContext(request=request, credentials=elevated_credentials) as iam_client:
            coros: list[Coroutine[Any, Any, None]] = []
            for collaborator_id in await _get_collaborators(request, bucket_id, iam_client):
                coros.append(_delete_collaborator(request, volume_id, bucket_id, collaborator_id, iam_client))
            await asyncio.gather(*coros)
...
async def __get_bucket(request: web.Request, volume_id: str, s3_client: S3Client, cache,
    ...
    async def _get_collaborators_for_bucket(b: AWSBucket):
        ...
        async with aws.IAMClientContext(request, credentials=elevated_credentials) as iam_client:
            b.collaborator_ids = await _get_collaborators(b.name, iam_client)
...
async def _get_collaborators(bucket_name: str, iam_client: IAMClient) -> list[str]:
    logger = logging.getLogger(__name__)
    logger.debug('Getting collaborators for bucket %s', bucket_name)
    try:
        assert bucket_name is not None, 'bucket_name cannot be None'
        return [policy_info.user_id async for policy_info in _all_collaborator_policies_gen(iam_client) if bucket_name in policy_info.bucket_names]
    except BotoClientError as e:
...
async def _delete_collaborator(request: web.Request, volume_id: str, bucket_name: str, collaborator_id: str, iam_client: IAMClient):
    logger = logging.getLogger(__name__)
    loop = asyncio.get_running_loop()
    sub = request.headers.get(SUB, NONE_USER)
    sub_headers = {SUB: sub}
    ...
    group_name = f'/{bucket_name} Collaborators/{collaborator_id}'
    try:
    ...
    organization = await client.get(request.app, URL(organization_url) / 'byaccountid' / aws_account.instance_id, Organization,
                                    headers=sub_headers)
    ...
    if organization is not None:
        ...
        collaborator.from_organization_id = organization.id
...
async def _put_collaborator(request: web.Request, volume_id: str, bucket_name: str, collaborator_id: str, iam_client: IAMClient):
    ...
    account_id = _extract_account_id_from(policy_arn)
    logger.debug('Got account %s', account_id)
    ...
    try:
        ...
        group = Group()
        group.group = f'/{bucket_name} Collaborators/{collaborator_id}'
        ...
        async def get_volume() -> Volume | None:
            assert aws_account is not None, 'aws_account cannot be None'
            ...
            user_volumes_gen = client.get_all(request.app, volume_url, Volume, headers=collab_sub_headers)
            ...
            async for volume in user_volumes_gen:
                ...
                if volume.owner == CREDENTIALS_MANAGER_USER:
                    if volume.account_id == aws_account.instance_id and volume.name != f'{aws_account.id}_Collaborator_{person.id}':
