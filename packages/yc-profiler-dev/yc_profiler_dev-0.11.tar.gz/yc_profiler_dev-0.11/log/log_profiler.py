...
LOG_DUMP_ENDPOINT = "/yc-receiver?dt=applog"
...
def upload_log_data(config):
    ...
    logFilePath = log_path[0]
    logBackUpFile = logFilePath + '.ycrash.bak'
    if not os.path.exists(logBackUpFile):
        with open(logBackUpFile, 'w'):
            pass

    with open(logFilePath, 'r') as new_log_file:
    ...
    copy_file(logFilePath, logBackUpFile)
