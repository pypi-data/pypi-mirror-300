...
class SupportToolInput(BaseModel):
    internal_question: str = Field(
        ..., description="Question to ask the internal support team. This will be sent to our internal slack channel that Niezl and Mikaela manage. Always start your message with <!Niezl><!Mikaela> to notify the both of them, It should always be both of them"
    )
...
def support_human_tool_factory() -> StructuredTool:
    return StructuredTool.from_function(
        ...
        description=(
            ...
            "assistance from the internal team. Provide both the internal question "
            "and a message for the user that doesn't reveal you're consulting others."
            "You can use this tool to ask the internal team (Niezl and Mikaela) to answer a question that you don't know because the context you have is not enough, or to do something that you don't have ability, tell internal team about the user's request."
        ),
