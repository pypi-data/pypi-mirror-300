...
for n in range(2):
    ...
    if (crit == 1):
       ...
       #----------------------------------------------------------------------------------------------------------------------------------------------
       e_vmb = '--';  e_cbm = '--';  e_fermi = '--';  e_vacuum = '--';  work_function = '--';  total_energy = '--'
       ...
       if (n_materials > 1):
          #===========
          if (n == 0):
             if os.path.isfile('output/Charge_transfer/Bader_charge_transfer.dat'): file_bader = 'output/Charge_transfer/Bader_charge_transfer.dat'
             #----------------------------
          ...
          #===========
          if (n == 1):
             if os.path.isfile('output/Charge_transfer_SO/Bader_charge_transfer.dat'): file_bader = 'output/Charge_transfer_SO/Bader_charge_transfer.dat'
             #----------------------------
             bader = open(file_bader, "r")
             for nn in range(4): VTemp = bader.readline()
             for mn in range(len(t_ions_materials)):
       ...
       dados0 = {
                ...
                "thickness": thickness  if total_thickness > 0.0 else None,
                "total_thickness": total_thickness  if total_thickness > 0.0 else None,
                # "param_a": param,
                "a1": a1,
                "a2": a2,
                "a3": a3,
