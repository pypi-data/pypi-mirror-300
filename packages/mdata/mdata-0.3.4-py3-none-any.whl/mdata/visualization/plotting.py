...
def create_overview_plot(md: MD, downsample_to=10_000, use_gl=True):
    md = as_base(md)
...
def create_timeseries_plot(md: MD, measurement_spec: str, obj: str,
    ...
    if measurement_spec in md.measurement_specs:
        ...
        for i, f in enumerate(features, start=1):
            ...
            data = data.dropna('rows', how='any')

            if ts_length > downsample_to:
                if feature_typing.has_numeric_type(data.iloc[:, 1]):
                    down = lttb.downsample(data.values, n_out=downsample_to, validators=[])
                    x, y = down[:, 0], down[:, 1]
                else:
...
def create_measurement_frequency_plot(md: MD, use_hz=True):
    md = as_base(md)
    fig = go.Figure()
    colorscale = plotly.colors.qualitative.Plotly
    objs = md.objects
