...
class ServiceDeclPortOp(_ods_ir.OpView):
  ...
  def __init__(self, inner_sym, toClientType, *, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_450')) else
      _ods_ir.AttrBuilder.get('anonymous_450')(toClientType, context=_ods_context))
    _ods_successors = None
    super().__init__(self.build_generic(attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip))
...
class ServiceImplClientRecordOp(_ods_ir.OpView):
  ...
  def __init__(self, relAppIDPath, servicePort, typeID, implDetails, *, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_450')) else
      _ods_ir.AttrBuilder.get('anonymous_450')(typeID, context=_ods_context))
...
def manifest_impl_conn(rel_app_id_path, service_port, type_id, impl_details, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ServiceImplClientRecordOp(relAppIDPath=rel_app_id_path, servicePort=service_port, typeID=type_id, implDetails=impl_details, loc=loc, ip=ip))
...
def manifest_req(requestor, service_port, type_id, *, std_service=None, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ServiceRequestRecordOp(requestor=requestor, servicePort=service_port, typeID=type_id, stdService=std_service, loc=loc, ip=ip))
...
def manifest_constants(symbol_ref, constants, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(SymbolConstantsOp(symbolRef=symbol_ref, constants=constants, loc=loc, ip=ip))
