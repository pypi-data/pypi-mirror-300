...
def information_gain_ME(data, target, current_split):
    ...
    for current_distribution in distributions_of_subsets:
        post_ME += (sum(current_distribution)/total_elements)*misclassification_error(current_distribution)

    # Elemente die für das Attribut nan beinhalten werden als Fehlklassifikationen gewertet
    post_ME += len(data[data[current_split.attribute].isna()])/total_elements
    
    info_gain = round(pre_ME - post_ME,6)
...
class DecisionTree:
    ...
    def __init__(self, crit = None, target = None, data = pd.DataFrame(), target_values = None):
        ...
        if (crit == None) or (crit == 'misclassification_error'):
    ...
    def grow_tree(self, data = pd.DataFrame(), target = None, crit = None, max_depth = float('inf'), act_depth = 0, min_gain = 0, min_leaf_cases = 0, drop = True):
        ...
        # Falls vorherige Abfragen nicht zutrafen wird ein weiterer Split gesucht um ihn anzuwenden
            ...
            if len(list_of_splits) > 0:
                ...
                best_split, best_gain = identify_best_split(data, target, list_of_splits, self.criterion)
                ...
                if best_gain >= min_gain:
                	...
                	for i in range(len(list_of_subsets)):
                		...
                		if drop:
                			...
                			new_input_subset = list_of_subsets[i]
                		self.grow_tree(new_input_subset, target, self.criterion, max_depth, act_depth+1, min_gain, min_leaf_cases, drop)
    ...
    def confusion_matrix(self, data, abs_freq = True, row_perc = False, col_perc = False ):
        #print('loading...')
    ...
    def prediction_accuracy2(self, data, detailed = False):
        ...
        for i in subset_list[1:]:
            data_predicted = pd.concat([data_predicted, i])
    ...
    def validation_pruning(self, validation_sample, root_node = None):                     #root_nr = 1
        ...
        if current_node.type == 'leaf':
            ...
            return None
        pass
    def manual_split(self, attribute = 'auto', threshold = 'auto', node_nr = 1, doc = False, drop = True):
        ...
        if attribute == 'auto':
            ...
            best_split, best_gain = identify_best_split(self.tree_nodes[node_nr].subset, self.target, list_of_splits, self.criterion)
            ...
            if self.tree_nodes[node_nr].subset[attribute].dtype in ['float64','int64', int, float,'float32','int32']:
                ...
                value_set = np.sort(self.tree_nodes[node_nr].subset[attribute].dropna().unique())                                              
        ...
        for i in range(len(list_of_subsets)):
            ...
            if drop: 
                ...
                self.return_leaf_node(list_of_subsets[i], self.target)
