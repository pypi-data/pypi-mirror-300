...
class ManifiestoInfo (EcuInfo):
	...
	def extractVehiculoInfo (self, type="VEHICULO", remolque=None):
		...
		try:
			placaPaisText            = Extractor.getValidValue (Utils.getValue (self.fields, keys ["placaPais"]))
			if placaPaisText:
				...
				vehiculo ["marca"]   = Extractor.getValidValue (Utils.getValue (self.fields, keys ["marca"]))
				vehiculo ["anho"]    = Extractor.getValidValue (Utils.getValue (self.fields, keys ["anho"]))

				if type == "VEHICULO":
					vehiculo ["chasis"]  = Extractor.getValidValue (Utils.getValue (self.fields, keys ["chasis"]))
		...
		print ("+++ Información extraída del vehiculo:", vehiculo)
		return vehiculo
