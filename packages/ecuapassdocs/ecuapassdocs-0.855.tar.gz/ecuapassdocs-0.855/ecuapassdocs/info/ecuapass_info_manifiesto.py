...
class ManifiestoInfo (EcuInfo):
	def __init__(self, docFieldsPath, runningDir):
		super().__init__ ("MANIFIESTO", docFieldsPath, runningDir)
	...
	def extractVehiculoInfo (self, type="VEHICULO", remolque=None):
		transport = {key:None for key in ["marca","anho","pais","placa","chasis","certificado","tipo"]}
		...
		if type == "VEHICULO":
			...
			print (f"ERROR: Tipo de transporte desconocido: '{type}'")
			return transport
 
		try:
			transport ["marca"]       = self.getTruckValue (type, Utils.getValue (self.fields, keys ["marca"]))
		...
		print ("+++ Extracted info vehiculo:", transport)
		return transport

	#-- Get valid value in vehicle/trailer info
	def getTruckValue (self, type, value):
		if value is None or value.upper() in ["", "N/A", "X", "XX", "XXX", "X X", "X X X"]:
	...
	def extractConductorInfo (self, type="CONDUCTOR"):
		...
		try:
			...
			conductor ["documento"]       = Utils.getValue (self.fields, keys ["documento"])
