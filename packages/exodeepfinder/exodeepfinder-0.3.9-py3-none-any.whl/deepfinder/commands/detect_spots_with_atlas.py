...
def detect_spots(tiffs_path, atlas_path, atlas_args, output_path):
    ...
    if not blobs.exists():
        ...
        subprocess.run([str(Path(atlas_path) / 'blobsref')])

    for frame in sorted(list(tiffs_path.iterdir())):
        if frame.suffix.lower() == '.tif' or frame.suffix.lower() == '.tiff':
            print(f'Processing {frame}...')
            subprocess.run([str(Path(atlas_path) / 'atlas'), '-i', str(frame), '-o', str(output_folder / frame.name)] + atlas_args.split(' '))
...
def add_args(parser):
    ...
    parser.add_argument('-ap', '--atlas_path', help='Optional path to atlas (Empty by default since atlas is supposed to be installed with conda thus directly accessible).', default='', type=str)
