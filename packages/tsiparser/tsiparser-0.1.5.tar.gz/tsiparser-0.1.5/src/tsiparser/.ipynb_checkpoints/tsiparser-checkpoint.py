...
def get_db(logger, inexceldb):
    ...
    logger.info("Reading the preovided excel file (--inexceldb)...")
...
def introduce_reactions(logger, db, model): 
    ...
    for rid, row in db['R'].iterrows():
        ...
        if row['author'] not in db['authors']['username'].to_list(): 
            logger.error(f"Reaction {rid} has invalid author: {row['author']}.")
            return 1
...
def check_completeness(logger, model, progress, module): 
...
def tsiparser(args, logger): 
    ...
    check_completeness(logger, model, args.progress, args.module)
