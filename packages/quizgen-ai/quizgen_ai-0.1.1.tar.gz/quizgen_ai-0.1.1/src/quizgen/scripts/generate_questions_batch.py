...
def generate_questions(input_dir, output_dir, title, file_filter, concept_model, questions_model):
...
def main():
    ...
    parser.add_argument("--input-dir", required=True, help="Input directory containing source files")
    parser.add_argument("--output-dir", required=True, help="Output directory for JSON question files")
    parser.add_argument("--title", required=True, help="Title of the course")
    parser.add_argument("--file-filter", default="*.md", help="File filter for source files")
    ...
    generate_questions(args.input_dir, args.output_dir, args.title, args.file_filter, args.concept_model, args.questions_model)
