...
def send_mattermost(url, whoami, subject):

    try:
        if not url:
            print_color('send_mattermost not sending text, since no URL given', colors.WARNING)
        if not whoami:
            whoami = send_mattermost.hostname
        s = f'```{whoami}@{send_mattermost.ip} : {subject}```'
        ...
        if response.status_code != 200:
            print_color(f'send_mattermost failed with status_code: {response.status_code} | text: {response.text}', colors.WARNING)
    except Exception as err:
        print_color(f'send_mattermost failed with exception: {err.__repr__()}', colors.WARNING)
    
send_mattermost.hostname = socket.gethostname()
send_mattermost.ip = socket.gethostbyname(socket.gethostname())

log_mattermost = None
def activate_logging_mattermost(mattermost_url, whoami):
...
def logfun(name, msg, color):
    ...
    if not log_mattermost is None:
        ...
        threading.Thread(target = log_mattermost, args=(emoji+msg, )).start()
...
class EventStreamHandler():
    ...
    def __init__(self, parent, dict_size = 100, verb=True) -> None:
    ...
    def start(self) -> None:
        if self.verb: log(f'{self.parent.ip} starting event stream in new thread...', color=colors.HEADER)
        ...
        self.thread.start()
        if self.verb: log(f'{self.parent.ip} --> eventstream started...', color=colors.OKGREEN)
        return self.thread
...
class DataStreamHandler():
    ...
    def wait_for_start(self, timeout=10.):
        if self.verb: log(f'{self.parent.ip} waiting for data stream to start...', color=colors.HEADER)
        t = time.time()
        while (time.time() - t) < timeout and not self.tickcount:
            time.sleep(0.1)
        if self.verb: log(f'{self.parent.ip} --> stream started OK', color=colors.OKGREEN)
    ...
    def start(self) -> None:
        if self.verb: log(f'{self.parent.ip} starting data stream in new thread...', color=colors.HEADER)
        assert not self.parent is None, 'parent can not be None'
        ...
        self.thread.daemon = True
        self.thread.start()
        if self.verb: log(f'{self.parent.ip} --> datasteam started...', color=colors.OKGREEN)
        return self.thread
...
class scu():
    ...
    HTTP rest API.
    """
    def __init__(self, address='', ip='', port='8080', debug=False, lims_az=(-270.0, 270, 3.0), lims_el=(15, 90, 1.35), dish_type='mke', post_put_delay=0.2, use_socket=True, influx_token='', kwargs_requests = None, wait_done_by_uuid=True, **kwargs):
        ...
        if not use_socket:
            assert not wait_done_by_uuid, 'can not wait by UUID if no websocket is requested!'

        self.wait_done_by_uuid = wait_done_by_uuid
        self.event_streamer = None
        if wait_done_by_uuid and use_socket:
    ...
    def getc(self, key=None, as_dict=False):
        ...
        if key is None and self.dish_type == 'skampi':
            ...
            return dict(keys, self._get_device_status_value(keys))
        elif key is None:
    ...
    def wait_uuid_done(self, uuid:str=None, dt_poll=0.1):
        if uuid is None:
            uuid = self.uuid_last
    ...
    def get_errors(self, warnings=True):
        if warnings:
            ...
            chans = [k for k in chans if fun(k)]
        else:
            chans =  [k for k in chans if 'err' in k.lower()]

        return {k:v for k, v in self.getc(chans, as_dict=True).items() if v}
    ...
    def move_to_azel(self, az_angle, el_angle, az_vel=None, el_vel=None, wait_settle=False):
        ...
        if az_vel is None and el_vel is None: 
            ...
            if wait_settle and self.wait_done_by_uuid:
                self.wait_uuid_done(uuid1)
                self.wait_uuid_done(uuid2)
    ...
    def acu_ska_track(self, BODY):
        ...
        uuid = self.scu_put('/acuska/programTrack', data = BODY)
        if self.wait_done_by_uuid:
            self.wait_uuid_done(uuid)
        return uuid
    ...
    def acu_ska_track_stoploadingtable(self):
        ...
        uuid = self.scu_put('/acuska/stopLoadingTable')
        if self.wait_done_by_uuid:
            self.wait_uuid_done(uuid)
        return uuid
    ...
    def start_logger(self, config_name='full_configuration', stop_if_need=True):
        ...
        if self.logger_state() == 'STOPPED':
            ...
            uuid = self.scu_put('/datalogging/start', params='configName=' + config_name)
        else:
...
#        log('logger state ')
    ...
    def start(self, az_start=None, el_start=None, band_start=None, az_speed=None, el_speed=None, send_default_configs=False, reset_after_start=True):
        ...
        if self.wait_done_by_uuid and not self.event_streamer.is_alive():
            self.start_event_stream()
...
if __name__ == '__main__':
    ...
    log("main")
    d119 = scu('http://10.96.66.10:8080/', debug=True, use_socket=True, wait_done_by_uuid=True)
