...
class RegistryServiceBase:
    ...
    async def _get_registry(self, registry_id: int) -> ContainerRegistry:
        ...
        async with ApiClient(self.api_config) as api_client:
            ...
            logger.info(
                "Making request to backend API with registry ID: %d", registry_id
            )
    ...
    async def _get_registry_creds(self, creds_id: int) -> ContainerRegistryAuth:
        ...
        async with ApiClient(self.api_config) as api_client:
            ...
            registries_api = RegistriesApi(api_client)
            logger.info("Making request to backend API with creds ID: %d", creds_id)
    ...
    async def _decrypt_password(self, creds: ContainerRegistryAuth) -> str:
        ...
        logger.debug(
            "RegistryServiceBase._decrypt_password() - encrypted password encoded [ %d chars ]",  # noqa
            len(encrypted_password),
        )
        cipher_suite: Fernet = self.auth_svc.get_cipher_suite()
        decrypted_password = cipher_suite.decrypt(encrypted_password).decode("utf-8")
        logger.debug(
            "RegistryServiceBase._decrypt_password() - decrypted password [ %d chars ]",
    ...
    async def authorize(self, ws_session: str) -> Result[None, str]:
        ...
        try:
            ...
            async with websockets.connect(ws_session) as ws:
                ...
                logger.debug(
                    "DockerRegistryService.authorize() - auth_data contains [ %d ] keys",  # noqa
                    len(auth_data.keys()),
                )
                creds_id = auth_data.get("creds_id", None)
                logger.info("Authorizing using credentials: %d", creds_id)
                if creds_id is None:
                ...
                logger.info("Got credentials for registry: %d", creds.registry_id)
                logger.debug(
                ...
                logger.info("Got registry: %d", registry.id)
                logger.debug(
