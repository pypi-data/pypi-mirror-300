...
def _hack_terminal_msgs():
    ...
    def terminal_message(key, msg:str, format:str="none"):
        ...
        This allows to give informations to the user even if the stdout is deactivated during a validation.

        @key:    Value to pass to allow the use of the function (see terminal \`STD_KEY\` macro argument).
    ...
    __builtins__.terminal_message = terminal_message
`,
...
def _hack_input_prompt():
    """ Put it in place once only, to limit memory leaks"""
...
`,
    ...
    // Cannot be done using the decorator, because called _before_ pyodide features refresh (in the tests).
    refresher: `
...
def _hack_stdout_up():
    ...
    __builtins__.true_print       = true_print      = print
    __builtins__.print            = bi_print
...
def _hack_stdout_down():
    ...
    __builtins__.print = __builtins__.true_print
