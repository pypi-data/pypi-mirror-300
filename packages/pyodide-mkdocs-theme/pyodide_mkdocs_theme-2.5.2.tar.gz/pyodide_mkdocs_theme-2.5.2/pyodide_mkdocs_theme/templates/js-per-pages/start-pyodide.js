...
/**Function injected into the python environment.
 * The second argument allows to add more info to the prompt window, that won't be sent to the
 * terminal. This is useful because nothing is visible in the terminal while the section is
 * run (no DOM update), so some users (aka Romain x) ) might want to add previous infos there.
 * */
function inputWithPrompt(text, beginning="") {
    let result = prompt(beginning+text);
    $.terminal.active().echo(
        txtFormat.none(text + result)
    );
    return result;
}
// Avoid the JS code "leaking" in the terminals (the repr of the js function is its code...!):
inputWithPrompt.toString = function(){ return "<function input>" }
