...
class MaestroFiles(BaseMaestro):
    ...
    @property
    def test_on_web(self):
        """ Check if the build is done for the online website or not (ie. local/serve) """
        return self.testing_include == PmtTests.site

    @property
    def do_add_testing_page(self):
        """ Check if the build is done for the online website or not (ie. local/serve) """
        return self.test_on_web or self.in_serve and self.testing_include == PmtTests.serve
