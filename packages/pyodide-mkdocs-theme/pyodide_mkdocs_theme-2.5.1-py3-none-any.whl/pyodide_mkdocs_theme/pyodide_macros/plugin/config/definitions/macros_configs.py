...
from ...maestro_tools_tests import CASES_OPTIONS
...
IDE = MacroConfigSrc(
    ...
    *PY_GLOBAL.subs_dct.values(),
    ...
    ConfigOptionIdeLink(
        'STD_KEY', str, default="",
        docs = """
            Clef à passer en argument de `terminal_message` pour autoriser son utilisation lorsque
            la sortie standard est désactivée pendant les tests.
        """,
        yaml_desc="""
            Key to allow to print messages directly in an IDE terminal when the stdout is deactivated.
        """,
    ),
    ConfigOptionIdeLink(
        ...
        """,
        yaml_desc="""
            Join the content of this editor to the zip archive, when extracting all the codes
...
MULTI_QCM = MultiQcmConfigSrc(
    ...
    ConfigOptionSrc(
        ...
        docs = """
            ...
            <br>Il est également possible de passer en unique argument un [chemin relatif vers un
            fichier `.json`](--qcms-json) comportant une configuration complète (ou partielle) pour
            déclarer un `multi_qcm`.
        """,
        yaml_desc = """
