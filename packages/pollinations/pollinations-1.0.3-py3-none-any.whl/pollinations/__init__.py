...
def text_models(*args, **kwargs) -> tuple:
    models: list = []
    request: requests.Request = requests.get(url=f"https://{TEXT_API}/models", headers=HEADER, timeout=30)
    for model in request.json():
        models.append(model["name"])
    return tuple(models)

def image_models(*args, **kwargs) -> list:
    request: requests.Request = requests.get(url=f"https://{IMAGE_API}/models", headers=HEADER, timeout=30)
...
class TextObject(object):
    ...
    def __str__(self) -> str:
        return f"TextObject({self.text=}, {self.prompt=}, {self.model=}, {self.time=}, self.messages=[...], self.params="+"{...})"
...
class TextModel(object):
    def __init__(
        ...
        *args, **kwargs
        ...
        self.assistant_create: object = lambda _ : {"role": "assistant", "content": str(_)}

        self.messages.append(self.system_create(self.system))

    def generate(self, prompt: str, display: bool=False, *args, **kwargs) -> str:
        ...
        if self.contextual: 
            ...
            request: requests.Request = requests.post(f"https://{TEXT_API}/", json=params, headers=HEADER, timeout=30)
        else:
            request: requests.Request = requests.get(f"https://{TEXT_API}/{prompt}", params=params, headers=HEADER, timeout=30)
        ...
        if display:
            for i, character in enumerate(content):
                if i > 0 and content[i - 1] not in set(string.ascii_letters + string.digits + ' \t\n'):
...
class ImageModel(object):
    def __init__(
        ...
        *args, **kwargs
        ...
        self.model: str = str(model)
        self.seed: int = int(seed) if str(seed).lower().strip() != "random" else random.randint(0, 9999999999)
    ...
    def generate(self, prompt: str, negative: str="", save: bool=False, file: str = "image-output.png", *args, **kwargs) -> Image:
        prompt: str = str(prompt)
        save: bool = bool(save)
        file: str = str(file)
        ...
        request: requests.Request = requests.get(url=f"https://{IMAGE_API}/prompt/{prompt}?{params}", headers=HEADER, timeout=30)
        try:
            image: Image = Image.open(io.BytesIO(request.content))
            if save: image.save(file)
            w_h: tuple = (self.width, self.height)
            if w_h == (1024, 1024): aspect_ratio: str = "1:1"
            elif w_h == (768, 1024): aspect_ratio: str = "3:4"
            elif w_h == (1024, 576): aspect_ratio: str = "16:9"
            else: aspect_ratio: str = "N/A"
            params = {
                "aspect": aspect_ratio,
                "seed": self.seed,
                "width": self.width,
...
class MultiModel(object):
    ...
    def generate(
        ...
        *args, **kwargs
        ...
        try: gen_type: str = gen_type.split("```", 1)[1].split("```", 1)[0].strip().lower()
        except: pass
        ...
        if gen_type == "y":
            content_1: str = self.main_model.generate(f"Dont wrap in quotes. Dont engage in conversation or say other stuff. Say something like: 'Sure!', 'Of course,', etc, 'Generating your image! but relate it to the user's prompt: {prompt}' (Might need to look at chat history)", display)
            enhanced: str = self.main_model.generate(f"Dont wrap in quotes. Say what the user wants to generate with images for the image generator, enhance the prompt very slightly. Only say the new prompt (no other text): (Might need to look at chat history) Prompt: {prompt}")

            if self.default == "None":
                model_guess: str = str(self.guesser_model.generate(enhanced.text).text)
                try: model_guess: str = model_guess.split("```", 1)[1].split("```", 1)[0].strip().lower()
                except: model_guess: str = "turbo"
            ...
            try: content_2: str = self.image_model.generate(enhanced.text, "", True)
            except: content_2: str = self.main_model.generate("Make an apology that the image generator didn't work.", display)
...
class SmartModel(object):
    def __init__(
        ...
        *args, **kwargs
        ...
        self.methods: dict = {
            ...
            "get_time_and_weather_and_search": lambda d_r: self.get_time_and_weather_and_search(d_r)
        }

    def get_time(self, timezone='UTC'):
        try:
            tz = timezone.split("(", 1)[-1].split(")", 1)[0].strip() if '(' in timezone else 'UTC'
            tz = pytz.timezone(tz)
        except:
    ...
    def generate(self, prompt: str, display: bool=False, provide_details: bool=False):
        ...
        if check == "n":
            ...
            try:
                distinguish_response: str = str(distinguish_response.text).split("```", 1)[1].split("```", 1)[0].lower().strip()
            except:
                distinguish_response: str = "n"

            distinguish_type: str = distinguish_response.split("(", 1)[0].strip()

            print(distinguish_type)
            print(distinguish_response)
            if distinguish_type in self.methods and self.methods[distinguish_type]:
                message: dict = self.main_model.main_model.system_create(f"{distinguish_response} -> {self.methods[distinguish_type](distinguish_response)}")
                print(message)
