...
def label_branches(array, peaks = 1, branch_removal = None, comp_dil = 0, max_vol = 0, down_factor = None, directory = None):
    """
...
class Network_3D:
    ...
    def isolate_mothers(self, directory = None, down_factor = 1, louvain = True):
        ...
        if down_factor > 1:
            ...
            for item in self._node_centroids:
                centroids[item] = np.round((self._node_centroids[item]) / down_factor)
            nodes = downsample(self._nodes, down_factor)
            partition = network_analysis.extract_mothers(nodes, self._network, directory = directory, centroid_dic = centroids, louvain = louvain)
        else:
