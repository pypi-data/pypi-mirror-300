...
path_to_exe = os.getenv('RELAP_EXE_PATH')
...
relapwin_loc = os.path.dirname(path_to_exe)
...
Defaults
    ...
    :hideplots: (optional) hideplots? True or False (could be deleted)
...
def make_strip(input_variables_file, strip_filename: str, form="fmtout"):
    ...
    :input_variables_file: [str] the file that lists all the variables
                           ...
                           [DataFrame] name of the variable that stores the list of
                                 variables
    :strip_filename: [str] the filename of the strip to be created
    :form: [str] (optional) field 'csv' or 'fmtout'
...
def read_stripf_df(case_folder='', names=''):
    ...
    """read_stripf performs the following consecutive actions:
            1.- reads the stripf file
            2.- stores the data in a narray to be used in python
    ...
    :returns:
        - a [ndarry] with the data with name 'case_folder'
    """
    if case_folder != '':
...
def run_strip(strip_file: str, restart: str, case_folder=''):
    ...
    """ run_strip simply performs a strip with the defined strip_file

    :strip_file: [str] strip file
    :restart: case name without extension
    :case_folder: (optional) folder of the case
    :returns: nothing
...
def general_fig(variables, dataCalc0, dataExp='' , annotations=[], labels=[]):
    ...
    print('annotation locations are as follows')
    for i in range(0,3):
        l = 0
...
def plot_variables(variable: list, variables_df, cases_df):
    ...
    plt.grid(b=True, which='major', linestyle=':', linewidth=1.0, alpha=0.8,
             color='grey')
    plt.legend()
    plt.tight_layout()
...
def read_exp(expdata_loc: str):
    ...
    try : 
        ...
        exp_file_bool = False
    return dataExp
def print_ss(case, exp='', llista= False, end_ss = 0, lenght = 60):
    """ Prints a table for steady state values
    TODO: 
    - Print the result in a text file

    :case: specific case for which you want to make the table
    :exp: (optional) If '', no experimental is to be printed
    ...
    :lenght: Number of time steps to be averaged as steady state value
    ...
    def avg_ss(cas, var):
        """ Makes the average of the variable for the specified lenght
        :var: name of the variable 
        """
        i = 0
        while cas['time'][i] < (end_ss - lenght):
             index_start = i
             i += 1
...
def mkdir(path: str):
    try :
        ...
        print(30*'-'+'WARNING'+30*'-')
        print('          directory ' + path + ' already present')
        print(67*'-')
