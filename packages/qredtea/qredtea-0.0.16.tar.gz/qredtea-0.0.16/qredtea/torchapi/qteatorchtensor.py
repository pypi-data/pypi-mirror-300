...
from qredtea.symmetries import QteaAbelianTensor, AbelianSymmetryInjector


ACCELERATOR_DEVICES = ["gpu", "xla"]
...
class QteaTorchTensor(_AbstractQteaBaseTensor):
    ...
    implemented_devices = ("cpu", "gpu", "cgpu", "xla")
    ...
    def __mul__(self, factor):
        ...
        return QteaTorchTensor.from_elem_array(
    ...
    def __truediv__(self, factor):
        ...
        return QteaTorchTensor.from_elem_array(
            elem, dtype=self.dtype, device=self.device
        )
    ...
    def copy(self, dtype=None, device=None):
        ...
        return self.from_elem_array(
            self._elem.detach().clone(), dtype=dtype, device=device
        )
    ...
    def read(cls, filehandle, dtype, device, base_tensor_cls, cmplx=True, order="F"):
        ...
        qteatensor = QteaTensor.read(
            filehandle, np.complex128, "cpu", base_tensor_cls, cmplx=cmplx, order=order
        )
    ...
    def set_missing_link(links, max_dim, are_links_outgoing=None):
        ...
        **Arguments**
        ...
        """
        return QteaTensor.set_missing_link(
            links, max_dim, are_links_outgoing=are_links_outgoing
        )

    def zeros_like(self):
        """Get a tensor same as `self` but filled with zeros."""
        return QteaTorchTensor(
            self.shape, ctrl="Z", dtype=self.dtype, device=self.device
        )
    ...
    def are_equal(self, other, tol=1e-7):
        ...
        return to.isclose(self._elem, other.elem, atol=tol).all().item()
    ...
    def conj(self):
        ...
        # For real tensors, conj() will not return a true copy,
        # but the same memory address. Assuming gradients should
        # be preserved in such a case, return a clone() and not
        # a detach().clone()
        return QteaTorchTensor.from_elem_array(
    ...
    def conj_update(self):
        ...
        to.conj(self._elem, out=self._elem)
    ...
    def _convert_check(self, device):
        ...
        if device is not None:
            ...
            if (device == "xla") and (not XLA_AVAILABLE):
    ...
    def convert(self, dtype=None, device=None, stream=None):
        ...
        if device is not None:
            ...
            if device == current:
                ...
                self._elem = self._elem.to(device="cuda")
            elif device == "cpu":
                # We go from gpu to cpu
                self._elem = self._elem.to(device="cpu")
            else:
    ...
    def convert_singvals(self, singvals, dtype, device):
        ...
        if device is not None:
            ...
            if device == current:
                ...
                singvals = singvals.to(device="cuda")
            elif device == "cpu":
                # We go from gpu to cpu
                singvals = singvals.to(device="cpu")
            elif device == "xla":
    ...
    def get_of(self, variable):
        ...
        if self.device_str(variable) in ACCELERATOR_DEVICES:
            return variable.detach().to(device="cpu")
    ...
    def get_entry(self):
        ...
        if self.device in ACCELERATOR_DEVICES:
            return self._elem.to(device="cpu").reshape(-1).item()
    ...
    def mpi_send(self, to_, comm, tn_mpi_types):
        ...
        **Arguments**
        ...
        """
        raise NotImplementedError("Pytorch has no MPI support yet.")
    ...
    def transpose(self, permutation):
        ...
        tens = QteaTorchTensor(None, ctrl=None, dtype=self.dtype, device=self.device)
        # pylint: disable-next=protected-access
        tens._elem = self._elem.permute(permutation)
        return tens
    ...
    def write(self, filehandle, cmplx=None):
        ...
        **Details**
        ...
        if self.device == "cpu":
            ...
            elem_np = self._elem.detach().to(device="cpu").numpy()
    ...
    def split_svd(
        ...
        if svd_ctrl in ("E", "X"):
            try:
                ...
                warn("SVD in mode E or X failed; falling back to mode D.")
                svd_ctrl = "D"
    ...
    def stack_link(self, other, link):
        ...
        **Returns**
        ...
        new_this = QteaTorchTensor(
            [dim1, new_dim, dim3], ctrl="N", dtype=self.dtype, device=self.device
        )
        # pylint: disable-next=protected-access
        new_this._elem[:, :dim2, :] = self._elem.reshape([dim1, dim2, dim3])
        # pylint: disable-next=protected-access
        new_this._elem[:, dim2:, :] = other._elem.reshape([dim1, dim4, dim3])
    ...
    def tensordot(self, other, contr_idx):
        ...
        tens = QteaTorchTensor.from_elem_array(
            elem, dtype=self.dtype, device=self.device
        )
        return tens

    @staticmethod
    def free_memory_device():
        """
    ...
    def from_elem_array(cls, tensor, dtype=None, device=None):
        ...
        **Arguments**
        ...
        if device is None:
            ...
            device = QteaTorchTensor.device_str(tensor)
    ...
    def kron(self, other, idxs=None):
        ...
        elem = to.einsum(subscipts, self._elem, other._elem).reshape(final_shape)
        tens = QteaTorchTensor.from_elem_array(
            elem, dtype=self.dtype, device=self.device
        )
        return tens
    ...
    def mask_to_host(self, mask):
        ...
        if to.is_tensor(mask):
            mask_on_host = mask.to(device="cpu").numpy()
        else:
            mask_on_host = mask
    ...
    def set_submatrix(self, row_range, col_range, tensor):
        ...
        self._elem[row1:row2, col1:col2] = tensor.elem.reshape(row2 - row1, col2 - col1)
    ...
    def _truncate_singvals(self, singvals, conv_params=None):
        ...
        normalization_factor = to.sqrt(norm_kept) / to.sqrt(norm_kept + norm_trunc)
    ...
    def _truncate_sv_norm(self, singvals, conv_params):
        ...
        total norm cut.
        """
        raise NotImplementedError("Not yet for QteaTorchTensor.")
    ...
    def device_str(obj):
        ...
        device = "cpu" if obj.get_device() == -1 else "gpu"
        if device == "gpu" and XLA_AVAILABLE:
    ...
    def _xla_convert(self, dtype, device, stream):
        ...
        if do_convert_dtype and (self.device != "cpu"):
            self._elem = self._elem.to(device="cpu")
            current = "cpu"
    ...
    def _split_svd_eigvl(self, matrix, svd_ctrl, max_bond_dimension, contract_singvals):
        ...
        if svd_ctrl == "E":
            ...
            warn("Falling back from X to E: pytorch has no eigsh.")
...
class DataMoverPytorch(_AbstractDataMover):
    ...
    def async_move(self, tensor, device):
        ...
        device: str
            The device where to move the tensor
        """
        warn("Moving stull sync for pytorch.")
