...
# async def ensure_valid_llm_response(runnable: Runnable, state: BaseModel | dict):
#     """
#     用途：确保 LLM 返回有效的回应
...
#     while True:
#         result = await runnable.ainvoke(state_dict)
...
#             state = {**state_dict, "messages": messages}
#         else:
#             break
#     return result
...
async def ensure_valid_llm_response_v2(runnable: Runnable, messages: list):
    ...
    messages2 = messages
    while True:
        result = await runnable.ainvoke(messages2)
        ...
        if not result.tool_calls and (
            ...
            messages2 = messages2 + [("user", "Respond with a real output.")]
            # state = {**state_dict, "messages": messages}
        else:
            break
    return result
