...
from mtmai.agents.article_gen import GenBookRequest
import asyncio
...
Here is some additional information about the author's desired goal for the book:\n\n {goal}"""),
    ...
    return BookOutline.model_validate( output)
...
@flow
async def flow_article_gen(goodbye=False):
    logger = get_run_logger()
    logger.info("开始编写大纲")

    # 创建大纲的逻辑
    logger = get_run_logger()
    logger.info("gen_book")
    # gen_book()
    state = GenBookRequest()
    ...
    for index, chapter_outline in enumerate(state.book_outline, start=1):
        logger.info(f"开始写章节正文 {index}/{len(state.book_outline)}: {chapter_outline.title}")
    ...
    logger.info(f"内容写入完成,数量:{len(chapters)}")
