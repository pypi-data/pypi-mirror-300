...
from mtmai.agents.states.research_state import Editor, InterviewState
...
async def _gen_queries(messages: list[AnyMessage]):
    ...
    ai_response = await ctx.call_model_chat(gen_queries_prompt, {})
    return Queries.model_validate(orjson.loads(ctx.repair_json(ai_response.content)))
...
async def _gen_answer(messages: list[AnyMessage]):
    ...
    return AnswerWithCitations.model_validate(
        orjson.loads(ctx.repair_json(ai_response.content))
    )
