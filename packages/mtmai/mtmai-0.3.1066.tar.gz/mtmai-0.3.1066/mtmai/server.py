...
def start_deamon_serve():
    ...
    if (
        ...
        if (
            ...
            mtutils.bash("prefect server start")
    ...
    #     logger.info("启动 嵌入的 worker 工作流")

    #     from mtmai.flows.hello_flow import flow_hello

    #     def start_flow_hello():
    #         flow_hello.serve(name="my-deployment3",
    #                     tags=["mtmai"],
    #                     parameters={},
    #                     interval=120)

    #     threading.Thread(target=start_flow_hello).start()
    #     # process.start()
