...
class LogWriter:
    ...
    def __sync_timer(self):
        while self.is_running:
            ...
            if self.config.flush_interval is None:
                raise ValueError("flush_interval is set to None. flush_interval has to be a number")
        ...
        try:
            ...
            if self.is_running_on_lambda():
                ...
                if self.can_access_filesystem():
                    ...
                    maximLogger.warning(f"Failed to push logs to server. Writing logs to file. Error: {e}")
                else:
                    for log in logs:
                        self.queue.put(log)
                        maximLogger.debug("Logs added back to queue for next attempt")
    ...
    def cleanup(self):
        ...
        if self.__flush_thread is not None:
