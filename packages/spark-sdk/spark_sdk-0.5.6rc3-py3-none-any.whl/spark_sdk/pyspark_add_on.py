...
def write_table_to_s3(spark_home, hadoop_home, hadoop_conf_dir,
                      endpoint, access_key, secret_key,
                      sql_query, s3_bucket_path, uris):
    """
    Function to write table to s3
    :param spark_home: "/opt/spark/spark-3.5.0-bin-hadoop3"
    :param sql_query: "select * from database.tbl limit 10"
    :param s3_bucket_path: s3a://bucket/dir/
    :param uris: thrift://abcd:9083
    :return: None, data writen
    """
    os.environ['SPARK_HOME'] = spark_home
    os.environ['HADOOP_HOME'] = hadoop_home
    os.environ['HADOOP_CONF_DIR'] = hadoop_conf_dir
    PS = PySpark(
        uris = ("spark.hadoop.hive.metastore.uris", uris),
        endpoint = ("spark.hadoop.fs.s3a.bucket.data-platform-labs.endpoint", endpoint),
        access_key = ("spark.hadoop.fs.s3a.bucket.data-platform-labs.access.key", access_key),
        secret_key = ("spark.hadoop.fs.s3a.bucket.data-platform-labs.secret.key", secret_key),
        )
    df = PS.spark.sql(f"""{sql_query}""")
