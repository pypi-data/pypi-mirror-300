...
def map_json(path):
    ...
    for key in object_lists.keys():
        ...
        for idx,go in enumerate(object_lists[key]):
            _str += f"self.{tab}{tab}go_{key}_{idx} = GameObject({go['position']},{go['angle']},\"{go['object_type']}\",r\"{go['img_path']}\",abs({round(go['scale_factor'],4)}));self.{key}.append(go_{key}_{idx})\n"
