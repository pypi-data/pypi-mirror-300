...
def add_multiplicative_gaussian_noise(df, series_id, column, column_flag = [], stddev=0.1, noise_type='proportional', row_axis_type = 'TIMECODE', row_axis = 'TD_TIMECODE'):
    ...
    def gen_query(df, series_id, column, stddev=0.1, noise_type='proportional', column_type = 'float'):
        """
    ...
    queries = [gen_query(df, series_id, column_, stddev_, noise_type_) for column_, stddev_, noise_type_ in
               zip(column, stddev, noise_type)] + \
            [gen_query(df, series_id, column_,  column_type='flag') for column_ in
     column_flag]
...
def add_missing_values(df, series_id, column, missing_ratio):
    ...
    def gen_query(df, series_id, column):
        """
    ...
    queries = [gen_query(df, series_id, column_) for column_ in column]
