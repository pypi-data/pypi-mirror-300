    ...
    def fuse(self, verbose=True):
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (Conv, Conv2, BeeF, DWConv)) and hasattr(m, "bn"):
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    if scales:
        ...
        if not scale:
            ...
            LOGGER.warning(f"ВНИМАНИЕ ⚠️ масштаб модели не пройден. При условии масштаба='{scale}'.")
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            args = [c1, c2, *args[1:]]
            if m in {BottleneckCSP, C1, C2, C2f, B2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB}:
