...
def energy_cal_gas(calculator, atoms_origin, F_CRIT_RELAX, save_path, gas_distance):
    while True:
        try:
            ...
            opt = LBFGS(atoms)
            opt.run(fmax=F_CRIT_RELAX, steps=2000)

            return atoms, atoms.get_potential_energy()

        except Exception as e:
            # If an error occurs, reduce gas_distance by 0.5 and try again
            print(f"Error occurred: {e}. Reducing gas_distance by 0.5 and retrying...")
            gas_distance -= 0.5
                ...
                if "gas" not in str(structure):
                    ...
                    if gas_tag in gas_energies:
                        ...
                        gas_CONTCAR, gas_energy = energy_cal_gas(
                            ...
                            F_CRIT_RELAX,
                            f"{save_directory}/gases/POSCAR_{gas_tag}",
                            gas_distance,
                        )
                        ...
                        write(f"{save_directory}/gases/CONTCAR_{gas_tag}", gas_CONTCAR)
...
def plotter_mono(DFT_values, NNP_values, NNP_name, tag, min_value, max_value, **kwargs):
...
def plotter_multi(ads_data, NNP_name, types, tag, min_value, max_value, **kwargs):
    colors = [
        "blue",
        "red",
        "green",
    ...
    ax.set_ylabel(f"{NNP_name} (eV)", fontsize=40)
...
def data_to_excel(main_data, outlier_data, NNPs_data, analysis_adsorbates, **kwargs):
...
def count_lbfgs_steps(log_path):
    ...
    for i, line in enumerate(reversed(lines)):
        if "Done!" in line:
            ...
            if previous_line.startswith("LBFGS:"):
...
def analysis_MLPs(**kwargs):
    ...
    for NNP_name in NNP_list:
        ...
        for reaction in NNP_result:
            ...
            if adsorbate in analysis_adsorbates:
                ...
                time_accum += sum(
                    value
                    for key, value in NNP_result[reaction]["final"].items()
                    if "time_total" in key
                )

                log_dir_path = f"{calculating_path}/{NNP_name}/log/{reaction}"
                ...
                if NNP_result[reaction]["outliers"]["slab_conv"]:
                    slab_conv += 1

                if NNP_result[reaction]["outliers"]["ads_conv"]:
                    ads_conv += 1

                if NNP_result[reaction]["outliers"]["slab_move"]:
                    slab_move += 1

                if NNP_result[reaction]["outliers"]["ads_move"]:
                    ads_move += 1

                if NNP_result[reaction]["outliers"]["slab_seed"]:
                    slab_seed += 1

                if NNP_result[reaction]["outliers"]["ads_seed"]:
                    ads_seed += 1

                if NNP_result[reaction]["outliers"]["ads_eng_seed"]:
                    ads_eng_seed += 1
        ...
        MAEs_normal_multi = plotter_multi(
            ads_data, NNP_name, ["normal"], "normal", min_value, max_value, **kwargs
        )
        MAEs_outlier_multi = plotter_multi(
            ads_data, NNP_name, ["outlier"], "outlier", min_value, max_value, **kwargs
        )
    ...
    data_to_excel(
        main_data, outlier_data, NNP_datas, list(analysis_adsorbates), **kwargs
    )
...
def analysis_MLPs_single(**kwargs):
    ...
    if os.path.exists(single_path):
        ...
        for NNP_name in NNP_single_list:
            print(NNP_name)
            with open(f"{single_path}/{NNP_name}/{NNP_name}_result.json", "r") as f:
                NNP_result = json.load(f)

            ads_data = {
                "all": {"DFT": np.array([]), "NNP": np.array([])},
            }
            ...
            for reaction in NNP_result:
                ...
                if adsorbate in analysis_adsorbates:
                    ...
                    ads_data[adsorbate]["all"]["NNP"] = np.append(
                        ads_data[adsorbate]["all"]["NNP"],
                        NNP_result[reaction]["SC_calc"]["ads_eng"],
                    )
                    ads_data["all"]["DFT"] = np.append(
                        ads_data["all"]["DFT"],
                        NNP_result[reaction]["cathub"]["ads_eng"],
                    )
                    ads_data["all"]["NNP"] = np.append(
                        ads_data["all"]["NNP"],
                        NNP_result[reaction]["SC_calc"]["ads_eng"],
                    )

            DFT_data = ads_data["all"]["DFT"]
            NNP_data = ads_data["all"]["NNP"]
            ...
            MAE_all = plotter_mono(
                DFT_data, NNP_data, NNP_name, "single", min_value, max_value, **kwargs
            )
            ...
            NNP_datas[NNP_name] = MAEs_all_multi
            total_num = len(DFT_data)

            main_data.append(
                {"NNP_name": NNP_name, "MAE (eV)": MAE_all, "Num_total": total_num}
            )

        data_to_excel_single(main_data, NNP_datas, list(analysis_adsorbates), **kwargs)
...
def execute_benchmark_single(calculator, **kwargs):
    ...
    save_directory = os.path.join(os.getcwd(), "result_single", NNP_name)
    print(f"Starting {NNP_name} Benchmarking")
    # Basic Settings==============================================================================
...
def data_to_excel_single(main_data, NNPs_data, analysis_adsorbates, **kwargs):
    benchmarking_name = kwargs.get("Benchmarking_name", os.path.basename(os.getcwd()))
