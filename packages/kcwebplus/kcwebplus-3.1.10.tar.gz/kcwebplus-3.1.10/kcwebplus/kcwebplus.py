...
def __get_cmd_par():
    ...
    if python_version[0:3]!='3.8':
        print("\033[1;31;40m kcwebplus-"+kcwebplus['version']+"依赖python3.8，与你现在的python"+python_version+"不兼容")
    try:
        ...
        try:
            ...
            gcs=''
        if gcs=='-v':
             print("kcwebplus-"+kcwebplus['version']) 
        else:
        ...
        if cmd_par['cli']:#通过命令行执行控制器的方法
            ...
            if get_sysinfo()['uname'][0]=='Linux':
                ...
                if __name__ == 'kcwebplus.kcwebplus':
                    ...
                    if types=='-stop':
                        os.system('pkill kcwebplus')
                    else:
                        os.system('kcwebplus intapp/index/pub/clistartplan --cli > app/runtime/log/server.log 2>&1 &')
                    ...
                    if cmd_par['plug']:
                        ...
                        if '应用创建成功' in t[1]:
                            if os.path.exists(cmd_par['project']):
                                ...
                                if not os.path.isfile("./"+cmd_par['project']+"/tempfile/cli.bat"):
                                    shutil.copy(remppath+'/tempfile/cli.py',cmd_par['project'])
