...
def to_fs_columns(dc: Array[DataContext]) -> FSharpList[FSharpList[FsCell]]:
    ...
    class ObjectExpr1193:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1192(x: str, y: str) -> bool:
                return x == y
    ...
    comment_keys: FSharpList[str] = to_list(distinct(collect(mapping_1, dc), ObjectExpr1193()))
    ...
    def _arrow1211(__unit: None=None, dc: Any=dc) -> IEnumerable_1[FSharpList[FsCell]]:
        def _arrow1210(__unit: None=None) -> IEnumerable_1[FSharpList[FsCell]]:
            def _arrow1209(dc_4: DataContext) -> FSharpList[FsCell]:
                dc_3: DataContext = dc_4
                def _arrow1208(__unit: None=None) -> IEnumerable_1[FsCell]:
                    def _arrow1200(__unit: None=None) -> FSharpList[FsCell]:
                    ...
                    def _arrow1207(__unit: None=None) -> IEnumerable_1[FsCell]:
                        def _arrow1206(__unit: None=None) -> IEnumerable_1[FsCell]:
                            def _arrow1205(__unit: None=None) -> IEnumerable_1[FsCell]:
                                def _arrow1204(__unit: None=None) -> IEnumerable_1[FsCell]:
                                    def _arrow1203(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        def _arrow1202(__unit: None=None) -> IEnumerable_1[FsCell]:
                                        ...
                                        return append(create_text(DataContext__get_GeneratedBy(dc_3)), delay(_arrow1202))

                                    return append(create_text(DataContext__get_Description(dc_3)), delay(_arrow1203))

                                return append(create_term(DataContext__get_ObjectType(dc_3)), delay(_arrow1204))

                            return append(create_term(DataContext__get_Unit(dc_3)), delay(_arrow1205))

                        return append(create_term(DataContext__get_Explication(dc_3)), delay(_arrow1206))

                    return append(_arrow1200(), delay(_arrow1207))

                return to_list(delay(_arrow1208))

            return map_1(_arrow1209, dc)

        return append(singleton_1(headers), delay(_arrow1210))

    return transpose(to_list(delay(_arrow1211)))
