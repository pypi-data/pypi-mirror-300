...
from ..fable_modules.fable_library.seq import (iterate_indexed, delay, append, singleton, map, exists, head, try_head, try_find, choose, try_pick, is_empty as is_empty_1, iterate)
...
def ArcAssay_toMetadataSheet(assay: ArcAssay) -> FsWorksheet:
    ...
    def _arrow1229(__unit: None=None, assay: Any=assay) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        assay_1: ArcAssay = assay
        def _arrow1228(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow1227(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                def _arrow1226(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    def _arrow1225(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        return to_rows_2("Assay Person", of_seq(assay_1.Performers))

                    return append(singleton(SparseRowModule_fromValues(to_enumerable(["ASSAY PERFORMERS"]))), delay(_arrow1225))

                return append(to_rows_1("Assay", singleton_1(assay_1)), delay(_arrow1226))

            return append(singleton(SparseRowModule_fromValues(to_enumerable(["ASSAY"]))), delay(_arrow1227))

        return delay(_arrow1228)

    iterate_indexed(action, _arrow1229())
    return sheet
        ...
        def loop(last_line_mut: str | None, assays_mut: FSharpList[ArcAssay], contacts_mut: FSharpList[Person], line_number_mut: int) -> ArcAssay:
            while True:
                ...
                if last_line is not None:
                    def _arrow1230(__unit: None=None, last_line: Any=last_line, assays: Any=assays, contacts: Any=contacts, line_number: Any=line_number) -> bool:
                        k: str = last_line
