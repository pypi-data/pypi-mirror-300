...
def encoder(id_map: Any | None, value: Value) -> IEncodable:
    if value.tag == 1:
        ...
        return ObjectExpr1840()



def _arrow1842(Item: int) -> Value:
    return Value(1, Item)


def _arrow1843(Item_1: float) -> Value:
    return Value(2, Item_1)


def _arrow1844(Item_2: OntologyAnnotation) -> Value:
    return Value(0, Item_2)


def _arrow1845(Item_3: str) -> Value:
    return Value(3, Item_3)


decoder: Decoder_1[Value] = one_of(of_array([map(_arrow1842, int_1), map(_arrow1843, float_1), map(_arrow1844, OntologyAnnotation_ISAJson_decoder), map(_arrow1845, string)]))
