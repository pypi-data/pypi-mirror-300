...
def encoder(dc: DataContext) -> IEncodable:
    ...
    def _arrow2376(oa: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa)

    def _arrow2382(oa_1: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_1)

    def _arrow2384(oa_2: OntologyAnnotation, dc: Any=dc) -> IEncodable:
        return OntologyAnnotation_encoder(oa_2)

    def _arrow2387(value: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2389(value_2: str, dc: Any=dc) -> IEncodable:
    ...
    def _arrow2392(value_4: str, dc: Any=dc) -> IEncodable:
    ...
    values: FSharpList[tuple[str, IEncodable]] = choose(chooser, of_array([("data", encoder_1(dc)), try_include("explication", _arrow2376, DataContext__get_Explication(dc)), try_include("unit", _arrow2382, DataContext__get_Unit(dc)), try_include("objectType", _arrow2384, DataContext__get_ObjectType(dc)), try_include("description", _arrow2387, DataContext__get_Description(dc)), try_include("generatedBy", _arrow2389, DataContext__get_GeneratedBy(dc)), try_include("label", _arrow2392, DataContext__get_Label(dc))]))
...
def _arrow2410(get: IGetters) -> DataContext:
    data: Data
    object_arg: IRequiredGetter = get.Required
    data = object_arg.Field("data", decoder_1)
    def _arrow2402(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_1: IOptionalGetter = get.Optional
        return object_arg_1.Field("explication", OntologyAnnotation_decoder)

    def _arrow2403(__unit: None=None) -> OntologyAnnotation | None:
        object_arg_2: IOptionalGetter = get.Optional
        return object_arg_2.Field("unit", OntologyAnnotation_decoder)

    def _arrow2404(__unit: None=None) -> OntologyAnnotation | None:
    ...
    def _arrow2408(__unit: None=None) -> str | None:
    ...
    return DataContext__ctor_Z780A8A2A(data.ID, data.Name, data.DataType, data.Format, data.SelectorFormat, _arrow2402(), _arrow2403(), _arrow2404(), _arrow2405(), _arrow2408(), _arrow2409(), data.Comments)


decoder: Decoder_1[DataContext] = object(_arrow2410)
