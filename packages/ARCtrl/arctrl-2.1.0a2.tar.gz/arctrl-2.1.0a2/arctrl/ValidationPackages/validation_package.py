...
class ValidationPackage:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackage = self
        def _arrow2761(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2760(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2759(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2758(__unit: None=None) -> IEnumerable_1[str]:
                        return singleton("}")

                    return append(singleton((" Version = " + value_1(this.Version)) + "") if (this.version is not None) else empty(), delay(_arrow2758))

                return append(singleton((" Name = " + this.Name) + ""), delay(_arrow2759))

            return append(singleton("{"), delay(_arrow2760))

        return join("\n", to_list(delay(_arrow2761)))
...
ValidationPackage_reflection = _expr2762
