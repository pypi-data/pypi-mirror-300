...
Init_reflection = _expr2971

def _expr2972() -> TypeInfo:
...
def Clone_createCloneContract_5000466F(remote_url: str, merge: bool | None=None, branch: str | None=None, token: tuple[str, str] | None=None, nolfs: bool | None=None) -> Contract:
    ...
    def _arrow2978(__unit: None=None, remote_url: Any=remote_url, merge: Any=merge, branch: Any=branch, token: Any=token, nolfs: Any=nolfs) -> IEnumerable_1[str]:
        def _arrow2977(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2976(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2975(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2974(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2973(__unit: None=None) -> IEnumerable_1[str]:
                            return singleton(".") if merge_1 else empty()

                        return append(singleton(remote_url_1), delay(_arrow2973))

                    return append(singleton(value(branch)) if (branch is not None) else empty(), delay(_arrow2974))

                return append(singleton(Clone_get_branchFlag()) if (branch is not None) else empty(), delay(_arrow2975))

            return append(singleton(Clone_get_noLFSConfig()) if nolfs_1 else empty(), delay(_arrow2976))

        return append(singleton(Clone_get_clone()), delay(_arrow2977))

    return create_git_contract(to_array(delay(_arrow2978)))
