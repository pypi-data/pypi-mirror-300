...
TemplateReadError_reflection = _expr1258
...
Metadata_Template_TemplateInfo_reflection = _expr1259
...
def Metadata_Template_TemplateInfo_ToSparseTable_48C39BE1(template: Template) -> SparseTable:
    ...
    def _arrow1260(__unit: None=None, template: Any=template) -> str:
        copy_of_struct: str = template.Id
        return str(copy_of_struct)

    def _arrow1261(__unit: None=None, template: Any=template) -> str:
        copy_of_struct_1: str = template.Id
        return str(copy_of_struct_1)

    add_to_dict(matrix.Matrix, ("Id", 1), "" if (_arrow1260().find("MISSING_IDENTIFIER_") == 0) else _arrow1261())
    ...
    class ObjectExpr1263:
        @property
        def Equals(self) -> Callable[[str, str], bool]:
            def _arrow1262(x: str, y: str) -> bool:
                return x == y
    ...
    return SparseTable(matrix.Matrix, matrix.Keys, reverse(List_distinct(comment_keys, ObjectExpr1263())), matrix.ColumnCount)
...
def Metadata_Template_toRows(template: Template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
    def _arrow1271(__unit: None=None, template: Any=template) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
        def _arrow1270(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
            def _arrow1269(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                def _arrow1268(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                    def _arrow1267(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                        def _arrow1266(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                            def _arrow1265(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                def _arrow1264(__unit: None=None) -> IEnumerable_1[IEnumerable_1[tuple[int, str]]]:
                                    return to_rows_1("Author", of_seq(template.Authors))

                                return append(singleton(SparseRowModule_fromValues(to_enumerable(["AUTHORS"]))), delay(_arrow1264))

                            return append(Metadata_Tags_toRows(None, to_list(template.Tags)), delay(_arrow1265))

                        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TAGS"]))), delay(_arrow1266))

                    return append(Metadata_ER_toRows(None, to_list(template.EndpointRepositories)), delay(_arrow1267))

                return append(singleton(SparseRowModule_fromValues(to_enumerable(["ERS"]))), delay(_arrow1268))

            return append(Metadata_Template_TemplateInfo_toRows_48C39BE1(template), delay(_arrow1269))

        return append(singleton(SparseRowModule_fromValues(to_enumerable(["TEMPLATE"]))), delay(_arrow1270))

    return delay(_arrow1271)
...
def Template_fromFsWorkbook(doc: FsWorkbook) -> Template:
    ...
    def _arrow1273(__unit: None=None, doc: Any=doc) -> ArcTable:
    ...
    return Template_fromParts(template_info, pattern_input[1], pattern_input[2], pattern_input[3], _arrow1273(), now())
