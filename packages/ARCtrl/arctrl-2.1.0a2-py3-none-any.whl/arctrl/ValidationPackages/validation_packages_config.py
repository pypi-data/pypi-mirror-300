...
class ValidationPackagesConfig:
    ...
    def __str__(self, __unit: None=None) -> str:
        this: ValidationPackagesConfig = self
        def _arrow2860(__unit: None=None) -> IEnumerable_1[str]:
            def _arrow2859(__unit: None=None) -> IEnumerable_1[str]:
                def _arrow2858(__unit: None=None) -> IEnumerable_1[str]:
                    def _arrow2857(__unit: None=None) -> IEnumerable_1[str]:
                        def _arrow2855(__unit: None=None) -> IEnumerable_1[str]:
                            def _arrow2854(__unit: None=None) -> IEnumerable_1[str]:
                                return singleton("}")

                            return append(singleton("]"), delay(_arrow2854))

                        return append(singleton(join((";" + "\n") + "", map(to_string, this.ValidationPackages))), delay(_arrow2855))

                    return append(singleton(" ValidationPackages = ["), delay(_arrow2857))

                return append(singleton((" ARCSpecification = " + value(this.ARCSpecification)) + "") if (this.ARCSpecification is not None) else empty(), delay(_arrow2858))

            return append(singleton("{"), delay(_arrow2859))

        return join("\n", to_list(delay(_arrow2860)))
    ...
    def StructurallyEquals(self, other: ValidationPackagesConfig) -> bool:
        ...
        def sort(arg: Array[ValidationPackage]) -> Array[ValidationPackage]:
            ...
            return sort_by(projection, list(arg), ObjectExpr2861())
        ...
        if equals(this.ARCSpecification, other.ARCSpecification):
            ...
            def _arrow2863(__unit: None=None) -> IEnumerable_1[bool]:
                def _arrow2862(i: int) -> bool:
                    return equals(item(i, a), item(i, b))

                return map(_arrow2862, range_big_int(0, 1, length(a) - 1))

            return fold(folder, True, to_list(delay(_arrow2863))) if (length(a) == length(b)) else False
...
ValidationPackagesConfig_reflection = _expr2871
