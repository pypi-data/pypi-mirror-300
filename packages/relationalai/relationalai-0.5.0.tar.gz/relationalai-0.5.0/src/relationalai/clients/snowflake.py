...
from typing import Any, Dict, Iterable, Literal, Tuple, List, cast
...
from ..errors import EngineAutoCreateFailed, EngineNotFoundException, EnginePending, Errors, RAIException, HexSessionException, SnowflakeAppMissingException, SnowflakeChangeTrackingNotEnabledException, SnowflakeImportMissingException, SnowflakeInvalidSource, SnowflakeRaiAppNotStarted, SnowflakeTableObject, SnowflakeTableObjectsException
...
class Resources(ResourcesBase):
    ...
    def create_engine(self, name:str, size:str|None=None, pool:str|None=None):
        if size is None:
            size = self.config.get("engine_size", None) or "HIGHMEM_X64_S"
        try:
    ...
    def create_engine_async(self, name:str, size:str|None=None):
        if size is None:
            size = self.config.get("engine_size", None) or "HIGHMEM_X64_S"
        try:
            with debugging.span("create_engine", name=name, size=size):
                self._exec(f"call {APP_NAME}.api.create_engine_async('{name}', '{size}');")
        except Exception as e:
            raise Exception(f"Failed to create engine {name} with size {size}: {e}") from e
    ...
    def clone_graph(self, target_name:str, source_name:str, nowait_durable=True):
    ...
    def get_export_code(self, params: ExportParams):
        ...
        with importlib.resources.open_text(
            __package__, "export_procedure.py.jinja"
        ) as f:
            template = f.read()
    ...
    def create_import_stream(self, source:ImportSource, model:str, rate = 1, options: dict|None = None):
        ...
        try:
            ...
            if "ensure that CHANGE_TRACKING is enabled on the source object" in str(e):
                print("\n")
                exception = SnowflakeChangeTrackingNotEnabledException(object)
                raise exception from None
            raise e
    ...
    def _exec_async_v2(
        ...
        with debugging.span("transaction") as txn_span:
            with debugging.span("create_v2"):
                ...
                use_graph_index = self.config.get("use_graph_index", False)
    ...
    def auto_create_engine(self, name: str | None = None):
        with debugging.span("auto_create_engine"):
            engine_name = name or self.get_user_based_engine_name()
            try:
    ...
    def auto_create_engine_async(self, name: str | None = None):
        with Spinner(
            ...
            with debugging.span("auto_create_engine_async"):
                engine_name = name or self.get_user_based_engine_name()
                try:
                    engine = self.get_engine(engine_name)
                    # if engine is gone, delete it and create new one
                    ...
                    if engine:
                        if engine["state"] == "PENDING":
                            ...
                            spinner.update_messages(
                                {
                                    "finished_message": f"Engine {engine_name} ready",
                                }
                            )
                            pass
                        elif engine["state"] == "GONE":
                            ...
                            try:
                                ...
                                # if engine is already deleted, we will get an exception
                                # we can ignore this exception and create a new engine asynchrounously
                                if isinstance(e, EngineNotFoundException):
                                    engine = None
                                    pass
                                else:
                    ...
                    if not engine:
                        ...
                        spinner.update_messages(
                            {
                                "finished_message": f"Starting engine {engine_name} in background",
                            }
                        )
                except Exception as e:
                    spinner.update_messages(
...
class SnowflakeTable(dsl.Type):
    ...
    def _lazy_init(self):
        ...
        if name not in parent._imported:
            ...
            if name not in parent._imported:
                use_graph_index = self._model._config.get("use_graph_index", False)
                if use_graph_index:
...
class SnowflakeClient(Client):
    def create_database(self, isolated=True, nowait_durable=True):
        ...
        # currently use_index support only tables and not views. Therefore, it could be safely hardcoded to 'TABLE',
        # because only tables could be passed as sources
        sources_object_references = [
            f"{app_name}.api.object_reference('TABLE', '{source}')"
            for source in sources
        ]
        sources_object_references_str = ", ".join(sources_object_references)

        model = self._source_database
        engine_name = self.resources.get_user_based_engine_name()

        with debugging.span("poll_use_index"):
            self.poll_use_index(
                app_name, sources_object_references_str, model, engine_name
            )
    ...
    def poll_use_index(
        self, app_name, sources_object_references_str, model, engine_name
    ):
        counter = 1
        ...
        def check_ready():
            ...
            with debugging.span(f"use_index_call_{counter}"):
                ...
                if ready:
                    ...
                    spinner.update_messages(
                        {
                            "finished_message": "Everything is setup and ready to go!",
                        }
                    )
                ...
                if not ready and all_data:
                    for data in all_data:
                        ...
                        if data.get("DATA_SYNC_STATUS") == "CREATED":
                            spinner.update_messages(
                                {
                                    "message": f"Table {data.get('FQ_OBJECT_NAME')} is created, waiting for sync to start...",
                                }
                            )
                        if (
                            ...
                            if batches_count:
                                spinner.update_messages(
                                    {
                                        "message": f"Table {data.get('FQ_OBJECT_NAME')} is syncing, remained batches: {data.get('PENDING_BATCHES_COUNT')}",
                                    }
                                )
                            else:
                                spinner.update_messages(
                                    {
                                        "message": f"Table {data.get('FQ_OBJECT_NAME')} is just started syncing...",
                                    }
                                )
                ...
                if not ready and engines:
                    for engine in engines:
                        if (
                            ...
                            message = f"{writer and 'Writer engine' or 'Engine'} {engine.get('name')} is pending, waiting for it to be ready..."
                            spinner.update_messages(
                                {
                                    "message": message,
                                }
                    ...
                    for error in errors:
                        if error.get("type") == "data":
                            if (
                                ...
                                tables_with_not_enabled_change_tracking.append(
                                    error.get("source")
                                )
                            else:
                                table_objects_with_other_errors.append(
        ...
        with Spinner("Indexing your data...") as spinner:
            poll_with_specified_overhead(check_ready, overhead_rate=0.1, max_delay=0.5)
...
def Graph(
    ...
    *,
    ...
    client_class = Client
    use_graph_index = config.get("use_graph_index", False)
