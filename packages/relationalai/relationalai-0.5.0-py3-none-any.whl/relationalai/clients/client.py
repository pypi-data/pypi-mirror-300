...
from .types import AvailableModel, EngineState, Import, ImportSource, ImportsStatus
...
class ResourcesBase(ABC):
    ...
    @abstractmethod
    def create_engine(self, name: str, size: str, pool:str|None=None) -> dict | None:
        pass
...
class ProviderBase(ABC):
    ...
    def create_engine(self, name:str, size:str):
        return self.resources.create_engine(name, size)
...
class Client():
    def __init__(
        ...
        use_graph_index = config.get("use_graph_index", False)
        if not dry_run and use_graph_index:
            try: 
                self.resources.auto_create_engine_async()
    ...
    def install_raw(self, code:str, name:str|None="pyrel_batch_0"):
        if not name:
            name = "pyrel_batch_0"
        task = dsl.build.raw_task(code)
        debugging.set_source(task)
    ...
    def _query(self, code:str, task:m.Task|None, end_span, readonly=True, raw_results=False, inputs={}, nowait_durable=None, headers={}, abort_on_error=True):
    ...
    def query(self, task:m.Task, rentrant=False, readonly=True, raw_results=False, inputs={}, format="pandas", tag=None, nowait_durable=None, headers={}, internal=False, abort_on_error=True) -> DataFrame|Any:
        ...
        with debugging.span("query", model=self._database, task=task, tag=tag, internal=internal) as end_span:
            ...
            start = time.perf_counter()
            if format == "pandas":
                results = self._query(code, task, end_span, readonly=readonly, raw_results=raw_results, inputs=inputs, nowait_durable=nowait_durable, headers=headers, abort_on_error=abort_on_error)
    ...
    def _install_batch_flush(self):
        ...
        if not self.dry_run:
            with debugging.span("install_batch", model=self._database) as end_span:
                ...
                dataframe = self._query(install_code, None, end_span, readonly=False, raw_results=True, abort_on_error=True)
