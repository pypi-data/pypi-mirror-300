...
def call_get_similar(user, vault, api_key, openai_key, text, num_items=4, include_distances=False, verbose=False, embeddings=None):
    url = f"{API_BASE_URL}/get_similar"
    payload = {
        'user': user,
        'vault': vault,
        'api_key': api_key,
...
def run_flow(flow_name, message, history='', user_email='', user_key='', ai_key='', vault='home', 
             conversation_user_id=None, groq_api=None, chat_ai='openai', fine_tuned_model=False):
...
def run_flow_stream(flow_name, message, history='', user_email='', user_key='', ai_key='', vault='home', 
                    conversation_user_id=None, groq_api=None, chat_ai='openai', fine_tuned_model=False):
