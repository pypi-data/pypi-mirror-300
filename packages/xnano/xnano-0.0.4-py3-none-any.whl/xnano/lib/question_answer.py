...
from .. import Completions, InstructorMode
...
def _qa(
    ...
    Args:
        ...
        chunk_size (int): The size of each chunk when processing large texts.
    ...
    chunks = chunker(input_text, chunk_size=chunk_size, model=model)
    ...
    def process_chunk(chunk):
        # Generate questions
        questions = generate_questions(completion_client, chunk, num_questions, model, mode, max_retries, temperature, progress_bar)
        
        # Generate answers for each question
        qa_pairs = generate_answers(completion_client, chunk, questions, model, mode, max_retries, temperature, progress_bar)
...
def generate_questions(completion_client, context, num_questions, model, mode, max_retries, temperature, progress_bar):
    system_message = (
...
def generate_answers(completion_client, context, questions, model, mode, max_retries, temperature, progress_bar):
    qa_pairs = []
...
def qa(
    ...
    Returns:
        Dataset: A dataset containing generated questions and answers.
    """
    return _qa(input_text, num_questions, chunk_size, model, api_key, base_url, organization, temperature, mode, max_retries, client, progress_bar, verbose)
