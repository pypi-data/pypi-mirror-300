...
__plugin_meta__ = PluginMetadata(
    ...
    extra=PluginExtraData(
        ...
        admin_level=ZxpmConfig.zxpm_ban_level,
        superuser_help="""
        超级管理员额外命令
        格式:
        ban [At用户/用户Id] ?[-t [时长]]
...
_ban_matcher = on_alconna(
    ...
    rule=admin_check(ZxpmConfig.zxpm_ban_level),
    priority=5,
    block=True,
)
...
async def _(
    ...
    if (gid := session.id3 or session.id2) and not group_id.available:
        if (
            ...
            _duration_text = "半 分钟"
            logger.info("尝试ban Bot 反被拿下", arparma.header_result, session=session)
