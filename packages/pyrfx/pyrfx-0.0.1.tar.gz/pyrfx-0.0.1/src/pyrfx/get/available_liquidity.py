...
class GetAvailableLiquidity(GetData):
    ...
    def _get_data_processing(self) -> dict[str, dict[str, float]]:
        ...
        for market_key in self.markets.info:
            ...
            market_symbol: str = self.markets.get_market_symbol(market_key)
            ...
            (long_pool_amount, long_reserve_factor, long_open_interest_reserve_factor) = self.get_max_reserved_usd(
                market=market_key, token=self._long_token_address, is_long=True
            )
            ...
            (short_pool_amount, short_reserve_factor, short_open_interest_reserve_factor) = self.get_max_reserved_usd(
                market=market_key, token=self._short_token_address, is_long=False
            )
            ...
            token_price: float = self._get_token_price(self._long_token_address, oracle_precision)
    ...
    def get_max_reserved_usd(self, market: str, token: str, is_long: bool) -> tuple[Any, Any, Any]:
        ...
        try:
            ...
            pool_amount_hash = pool_amount_key(market, token)
            reserve_factor_hash = reserve_factor_key(market, is_long)
            open_interest_reserve_factor_hash = open_interest_reserve_factor_key(market, is_long)

            pool_amount = datastore.functions.getUint(pool_amount_hash)
            reserve_factor = datastore.functions.getUint(reserve_factor_hash)
            open_interest_reserve_factor = datastore.functions.getUint(open_interest_reserve_factor_hash)
    ...
    def _log_liquidity(
        ...
        self.logger.info(f"Available Long Liquidity: ${numerize.numerize(long_liquidity)}")
        ...
        self.logger.info(f"Available Short Liquidity: ${numerize.numerize(short_liquidity)}")
