...
class GetFundingFee(GetData):
    ...
    def _load_open_interest_data(self) -> dict[str, Any]:
        ...
        if self.use_local_datastore:
            open_interest_file: Path = (
                Path(__file__).parent.parent / "data_store" / f"{self.config.chain}_open_interest.json"
            )
            ...
            return OpenInterest(config=self.config).get_data(to_json=False)
