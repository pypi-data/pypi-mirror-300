...
class SkinLesionClassifier:
    ...
    def __init__(self):
        ...
        self.cr_test_path = os.path.join(current_dir, "cr_test.pkl")
        ...
        self.props = self.load_model_props()
    ...
    def load_cr_test(self):
        if os.path.exists(self.cr_test_path):
            with open(self.cr_test_path, "rb") as f:
                return pickle.load(f)
        else:
            raise FileNotFoundError(
                f"Model test performance' file not found: {self.cr_test_path}"
            )
