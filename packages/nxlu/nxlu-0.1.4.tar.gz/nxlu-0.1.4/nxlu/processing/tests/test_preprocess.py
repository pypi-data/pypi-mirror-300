...
def test_symmetrize_graph(weighted_graph):
    ...
    # Updated the match string to reflect the actual error message
    with pytest.raises(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        SubgraphSelectionConfig(hubness="betweenness", sd_cutoff=1),
    ],
)
def test_subgraph_selection(varied_graph, config):
    ...
    assert isinstance(result, nx.Graph)
    if varied_graph.number_of_nodes() > 0:
        assert (
            result.number_of_nodes() >= 0
        )  # Allow for empty subgraph if no nodes meet criteria
    if config.use_lcc:
        assert nx.is_connected(result) or result.number_of_nodes() == 0
    if config.hubness:
        assert result.number_of_nodes() <= varied_graph.number_of_nodes()
