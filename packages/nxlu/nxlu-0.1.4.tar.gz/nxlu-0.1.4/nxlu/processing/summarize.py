...
def characterize_graph(
    ...
    truncated_node_set = {node[0] for node in truncated_nodes}  # Extract node IDs

    # Get all edges where both nodes are in the truncated node set
    truncated_edges = [
        (u, v, d)
...
def format_numeric_value(value: float) -> Any:
    ...
    if isinstance(value, (int, float)):
        if abs(value) >= 1e10:
            return float("inf")  # Return as float instead of string
        if abs(value) <= 1e-4:
            return 0.0
...
def format_algorithm_results(results):
    ...
    def should_exclude(vals):
        # Exclude if all values are 0.0 or 1.0
    ...
    for alg, vals in edge_results.items():
        if isinstance(vals, list) and all(isinstance(v, (int, float)) for v in vals):
            ...
            # Non-numeric edge-specific results (e.g., list of tuples or sets)
