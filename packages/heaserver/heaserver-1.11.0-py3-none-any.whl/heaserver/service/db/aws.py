...
from ..util import async_retry, now
from .. import client
...
class S3(Database):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.__locks: dict[str | None, asyncio.Lock] = defaultdict(asyncio.Lock)
    ...
    async def __get_temporary_credentials(self, request: web.Request, credentials: AWSCredentials,
        ...
        async with self.__locks[credentials.id]:
            ...
            if credentials.has_expired(self.MAX_EXPIRATION_LIMIT if sub != CREDENTIALS_MANAGER_USER else self.MIN_EXPIRATION_LIMIT):
...
class AWSPermissionContext(HEAServerPermissionContext[AWSDesktopObjectTypeVar], ABC, Generic[AWSDesktopObjectTypeVar]):
    ...
    def __init__(self, request: Request, volume_id: str, actions: Sequence[str], **kwargs):
        ...
        self.__cache: TTLCache[bytes, list[Permission]] = TTLCache(maxsize=128, ttl=30)
    ...
    async def is_account_owner(self) -> bool:
        ...
        if self.__is_account_owner is None:
            self.__is_account_owner = await is_account_owner(self.request, self.volume_id)
    ...
    async def _simulate_perms(self, obj: AWSDesktopObjectTypeVar, actions: Sequence[str]) -> list[Permission]:
        ...
        try:
            logger.debug('Elevating privileges to check permissions')
