...
class ServiceError(BaseModel):
    ...
    def __init__(self, **kwargs):
        if "error_type" not in kwargs:
            kwargs["error_type"] = "INTERNAL"
        super().__init__(**kwargs)

    # noqa: E501
    error_type: StrictStr
    message: StrictStr = Field(description="A human-readable error message")
    error: Optional[StrictStr] = Field(default=None, description="Additional information about the error")
    __properties: ClassVar[List[str]] = ["error_type", "message", "error"]

    @field_validator("error_type")
    def error_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(["INTERNAL"]):
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        if not isinstance(obj, dict):
            if not obj.error_type:
                obj.error_type = "INTERNAL"
            return cls.model_validate(obj)

        _obj = cls.model_validate(
            {"error_type": obj.get("error_type"), "message": obj.get("message"), "error": obj.get("error")}
        )
        return _obj
