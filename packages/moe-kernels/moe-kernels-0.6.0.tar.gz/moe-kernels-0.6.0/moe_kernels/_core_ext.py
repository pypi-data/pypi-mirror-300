...
core_C_available = importlib.util.find_spec('._moe_kernels_ops', 'moe_kernels') is not None
...
if TYPE_CHECKING or not core_C_available:
    ...
    import moe_kernels._moe_kernels_ops  # noqa: F401

    ScalarType = torch.classes._moe_kernels_ops.ScalarType

    if hasattr(torch, "_library") and hasattr(torch._library, "register_fake_class"):
        # Needed for dynamo support of ScalarType.
        @torch._library.register_fake_class("_moe_kernels_ops::ScalarType")
        class FakeScalarType:
            ...
            def __obj_flatten__(self) -> Tuple[Tuple[str, Any], ...]:
                return torch.classes._moe_kernels_ops.ScalarType.__obj_flatten__(self.ScalarType)

            @classmethod
            def __obj_unflatten__(
                cls, flat_type: Tuple[Tuple[str, Any], ...]
            ) -> "ScalarType":
                return cls(torch.classes._moe_kernels_ops.ScalarType.__obj_unflatten__(flat_type))
