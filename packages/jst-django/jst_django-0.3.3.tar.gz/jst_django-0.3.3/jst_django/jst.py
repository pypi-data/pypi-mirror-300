...
class Config:
    ...
    def register_app(self, app_name, class_name=None):
        app_module = f"core.apps.{app_name}"

        if class_name:
            class_name_path = f"{app_module}"
            self.config.setdefault("modules", []).append(class_name_path)
            self.write()
...
def install_module(
    ...
    module: str = typer.Option(prompt=True, default="default"),
):
...
@app.command(name="create:project", help="Yangi loyiha yaratish")
def create_project(
...
def version_callback(value: bool):
    if value:
        print("[green bold]MyApp version {}[/green bold]".format("0.3.3"))
        raise typer.Exit()


@app.callback(invoke_without_command=True)
def init(version: bool = typer.Option(None, "--version", callback=version_callback, is_eager=True, help="Show the application's version and exit.")):
    typer.echo("Running the main command")
