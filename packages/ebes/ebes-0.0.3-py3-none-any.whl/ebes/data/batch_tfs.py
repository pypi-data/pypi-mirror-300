...
class RandomEventsPermutation(BatchTransform):
    ...
    Params:
        keep_last: if True the last event remains on its place, other are permuted
    """
...
class RescaleTime(BatchTransform):
    ...
    Params:
        loc: location to subtract from time.
        scale: scale to divide time by.
    """
...
class TimeToFeatures(BatchTransform):
    ...
    Params:
        time_name: name of new feature with time, default "time".
        diff: whether to add time difference instead of time. The first difference
...
class DatetimeToFloat(BatchTransform):
    ...
    Args:
        loc: location to subtract. If string is passed, it is converted to
            np.datetime64.
        scale: scale to divide time by. If tuple is passed, it is passed to the
            np.timedelta64 function. The first item is a value and the second is a unit.
    """
...
class ForwardFillNans(BatchTransform):
    ...
    Params:
        backward: wether to do backward fill after the forwad fill.
    """
...
class FillNans(BatchTransform):
    ...
    Params:
        fill_value: if float, all NaNs in all numerical features will be replaced with
            the `fill_value`. Mapping sets feature-specific replacement values.
    """
...
class RandomSlices(BatchTransform):
    ...
    Params:
        split_count: how many sample slices to draw for each input sequence.
        cnt_min: minimal sample sequence length.
        cnt_max: maximal sample sequence length.
        short_seq_crop_rate: float from (0, 1]. If `short_seq_crop_rate` < 1, and if a
