...
from pyspark.sql.functions import array, lit, struct, expr, concat_ws, collect_list, col, trim, regexp_replace, lower, \
    upper, initcap, when, isnan
...
# todo: move this method from basics to here
# def clean_data(
#         df: DataFrame,
#         columns: list,
#         replaceBlankForTextFields: bool = False,
#         replaceZeroForNumberFields: bool = False,
...
#         null_operation: str = None  # Add null operation flag
# ) -> DataFrame:
#     # Step 1: Apply data cleansing operations
#     for col_name in columns:
#         # Replace blank text fields with None
#         # TODO: ask user for the value to replace null with
#         if replaceBlankForTextFields:
#             df = df.withColumn(col_name, when(col(col_name) == None, '').otherwise(col(col_name)))
#
#         # Replace 0 for number fields
#         if replaceZeroForNumberFields:
#             df = df.withColumn(col_name, when(col(col_name) == None, 0).otherwise(col(col_name)))
#
#         # Trim whitespace
#         if trimWhiteSpace:
...
#             df = df.withColumn(col_name, regexp_replace(col(col_name), r'\s+', ''))
#
#         # Clean letters (remove non-letter characters)
#         if cleanLetters:
#             df = df.withColumn(col_name, regexp_replace(col(col_name), r'[^A-Za-z]', ''))
#
#         # Clean punctuations (remove punctuation characters, not word[a-zA-Z0-9) or space
#         if cleanPunctuations:
#             df = df.withColumn(col_name, regexp_replace(col(col_name), r'[^\w\s]', ''))
#
...
#
#     # Step 2: Handle null_operation flag
...
#                           df.filter(df[col_name].isNotNull() | isnan(df[col_name])).count() > 0]
