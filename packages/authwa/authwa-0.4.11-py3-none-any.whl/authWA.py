...
DBs = ['MSSQL','SQLITE','POSTGRESSQL','MySQL' ]
...
class WA:
    ...
    def sql_connection(self, uid:str, pwd:str, server:str, database:str, driver:str):
...
class Parcial:
    def __init__(self, uid, pwd, server, db, driver, hora_inicio = 0, hora_final = 23,):
        self.hora_inicio = hora_inicio
        self.hora_final = hora_final
        
        if uid and pwd and server and db:
            self.engine = self.whats.sql_connection(uid, pwd, server, db, driver)
            self.whats = WA(self.engine)
        else: self.whats = WA()
...
if __name__ == '__main__':
    p = Parcial('guilherme.breve','Gtaiv@130520','10.56.6.56','Vista_Replication_PRD', MSSQL[0])
