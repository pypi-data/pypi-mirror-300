...
class BlockchainClient:
    ...
    def fetch_latest_transaction(self, app_id ):
        """Fetch the latest transaction for a given address."""
        try:
            response = self.indexer_client.search_transactions(application_id= app_id)
            transactions = response["transactions"]

            latest_transaction_details = {}

            # Process only the first transaction (latest)
            for txn in transactions:
                global_state_delta = txn.get('global-state-delta')
                if global_state_delta:
                    for delta in global_state_delta:
                        ...
                        latest_transaction_details[decoded_attribute] = decoded_value
                    break
            return latest_transaction_details
        except Exception as e:
            print(f"Error occurred: {e}")
            return None
