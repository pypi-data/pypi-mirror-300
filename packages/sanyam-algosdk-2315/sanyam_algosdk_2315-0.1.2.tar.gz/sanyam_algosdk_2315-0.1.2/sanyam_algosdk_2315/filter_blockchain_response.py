...
class BlockchainClient:
    ...
    def fetch_latest_transaction_string(self, app_id ):
        ...
        try:
            ...
            global_data = transactions[-1].get("global-state-delta")
            attribute = global_data[0].get("key")
            decoded_attribute = base64.b64decode(attribute.encode()).decode("utf-8")
            value = global_data[0].get("value").get("bytes")
            decoded_value = base64.b64decode(value.encode()).decode("utf-8")
            return {"attribute":decoded_attribute , "value":decoded_value}
    ...
    def fetch_latest_transaction_integer(self,app_id):
        ...
        try:
            ...
            attribute = global_data[0].get("key")
            decoded_attribute = base64.b64decode(attribute.encode()).decode("utf-8")
            value = global_data[0].get("value").get("uint")
            return {"attribute":decoded_attribute , "value":value}
