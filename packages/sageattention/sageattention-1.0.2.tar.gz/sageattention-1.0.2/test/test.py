...
q = (torch.randn((B, H, S, D), dtype=torch.bfloat32, device="cuda"))
k = torch.randn((B, H, S, D), dtype=torch.bfloat32, device="cuda") + torch.randn((B, H, S, D), dtype=dtype, device="cuda") * 200 * (torch.randn((B, H, S, D), dtype=dtype, device="cuda") > 1.5)
...
def test_acc(is_causal):
    torch_stand = torch.nn.functional.scaled_dot_product_attention(q.to(torch.float16), k.to(torch.float16), v.to(torch.float16),is_causal=is_causal)
