...
class Pipeline:
    def __init__(self, name: str, QA_flag: str = True):
        """
    ...
    def get_task_table(self, task_name):
        """
        Retrieves the temporary table created by a previous task and returns it as a pandas DataFrame.
        
        Args:
            task_name (str): The name of the task whose temporary table you want to retrieve.
        
        Returns:
            pd.DataFrame: The data from the temporary table as a DataFrame.
        
        Raises:
            ValueError: If the temporary table for the specified task is not found.
        """
        # Check if the task exists in the temp_tables dictionary
        if task_name not in self.temp_tables:
            raise ValueError(f"Temporary table for task '{task_name}' not found.")
        
        temp_table_name = self.temp_tables[task_name]
        
        # Query the temporary table and return as DataFrame
        query = f"SELECT * FROM `{temp_table_name}`"
        self.logger.log(f"Retrieving data from temporary table '{temp_table_name}' for task '{task_name}'.")
        
        try:
            df = self.client.query(query).to_dataframe()
            return df
        except Exception as e:
            self.logger.log(f"Failed to retrieve data from temporary table '{temp_table_name}': {str(e)}", 'error')
            raise
    ...
    def execute_python_task(self, task):
        self.logger.log(f"Executing Python task '{task.name}'")
        try:
            if not callable(task.python_callable):
                raise TaskError(f"Python callable for task '{task.name}' is not callable or not provided.")

            # Import 'types' to manipulate function
            import types

            func = task.python_callable

            # Copy the function's globals to modify them
            func_globals = func.__globals__.copy()

            # Check if any instance of 'Pipeline' exists in the globals
            pipeline_instance = None
            for var_name, var_value in func_globals.items():
                if isinstance(var_value, Pipeline):
                    pipeline_instance = var_value
                    break

            # If no Pipeline instance is found, inject the current one
            if not pipeline_instance:
                self.logger.log("No Pipeline instance found in function globals. Injecting the current Pipeline instance.")
                func_globals['pipeline'] = self
            else:
                self.logger.log(f"Pipeline instance '{var_name}' already exists in function globals. Using it.")
            ...
            # Execute the function without arguments and keyword arguments
            result = wrapped_func()
            task.result = result

            # Mark the task as completed
            self.update_status(task, 'Completed')
            self.logger.log(f"Python task '{task.name}' completed successfully.")

        except Exception as e:
            error_message = f"Python task '{task.name}' failed with exception: {str(e)}"
            self.logger.log(error_message, 'error')
    ...
    def clone_datasets(self, datasets, exclude_tables=[]):
        """
        ...
        for row in datasets_result:
            source_dataset = f'aic-production-core.{row["prod_dataset"]}'
            target_dataset = f'aic-production-core.{row["qa_dataset"]}'
