...
class Finic:
    ...
    def save_browser_context(self, context: BrowserContext, browser_id: str):
        if self.environment == FinicEnvironment.LOCAL:
            path = os.path.join(os.getcwd(), f"browser_{browser_id}.json")
    ...
    def get_browser_context(self, browser_id: str) -> Optional[Dict[str, Any]]:
        if self.environment == FinicEnvironment.LOCAL:
            path = os.path.join(os.getcwd(), f"browser_{browser_id}.json")
            if os.path.exists(path):
                with open(path, 'r') as f:
    ...
    def get_session_results(self, session_id: Optional[str] = None) -> Dict:
        if self.environment == FinicEnvironment.LOCAL:
            path = os.path.join(os.getcwd(), f"results.json")
            with open(path, 'r') as f:
                return json.load(f)
