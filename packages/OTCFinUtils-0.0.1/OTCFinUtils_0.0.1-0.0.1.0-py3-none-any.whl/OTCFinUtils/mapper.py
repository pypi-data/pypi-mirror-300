...
class FileMapper:
    def __init__(self, dvh: DVHandler, owner_id: str) -> None:
        """
    ...
    def _table_definition(self, table: str) -> dict:
        """
        Uses a caching approach to minimize number of queries to DV.
        """
        if table not in self.table_definitions:
            self.table_definitions[table] = self.dvh.get_table_definition(table)
    ...
    def _resolve_empty_id_conditions(self) -> None:
        ...
        TODO: 
        Need to make sure this won't affect the cases where
        we have a lookup & key simultaneously - check the 
        definition of "IDParams".
        """
    ...
    def _entity_ids(self) -> list[str]:
        """
        Send a request to the DV Handler using the category 
        buffer for the ids. Resets the category buffer.
        """
        self._resolve_empty_id_conditions()
        entity_ids = self.dvh.get_buffer_ids(self.buffer.category_id_params)
        return entity_ids
    

    def _lookups(self) -> list[EntityLookups]:
        """
        Send a request to the DV Handler using the category 
        buffer for the lookups. Resets the category buffer.
        """
        lookups = self.dvh.get_buffer_lookups(self.buffer.category_lookup_params)
        return lookups
    ...
    def _extract_lookup_vars(self, mapping: dict) -> tuple:
        ...
        """
        try:
            table: str = mapping[Mapping.LOOKUP_TABLE]
            relationship: str = mapping[Mapping.LOOKUP_RELATIONSHIP]
            column = extract_lookup_column(mapping)
            
            table_definition: dict = self._table_definition(table)
    ...
    def _lookup_logic(self, row: pd.Series, mapping: dict) -> None:
        ...
        dv_column: str = mapping[Mapping.DV_COLUMN]
    ...
    def _key_logic(self, row: pd.Series, mapping: dict) -> None:
        ...
        """
        try:
            file_column: str = mapping[Mapping.FILE_COLUMN]
    ...
    def _save_key_lookup_column(self, mapping: dict) -> None:
        ...
        """
        dv_column = mapping[Mapping.DV_COLUMN]
        table = mapping[Mapping.LOOKUP_TABLE]
    ...
    def _default_value_logic(self, mapping: dict) -> Any:
        ...
        try:
            dv_column = mapping[Mapping.DV_COLUMN]
            default_value_string = mapping[Mapping.DEFAULT_VALUE]
    ...
    def _type_conversion_logic(self, row: pd.Series, mapping: dict) -> None:
        """
        In the case there are mismatches between the file type and DV type.
        """
        try:
            dv_column: str = mapping[Mapping.DV_COLUMN]
            file_column = mapping[Mapping.FILE_COLUMN]
            data_type: str = mapping[Mapping.DATA_TYPE].lower()
            self.buffer.entity_data[dv_column] = cast_data(row[file_column], data_type)
        except Exception as e:
            raise RuntimeError(f"Error in type conversion logic: [{e}] for mapping [{mapping}]")
    ...
    def _condition_logic(self, row: pd.Series, mapping: dict) -> None:
        condition_string = mapping[Mapping.CONDITION]
    ...
    def _run_mapping_cycle(self, row: pd.Series, mapping: dict) -> None:
        ...
        """
        try:
            is_lookup: bool = mapping[Mapping.IS_LOOKUP]
            is_key: bool = mapping[Mapping.IS_KEY]
            choice_dictionary: str = mapping[Mapping.CHOICE_DICTIONARY]
            default_value_string: str = mapping[Mapping.DEFAULT_VALUE]
            skip_entity: bool = get_skip_flag(row, mapping)
            
            # data_type: str = mapping[Mapping.DATA_TYPE]
            ...
            # elif data_type:
            #     self._type_conversion_logic(row, mapping)
    ...
    def _attach_entity_lookups_to_buffer(self, entity_index: int, entity_lookups: EntityLookups) -> None:
        ...
        for lookup in entity_lookups:
            ...
            condition = FileMapper._create_key_lookup_condition(entity_params, lookup)
    ...
    def extract_category_key_columns(self, category: Category) -> list[str]:
        category_mappings = self.grouped_mappings[category]
        key_mappings = [mapping for mapping in category_mappings if mapping[Mapping.IS_KEY] == True]
        key_file_columns = []
        for mapping in key_mappings:
            key_file_column = mapping[Mapping.FILE_COLUMN]
    ...
    # TODO: 
    # - Add sheet name to parameter list - not needed anymore?
    # - Remove table name and sheet name from categories - not needed anymore?
    ...
    def map_data(
        ...
        for sheet in sheets:
            ...
            sheet_response_ids = self._load_sheet(input_data, sheet_categories, sheet, header_row_number)
