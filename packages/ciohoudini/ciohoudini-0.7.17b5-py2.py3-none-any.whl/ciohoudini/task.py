...
def get_task_cmd(node, cmd_template_name='hython', **kwargs):
    ...
    if rop_node.type().name() == 'usdrender_rop': 
        ...
        try:
            if output_image_path:
                output_image_path = output_image_path.with_suffix("")
        
        except Exception as err_msg:
            raise Exception("output_image_path ({}):\n{}".format(rop_node, err_msg))
            
        output_usd_path = pathlib.Path(rop_node.parm("lopoutput").eval())
        
        LOGGER.debug("output_image: %s", output_image_path)
        ...
        #usd_file.replace("__render__", output_image_path)
        LOGGER.debug("usd_file2: %s", usd_file)
    ...
    return CMD_TEMPLATES[cmd_template_name].format(**data)
...
def resolve_payload(node, **kwargs):
    ...
    for i, chunk in enumerate(chunks):
        ...
        cmd = get_task_cmd(node, cmd_template, **kwargs)
