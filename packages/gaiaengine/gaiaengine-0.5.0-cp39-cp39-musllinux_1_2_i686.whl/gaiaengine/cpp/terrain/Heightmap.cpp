...
void Heightmap::setIsNavigable(const glm::ivec2& coordinates, bool navigable) {
  if (!isOutsideBounds(coordinates)) {
    _isNavigable[coordinates.x * getNbCells().y + coordinates.y] = navigable;
  }
}
...
void Heightmap::setHeight(const glm::ivec2& coordinates, float height) {
  if (!isOutsideBounds(coordinates)) {
    _heights[coordinates.x * _nbVertices.y + coordinates.y] = height;
    _geometryDirty = true;
  }
}
...
int Heightmap::getTextureID(const glm::vec2& pos) const {
  ...
  return _textureIDs[(int)pos.x * (_nbVertices.y - 1) + (int)pos.y];
}

void Heightmap::setTextureID(const glm::ivec2& coordinates, int textureID) {
  if (!isOutsideBounds(coordinates)) {
    _textureIDs[coordinates.x * (_nbVertices.y - 1) + coordinates.y] = textureID;
    _textureIDsDirty = true;
  }
}
...
void Heightmap::setColor(const glm::ivec2& coordinates, const Color& color) {
  if (!isOutsideBounds(coordinates)) {
    _colors[coordinates.x * _nbVertices.y + coordinates.y] = color;
    _geometryDirty = true;
  }
}
