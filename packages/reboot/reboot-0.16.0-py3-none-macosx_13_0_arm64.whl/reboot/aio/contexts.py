...
class React:
    ...
    class Querier(Generic[ResponseT]):
        ...
        # Service, state, method we are invoking.
    ...
    async def call(
        ...
        *,
        ...
        key = f'{service}.{method}/{state_ref}'
    ...
    def invalidate(
        ...
        *,
        ...
        for key, querier in self._queriers.items():
            if key.startswith(f'{service}.') and key.endswith(f'/{state_ref}'):
...
class Context(ABC, IdempotencyManager):
    ...
    def state_type_name(self) -> StateTypeName:
        ...
        """
        return service_to_state_type(self._service_name)
        ...
        *,
        ...
        if headers.transaction_ids is None:
            headers = dataclasses.replace(
                ...
                # coordinator.
                transaction_coordinator_service=service_name,
...
UntilT = TypeVar('UntilT')


async def until(
    context: WorkflowContext,
    condition: Callable[[], Awaitable[UntilT]],
) -> UntilT:
