...
class StreamEventHandler(AssistantEventHandler):
    ...
    def handle_message_delta(self, event, assistant_name="Assistant"):
        """
        Handles incoming message deltas (streaming content) and logs or prints the assistant's responses.
        Accepts an assistant_name to allow dynamic printing of the assistant's name.
        """
        ...
        if message_content:
            self.logger.info(f"Message content for ID {message_id}: {message_content}")
            # Use the dynamically provided assistant name
            print(f"\r{assistant_name}: {message_content}", end='', flush=True)
