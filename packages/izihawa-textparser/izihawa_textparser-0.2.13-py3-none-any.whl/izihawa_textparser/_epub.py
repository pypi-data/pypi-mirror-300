    ...
    with tempfile.NamedTemporaryFile(mode="w+b", delete=False) as t_file:
        ...
        for item in items:
            if _ := (
                ...
                or re.search('^(?!.*(contributors|acknowledgements|content|list_figure|cover|title_page|title|titlePage|copyright|backmatter|dedication|epigraph|image|'
                             'index|credits|bibliography|footnote|navdoc|frontmatter|reference|toc|series|nav|copy|ack\.)).*$', item.get_name(), flags=re.IGNORECASE)
            ):
