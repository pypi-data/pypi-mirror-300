...
class GrobidParser(AioThing):
    ...
    def parse_response(self, grobid_response):
        ...
        for el in list(article.find_all(
            "div",
                attrs={"type": re.compile("^(annex|acknowledgment|availability|funding)$")}
        )):
            el.extract()
    ...
    def parse_body(self, divs):
        ...
        for div in divs:
            for el in list(div.select("table, nav, formula, math, figure, img, titlePage, front, note")):
                el.extract()
            ...
            if not div_list:
                ...
                if header_text:
                    mapped_heading = SECTIONS_MAPS.get(header_text.lower().strip(), header_text)
