...
def _process_single_pubmed_central_part(soup: BeautifulSoup, banned_sections: set, section_maps: dict[str, str]):
    secs = []
    for rep in reversed(range(1, 5)):
        for h in soup.select("section > " * rep + "title"):
            if not h.text.strip():
                ...
                h.name = f"h{rep + 1}"
            title = (
                section_maps.get(h.text) or section_maps.get(h.text.lower()) or h.text
            )
