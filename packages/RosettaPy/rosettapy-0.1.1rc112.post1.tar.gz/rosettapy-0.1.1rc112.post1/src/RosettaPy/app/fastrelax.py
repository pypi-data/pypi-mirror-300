...
class FastRelax:
    ...
    def get_relax_scripts_from_db(script_name: str) -> str:
        ...
        all_scripts=[os.path.join(ROSETTA3_DB, f[:-4]) for f in os.listdir(f'{ROSETTA3_DB}/sampling/relax_scripts/') if f.endswith('.txt') and f != 'README.txt' and 'dualspace' not in f]
        ...
        raise RuntimeError(f"No such relax script - {script_name}, All available scripts: {[os.path.basename(f).replace('.txt','') for f in all_scripts]}")
    ...
    def run(self, nstruct: int=8) -> RosettaEnergyUnitAnalyser:
        ...
        rosetta = Rosetta(
            ...
            opts=[
                ...
                "-relax:script",
                self.relax_script,
                '-relax:default_repeats', str(self.default_repeats),
                '-out:prefix', f"{self.instance}_fastrelax_",
                '-out:file:scorefile', f"{self.instance}_fastrelax.sc",
                '-score:weights', 'ref2015_cart' if self.dualspace else 'ref2015',
                '-relax:dualspace', 'true' if self.dualspace else 'false'
