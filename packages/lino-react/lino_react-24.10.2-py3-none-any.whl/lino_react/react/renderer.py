...
use_dashboard_layouts = settings.SITE.get_plugin_setting(
    'system', 'use_dashboard_layouts', False)
...
class Renderer(JsCacheRenderer):
    ...
    def write_lino_js(self, f):
        ...
        data = dict(
            ...
            use_dashboard_layouts=use_dashboard_layouts,
        )
    ...
    def action_call(self, ar, bound_action, status):
        ...
        if ar is not None and ar.actor is bound_action.actor:
        ...
        return "window.App.runAction(%s)" % py2js(dict(
            ...
            actorId=actorId,
            status=status,
            rp=None if an in ['detail', 'show', 'grid'] else rp))
    ...
    def actor2json(self, v):
        ...
        result['extra_display_modes'] = sorted(v.extra_display_modes)
