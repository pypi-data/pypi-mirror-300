...
def graph_betweenness(graph: ig.Graph, weighted=False, cutoff=None, sources=None, targets=None, normalized=False) -> pd.DataFrame:
    result = pd.DataFrame()
    directed = graph.is_directed()
    ...
    if weighted:
        weighted_betweenness = graph.betweenness(weights='weight',  directed=directed, cutoff=cutoff, sources=sources, targets=targets)
...
def graph_closeness(graph: ig.Graph, weighted, cutoff=None, normalized=False) -> pd.DataFrame:
    ...
    if normalized:
        if directed:
            ...
            if weighted:
                normalized_weighted_in_closeness = graph.closeness(weights='weight', mode="in", cutoff=cutoff, normalized=True)
                normalized_weighted_out_closeness = graph.closeness(weights='weight', mode="out", cutoff=cutoff, normalized=True)
                normalized_weighted_all_closeness = graph.closeness(weights='weight', mode="all", cutoff=cutoff, normalized=True)
...
# def weighted_degree(graph: ig.Graph, node: int) -> float:
#     if graph.is_directed():
#         weighted_degree = graph.strength(node, weights='weight')