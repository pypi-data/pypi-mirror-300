...
class ReadPdf:
    ...
    def 读取PDF中的单个字符信息(self,起始页=1,结束页:int=None) -> dict:
        ...
        for 页码, 页面 in enumerate(self.pdf_页面列表[起始页-1:结束页 or self.总页数], start=起始页):
            页面字符信息={'页码':页面.page_number,'页面宽度':页面.width,'页面高度':页面.height,'字符信息':[]}
    ...
    def 读取PDF中的文本块信息(self,起始页=1,结束页:int=None) -> dict:
        self.读取PDF()
        for 页码, 页面 in enumerate(self.pdf_页面列表[起始页 - 1:结束页 or self.总页数], start=起始页):
            页面文本块信息 = {'页码': 页面.page_number, '页面宽度': 页面.width, '页面高度': 页面.height, '文本块信息': []}
            for 文本块信息 in 页面.extract_words():
                页面文本块信息['文本块信息'].append({
    ...
    def 读取PDF中的图片(self,起始页=1,结束页:int=None,图片保存路径=None) -> None:
        """
        :param 图片保存路径:当不传入时，默认在pdf所在目录下创建与pdf相同名称的文件夹，并将图片保存在该文件夹下
        """
        if not 图片保存路径:
            图片保存路径 = self.pdf_路径[:-4]
            makedirs(图片保存路径, exist_ok=True)

        # 使用PyMuPDF获取页面
        document = fitz.open(self.pdf_路径)

        for 页码, 页面 in enumerate(document[起始页 - 1:结束页 or len(document)], start=起始页):
            # 获取页面上的所有图像
            图片列表 = document[页码-1].get_images(full=True)

            # 保存页面上的所有图像
            for i, 图片 in enumerate(图片列表):
                图片 = document.extract_image(图片[0])
                图片后缀 = 图片.get('ext','png').lower()
                if 图片后缀 == 'jpeg':图片后缀 = 'jpg'
                图片保存名称=f"第{页码}页_第{i + 1}张图.{图片后缀}"
                with open(join(图片保存路径,图片保存名称), "wb") as image_file:
                    image_file.write(图片["image"])

                logger.success(f"{图片保存名称}已保存")

        logger.info(f"图片保存路径：{图片保存路径}")
