...
class HDF5Writer(Postprocessor):
    def __init__(self, file_path, dataset_names=None, name="hdf5_writer"):
    ...
    def run(self, arrays, dataset_name, chunk_shape=1000, column_names=None):
...
class NtupleHDF5Writer(HDF5Writer):
    def __init__(
        ...
        save_node : str, optional
            Name of the node in the processors graph to save arrays from, by default "unweighted_output".
        ...
        """
        super(NtupleHDF5Writer, self).__init__(file_path, name="ntuple_hdf5_writer")
...
if __name__ == "__main__":
    ...
    a, b = np.random.rand(10, 10), np.random.rand(10, 10)
    c = np.random.rand(10, 10)

    w = HDF5Writer("test.hdf5", dataset_names=["dir_a/test_a", "test_b"])
    w.create_datasets(shape=(10, 10), chunks=(5, 10), maxshape=(None, 10), dtype="float32")

    w.add_data(a, "dir_a/test_a", idx=(0, 10))
    w.add_data(b, "test_b", idx=(0, 10))

    w.add_data(c, "dir_a/test_a", idx=(10, 20), resize=(20, 10))

    metadata = {"test_a": "a", "test_a_int": 1, "test_a_lst": [1, 2], "test_a_dct": {"a": 1}}
    w.add_metadata(metadata, "dir_a")

    f = w.get_handle()

    print(f["dir_a"], f["dir_a/test_a"], f["test_b"])
    print(w.get_metadata("dir_a"))
