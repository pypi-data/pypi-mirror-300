    ...
    def analyze_linha_pwf(self, list_dic_dlin):
        ...
        for index, dic in enumerate(list_dic_dlin):
            ...
            if dic["operacao"] == "" or dic["operacao"] == "0" or dic["operacao"] == "A":
                ...
                # Check crítico A.3 - Capacidade Equipamento Vazio
                # if (self.capac_equip == ""):
                #     errors.append(f"ERRO L{row_error} - LINHA {self.barra_de}-{self.barra_para}-{self.num_circuito}: Capacidade Equipamento está vazio!")
                    
                # Check crítico A.4 - Capacidade Emergência Vazio
                # if (self.capac_emerg == ""):
                #     errors.append(f"ERRO L{row_error} - LINHA {self.barra_de}-{self.barra_para}-{self.num_circuito}: Capacidade Emergência está vazio!")
                    
                # Check crítico A.5 - Número de taps vazio
                if (dic['numero_taps'] == "") and (dic["tap_minimo"] != ""):
                    if float(dic["tap_minimo"]) != float(dic["tap_maximo"]):
                        errors.append(f"ERRO L{row_error} - LINHA {dic['barra_de']}-{dic['barra_para']}-{dic['num_circuito']}: Número de taps está vazio!")

                # Check crítico A.6 - Número de taps vazio
                ...
                # Check crítico A.7 - Barra controlada se tap fixo
                ...
                # Check crítico A.1 - Número do circuito vazio
                # if (self.num_circuito == ""):
                #     errors.append(f"ERRO L{row_error} - LINHA {self.barra_de}-{self.barra_para}-{self.num_circuito}: Numero do circuito está vazio!")
    ...
    def analyze_linha_ana(self, list_dic_dlin, dic_dbar):
        ...
        for index, dic in enumerate(list_dic_dlin):
            ...
            if dic["codigo_atualizacao"] == "" or dic["codigo_atualizacao"] == "0" or dic["codigo_atualizacao"] == "A":
                ...
                # Check crítico 2 - Tipo Circuito L com conexão
                ...
                if (dic["suscept_pos"] != "") and (dic["suscept_zer"] == ""):
                ...
                # Check crítico 2 - Barra DE tipo MIDPOINT com Conexão DE
                ...
                # Check crítico 3 - Barra PARA tipo MIDPOINT com Conexão PARA
                if (dic["tipo_circuito"] == "T"):
                    # Verifica se a barra existe no servidor SIGER
