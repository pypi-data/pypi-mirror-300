import os
import json
from typing import Any, Dict, List, Union, TypeVar, Generic, Type
...
class PersistentStorage:
    ...
    For writes that are done by the user directly, use `write_unprivileged()` to prevent writes to protected values.
    """
    ...
    def _load_data(self) -> Dict[str, Any]:
        if os.path.exists(self.file_path):
            ...
            properties = {k: v.default for k, v in self.property_list.items()}
            self._save_data(properties)
            return properties
    ...
    def write_unprivileged(self, key: str, value: object) -> None:
        """Same as a direct properties[] write, but protected data fails to write."""
        if key not in self.property_list:
            raise KeyError(f"Invalid property: {key}")
