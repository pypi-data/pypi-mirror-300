...
from aurora_sdk.aurora_certvalidator.authority import AuthorityWithCert
from aurora_sdk.aurora_certvalidator.errors import InvalidCertificateError
from aurora_sdk.aurora_certvalidator.path import ValidationPath

from aurora_sdk.aurora_signature.pdf_utils import generic
from aurora_sdk.aurora_signature.pdf_utils.generic import pdf_name, pdf_string
from aurora_sdk.aurora_signature.pdf_utils.misc import (
    OrderedEnum,
    PdfReadError,
    get_and_apply,
)
from aurora_sdk.aurora_signature.sign.general import SigningError, UnacceptableSignerError
from aurora_sdk.aurora_signature.pdf_utils.rw_common import PdfHandler
...
class SigSeedValFlags(Flag):
    ...
    .. warning::
        ...
        * aurora does not support legal attestations at all, so given that
          the :attr:`LEGAL_ATTESTATION` requirement flag only restricts the
          legal attestations that can be used by the signer, aurora can safely
          ...
          constraint.
        * Since aurora does not define any named appearances, setting
          the :attr:`APPEARANCE_FILTER` flag and the
          :attr:`~.SigSeedValueSpec.appearance` entry in the seed value
          dictionary will make aurora refuse to sign the document.

          When validating, the situation is different: since aurora has no
    ...
    FILTER = 1
    """
    Makes the signature handler setting mandatory. aurora only supports
    ``/Adobe.PPKLite``.
    """
...
class SigCertConstraints:
    ...
    .. note::
        aurora ignores this value, but we include it for compatibility.
    """
...
class SigSeedValueSpec:
    ...
    processor. If ``None``, aurora will compute an appropriate value.
    
    .. note::
        You may also specify this value directly as an integer.
        This covers potential future versions of the standard that aurora
    ...
    .. caution::
        ...
        enforced.
        Regardless, since aurora's validator is also unaware of legal
        attestation settings, it will refuse to validate signatures
        where this seed value constitutes a mandatory constraint.
        
        Additionally, since aurora does not support legal attestation
    ...
    .. caution::
        ...
        aurora currently does not define any named appearances.        
    """
    ...
    def build_timestamper(self):
        ...
        :return:
            A :class:`~.aurora_sdk.aurora_signature.sign.timestamps.HTTPTimeStamper`.
        """
        from aurora_sdk.aurora_signature.sign.timestamps import HTTPTimeStamper
