...
from aurora_sdk.aurora_certvalidator._asyncio_compat import to_thread
from aurora_sdk.aurora_certvalidator.registry import (
    CertificateStore,
    SimpleCertificateStore,
)

from aurora_sdk.aurora_signature.pdf_utils import misc
...
from aurora_sdk.aurora_signature.sign.attributes import (
...
class Signer:
    ...
    As of now, aurora provides two implementations:

    * :class:`.SimpleSigner` implements the easy case where all the key material
      can be loaded into memory.
    * :class:`~aurora_sdk.aurora_signature.sign.pkcs11.PKCS11Signer` implements a signer that is
...
class SimpleSigner(Signer):
    ...
    def load_bytes(
        ...
        """
        Load certificates and key material from PEM/DER bytes.
        ...
        try:
            print('0')
            # load cryptographic data (both PEM and DER are supported)
            signing_key = load_private_key_from_bytes(
                key_bytes, passphrase=key_passphrase
            )
            print('1')
            signing_cert = load_certs_from_bytes(cert_bytes)
            print('load_bytes', type(signing_cert))
        except (IOError, ValueError, TypeError) as e:
            logger.error('Could not load cryptographic material', exc_info=e)
            return None
