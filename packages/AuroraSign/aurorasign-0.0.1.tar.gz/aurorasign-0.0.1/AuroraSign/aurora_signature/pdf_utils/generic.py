...
class TrailerReference(Dereferenceable):
    ...
    :param reader:
        a :class:`~aurora_sdk.aurora_signature.pdf_utils.reader.PdfFileReader`
    """
        ...
        if stream_data is not None:
            ...
            if as_metadata_stream:
                try:
                    ...
                    from aurora_sdk.aurora_signature.pdf_utils.metadata.xmp_xml import (
                        MetadataStream,
                    )
...
class StreamObject(DictionaryObject):
    ...
    :param handler:
        ...
        :class:`.aurora_sdk.aurora_signature.pdf_utils.crypt.SecurityHandler`.
    ...
    def apply_filter(
        ...
        *Note:* Calling this method on an encoded stream will first cause the
        ...
        :param filter_name:
            Name of the filter
            (see :const:`~aurora_sdk.aurora_signature.pdf_utils.filters.DECODERS`)
        :param params:
...
class DecryptedObjectProxy(PdfObject):
    ...
    .. warning::
        ...
        which aurora's object handling code ultimately derives, there are
        some Python builtins that might cause these wrapper objects to
        inadvertently "leak". Please `tell us about such cases
        <https://github.com/MatthiasValvekens/aurora/discussions>`_ so we can
