...
class Detector(PropertiesReport):
    ...
    def _generate_shot_noise(self, photocurrent: Quantity) -> np.ndarray:
        r"""
        Generates shot noise based on the photocurrent and bandwidth.

        Shot noise arises from the discrete nature of photons and electrons. It is given by:
        \[
        \sigma_{\text{shot}} = \sqrt{2 e I_{\text{ph}} B}
        \]
        Where:
        ...
        -------
        np.ndarray
            An array of shot noise values.
        """
        if not self.include_shot_noise or not self.include_noises:
            return np.zeros(len(self.dataframe))

        e = 1.602176634e-19 * coulomb  # Elementary charge in coulombs
        noise_std = np.sqrt(2 * e * photocurrent * self.bandwidth.to('Hz')) * self.resistance

        return np.random.normal(0, noise_std, size=len(self.dataframe))
    ...
    def _add_photon_shot_noise_to_raw_signal(self, optical_power: Quantity) -> None:
        ...
        """
        if not self.include_shot_noise:
            return
    ...
    def plot(self, show: bool = True, figure_size: tuple = None, color: str = 'C0', ax: plt.Axes = None) -> None:
        ...
        with plt.style.context(mps):
            ...
            ax.set_ylabel(f"Signal [{signal_unit:P}]")
