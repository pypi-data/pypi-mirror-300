...
import { B as Block, S as StatusTracker, N as BlockTitle, Z as Clear, j as Music, ao as Video, a1 as File$1, ap as Paperclip, aq as Send, ar as Square } from './2-B1kq23UX.js';
import { U as Upload } from './ModifyUpload-D1AmP-fT.js';
import { I as Image$1 } from './Image-DFqHtuJN.js';
export { default as BaseExample } from './Example17-D8frBwiU.js';
...
const MultimodalTextbox = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    can_submit = value.text !== "" || value.files.length > 0;
    $$rendered = `<div class="${["full-container svelte-1tt7gi3", dragging ? "dragging" : ""].join(" ").trim()}" role="group" aria-label="Multimedia input field"${add_attribute("this", full_container, 0)}> <label${add_classes((container ? "container" : "").trim())}>${validate_component(BlockTitle, "BlockTitle").$$render($$result, { root, show_label, info }, {}, {
      default: () => {
        return `${escape(label)}`;
      }
    })} ${value.files.length > 0 || uploading ? `<div class="thumbnails scroll-hide svelte-1tt7gi3" aria-label="Uploaded files" data-testid="container_el" style="${"display: " + escape(value.files.length > 0 || uploading ? "flex" : "none", true) + ";"}">${each(value.files, (file, index) => {
      return `<span role="listitem" aria-label="File thumbnail"><button class="thumbnail-item thumbnail-small svelte-1tt7gi3"><button class="${["delete-button svelte-1tt7gi3", disabled ? "disabled" : ""].join(" ").trim()}">${validate_component(Clear, "Clear").$$render($$result, {}, {}, {})}</button> ${file.mime_type && file.mime_type.includes("image") ? `${validate_component(Image$1, "Image").$$render(
        $$result,
        {
          src: file.url,
          title: null,
    ...
    })} ${uploading ? `<div class="loader svelte-1tt7gi3" role="status" aria-label="Uploading"></div>` : ``}</div>` : ``} <div class="input-container svelte-1tt7gi3">${validate_component(Upload, "Upload").$$render(
      $$result,
      {
        file_count,
        filetype: file_types,
      ...
      {}
    )} <button data-testid="upload-button" class="upload-button svelte-1tt7gi3">${validate_component(Paperclip, "Paperclip").$$render($$result, {}, {}, {})}</button> <textarea data-testid="textbox" class="${["scroll-hide svelte-1tt7gi3", !show_label ? "no-label" : ""].join(" ").trim()}"${add_attribute("dir", rtl ? "rtl" : "ltr", 0)}${add_attribute("placeholder", placeholder, 0)}${add_attribute("rows", lines, 0)} ${disabled ? "disabled" : ""} ${autofocus ? "autofocus" : ""}${add_attribute("style", text_align ? "text-align: " + text_align : "", 0)}${add_attribute("this", el, 0)}>${escape(value.text || "")}</textarea> ${submit_btn ? `<button class="${["submit-button svelte-1tt7gi3", submit_btn !== true ? "padded-button" : ""].join(" ").trim()}" ${!can_submit ? "disabled" : ""}>${submit_btn === true ? `${validate_component(Send, "Send").$$render($$result, {}, {}, {})}` : `${escape(submit_btn)}`}</button>` : ``} ${stop_btn ? `<button class="${["stop-button svelte-1tt7gi3", stop_btn !== true ? "padded-button" : ""].join(" ").trim()}">${stop_btn === true ? `${validate_component(Square, "Square").$$render($$result, { fill: "none", stroke_width: 2.5 }, {}, {})}` : `${escape(stop_btn)}`}</button>` : ``}</div></label> </div>`;
  } while (!$$settled);
  return $$rendered;
});
...
const Index = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `   ${validate_component(Block, "Block").$$render(
      ...
      {
        default: () => {
          return `${loading_status ? `${validate_component(StatusTracker, "StatusTracker").$$render($$result, Object.assign({}, { autoscroll: gradio.autoscroll }, { i18n: gradio.i18n }, loading_status), {}, {})}` : ``} ${validate_component(MultimodalTextbox$1, "MultimodalTextbox").$$render(
            $$result,
            {
              file_types,
              root,
...
//# sourceMappingURL=Index38-C3aPASSV.js.map
