...
def status(flag=False):
    ...
    if flag:
        ...
        print(scr_line("-" * ll*3))
        print(scr_str(f"Mecord{'[auto]' if os.path.exists(auto_restart) else ''}{'[short]' if os.path.exists(short_flag) else ''}".ljust(ll*3-2)))
        ...
        def task_status():
            ...
            while monitor_run:
                ...
                stdscr.addstr(idx, 0, scr_str(
                    f"Mecord{'[auto]' if os.path.exists(auto_restart) else ''}{'[short]' if os.path.exists(short_flag) else ''}".ljust(ll * 3 - 2)), )
                idx += 1
