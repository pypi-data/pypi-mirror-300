...
def _domain_post(url, data):
    try:
        ...
        s.keep_alive = False
        res = s.post(url=url, data=data, timeout=3, headers={'Connection':'close'})
...
def _ip_post(url, data, direct_ip):
    try:
        ...
        res = s.post(url=real_url, data=data, timeout=3, headers={'Connection':'close','Host':domain}, verify=False)
        if res.status_code == 200:
            res_content = res.content
            res.close()
            return True, res_content 
