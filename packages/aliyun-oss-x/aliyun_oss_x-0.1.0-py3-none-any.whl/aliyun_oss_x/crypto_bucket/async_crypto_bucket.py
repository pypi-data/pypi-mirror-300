...
class AsyncCryptoBucket(AsyncBucket):
    ...
    用法（假设Bucket属于杭州区域） ::
        ...
        >>> bucket = aliyun_oss_x.AsyncCryptoBucket(auth, 'http://oss-cn-hangzhou.aliyuncs.com', 'your-bucket', aliyun_oss_x.LocalRsaProvider())
    ...
    def __init__(
        ...
        logger.debug("Init AsyncCryptoBucket: {0}".format(bucket_name))
        super(AsyncCryptoBucket, self).__init__(
            auth,
            endpoint,
            bucket_name,
            is_cname,
    ...
    def put_object_with_url(self, sign_url, data, headers=None, progress_callback=None):
        ...
        :return:
        """
        raise ClientError("The operation is not support for AsyncCryptoBucket now")

    def append_object(self, key, position, data, headers=None, progress_callback=None, init_crc=None):
        raise ClientError("The operation is not support for AsyncCryptoBucket")
    ...
    async def get_object(self, key, byte_range=None, headers=None, progress_callback=None, process=None, params=None):
        ...
        if byte_range:
            ...
            logger.debug(
                "adjust range of get object, byte_range: {0}, adjust_byte_range: {1}, discard: {2}".format(
    ...
    async def get_object_with_url(self, sign_url, byte_range=None, headers=None, progress_callback=None):
        ...
        logger.debug(
            "Start to get object with url from AsyncCryptoBucket: {0}, sign_url: {1}, range: {2}, headers: {3}".format(
    ...
    async def init_multipart_upload(self, key, headers=None, params=None, upload_context=None):
        """客户端加密初始化分片上传。
        ...
        logger.info(
            "Start to init multipart upload by AsyncCryptoBucket, data_size: {0}, part_size: {1}".format(
        ...
        if upload_context.part_size:
            ...
            if not res:
                raise ClientError("part_size is invalid for multipart upload for AsyncCryptoBucket")
        else:
        ...
        resp = await super(AsyncCryptoBucket, self).init_multipart_upload(key, headers)
    ...
    async def upload_part(
        ...
        """
        logger.info(
            "Start to upload multipart of AsyncCryptoBucket, upload_id = {0}, part_number = {1}".format(
        ...
        if upload_context:
            ...
            raise ClientError(
                "Could not init upload context, upload contexts flag is False and upload context is none"
            )

        content_crypto_material = context.content_crypto_material

        if (
            content_crypto_material.cek_alg != self.crypto_provider.cipher.alg
            or content_crypto_material.wrap_alg != self.crypto_provider.wrap_alg
        ):
