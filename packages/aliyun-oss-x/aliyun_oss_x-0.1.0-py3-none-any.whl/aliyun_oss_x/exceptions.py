...
class ExceptionMeta(type):
    ...
    def __init__(cls, name, bases, dct):
        super(ExceptionMeta, cls).__init__(name, bases, dct)
        status = getattr(cls, "status", None)
        code = getattr(cls, "code", None)
        if status is not None and code is not None:
            _OSS_ERROR_TO_EXCEPTION[(status, code)] = cls
...
class OssError(Exception):
    ...
    def __str__(self):
        error = {"status": self.status, OSS_REQUEST_ID: self.request_id, "details": self.details}
        return str(error)

    def _str_with_body(self):
        error = {"status": self.status, OSS_REQUEST_ID: self.request_id, "details": self.body}
        return str(error)
...
class InconsistentError(OssError):
    def __init__(self, message, request_id=""):
        OssError.__init__(
            self, OSS_INCONSISTENT_ERROR_STATUS, {OSS_REQUEST_ID: request_id}, "InconsistentError: " + message, {}
        )
...
class ServerError(OssError, metaclass=ExceptionMeta):
    def __init__(self, status, headers, body, details):
        OssError.__init__(self, status, headers, body, details)
...
class InvalidEncryptionRequest(ServerError):
    ...
    def __init__(self, message):
        super(InvalidEncryptionRequest, self).__init__(
            InvalidEncryptionRequest.status, {}, "", {"Code": "InvalidEncryptionRequest", "Message": message}
        )
...
class SelectOperationFailed(ServerError):
    ...
    def __str__(self):
        error = {"status": self.status, "code": self.code, "details": self.message}
        return str(error)
...
class SelectOperationClientError(OssError):
    def __init__(self, message, request_id):
        OssError.__init__(
            self,
            OSS_SELECT_CLIENT_ERROR_STATUS,
    ...
    def __str__(self):
        error = {"x-oss-request-id": self.request_id, "message": self.message}
        return str(error)
...
def make_exception(resp: OSSResponse):
    status = resp.status
...
def _parse_error_body(body):
    try:
        ...
        return details
    except (ElementTree.ParseError, expat.ExpatError):
