...
class SelectResponseAdapter:
    ...
    def next(self):
        ...
        while self.finished == 0:
            if self.frame_off_set < self.frame_length:
                ...
                if (
                    self.frames_since_last_progress_report >= SelectResponseAdapter._FRAMES_FOR_PROGRESS_UPDATE
    ...
    def read_next_frame(self):
        ...
        if (
            ...
            logger.warning(
                "Unexpected frame type: {0}. RequestId:{1}. This could be due to the old version of client.".format(
                    frame_type_val, self.request_id
                )
            )
