...
class Service(_Base):
    ...
    def write_get_object_response(self, route, token, fwd_status, data, headers=None):
        ...
        logger.debug(
            "write get object response done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
    ...
    def list_user_data_redundancy_transition(self, continuation_token="", max_keys=100):
        ...
        """
        logger.debug(
            "Start to list user data redundancy transition, continuation token: {0}, max keys: {1}".format(
                continuation_token, max_keys
            )
        )
        ...
        resp = self._do(
            ...
            "",
            params={
                Bucket.REDUNDANCY_TRANSITION: "",
                "continuation-token": continuation_token,
                "max-keys": str(max_keys),
            },
        )
        logger.debug(
            "List user data redundancy transition done, req_id: {0}, status_code: {1}".format(
    ...
    def list_access_points(self, max_keys=100, continuation_token=""):
        ...
        resp = self._do(
            ...
            "",
            "",
            params={Bucket.ACCESS_POINT: "", "max-keys": str(max_keys), "continuation-token": continuation_token},
        )
        logger.debug(
            "query list access point done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
    ...
    def put_public_access_block(self, block_public_access=False):
        ...
        logger.debug(
            "Put public access block done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
        return RequestResult(resp)
    ...
    def get_public_access_block(self):
        ...
        logger.debug(
            "Get public access block done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
    ...
    def delete_public_access_block(self):
        ...
        logger.debug(
            "Delete public access block done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
    ...
    def list_resource_pools(self, continuation_token="", max_keys=100):
        ...
        """
        logger.debug(
            "Start to list resource pools, continuation_token: {0}, max_keys: {1}".format(continuation_token, max_keys)
        )
        ...
        resp = self._do(
            ...
            "",
            "",
            params={Service.RESOURCE_POOL: "", "continuation-token": continuation_token, "max-keys": str(max_keys)},
        )
    ...
    def get_resource_pool_info(self, resource_pool_name):
        ...
        resp = self._do(
            "GET", "", "", params={Service.RESOURCE_POOL_INFO: "", Service.RESOURCE_POOL: resource_pool_name}
        )
    ...
    def list_resource_pool_buckets(self, resource_pool_name, continuation_token="", max_keys=100):
        ...
        """
        logger.debug(
            "Start to list resource pool buckets, resource_pool_name:{0} continuation_token: {1}, max_keys: {2}".format(
        ...
        resp = self._do(
            ...
            "",
            params={
                Service.RESOURCE_POOL_BUCKETS: "",
                Service.RESOURCE_POOL: resource_pool_name,
                "continuation-token": continuation_token,
                "max-keys": str(max_keys),
            },
        )
        logger.debug(
            "List resource pool buckets done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
    ...
    def put_resource_pool_requester_qos_info(self, uid, resource_pool_name, qos_configuration):
        ...
        """
        logger.debug(
            "Start to put resource pool requester qos info, uid: {0}, resource_pool_name: {1}, qos_configuration: {2}".format(
        ...
        resp = self._do(
            ...
            data=data,
            params={
                Service.REQUESTER_QOS_INFO: "",
                Service.QOS_REQUESTER: uid,
                Service.RESOURCE_POOL: resource_pool_name,
            },
        )
        logger.debug(
            "Put resource pool requester qos info done, req_id: {0}, status_code: {1}".format(
                resp.request_id, resp.status
            )
        )
        return RequestResult(resp)
    ...
    def get_resource_pool_requester_qos_info(self, uid, resource_pool_name):
        ...
        """
        logger.debug(
            "Start to get resource pool requester qos info, uid: {0}, resource_pool_name: {1}.".format(
                uid, resource_pool_name
            )
        )
        if not uid:
            raise ClientError("uid should not be empty")
        ...
        logger.debug(
            "Get resource pool requester qos info done, req_id: {0}, status_code: {1}".format(
                resp.request_id, resp.status
            )
        )
    ...
    def list_resource_pool_requester_qos_infos(self, resource_pool_name, continuation_token="", max_keys=100):
        ...
        """
        logger.debug(
            "Start to list resource pool requester qos infos, resource_pool_name:{0} continuation_token: {1}, max_keys: {2}".format(
                resource_pool_name, continuation_token, max_keys
            )
        )
        if not resource_pool_name:
        ...
        logger.debug(
            "List resource pool requester qos infos done, req_id: {0}, status_code: {1}".format(
                resp.request_id, resp.status
            )
        )
        return self._parse_result(
    ...
    def delete_resource_pool_requester_qos_info(self, uid, resource_pool_name):
        ...
        """
        logger.debug(
            "Start to delete resource pool requester qos info, uid: {0}, resource_pool_name: {1}.".format(
                uid, resource_pool_name
            )
        )
        if not uid:
...
class Bucket(_Base):
    ...
    def copy_object(self, source_bucket_name, source_key, target_key, headers=None, params=None):
        ...
        if params and Bucket.VERSIONID in params:
            headers[OSS_COPY_OBJECT_SOURCE] = (
                "/" + source_bucket_name + "/" + quote(source_key, "") + "?versionId=" + params[Bucket.VERSIONID]
            )
        else:
    ...
    def batch_delete_objects(self, key_list, headers=None):
        ...
        resp = self.__do_bucket("POST", data=data, params={"delete": "", "encoding-type": "url"}, headers=headers)
    ...
    def complete_multipart_upload(self, key, upload_id, parts, headers=None):
        ...
        resp = self.__do_object("POST", key, params={"uploadId": upload_id}, data=data, headers=headers)
    ...
    def abort_multipart_upload(self, key, upload_id, headers=None):
        ...
        resp = self.__do_object("DELETE", key, params={"uploadId": upload_id}, headers=headers)
    ...
    def upload_part_copy(
        ...
        resp = self.__do_object("PUT", target_key, params=params, headers=headers)
    ...
    def list_live_channel(self, prefix="", marker="", max_keys=100):
        ...
        resp = self.__do_bucket(
            "GET", params={Bucket.LIVE: "", "prefix": prefix, "marker": marker, "max-keys": str(max_keys)}
        )
    ...
    def post_vod_playlist(self, channel_name, playlist_name, start_time=0, end_time=0):
        ...
        resp = self.__do_object(
            "POST", key, params={Bucket.VOD: "", "startTime": str(start_time), "endTime": str(end_time)}
        )
    ...
    def get_vod_playlist(self, channel_name, start_time, end_time):
        ...
        resp = self.__do_object(
            "GET", channel_name, params={Bucket.VOD: "", "startTime": str(start_time), "endTime": str(end_time)}
        )
    ...
    def process_object(self, key, process, headers=None):
        ...
        process_data = f"{Bucket.PROCESS}={process}"
    ...
    def put_bucket_policy(self, policy):
        ...
        resp = self.__do_bucket(
            "PUT", data=policy, params={Bucket.POLICY: ""}, headers={"Content-MD5": utils.content_md5(policy)}
        )
    ...
    def put_bucket_request_payment(self, payer):
        ...
        resp = self.__do_bucket(
            "PUT", data=data, params={Bucket.REQUESTPAYMENT: ""}, headers={"Content-MD5": utils.content_md5(data)}
        )
    ...
    def get_bucket_storage_capacity(self):
        ...
        resp = self.__do_bucket("GET", params={Bucket.USER_QOS: ""})
    ...
    def put_async_fetch_task(self, task_config):
        ...
        headers = http.Headers()
        headers["Content-MD5"] = utils.content_md5(data)
        resp = self.__do_bucket("POST", data=data, params={Bucket.ASYNC_FETCH: ""}, headers=headers)
    ...
    def get_async_fetch_task(self, task_id):
        ...
        resp = self.__do_bucket("GET", headers={OSS_TASK_ID: task_id}, params={Bucket.ASYNC_FETCH: ""})
        logger.debug(f"Put async fetch task done, req_id: {resp.request_id}, status_code: {resp.status}")
    ...
    def put_bucket_inventory_configuration(self, inventory_configuration):
        ...
        resp = self.__do_bucket(
            "PUT",
            data=data,
            params={Bucket.INVENTORY: "", Bucket.INVENTORY_CONFIG_ID: inventory_configuration.inventory_id},
            headers=headers,
        )
    ...
    def extend_bucket_worm(self, worm_id=None, retention_period_days=None):
        ...
        resp = self.__do_bucket(
            "POST", data=data, params={Bucket.WORM_ID: worm_id, Bucket.WORM_EXTEND: ""}, headers=headers
        )
    ...
    def delete_bucket_replication(self, rule_id):
        ...
        logger.debug(
            "Delete bucket replication done, req_id: {0}, status_code: {1}".format(resp.request_id, resp.status)
        )
    ...
    def create_bucket_data_redundancy_transition(self, targetType):
        ...
        resp = self.__do_bucket(
            "POST", params={Bucket.REDUNDANCY_TRANSITION: "", Bucket.TARGET_REDUNDANCY_TYPE: targetType}
        )
        logger.debug(
    ...
    def get_bucket_data_redundancy_transition(self, taskId):
        ...
        resp = self.__do_bucket(
            "GET", params={Bucket.REDUNDANCY_TRANSITION: "", Bucket.REDUNDANCY_TRANSITION_TASK_ID: taskId}
        )
        logger.debug(
    ...
    def delete_bucket_data_redundancy_transition(self, taskId):
        ...
        resp = self.__do_bucket(
            "DELETE", params={Bucket.REDUNDANCY_TRANSITION: "", Bucket.REDUNDANCY_TRANSITION_TASK_ID: taskId}
        )
        logger.debug(
    ...
    def put_access_point_policy(self, accessPointName, accessPointPolicy):
        ...
        headers["x-oss-access-point-name"] = accessPointName
        resp = self.__do_bucket(
            "PUT", data=accessPointPolicy, params={Bucket.ACCESS_POINT_POLICY: ""}, headers=headers
        )
    ...
    def put_access_point_public_access_block(self, access_point_name, block_public_access=False):
        ...
        resp = self.__do_bucket(
            "PUT", data=data, params={Bucket.PUBLIC_ACCESS_BLOCK: "", Bucket.OSS_ACCESS_POINT_NAME: access_point_name}
        )
    ...
    def get_access_point_public_access_block(self, access_point_name):
        ...
        resp = self.__do_bucket(
            "GET", params={Bucket.PUBLIC_ACCESS_BLOCK: "", Bucket.OSS_ACCESS_POINT_NAME: access_point_name}
        )
        logger.debug(
    ...
    def delete_access_point_public_access_block(self, access_point_name):
        ...
        resp = self.__do_bucket(
            "DELETE", params={Bucket.PUBLIC_ACCESS_BLOCK: "", Bucket.OSS_ACCESS_POINT_NAME: access_point_name}
        )
        logger.debug(
