...
from .cloudwatch_client import create_client
from .logging_config import get_logger
from logging import ERROR, INFO, DEBUG, getLevelName
...
class CloudLogger:
  ...
  def _log_to_cloudwatch(self, message: str, level = INFO):
    sys.stdout.flush()
    timestamp = int(round(time.time() * 1000))
    message = {'timestamp': timestamp, 'log_level': getLevelName(level), 'message': message, 'app_name': 'positron_wrapper'}
    log_events = [{
        'timestamp': timestamp,
