...
def _pack_one(arch: str, adapter_name: str, parent_dir: str = None):
    ...
    try:
        ...
        if run_mode == AdapterRunMode.INDEPENDENT.value:
            ...
            if arch == "x86_64":
                ...
                platforms.append("linux_x86_64-cp-3.10.12-cp310")
            elif arch == "arm64":
                arches.append("aarch64")
                platforms.append("linux_aarch64-cp-3.10.12-cp310")
            elif arch == "both":
                arches.append("x86_64")
                platforms.append("linux_x86_64-cp-3.10.12-cp310")
                arches.append("aarch64")
            ...
            for index, arch in enumerate(arches):
                ...
                cmd = f"pex -v -r {requirements_path} -r {extra_requirements_path} -o {pex_path} --platform {platform_name} --index-url https://pypi.tuna.tsinghua.edu.cn/simple --no-pypi"
...
def _dist(only_one=False, adapter_name=None, version=None):
    now = datetime.now()
    if only_one:
        dist_file_path = os.path.join(
            _DIST_DIR, f"{adapter_name}-{version}-adapter_{now.isoformat().replace(':', '_')}.tar.gz"
        )
    else:
        dist_file_path = os.path.join(_DIST_DIR, f"adapter_{now.isoformat().replace(':', '_')}.tar.gz")
...
def _pack(arch, adapter_names: Union[str, List[str]] = None, parent_dir: str = None):
    ...
    if isinstance(adapter_names, str):
        only_one = True
        adapter_name, version = _pack_one(arch, adapter_names, parent_dir)
        if adapter_name and version:
            _dist(only_one, adapter_name, version)
    else:
        for adapter_name in adapter_names:
            _pack_one(arch, adapter_name, parent_dir)


def pack(arch, adapter_names: Union[str, List[str]] = None):
    _clean()
    _prepare()
    _pack(arch, adapter_names)

    if isinstance(adapter_names, List):
        _dist()


def pack_dirs(arch, parent_dirs: Union[str, List[str]] = None):
    ...
    for parent_dir in parent_dirs:
        _pack(arch, parent_dir=parent_dir)
    _dist()


if __name__ == "__main__":
    os.chdir("../../")
    # pack(, "weilansec_simple_fetch")
    pack_dirs("x86_64", ".")
