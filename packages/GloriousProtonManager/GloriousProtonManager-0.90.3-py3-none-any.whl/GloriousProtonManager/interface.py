...
def main():
    ...
    col2 = [
        ...
        [
            ...
            sg.InputText(size=[20, 20], font=("DejaVu 12"), key="-VERSION-"),
        ],
        [
            sg.Button(
    ...
    col3 = [
        ...
        [
            sg.Text("2. Pick one (e.g. 7-15):", font=("DejaVu 9")),
            sg.InputText(size=[20, 20], font=("DejaVu 12"), key="-DELETE_VERSION-"),
        ],
        [
            sg.Button(
    ...
    while True:
        ...
        if event == "3. Install past Proton-GE version":
            install_old_input = values["-VERSION-"]
            old_version = f"https://github.com/GloriousEggroll/proton-ge-custom/releases/download/GE-Proton{install_old_input}/GE-Proton{install_old_input}.tar.gz"
            release = last_fifteen_versions()
            ...
            elif install_old_input not in str(listdir(DEFAULT_DIR)) and (
                len(install_old_input) <= 5
            ):
                print(
                    f"Downloading and extracting Proton-GE{install_old_input}. It might take a while.\n"
                )
                window.refresh()
                install_release(old_version)
                sg.popup(
                    f"Proton-GE{install_old_input} successfully installed",
                    font=("DejaVu 9"),
                    title="Glorious Proton Manager",
                )
            elif install_old_input not in str(listdir(DEFAULT_DIR)) and (
        ...
        if event == "3. Delete Proton-GE version":
            delete_input = values["-DELETE_VERSION-"]
            ge_del_version = f"GE-Proton{delete_input}"
            if exists(DEFAULT_DIR):
                installed = list_installed_versions()
