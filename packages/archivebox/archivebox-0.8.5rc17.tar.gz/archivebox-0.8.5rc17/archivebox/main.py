...
def install(out_dir: Path=DATA_DIR) -> None:
    ...
    for binprovider in reversed(list(settings.BINPROVIDERS.values())):
        print(f'[+] Setting up package manager [yellow]{binprovider.name}[/yellow]...')
        try:
            print(binprovider.setup())
        except Exception as e:
            print(f'[red]:cross_mark: Failed to set up [yellow]{binprovider.name}[/yellow]: {e}[/red]')
