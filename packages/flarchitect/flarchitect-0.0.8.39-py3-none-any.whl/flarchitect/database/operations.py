import random
from typing import List, Dict, Union, Callable, Optional, Any, Type

from flask import request
from numpy.core.defchararray import isnumeric
from sqlalchemy import inspect, desc, and_, Column
...
from flarchitect.database.utils import (
    ...
    get_select_fields,
    get_primary_keys,
    get_primary_key_filters, get_related_b_query,
)
from flarchitect.exceptions import CustomHTTPException
from flarchitect.utils.decorators import add_page_totals_and_urls, add_dict_to_query
...
def apply_sorting_to_query(
    args_dict: Dict[str, Union[str, int]], query: Query, base_model: Callable
) -> Query:
...
class CrudService:
    ...
    def filter_query_from_args(
        self, args_dict: Dict[str, Union[str, int]], query=None
    ) -> Query:
        ...
        **kwargs,
        ...
        return {
            "query": (
                ...
                else final_query.items if hasattr(final_query, "items") else final_query
            ),
            "limit": (
    ...
    def update_object(
        self, lookup_val: Union[int, str], data_dict: Dict[str, Any], *args, **kwargs
    ) -> Callable:
    ...
    def delete_object(self, lookup_val: Union[int, str], *args, **kwargs) -> None:
        """
        ...
        with self.session.no_autoflush:
            ...
            try:
                ...
                if get_config_or_model_meta(
                    ...
                    error_msg = f"Error deleting object, use url parameter `cascade_delete=1` to attempt cascade delete"
                else:
                    error_msg = f"Error deleting object"
