...
from prim.waveform_generator import (
    gen_td_modes_wf,
    gen_td_modes_wf_params,
    td_amp_scale,
)
...
def generate_spliced_pn_waveform(
    ...
    t, hlm = gen_td_modes_wf(p, modes=[(2, 2)])
    amp_scale = td_amp_scale(M, p["r"])  # pyright: ignore
    for lm in hlm.keys():
...
def pn_h_22(x, eta, phi, chi1, chi2):
    ...
    Hlm = np.sqrt(16 * np.pi / 5) * hatHlm * np.exp(-1.0j * mm * phi)
