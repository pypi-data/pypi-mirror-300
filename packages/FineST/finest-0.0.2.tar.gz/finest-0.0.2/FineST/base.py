...
class MixedGaussian:
    ...
    def run_cluster(self,Y,K,pi=None,mean=None,sigma_sq=None,delta=None,iter=500):
        ...
        omega = np.ones((self.G,self.K))/self.K
        converge = False
        count = 0
            ...
            for k in range(self.K):
                if self.pi[k] == 0:
                    ...
                    for g in range(self.G):
                        ...
                        omega[g,k] = self.pi[k]/np.sum(self.pi * _pexp((self.ll[g]-self.ll[g][k])/np.sqrt(self.N)))
            new_mean = self.update_param(omega)
            count += 1
