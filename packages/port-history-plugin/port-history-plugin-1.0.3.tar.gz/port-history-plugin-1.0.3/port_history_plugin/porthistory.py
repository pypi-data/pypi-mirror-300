...
class BaseCheckPorts():
    ...
    def __init__(self):
        ...
        self.device_role = DeviceRole.objects.filter(slug__in=self.SWITCHES_ROLE_SLUG)
        self.alert_devices = []
    ...
    def get_base_netbox_data(self):
        nb_devices = Device.objects.filter(device_role__in=self.device_role)
                ...
                for cnt_on_port, dst_host_dict in ifindex.items():
                    try:
                        if ('device' and 'mac') in dst_host_dict.keys():
                            ...
                            if dst_host_dict['mac'] != self.device_dict[device_ip]['cable'][ifindex['name']]['mac']:
                                status = 'failed'
                            else:
                                status = 'passed'
                            self.writable_devices.append({'device_ip': device_ip,
