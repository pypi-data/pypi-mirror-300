...
class OptimizeConfigResponse(BaseModel):
    ...
    optimization_objective: Optional[StrictStr] = Field(None, alias="optimizationObjective", description="Hyperparameter optimization objective")
    ...
    def optimization_objective_validate_enum(cls, v):
        if v is None:
            return v

        if v not in ('accuracy', 'ram', 'rom', 'latency'):
            raise ValueError("must validate the enum values ('accuracy', 'ram', 'rom', 'latency')")
        return v
