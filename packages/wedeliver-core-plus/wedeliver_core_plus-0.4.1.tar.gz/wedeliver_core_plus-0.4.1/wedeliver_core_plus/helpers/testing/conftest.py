...
def model_factory(db_session):
    ...
    # Cleanup: Automatically delete models based on dependencies
    # Cleanup: Delete all created models
    for model_instance in reversed(created_models):
    ...
    if created_models:
        try:
            ...
            db_session.rollback()
            pytest.fail(f"CircularDependencyError during cleanup: {e}")
...
def web_auth_token(test_app):
    def get_token(role=Role.SYSTEM_ADMIN):
...
def app_auth_token(test_app):
    def get_token(role=Role.APP_USER, customer_id=1, account_id=2, device_id=1, language='ar', currency='SAR',
                  country_code='sa', nick_name='Test Nickname', image=""):
        customer = dict(
            id=customer_id,
            nick_name=nick_name,
            image=image
