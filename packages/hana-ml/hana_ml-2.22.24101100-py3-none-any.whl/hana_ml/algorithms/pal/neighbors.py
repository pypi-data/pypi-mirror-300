...
class _KNNBase(PALBase):
    ...
    def _fit(self, data, key=None, features=None, label=None,
        ...
        if not self._disable_hana_execution:
            ...
            if not data.dtypes([self.label])[0][1] in ('INT', 'FLOAT', 'DOUBLE') or self.categorical_variable is not None and self.label in self.categorical_variable:
                if self.functionality == self.func_map['regression']:
                    msg = ("The label column data must be numerical for regression.")
                    logger.error(msg)
                    raise ValueError(msg)
            if data.dtypes([self.label])[0][1] in ('FLOAT', 'DOUBLE') and self.functionality == self.func_map['classification']:#pylint:disable=line-too-long
...
class KNNClassifier(_KNNBase):
    ...
    with no explicit training phase. It assumes similar instances should have similar labels or values.

    In the prediction phase, given a query sample x, its top K nearest samples are found in the training set first, then the label or value of x is assigned with some metric using the K nearest neighbors.
    In order to speed up the search, the KD-tree searching method is provided.
    ...
    def fit(self, data, key=None, features=None, label=None,
        ...
        features : str or a list of str, optional
            ...
            Defaults to non-key, non-label columns.
        label : str, optional
    ...
    def predict(self, data, key=None, features=None,
        ...
        Training dataset must be constructed through the fit function first.
...
class KNNRegressor(_KNNBase):
    ...
    >>> knn  = KNNRegressor(algorithm='kd_tree',
    ...
    def fit(self, data, key=None, features=None, label=None,
        ...
        Returns
        -------
        A fitted object of class "KNNRegressor".
    ...
    def predict(self, data, key=None, features=None,
                ...
                random_state=None):
        r"""
        Prediction for the input data with the training dataset. Training dataset must be
...
class KNN(PALBase): # pragma: no cover
    """
    ...
    def predict(self, data, key, features=None):#pylint:disable=too-many-locals
        ...
        if not hasattr(self, '_training_set'):
            raise FitIncompleteError("_training_set is None. Please call the fit() method first.")
        key = self._arg('key', key, str, True)
        features = self._arg('features', features, ListOfStrings)
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        col_left = data.columns
    ...
    def score(self, data, key, features=None, label=None):#pylint:disable=too-many-locals
        """
        Returns the scalar accuracy on the given test data and labels.
