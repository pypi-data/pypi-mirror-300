...
class RequestMaxLimitExceeds(Exception):
    """
    Exception class which is thrown when `post`, `patch` or `put` request body exceeds 8KB
    """

    def __init__(self, request_size: int):
        super().__init__(
            f"Request body exceeds limit. Max Limit: {REQUEST_BODY_LIMIT} bytes"
        )
...
class HTTPWrapper:
    ...
    def _validate_json_size(self, json: json_types):
        ...
        if payload_size > REQUEST_BODY_LIMIT:
            raise RequestMaxLimitExceeds(payload_size)
