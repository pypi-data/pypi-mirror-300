...
class CodeObjectCreateInput(RhinoBaseModel):
    ...
    output_data_schema_uids: List[Optional[str]] = []
...
class CodeObject(WithinProjectModel, CodeObjectCreateInput, UserCreatedModel):
    ...
    input_data_schema_uids: Annotated[
        List[str],
        UIDField(
            model_fetcher=lambda session, uids: session.data_schema.get_data_schemas(uids),
            model_property_name="input_data_schemas",
            model_property_type="DataSchema",
            is_list=True,
        ),
    ] = []
    """@autoapi True A list of uids of data schemas this CodeObject expects input datasets to adhere to."""
    output_data_schema_uids: Annotated[
        List[Optional[str]],
        UIDField(
            model_fetcher=lambda session, uids: session.data_schema.get_data_schemas(uids),
            model_property_name="output_data_schemas",
            model_property_type="DataSchema",
            is_list=True,
        ),
    ] = []
