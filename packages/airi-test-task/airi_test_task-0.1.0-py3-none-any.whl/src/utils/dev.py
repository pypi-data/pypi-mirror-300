...
class LargeParamMLP(nn.Module):
    def __init__(self, input_size, hidden_sizes, output_size, activation, dropout_rate, batch_norm):
...
def train_and_evaluate_model(model, optimizer, train_dataloader, val_dataloader, run_name, num_epochs=10):
    with mlflow.start_run(
        ...
        lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="min", patience=14, factor=0.2, verbose=True)
...
def objective(config):
    ...
    model = LargeParamMLP(input_size=358, hidden_sizes=hidden_sizes, output_size=1,
                          activation=activation, dropout_rate=dropout_rate, batch_norm=batch_norm).cuda()
    ...
    with open(fr"D:\\Python_Projects\\Jupyter\\test-job-tasks\\airi\\data\\secondary\\dataloaders\dataloaders.pkl", "rb") as file:
        train_dataloader, val_dataloader, test_dataloader = pickle.load(file)
    
    validation_loss = train_and_evaluate_model(model, optimizer, train_dataloader, val_dataloader, run_name=run_name, num_epochs=300)
