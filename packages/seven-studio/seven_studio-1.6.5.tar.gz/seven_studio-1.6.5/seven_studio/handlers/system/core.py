...
:LastEditTime: 2024-10-09 11:14:26
...
class VerifyCodeHandler(StudioBaseHandler):
    ...
    def get_async(self):
        ...
        elif check_level=="verify_code" or check_level=="safety" or check_level=="safety_strong":
            # 默认模式，白名单无需手机验证
        ...
        if user_info and user_info["Phone"]:
            if check_level=="verify_code":
                ...
                return self.response_common("Check All", "安全验证模式")
    ...
    def post_async(self):
        ...
        resposne = HTTPHelper.post("https://ps.gao7.com/verify_code/sms",request_data,headers={'Content-Type': 'application/json'})
...
class LoginPlatformHandler(StudioBaseHandler):
    ...
    def login(self, account="", password="", verify_code=""):
        ...
        user_info = user_info_model.get_entity("Account=%s or Phone=%s", params=[account,account])
        if not user_info:
        ...
        if verify==True and verify_code:
            # 验证用户验证码
            resposne = HTTPHelper.get(f"https://ps.gao7.com/verify_code/sms?mobile={user_info.Phone}&project_name={config.get_value('project_name')}")

            if resposne:
                verify_json = json.loads(resposne.text)
                if verify_json["result"]==0:
                    invoke_result.ResultCode = "VerifyCodeExpired"
                    invoke_result.ResultMessage = f"验证码已过期"
                    verify=False
