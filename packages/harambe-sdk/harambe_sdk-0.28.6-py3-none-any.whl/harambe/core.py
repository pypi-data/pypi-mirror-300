...
class SDK:
    ...
    async def ai(self, element: ElementHandle, fields: list[str], guidance: str) -> dict[str, Any]:
        """
        Use an LLM to dynamically generate field values based on an unstructured text element on the page.

        Only use this when the text element is unstructured and there is no reliable way to extract the data
        out of the element. For example, when there are lots of fields within a single paragraph.
        """
        if not self._llm:
            raise RuntimeError("The SDK.ai method cannot be run without an llm instance provided.")

        # TODO: We should use instructor and get an actual pydantic model to represent the subset of the schema


        return self._llm.get_fields(element, fields, guidance)
