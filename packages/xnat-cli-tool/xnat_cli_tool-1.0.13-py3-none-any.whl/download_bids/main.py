...
import os
import shutil
from .xnat2bids import convert2bids
import json

def has_bids_and_scans_files(directory):
    """
    Check if a directory contains subdirectories named 'BIDS' and 'SCANS' at any level
    and if they contain any files.
    """
    has_bids_files = False
    has_scans_files = False

    for dirpath, dirnames, filenames in os.walk(directory):
        if 'BIDS' in dirnames:
            bids_path = os.path.join(dirpath, 'BIDS')
            for bids_dirpath, bids_dirnames, bids_filenames in os.walk(bids_path):
                if bids_filenames:
                    has_bids_files = True
                    break  
        
        if 'SCANS' in dirnames:
            scans_path = os.path.join(dirpath, 'SCANS')
            for scans_dirpath, scans_dirnames, scans_filenames in os.walk(scans_path):
                if scans_filenames:
                    has_scans_files = True
                    break 
        
       
        if has_bids_files and has_scans_files:
            return True

    return has_bids_files and has_scans_files
...
def download_subject_data(session, subject, head_path, temp_dir):
    ...
    for experiment in subject.experiments.values():
        ...
        # Check for BIDS resource at the session level
        bids_resource = experiment.resources.get('BIDS')
        if bids_resource:
            try:
                bids_resource.download_dir(temp_dir)
                session_bids_dir = os.path.join(head_path, experiment_id, 'RESOURCES/BIDS')
                os.makedirs(session_bids_dir, exist_ok=True)
                move_files(temp_dir, session_bids_dir)
                print(f"BIDS data for experiment {experiment_id} downloaded")
            except Exception as e:
                print(f'Error downloading BIDS data for experiment {experiment_id}: {e}')
...
def download_data(output_dir=None, subjects=None, project=None):
    ...
    with connect(server, username, secret) as session:
        ...
        for project_dir in os.listdir('./input'):
            project_path = os.path.join('./input', project_dir)
            if os.path.isdir(project_path):
                for subject_dir in os.listdir(project_path):
                    subject_path = os.path.join(project_path, subject_dir)
                    if os.path.isdir(subject_path):
                        for session_dir in os.listdir(subject_path):
                            session_path = os.path.join(subject_path, session_dir)
                            if os.path.isdir(session_path):
                                if has_bids_and_scans_files(session_path):
                                    output_project_path = os.path.join(output_dir, project_dir)
                                    if not os.path.exists(output_project_path):
                                        os.makedirs(output_project_path)
                                    convert2bids(session_path, output_project_path)
...
def download_bids_script(args):
    """
    Script entry point for downloading BIDS data based on provided arguments.
    """
