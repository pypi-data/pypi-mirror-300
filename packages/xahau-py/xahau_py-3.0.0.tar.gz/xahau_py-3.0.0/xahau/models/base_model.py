...
def _key_to_json(field: str) -> str:
    ...
    Raises:
        XAHLModelException: If the input is invalid
    """
    if not re.fullmatch(pattern=_PASCAL_OR_CAMEL_CASE, string=field):
        raise XAHLModelException(f"Key {field} is not in the proper XAHL format.")
...
def _value_to_json(value: XAHL_VALUE_TYPE) -> XAHL_VALUE_TYPE:
...
class BaseModel(ABC):
    ...
    @classmethod
    def from_dict(cls: Type[Self], value: Dict[str, XAHL_VALUE_TYPE]) -> Self:
        """
    ...
    def _from_dict_single_param(
        ...
        if (
            ...
            # expected an XAHL Model, received a Dict
    ...
    def _process_xrpl_json(
        ...
        """
        Creates a dictionary object based on a JSON or dictionary in the standard XAHL
        format.
        ...
        formatted_dict = {
            ...
            for (k, v) in cast(Dict[str, XAHL_VALUE_TYPE], value).items()
        }
    ...
    def validate(self: Self) -> None:
        ...
        Raises:
            XAHLModelException: if this object is invalid.
        """
        errors = self._get_errors()
        if len(errors) > 0:
            raise XAHLModelException(str(errors))
