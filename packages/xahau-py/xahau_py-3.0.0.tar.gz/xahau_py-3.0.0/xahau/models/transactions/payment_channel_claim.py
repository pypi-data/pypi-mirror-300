...
class PaymentChannelClaimFlag(int, Enum):
    ...
    XAH allocated to it after processing the current claim, or if the destination
    address uses it. If the source address uses this flag when the channel still
    holds XAH, this schedules the channel to close after `SettleDelay` seconds have
    ...
    uses this flag when the channel still holds XAH, any XAH that remains after
...
class PaymentChannelClaim(Transaction):
    ...
    transaction, which claims XAH from a `payment channel
