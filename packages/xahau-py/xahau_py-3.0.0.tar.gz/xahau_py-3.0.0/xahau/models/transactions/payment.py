...
from xahau.models.amounts import Amount, is_xrp
from xahau.models.flags import FlagInterface
from xahau.models.path import Path
...
class Payment(Transaction):
    ...
    omitted for XAH-to-XAH transactions.
    """
    ...
    exchange rates, and slippage. Does not include the XAH destroyed as a
    cost for submitting the transaction. Must be supplied for cross-currency
    or cross-issue payments. Must be omitted for XAH-to-XAH payments.
    """
    ...
    def _get_errors(self: Self) -> Dict[str, str]:
        ...
        # XAH transaction errors
        if is_xrp(self.amount) and self.send_max is None:
            if self.paths is not None:
                errors["paths"] = "An XAH-to-XAH payment cannot contain paths."
            if self.account == self.destination:
                errors["destination"] = (
                    "An XAH payment transaction cannot have the same sender and "
                    "destination."
                )
