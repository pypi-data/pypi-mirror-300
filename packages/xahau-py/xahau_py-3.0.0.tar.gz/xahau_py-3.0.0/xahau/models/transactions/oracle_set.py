...
class OracleSet(Transaction):
    ...
    Create or own the XAHL account in the Owner field and have enough XAH to meet the
    reserve and transaction fee requirements.
    Publish the XAHL account public key, so it can be used for verification by dApps.
    ...
    """
    This field must be hex-encoded. You can use `xahau.utils.str_to_hex` to
