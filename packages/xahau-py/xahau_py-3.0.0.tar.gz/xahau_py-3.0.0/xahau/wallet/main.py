"""The information needed to control an XAHL account."""
...
from xahau.core.addresscodec import classic_address_to_xaddress, ensure_classic_address
from xahau.core.addresscodec.exceptions import XAHLAddressCodecException
from xahau.core.keypairs import derive_classic_address, derive_keypair, generate_seed


class Wallet:
    """
    The cryptographic keys needed to control an XAH Ledger account. See
    ...
    def address(self: Self) -> str:
        """
        The XAHL address that publicly identifies this wallet,
    ...
    def classic_address(self: Self) -> str:
        ...
        destination tag, and XAHL address into a single value.
        ...
        *,
        ...
        if seed is not None:
            try:
                ...
                raise XAHLAddressCodecException(
    ...
    def from_entropy(
        ...
        *,
        ...
        Raises:
            XAHLException: If passed in entropy is not a bytestring.
        """
    ...
    def from_secret_numbers(
        ...
        *,
        ...
        Raises:
            XAHLException: If the number of secret numbers is not 8. If the length of
                any secret number is not 6. If the checksum of any secret number is
                invalid.
        """
        # Logic adapted from xahau-secret-numbers secretToEntropy function
        # https://github.com/XAHLF/xrpl.js/blob/main/packages/secret-numbers/src/utils/index.ts
        ...
        if len(parsed_secret_numbers) != 8:
            raise XAHLException("There must be 8 secret numbers.")
        ...
        for i, secret_number in enumerate(parsed_secret_numbers):
            ...
            if len(secret_number) != 6:
                raise XAHLException("Each secret number must be 6 digits long.")
            if no * (i * 2 + 1) % 9 != checksum:
                raise XAHLException(f"Checksum of secret number {i} is invalid.")
