...
from xahau.core.keypairs.main import sign as keypairs_sign
from xahau.models.requests import ServerInfo, ServerState, SubmitOnly
from xahau.models.response import Response
from xahau.models.transactions import EscrowFinish
from xahau.models.transactions.transaction import Signer, Transaction
from xahau.models.transactions.transaction import (
...
from xahau.utils import drops_to_xah, xah_to_drops
...
# xahau-py 1.0, and we decided it wasn't worth breaking people's imports to move
# It to a central location as part of the xahau-py 2.0 changes. It is aliased in
...
def _prepare_transaction(
    ...
    Raises:
        XAHLException: if both LastLedgerSequence and `ledger_offset` are provided, or
...
def _validate_account_xaddress(
    ...
    Raises:
        XAHLException: if both an X-Address containing a tag and a tag field are
    ...
    if is_valid_xaddress(json[account_field]):
        ...
        if tag_field in json and json[tag_field] != tag:
            raise XAHLException(f"{tag_field} value does not match X-Address tag")
        json[tag_field] = tag
...
async def _check_fee(
    ...
    Raises:
        XAHLException: if the transaction fee is higher than the expected fee.
    """
    expected_fee = max(
        xah_to_drops(0.1),  # a fee that is obviously too high
    ...
    if transaction.fee and int(transaction.fee) > int(expected_fee):
        raise XAHLException(
            f"Fee value: {str(drops_to_xah(transaction.fee))} XAH is likely entered "
...
async def _calculate_fee_per_transaction_type(
    ...
    if client is None:
        ...
        net_fee = int(await get_fee(client))  # Usually 0.00001 XAH (10 drops)
