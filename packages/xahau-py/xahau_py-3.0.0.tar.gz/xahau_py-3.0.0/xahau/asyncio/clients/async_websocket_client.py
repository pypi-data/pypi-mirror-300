...
from xahau.asyncio.clients.client import REQUEST_TIMEOUT
from xahau.asyncio.clients.exceptions import XAHLWebsocketException
from xahau.asyncio.clients.websocket_base import WebsocketBase
...
class AsyncWebsocketClient(AsyncClient, WebsocketBase):
    ...
    example can be copied directly into a file and run with no changes::
        ...
        from xahau.asyncio.clients import AsyncWebsocketClient
        from xahau.models import Subscribe, Unsubscribe, StreamParameter
    ...
    async def send(self: Self, request: Request) -> None:
        ...
        Raises:
            XAHLWebsocketException: If there is already an open request by the
                request's ID, or if this WebsocketBase is not open.
        """
        if not self.is_open():
            raise XAHLWebsocketException("Websocket is not open")
