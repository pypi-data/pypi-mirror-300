...
from xahau.core.addresscodec.codec import encode_classic_address
...
def parse_nftoken_id(nft_id: str) -> NFTokenID:
    ...
    |
    `---> Flags: 11 -> lsfBurnable, lsfOnlyXAH and lsfTransferable

    Args:
        nft_id: A hex string which identifies an NFToken on the ledger.

    Raises:
        XAHLException: when given an invalid Token ID as nft_id.
