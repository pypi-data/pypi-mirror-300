...
from xahau.utils.txn_parser.utils import NormalizedNode, normalize_nodes
from xahau.utils.txn_parser.utils.parser import get_value, group_by_account
from xahau.utils.txn_parser.utils.types import (
    AccountOfferChange,
    AccountOfferChanges,
    CurrencyAmount,
    OfferChange,
)
from xahau.utils.xah_conversions import drops_to_xah
...
def _derive_currency_amount(
    ...
    if isinstance(currency_amount, str):
        return CurrencyAmount(currency="XAH", value=str(drops_to_xah(currency_amount)))
    else:
