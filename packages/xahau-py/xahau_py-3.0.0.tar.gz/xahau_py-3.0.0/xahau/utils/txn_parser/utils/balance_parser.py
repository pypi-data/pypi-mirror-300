...
from xahau.utils.txn_parser.utils.parser import group_by_account
from xahau.utils.txn_parser.utils.types import AccountBalance, AccountBalances, Balance
...
def _get_xrp_quantity(
    ...
    xrp_value = (
        drops_to_xah(str(absolute_value)).copy_negate()
        if value.is_signed()
        else drops_to_xah(str(absolute_value))
    )
