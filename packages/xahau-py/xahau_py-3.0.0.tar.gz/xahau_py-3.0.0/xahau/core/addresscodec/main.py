...
from xahau.core.addresscodec.codec import decode_classic_address, encode_classic_address
...
def classic_address_to_xaddress(
    ...
    Raises:
        XAHLAddressCodecException: If the classic address does not have enough bytes
            or the tag is invalid.
    """
    classic_address_bytes = decode_classic_address(classic_address)
    if len(classic_address_bytes) != 20:
        raise XAHLAddressCodecException("Account ID must be 20 bytes")

    if tag is not None and tag > MAX_32_BIT_UNSIGNED_INT:
        raise XAHLAddressCodecException("Invalid tag")
    ...
    return base58.b58encode_check(bytestring, alphabet=XAHL_ALPHABET).decode("utf-8")
...
def xaddress_to_classic_address(xaddress: str) -> Tuple[str, Optional[int], bool]:
    ...
    decoded = base58.b58decode_check(
        xaddress, alphabet=XAHL_ALPHABET
...
def ensure_classic_address(account: str) -> str:
    ...
    Raises:
        XAHLAddressCodecException: if the X-Address has an associated tag.
    """
...
def _is_test_address(prefix: bytes) -> bool:
    ...
    Raises:
        XAHLAddressCodecException: If the prefix is invalid.
    """
    if _PREFIX_BYTES_MAIN == prefix:
        return False
    if _PREFIX_BYTES_TEST == prefix:
        return True
    raise XAHLAddressCodecException("Invalid X-Address: bad prefix")
...
def _get_tag_from_buffer(buffer: bytes) -> Optional[int]:
    ...
    flag = buffer[0]
    if flag >= 2:
        raise XAHLAddressCodecException("Unsupported X-Address")
    ...
    if flag != 0:
        raise XAHLAddressCodecException("Flag must be zero to indicate no tag")
    if bytes.fromhex("0000000000000000") != buffer[1:9]:
        raise XAHLAddressCodecException("Remaining bytes must be zero")
    return None
...
def is_valid_xaddress(xaddress: str) -> bool:
    ...
    try:
        ...
        return True
    except (XAHLAddressCodecException, ValueError):
        return False
