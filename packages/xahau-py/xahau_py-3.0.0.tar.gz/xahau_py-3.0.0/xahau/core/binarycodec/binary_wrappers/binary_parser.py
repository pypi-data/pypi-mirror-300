...
from xahau.core.binarycodec.definitions import definitions
...
# Max length that can be represented in a single byte per XAHL serialization encoding
_MAX_SINGLE_BYTE_LENGTH: Final[int] = 192
# Max length that can be represented in 2 bytes per XAHL serialization restrictions
...
class BinaryParser:
    """Deserializes from hex-encoded XAHL binary format to JSON fields and values."""
    ...
    def skip(self: Self, n: int) -> None:
        ...
        Raises:
            XAHLBinaryCodecException: If n bytes can't be skipped.
        """
        if n > len(self.bytes):
    ...
    def read_field_header(self: Self) -> FieldHeader:
        ...
        Raises:
            XAHLBinaryCodecException: If the field ID cannot be read.
        """
    ...
    def read_field_value(self: Self, field: FieldInstance) -> SerializedType:
        ...
        Raises:
            XAHLBinaryCodecException: If a parser cannot be constructed from field.
        """
