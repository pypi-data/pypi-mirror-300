...
counterparty (issuer) on the trustline that holds the value. For XAH, there is no
counterparty.
"""
...
def is_xrp(amount: Amount) -> bool:
    """
    Returns whether amount is an XAH value, as opposed to an issued currency.

    Args:
        amount: The Amount object whose type is being checked.

    Returns:
        Whether the amount is an XAH value.
    """
...
def is_issued_currency(amount: Amount) -> bool:
    """
    Returns whether amount is an issued currency value, as opposed to an XAH value.
