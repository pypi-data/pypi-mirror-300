...
class XChainCreateBridge(Transaction):
    ...
    """
    The minimum amount, in XAH, required for a ``XChainAccountCreateCommit``
    transaction. If this isn't present, the ``XChainAccountCreateCommit``
    transaction will fail. This field can only be present on XAH-XAH bridges.
    ...
    def _get_errors(self: Self) -> Dict[str, str]:
        ...
        if (bridge.locking_chain_issue == XAH()) != (
            bridge.issuing_chain_issue == XAH()
        ):
            errors["issue"] = "Bridge must be XAH-XAH or IOU-IOU."

        if (
            self.min_account_create_amount is not None
            and bridge.locking_chain_issue != XAH()
        ):
            errors[
