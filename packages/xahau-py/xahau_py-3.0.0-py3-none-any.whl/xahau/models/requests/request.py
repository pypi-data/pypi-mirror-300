...
class RequestMethod(str, Enum):
    ...
    # (there is no XAHL analog, this model is specific to xahau-py)
...
class Request(BaseModel):
    ...
    def from_dict(cls: Type[Self], value: Dict[str, Any]) -> Self:
        ...
        if cls.__name__ == "Request":
            if "method" not in value:
                raise XAHLModelException("Request does not include method.")
    ...
    def get_method(cls: Type[Self], method: str) -> Type[Request]:
        ...
        if method == RequestMethod.NO_RIPPLE_CHECK:
            return xahau.models.requests.NoRippleCheck
        if method == RequestMethod.ACCOUNT_NFTS:
            return xahau.models.requests.AccountNFTs
        if method == RequestMethod.NFT_BUY_OFFERS:
            return xahau.models.requests.NFTBuyOffers
        if method == RequestMethod.NFT_SELL_OFFERS:
            return xahau.models.requests.NFTSellOffers
        if method == RequestMethod.NFT_INFO:
            return xahau.models.requests.NFTInfo
        if method == RequestMethod.NFT_HISTORY:
            return xahau.models.requests.NFTHistory
        if method == RequestMethod.NFTS_BY_ISSUER:
            return xahau.models.requests.NFTsByIssuer
        parsed_name = "".join([word.capitalize() for word in method.split("_")])
        if parsed_name in xahau.models.requests.__all__:
            return cast(Type[Request], getattr(xahau.models.requests, parsed_name))
        return xahau.models.requests.GenericRequest
