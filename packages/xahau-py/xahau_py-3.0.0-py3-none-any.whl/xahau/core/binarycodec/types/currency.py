"""Codec for currency property inside an XAHL issued currency amount json."""
...
def _iso_to_bytes(iso: str) -> bytes:
    ...
    if not _is_iso_code(iso):
        raise XAHLBinaryCodecException(f"Invalid ISO code: {iso}")

    if iso == "XAH":
        # This code (160 bit all zeroes) is used to indicate XAH in
        # rare cases where a field must specify a currency code for XAH.
...
class Currency(Hash160):
    ...
    def __init__(self: Self, buffer: Optional[bytes] = None) -> None:
        ...
        if self.buffer[0] != 0:
            ...
            # the special case for literal XAH
            self._iso = "XAH"
        else:
    ...
    def from_value(cls: Type[Self], value: str) -> Self:
        ...
        Raises:
            XAHLBinaryCodecException: If the Currency representation is invalid.
        """
        ...
        raise XAHLBinaryCodecException("Unsupported Currency representation: {value}")
