...
def _encode(bytestring: bytes, prefix: List[int], expected_length: int) -> str:
    ...
    if expected_length and len(bytestring) != expected_length:
        ...
        raise XAHLAddressCodecException(error_message)
    encoded_prefix = bytes(prefix)
    payload = encoded_prefix + bytestring
    return base58.b58encode_check(payload, alphabet=XAHL_ALPHABET).decode("utf-8")
...
def _decode(b58_string: str, prefix: bytes) -> bytes:
    ...
    decoded = base58.b58decode_check(b58_string, alphabet=XAHL_ALPHABET)
    if decoded[:prefix_length] != prefix:
        raise XAHLAddressCodecException("Provided prefix is incorrect")
...
def encode_seed(entropy: bytes, encoding_type: CryptoAlgorithm) -> str:
    ...
    Raises:
        XAHLAddressCodecException: If entropy is not of length SEED_LENGTH
            or the encoding type is not one of CryptoAlgorithm.
    """
    if len(entropy) != SEED_LENGTH:
        raise XAHLAddressCodecException(f"Entropy must have length {SEED_LENGTH}")
...
def decode_seed(
    ...
    Raises:
        XAHLAddressCodecException: If the seed is invalid.
    """
    if algorithm is not None:
        ...
        for prefix in _ALGORITHM_TO_PREFIX_MAP[algorithm]:
            ...
            except XAHLAddressCodecException:
                # prefix is incorrect, wrong prefix
                continue
        raise XAHLAddressCodecException("Wrong algorithm for the seed type.")
