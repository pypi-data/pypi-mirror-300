...
from xahau.asyncio.transaction.main import _check_fee
from xahau.asyncio.transaction.main import autofill as _autofill
from xahau.asyncio.transaction.main import sign, submit
...
class XAHLReliableSubmissionException(XAHLException):
    """General XAHL Reliable Submission Exception."""
...
async def _wait_for_final_transaction_outcome(
    ...
    if current_ledger_sequence >= last_ledger_sequence:
        raise XAHLReliableSubmissionException(
    ...
    if not transaction_response.is_successful():
        if transaction_response.result["error"] == "txnNotFound":
            ...
            raise XAHLRequestFailureException(transaction_response.result)
    ...
    if "validated" in result and result["validated"]:
        ...
        if return_code != "tesSUCCESS":
            raise XAHLReliableSubmissionException(f"Transaction failed: {return_code}")
...
async def _send_reliable_submission(
    ...
    Raises:
        XAHLReliableSubmissionException: if the transaction fails, is malformed, or is
    ...
    if prelim_result[0:3] == "tem":
        ...
        raise XAHLReliableSubmissionException(f"{prelim_result}: {error_message}")
...
async def _get_signed_tx(
    ...
    Raises:
        XAHLException: if the transaction is unsigned and a wallet is not
