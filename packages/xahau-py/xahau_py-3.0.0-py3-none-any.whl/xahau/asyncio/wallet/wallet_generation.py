...
from xahau.asyncio.account import get_balance, get_next_valid_seq_number
...
class XAHLFaucetException(XAHLException):
...
async def generate_faucet_wallet(
    ...
    user_agent: Optional[str] = "xahau-py",
) -> Wallet:
    """
    Generates a random wallet and funds it using the XAHL Testnet Faucet.
    ...
    Args:
        ...
        user_agent: A string representing the user agent (software/ client used)
            for the HTTP request. Default is "xahau-py".


    Returns:
        A Wallet on the testnet that contains some amount of XAH.

    Raises:
        XAHLFaucetException: if an address could not be funded with the faucet.
        XAHLRequestFailureException: if a request to the ledger fails.
...
def get_faucet_url(url: str, faucet_host: Optional[str] = None) -> str:
    ...
    Raises:
        XAHLFaucetException: if the provided URL is not for the testnet or devnet.
    """
    if faucet_host is not None:
        return process_faucet_host_url(faucet_host)
    if "xahau-test" in url or "testnet" in url:  # testnet
...
async def _check_wallet_balance(address: str, client: Client) -> int:
    ...
    except XAHLRequestFailureException as e:
