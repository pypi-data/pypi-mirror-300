...
def enum(array, start=1):
    return [i for i in range(start, start + len(array))]

def is_online(url="https://www.google.com", timeout=1):
    try:
        return rq.get(url, timeout=timeout).status_code == 200
...
def divisors(n):
    ...
    for i in range(1, int(abs(n)**0.5) + 1):
        if abs(n) % i == 0:
            ...
            if i != abs(n) // i:
                positive_divisors.append(abs(n) // i)
    if n < 0:
        negative_divisors = [-d for d in positive_divisors]
        return sorted(negative_divisors + positive_divisors)
    return sorted(positive_divisors)
...
def decompose_matrice(n):
    ...
    factors = get_prime_factors(n)
    matrice = [[prime, exponent] for prime, exponent in sorted(factors.items())]
