...
async def run(account: LocalAccount, logger: Logger) -> AsyncGenerator[AaveEvent, None]:
    ...
    while True:
        ...
        for token in filter(lambda token: token.chain != next_token.chain, tokens):
            try:
                if (withdrawn_amount := await withdraw(token, account, logger)) > 0:
        ...
        try:
            if (amount := await supply(next_token, account, logger)) <= 0:
                continue
