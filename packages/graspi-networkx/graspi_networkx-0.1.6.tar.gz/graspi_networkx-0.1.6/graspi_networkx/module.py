...
def createGraph(txt):
    ...
    with open(txt, 'r') as file:
        ...
        for a in range(y + 1):
            if (
                    a == 0 or a == 1):  # skips over the metadata and the first row of the graph, since those two lines have already been processed
                continue
            ...
            for i in range(x):
                ...
                if (i != 0):
                    ...
                    g.add_edge(nodeIndex, nodeIndex - (x + 1),
                               weight=math.sqrt(2))  # node to the bottom left of the current node
                if (i != x - 1):
                    g.add_edge(nodeIndex, nodeIndex - (x - 1),
                               weight=math.sqrt(2))  # node to the bottom right of the current node
                nodeIndex += 1
    print(nodeIndex)
...
def filterGraph(inputG):
    filter_edges = []
    filtered_edges = [(u, v) for u, v, d in inputG.edges(data=True) if
                      ((inputG.nodes[v]['color'] == 'black') and (inputG.nodes[u]['color'] == 'black')) or (
                                  (inputG.nodes[v]['color'] == 'red') and (inputG.nodes[u]['color'] == 'black')) or (
                                  (inputG.nodes[v]['color'] == 'black') and (inputG.nodes[u]['color'] == 'red')) or (
                                  (inputG.nodes[v]['color'] == 'white') and (inputG.nodes[u]['color'] == 'white')) or (
                                  (inputG.nodes[v]['color'] == 'white') and (inputG.nodes[u]['color'] == 'blue')) or (
                                  (inputG.nodes[v]['color'] == 'blue') and (inputG.nodes[u]['color'] == 'white'))]
    H = nx.Graph()
...
def bfs(fitleredGraph, source=1):
    return nx.single_source_shortest_path(fitleredGraph, source)