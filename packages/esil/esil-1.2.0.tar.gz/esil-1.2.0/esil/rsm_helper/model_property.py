...
LastEditors: Devin
LastEditTime: 2024-06-18 22:05:10
...
class model_attribute:
    ...
    def __init__(self, nc_file):
        ...
        self.lon_start, self.lat_start = projection(self.min_x, self.min_y , inverse=True)
        self.lon_end, self.lat_end = projection(self.max_x, self.max_y, inverse=True)
        import cartopy.crs as ccrs
        self.projection = ccrs.LambertConformal(central_longitude=center_lon, central_latitude=center_lat,
        ...
        if  self.is_BC:
            ...
            if  self.start_date:
                ...
                self.start_date= date_helper.convert_julian_regular_date(self.start_date)
            ...
            self.x_coords_bc, self.y_coords_bc,  self.x_coords, self.y_coords = self.get_xy_coords()

            #self.lons, self.lats = projection(self.x_coords, self.y_coords , inverse=True)
            # 将 Lambert 投影坐标系下的 x、y 转换为经纬度坐标系下的 x、y
            self.lats, self.lons = transform(projection, 'EPSG:4326', self.x_coords_bc,  self.y_coords_bc)  # 'EPSG:4326' 表示经纬度坐标系
    ...
    return {numpy.ndarray(1D), numpy.ndarray(1D)}
    '''    
    def get_xy_coordinates(self,  show_lonlat=False):
        if show_lonlat:
            lon_start, lat_start, lon_end, lat_end = self.lon_start,self.lat_start,self.lon_end,self.lat_end
            x = np.linspace(lon_start, lon_end, self.cols)
            y = np.linspace(lat_start, lat_end, self.rows)
        else:
