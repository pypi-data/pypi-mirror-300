...
class IngestionJob:
    ...
    def __init__(self, parameters:dict=None, debug_mode:bool=False, process_history=False) -> None:
        """
...
"""
    ...
    def __get_process_history(self, args, print_level:int):
        if args is False or args is None or not isinstance(args, dict):
            return False
    ...
    def get_split_part(self, s:str, d="-", p=0) -> str:
        ...
        self.debug_print(f"def GET_SPLIT_PART >> parameters >> s={s}; d={d}, p={p}")
        parts = s.split(d,1)
    ...
    def __read_data_from_ftp(self, kwargs:dict, print_level=1) -> DataFrame:
        ...
        self.pprint("Getting secret in AWS Secrets Manager and connecting to FTP", lvl=print_level)
        ...
        with FTP(**connection_options) as ftp:
            ...
            for path in directories:
                files += [f for f in self.ls(path, env_type="ftp", return_dirs=False, connection=ftp) if kwargs["name"] in f]
    ...
    def __read_data_from_sap(self, kwargs:dict, print_level=1) -> DataFrame:
        ...
        connection_options = {k:v for k,v in kwargs.items() if k in ["user", "passwd", "ashost", "sysnr", "client", "lang"]}
    ...
    def process_parallel_read(self) -> None:
        """
        Process all readings in parallel to reduce the final execution time. This method has no parameters.
        ...
        with ThreadPoolExecutor() as executor:
            future_options = {executor.submit(self.read_data, options): key for key, options in self.__parallel_read_options.items()}
    ...
    def __data_quality_process_failed_expectation(self, ex:dict, original_df:DataFrame, quarantine_df:DataFrame, quarantine_options:dict, print_level:int) -> DataFrame:
        ...
        success_filter = " and ".join(success_clauses)
        quarantine_filter = " or ".join(quarantine_clauses)
    ...
    def __data_quality_run_validation(self, df:DataFrame, args:dict, print_level:int) -> dict:
        self.pprint("Converting Spark DataFrame to Great Expectations DataFrame, applying the Expectation Suite", lvl=print_level)
        import great_expectations as gx
        gx_df = gx.dataset.SparkDFDataset(df)

        self.pprint("Running data quality validation on DataFrame", lvl=print_level)
        validation_results = gx_df.validate(expectation_suite=args, only_return_failures=True, result_format="COMPLETE")
    ...
    def __data_quality_transform_failed_result(self, r:dict, quarantine_path:str, print_level:int) -> dict:
        ...
        if r['expectation_config']['expectation_type'] == "expect_column_distinct_values_to_be_in_set":
        ...
        return {
            ...
            "unexpected_list": list(set(r["result"].get("unexpected_list"))),
    ...
    def data_quality(self, df:DataFrame, args:dict, print_level=1, comment=None):
        ...
        validation_results = self.__data_quality_run_validation(quarantine_df, args, print_level+1)
    ...
    def __prepare_delete_statement(self, delete_keys:list, target_table:str):
        ...
        on_clause = " and ".join([f"source.{c} = target.{c}" for c in self.to_list(delete_keys)])
    ...
    def __prepare_merge_statement(self, merge_keys:list, merge_filter:list, target_table:str):
        ...
        on_clause = " and ".join([f"source.{c} = target.{c}" for c in self.to_list(merge_keys)] + self.to_list(merge_filter))
