...
WITH ranked_token AS (
    ...
    FROM (
    	SELECT ats.overall_rank, ats.token_rank, ats.pair_rank, ats.symbol, ats.name, ats.is_coin, ats.chain_id, 
    	ats.token_id, ats.pair_id, ats.vol_24_hr, ats.liquidity, 
        ats.marketcap, 
        CASE 
        ...
        CASE
            WHEN ats.symbol = '{search_term}' OR ats.name = '{search_term}' THEN 1
            ELSE 2
        END AS match_priority,
        CASE
            WHEN ats.security_tag = "scam" 
            THEN NULL
            ELSE ((ats.liquidity * 7) + (ats.marketcap * 2) + (ats.vol_24_hr * 7))
        END AS score,
        t.is_honeypot, t.can_mint, t.is_proxy, t.is_blacklisted, t.can_burn, t.is_scam, t.can_freeze, t.is_contract_verified,
        t.pc_24_hr
	    FROM blockchains.active_symbols AS ats
	    LEFT JOIN blockchains.tokens AS t ON ats.token_id = t.address
	    WHERE NOT is_coin 
	    AND ({search_condition})
    ) AS sub
