...
class SUtil():
    @staticmethod
    def ExcuteCommandResult(cmd_path,command):
        os.chdir(cmd_path)
    ...
    @staticmethod
    def formatPageData(database_pagination):
        '''转换分页返回数据'''
    ...
    @staticmethod
    def formatPageList(data: list, PageIndex:int =1, PageLimit: int= 10):
        if len(data) == 0:
            return {"list": [], "count": 0}
        ret = {"list": data[(PageIndex-1)*PageLimit:PageIndex*PageLimit], "count": len(data)}
        return ret


    @staticmethod
    def formatPageEntityData(database_pagination):
        '''转换指定字段的分页返回数据,推荐使用'''
        if len(database_pagination.items) == 0:
            return {"list": [], "count": 0}
    ...
    @staticmethod
    # 列表value转字典格式
    def CustomRowsToDict(rows):
        '''用于将查询指定字段的返回体序列化'''
        return [SUtil.CustomRowToDict(row) for row in rows]
    ...
    def formatAntdPageFieldsData(total,datas):
        '''转换查找指定字段分页数据'''
        data = {
            "data": [SUtil.toSingleEntity(data) for data in datas],
            "total": total
        }
        return data
