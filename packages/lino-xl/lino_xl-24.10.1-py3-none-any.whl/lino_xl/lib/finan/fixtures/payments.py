...
def add_demo_account(partner):
    ...
    if acct is None:
        ...
        if len(ibans) == 0:
            print("20220512 No sample IBAN for country {}".format(
                partner.country))
            return None
...
def objects(refs="PMO BNK"):  # welfare calls it with customized refs.
    ...
    if dd.is_installed("vat") and dd.plugins.vat.declaration_plugin is None:
        dd.logger.warning(
            "No demo payments because declaration_plugin is None")
        return
    ...
    for ref in refs.split():
        ...
        if ref == 'PMO':
            ...
            if site_company.country is None:
                raise Exception("Oops, site company {} has no country".format(
                    site_company))
