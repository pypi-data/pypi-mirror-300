...
def objects():
    ...
    cal_users = [
        ut for ut in rt.models.users.UserTypes.get_list_items()
        if ut.has_required_roles([OfficeUser])
    ]
    if len(cal_users) == 0:
        raise Exception("No user type inherits from OfficeUser")
    USERS = Cycler(
        settings.SITE.user_model.objects.exclude(email='').filter(
    ...
    if dd.get_plugin_setting('cal', 'with_demo_appointments'):
        ...
        SUMMARIES = Cycler(
            (dict(en='Lunch', de=u"Mittagessen", fr=u"Diner"),
             ...
             dict(en='Breakfast', de=u"Frühstück",
                  fr=u"Petit-déjeuner"), dd.str2dict(_('Meeting')),
