...
def objects():
    ...
    if False:
        yield trading.InvoicingMode(**dd.babel_values(
            'name', en='Default', de="Standard", fr="Standard"))

    if accounting:
        Invoice = dd.resolve_model('trading.VatProductInvoice')
        InvoiceItem = dd.resolve_model('trading.InvoiceItem')
        vt = accounting.VoucherTypes.get_for_model(Invoice)
