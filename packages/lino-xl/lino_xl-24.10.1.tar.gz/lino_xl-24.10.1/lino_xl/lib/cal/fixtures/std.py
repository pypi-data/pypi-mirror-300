...
def objects():
    ...
    settings.SITE.site_config.update(site_calendar=general,
                                     hide_events_before=d)

    # yield settings.SITE.site_config

    event_type = Instantiator('cal.EventType').build

    yield event_type(planner_column=PlannerColumns.external,
                     is_appointment=True,
                     fill_presences=True,
                     max_days=0,
                     **dd.str2kw('name', _("Absences")))

    holidays = event_type(planner_column=PlannerColumns.external,
    ...
    meeting = event_type(is_appointment=True,
                         fill_presences=True,
                         planner_column=PlannerColumns.external,
    ...
    if dd.get_plugin_setting('cal', 'with_demo_appointments'):
        yield event_type(planner_column=PlannerColumns.internal,
                         is_appointment=False,
                         fill_presences=True,
                         transparent=True,
    ...
    def holiday(month, day, en, de, fr, et=None):
        if et is None:
            et = en
        return add(every_unit=Recurrences.yearly,
                   monday=True,
                   tuesday=True,
                   wednesday=True,
                   thursday=True,
    ...
    def relative_holiday(offset, name):
        return add(every_unit=Recurrences.easter,
                   every=1,
    ...
    exam_policy = Instantiator('cal.EventPolicy',
                               'every',
    ...
    yield exam_policy(3, **dd.str2kw('name', _("Every 3 months")))
