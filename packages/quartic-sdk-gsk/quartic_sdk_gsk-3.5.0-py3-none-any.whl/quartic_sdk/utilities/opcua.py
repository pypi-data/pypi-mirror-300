...
def get_client(config: OPCUAConfig, client_cls: Union[AsyncClient, Client]):
    client = client_cls(config.opcua_url)
    security_config = config.security_conifg
    if security_config:
        client.set_security_string(
            f"{security_config.security_policy},{security_config.security_mode},{security_config.certificate_path},{security_config.private_key_path}"
        )
