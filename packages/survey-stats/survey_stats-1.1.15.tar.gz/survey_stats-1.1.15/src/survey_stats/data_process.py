...
class Data:
    ...
    def set_index(self, variable:str|list, drop_variable:bool=True, duplicate_sum_vars:list=[], index_name:str='',
        ...
        print(a)\n
        duplicate_sum_vars: include variables that execute 'sum' on their values.
        '''
        if print_progress:
                    ...
                    for v in self.variables():
                        if v in duplicate_sum_vars:
                            ...
                            vals = {variable[i]:val
                                  for i, val in enumerate(self.values[v].values())}
                        self.values[v] = vals
            elif variable[0] in self.variables():
                new_index = []
                for i in index:
                    j = ''
                    for v in variable:
                        try:
                            ...
                            raise ValueError(f'Error! {v} is not in variables.')
                    new_index.append(j[1:])
                self.set_index(new_index, drop_variable, duplicate_sum_vars, index_name, print_progress, indent)
            else:
