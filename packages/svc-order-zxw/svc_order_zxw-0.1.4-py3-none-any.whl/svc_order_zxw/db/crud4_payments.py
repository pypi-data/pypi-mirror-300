...
from .models import Payment, PaymentMethod, OrderStatus, Order, Product, Application
...
async def create_payment(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException_AppToolsSZXW(ErrorCode.数据库错误, f"创建支付记录失败: {str(e)}")
...
async def get_payment(
    ...
    if payment_id is None and order_number is None:
        raise HTTPException_AppToolsSZXW(ErrorCode.参数错误, "必须提供payment_id或order_number")
...
async def update_payment(
    ...
    try:
        ...
        if not payment:
            raise HTTPException_AppToolsSZXW(ErrorCode.支付记录未找到, f"未找到要更新的支付记录: {payment_id}")
        ...
        await db.rollback()
        raise HTTPException_AppToolsSZXW(ErrorCode.数据库错误, f"更新支付记录失败: {str(e)}")
...
async def delete_payment(db: AsyncSession, payment_id: int) -> bool:
    try:
        ...
        if result.rowcount == 0:
            raise HTTPException_AppToolsSZXW(ErrorCode.支付记录未找到, f"未找到要删除的支付记录: {payment_id}")
        return True
    except Exception as e:
        await db.rollback()
        raise HTTPException_AppToolsSZXW(ErrorCode.数据库错误, f"删��支付记录失败: {str(e)}")
