...
from .models import Order, Product, Application, Payment
from typing import List, Optional, Union
from sqlalchemy.orm import joinedload
from svc_order_zxw.errors import HTTPException_AppToolsSZXW, ErrorCode
from app_tools_zxw.支付服务_代码规范 import 支付服务
...
async def create_order(
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException_AppToolsSZXW(ErrorCode.数据库错误, f"创建订单失败: {str(e)}")
...
async def get_order(
    ...
    if not order:
        return None
        # raise HTTPException_AppToolsSZXW(ErrorCode.订单未找到, f"未找到订单: {order_identifier}")
...
async def update_order(
    ...
    try:
        ...
        if not order:
            raise HTTPException_AppToolsSZXW(ErrorCode.订单未找到, f"未找到要更新的订单: {order_id}")
        ...
        await db.rollback()
        raise HTTPException_AppToolsSZXW(ErrorCode.数据库错误, f"更新订单失败: {str(e)}")
...
async def delete_order(db: AsyncSession, order_id: int) -> bool:
    try:
        ...
        if result.rowcount == 0:
            raise HTTPException_AppToolsSZXW(ErrorCode.订单未找到, f"未找到要删除的订单: {order_id}")
        return True
    except Exception as e:
        await db.rollback()
        raise HTTPException_AppToolsSZXW(ErrorCode.数据库错误, f"删除订单失败: {str(e)}")
