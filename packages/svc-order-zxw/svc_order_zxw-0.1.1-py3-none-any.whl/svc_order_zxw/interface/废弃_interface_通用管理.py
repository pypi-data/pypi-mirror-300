...
from svc_order_zxw.interface.__interface通用方法__ import 验证请求异常sync
...
class 通用管理:
    ...
    async def 创建产品(self, name: str, app_id: str, price: float) -> 返回_创建产品:
        async with httpx.AsyncClient() as client:
            data = {"name": name, "app_name": app_id, "price": price}
    ...
    async def 创建订单(self, user_id: str, app_id: str, total_amount: float, product_id: int) -> 返回_创建订单:
        async with httpx.AsyncClient() as client:
            data = {"user_id": user_id, "app_name": app_id, "total_price": total_amount, "product_id": product_id}
