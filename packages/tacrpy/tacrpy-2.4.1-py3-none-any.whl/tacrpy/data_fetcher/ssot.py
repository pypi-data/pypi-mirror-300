    ...
    def _get_projects(self) -> pd.DataFrame:
       ...
       """
        credentials = service_account.Credentials.from_service_account_file(
            '/content/drive/Shareddrives/Analytické oddělení/Data/Zdrojová data/Zdroj pravdy (BigQuery)/data-poc-424211-4a1e7b9ab7e2.json')
        project_id = 'data-poc-424211'
        client = bigquery.Client(credentials=credentials, project=project_id)

        query = """
        SELECT *
        FROM `data-poc-424211.ssot.Projekty_ssot`
        """
...
class Organizations:
    ...
    def _get_organizations(self) -> pd.DataFrame:
        ...
        query = """
        SELECT *
        FROM `data-poc-424211.ssot.Organizace_ssot`
        """
        df = client.query(query).to_dataframe()
        return df
    ...
    def _data_preparing(self):
        ...
        """
        self.organizations["kod_program"] = self.organizations["kod_projektu"].str[:2]
        if ["kod_program"] == 'TG':
            self.organizations["kod_vs"] = self.organizations["kod_projektu"].str[:6]
        else:
            self.organizations["kod_vs"] = self.organizations["kod_projektu"].str[:4]
...
class Administrovane_projekty:
    ...
    def create_output(self, df:pd.DataFrame, agg_col: str):
        ...
        df_for_hodnoceno = df.copy()
        df_hodnoceno = df_for_hodnoceno.groupby(agg_col)['hodnocen'].sum().reset_index()
        total_count_hodnoceno = df_hodnoceno['hodnocen'].sum()
        df_hodnoceno_final = pd.concat([df_hodnoceno, pd.DataFrame({agg_col: ['Součet'], 'hodnocen': [total_count_hodnoceno]})], ignore_index=True)

        df_for_realizace = df.copy()
        df_realizace = df_for_realizace.groupby(agg_col)['realizace'].sum().reset_index()
        total_count_realizace = df_realizace['realizace'].sum()
        df_realizace_final = pd.concat([df_realizace, pd.DataFrame({agg_col: ['Součet'], 'realizace': [total_count_realizace]})], ignore_index=True)

        df_for_implementace = df.copy()
        df_implementace = df_for_implementace.groupby(agg_col)['implementace'].sum().reset_index()
        total_count_implementace = df_implementace['implementace'].sum()
        df_implementace_final = pd.concat([df_implementace, pd.DataFrame({agg_col: ['Součet'], 'implementace': [total_count_implementace]})], ignore_index=True)

        df_for_administrovan_vse = df.copy()
        df_administrovan_vse = df_for_administrovan_vse.groupby(agg_col)['administrovan_vse'].sum().reset_index()
        total_count_administrovan_vse = df_administrovan_vse['administrovan_vse'].sum()
        df_administrovan_vse_final = pd.concat([df_administrovan_vse, pd.DataFrame({agg_col: ['Součet'], 'administrovan_vse': [total_count_administrovan_vse]})], ignore_index=True)

        df_for_administrovan_bez_impl = df.copy()
        df_administrovan_bez_impl = df_for_administrovan_bez_impl.groupby(agg_col)['administrovan_bez_impl'].sum().reset_index()
        total_count_administrovan_bez_impl = df_administrovan_bez_impl['administrovan_bez_impl'].sum()
        df_administrovan_bez_impl_final = pd.concat([df_administrovan_bez_impl, pd.DataFrame({agg_col: ['Součet'], 'administrovan_bez_impl': [total_count_administrovan_bez_impl]})], ignore_index=True)

        df_for_pouze_implementace = df.copy()
        df_pouze_implementace = df_for_pouze_implementace.groupby(agg_col)['pouze_implementace'].sum().reset_index()
        total_count_pouze_implementace = df_pouze_implementace['pouze_implementace'].sum()
        df_pouze_implementace_final = pd.concat([df_pouze_implementace, pd.DataFrame({agg_col: ['Součet'], 'pouze_implementace': [total_count_pouze_implementace]})], ignore_index=True)


        self.hodnoceno = df_hodnoceno_final
        self.realizace = df_realizace_final
        self.implementace = df_implementace_final
        self.administrovan_vse = df_administrovan_vse_final
        self.administrovan_bez_impl = df_administrovan_bez_impl_final
        self.pouze_implementace = df_pouze_implementace_final


def _roky_reseni(zacatek, konec):
    """Pomocná funkce pro získání rozmezí let řešení ze sloupců nesoucí infroamci o začátku a konci projektu"""
    return list(range(zacatek.year, konec.year + 1))
...
def podporene_VOPO(year, typ_agg):
      ...
      :param year: rok, za který chceme podpořené organizace spočítat
      ...
      org_podp_terminy['roky_reseni'] = org_podp_terminy.apply(lambda row: _roky_reseni(row['zacatek_reseni'], row['konec_reseni']), axis=1)
      PO_list = ['MP', 'SP', 'VP', 'UP']

      org_podp_terminy['VOPO_kod'] = org_podp_terminy['typeorg_kod'].apply(lambda typ: _get_VOPO_kod(typ, PO_list))
      org_podp_roky = org_podp_terminy.explode('roky_reseni')
      if typ_agg == 'celkem':
          org_prog_roky_typ = org_podp_roky.groupby(['kod_program', 'VOPO_kod','roky_reseni']).agg({'ICO_organizace': 'count'}).reset_index()
      elif typ_agg == 'unikatni':
          org_prog_roky_typ = org_podp_roky.groupby(['kod_program', 'VOPO_kod','roky_reseni']).agg({'ICO_organizace': 'nunique'}).reset_index()
      else:
          raise ValueError(f'Neznámý typ agregace {typ_agg}, platné typy jsou celkem a unikatni')
      org_prog_roky_typ=org_prog_roky_typ[org_prog_roky_typ['roky_reseni']==year]
      org_prog_roky_typ.drop(columns=['roky_reseni'], inplace=True)
      org_prog_roky_typ_pivot = org_prog_roky_typ.pivot(
          index='kod_program', columns='VOPO_kod',
          values="ICO_organizace")

      org_prog_roky_typ_pivot = org_prog_roky_typ_pivot.fillna(0)
      org_prog_roky_typ_pivot = org_prog_roky_typ_pivot.astype(int)
      org_prog_roky_typ_pivot = org_prog_roky_typ_pivot[['VO', 'PO','O']]
      new_names={'PO': f"Počet podpořených podniků v roce {year}",'VO': f"Počet podpořených VO v roce {year}",'O': f"Počet podpořených ostatních v roce {year}"}
      org_prog_roky_typ_pivot.rename(columns=new_names, inplace=True)

      org_prog_roky_typ_pivot.loc['Celkem'] = org_prog_roky_typ_pivot.sum(numeric_only=True)
      return org_prog_roky_typ_pivot