...
def is_valid(name_or_value_or_enum: any, enum: YTAEnum):
    ...
    ParameterValidator.validate_is_enum(enum)

    return isinstance(name_or_value_or_enum, enum) or is_valid_name(name_or_value_or_enum, enum) or is_valid_value(name_or_value_or_enum)
...
def is_valid_name(name: any, enum: YTAEnum):
    """
...
def from_name(name: any, enum: YTAEnum):
    """
    This method returns the 'enum' YTAEnum item instance if the
    provided 'name' is a valid name of that 'enum' YTAEnum class,
    or raises an Exception if not.
    """
    if is_valid_name(name, enum):
        return enum[name]
