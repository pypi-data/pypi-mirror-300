...
from clairview.exception_capture import DEFAULT_DISTINCT_ID, Integrations  # noqa: F401
...
def capture(
    ...
    """
    Capture allows you to capture anything a user does within your system, which you can later use in ClairView to find patterns in usage, work out which features to improve or where people are giving up.
    ...
    For example:
    ```python
    clairview.capture('distinct id', 'opened app')
    clairview.capture('distinct id', 'movie played', {'movie_id': '123', 'category': 'romcom'})

    clairview.capture('distinct id', 'purchase', groups={'company': 'id:5'})
    ```
    """
    return _proxy(
        "capture",
...
def identify(
    ...
    # type: (...) -> Tuple[bool, dict]
    """
    Identify lets you add metadata on your users so you can more easily identify who they are in ClairView, and even do things like segment users by these properties.
    ...
    For example:
    ```python
    clairview.identify('distinct id', {
...
def set_once(
    ...
    This will not overwrite previous people property values, unlike `identify`.
     ...
     For example:
     ```python
     clairview.set_once('distinct id', {
         'referred_by': 'friend',
     })
     ```
    """
...
def group_identify(
    ...
    Set properties on a group
     ...
     For example:
     ```python
     clairview.group_identify('company', 5, {
         'employees': 11,
     })
     ```
    """
...
def alias(
    ...
    For example:
    ```python
    clairview.alias('anonymous session id', 'distinct id')
    ```
    """
    return _proxy(
        "alias",
...
def capture_exception(
    ...
    try:
        ...
        clairview.capture_exception(e, 'my specific distinct id')
        clairview.capture_exception(distinct_id='my specific distinct id')
...
def feature_enabled(
    ...
    For example:
    ```python
    if clairview.feature_enabled('beta feature', 'distinct id'):
        # do something
    if clairview.feature_enabled('groups feature', 'distinct id', groups={"organization": "5"}):
        # do something
    ```

    You can call `clairview.load_feature_flags()` before to make sure you're not doing unexpected requests.
    """
    return _proxy(
        "feature_enabled",
        key=key,
...
def get_feature_flag(
    ...
    Example:
    ```python
    if clairview.get_feature_flag('beta-feature', 'distinct_id') == 'test-variant':
        # do test variant code
    if clairview.get_feature_flag('beta-feature', 'distinct_id') == 'control':
        # do control code
    ```
    ...
    ```python
        group_properties={"organization": {"name": "ClairView", "employees": 11}}
    ```
    """
    return _proxy(
        "get_feature_flag",
...
def get_all_flags(
    ...
    Example:
    ```python
    flags = clairview.get_all_flags('distinct_id')
    ```
...
def load_feature_flags():
    """Load feature flag definitions from ClairView."""
