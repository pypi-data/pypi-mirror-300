...
from clairview.feature_flags import InconclusiveMatchError, match_property, relative_date_parse_for_feature_flag_matching
from clairview.request import APIError
...
class TestLocalEvaluation(unittest.TestCase):
    ...
    def test_load_feature_flags_wrong_key(self):
        ...
        with self.assertLogs("clairview", level="ERROR") as logs:
            client.load_feature_flags()
            self.assertEqual(
                logs.output[0],
                "ERROR:clairview:[FEATURE FLAGS] Error loading feature flags: To use feature flags, please set a valid personal_api_key. More information: https://clairview.com/docs/api/overview",
            )
        client.debug = True
    ...
    def test_get_feature_flag_with_variant_overrides(self, patch_decide):
        ...
        self.assertEqual(
            client.get_feature_flag("beta-feature", "test_id", person_properties={"email": "test@clairview.com"}),
            "second-variant",
        )
    ...
    def test_flag_with_clashing_variant_overrides(self, patch_decide):
        ...
        self.assertEqual(
            client.get_feature_flag("beta-feature", "example_id", person_properties={"email": "test@clairview.com"}),
            "second-variant",
        )
        # decide not called because this can be evaluated locally
        self.assertEqual(patch_decide.call_count, 0)
    ...
    def test_multivariate_feature_flag_payloads(self, patch_decide):
        ...
        self.assertEqual(
            self.client.get_feature_flag_payload(
                "beta-feature", "test_id", person_properties={"email": "test@clairview.com"}
            ),
            {"a": "json"},
        )
        self.assertEqual(
            self.client.get_feature_flag_payload(
                "beta-feature", "test_id", match_value="third-variant", person_properties={"email": "test@clairview.com"}
            ),
            {"a": "json"},
        )
        ...
        self.assertEqual(
            self.client.get_feature_flag_payload(
                "beta-feature", "test_id", match_value="first-variant", person_properties={"email": "test@clairview.com"}
            ),
            "some-payload",
        )
        self.assertEqual(patch_decide.call_count, 0)
...
class TestCaptureCalls(unittest.TestCase):
    ...
    @mock.patch("clairview.client.MAX_DICT_SIZE", 100)
    @mock.patch.object(Client, "capture")
    @mock.patch("clairview.client.decide")
