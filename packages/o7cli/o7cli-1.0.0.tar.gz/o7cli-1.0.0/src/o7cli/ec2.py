...
class Ec2(Base):
    ...
    def display_instances(self):
        ...
        params = TableParam(
            columns=[
                ...
                ColumnParam(
                    title="Private IP", type="str", data_col="PrivateIpAddress"
                ),
    ...
    def menu_instance(self, index):
        ...
        obj.add_option(
            o7m.Option(
                key="rdp", name="RDP Forwarding", callback=self.start_forward_rdp
            )
        )
        obj.add_option(
            o7m.Option(
                key="pf", name="Port Forwarding", callback=self.start_forward_port
            )
        )
