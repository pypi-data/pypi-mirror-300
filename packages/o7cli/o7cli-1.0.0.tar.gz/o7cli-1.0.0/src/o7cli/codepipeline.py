...
class CodePipeline(Base):
    ...
    def __init__(self, **kwargs):
        ...
        self.client = self.session.client(
            ...
            config=botocore.config.Config(
                connect_timeout=5, retries={"max_attempts": 0}
            ),
        )
    ...
    def display_pipeline_details(self):
        ...
        for stage in stages:
            ...
            for rev in stage.get("latestExecutionDetails", {}).get(
                ...
                rev_summry.append(
                    rev.get("revisionSummary", "NA").split("\n")[0].strip()
                )
    ...
    def display_execution_details(self):
        ...
        for action in actions:
            ...
            action_output = action.get("output", {}).get("output_variables", {})
    ...
    def menu_action_details(self, index):
        ...
        for stage in self.pipeline.get("stages", []):
            ...
            for action in stage.get("actions", []):
                if action["id"] == index:
                    exec_id = stage.get("latestExecution", {}).get(
        ...
        if status == "Failed":
            ...
            if key == "r":
                self.retry_fail_stage(
                    pl_name=self.pipeline_name, stage_name=stage_name, exec_id=exec_id
                )
            elif key == "d":
                o7cli.codebuild.CodeBuild(session=self.session).MenuBuildDetail(
    ...
    def menu_execution_details(self, index):
        ...
        obj.add_option(
            o7m.Option(
                key="int",
                short="Raw Action",
                callback=lambda x: pprint.pprint(
                    self.execution["actions"][x - 1]
                    if 0 < x <= len(self.execution["actions"])
                    else None
                ),
            )
        )
    ...
    def menu_pipeline_details(self, index):
        ...
        obj.add_option(
            o7m.Option(key="s", short="Start", callback=self.start_execution)
        )
        obj.add_option(
            o7m.Option(
                key="int",
