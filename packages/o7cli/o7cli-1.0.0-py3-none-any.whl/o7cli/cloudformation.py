...
class Cloudformation(Base):
    ...
    def init_drift_detection(self):
        ...
        if (
            o7i.is_it_ok(f"Confirm Drift Detection for Stack: {self.stack_name}")
            is False
        ):
            return
            ...
            if bucket["Region"] == self.session.region_name:
                ...
                if len(names) >= 3:
                    if names[0] == "cf" and names[1] == "templates":
                        logger.info(
                            f'find_s3_for_upload: found bucket {bucket["Name"]}'
                        )
    ...
    def display_stacks(self):
        ...
        params = TableParam(
            columns=[
                ColumnParam(title="id", type="i", min_width=4),
                ColumnParam(
                    title="Name", type="str", max_width=50, data_col="StackName"
                ),
    ...
    def display_events(self):
        ...
        params = TableParam(
            columns=[
                ...
                ColumnParam(
                    title="Reason", type="str", data_col="ResourceStatusReason"
                ),
            ]
        )
