...
class SegFunc(TimeFunc):
    ...
    def to_xml(self, tag_name:str="seg", tag_item:str="item", attr_time:str="time", attr_value:str="val", init_space:int=0, tab_space:int=2)->str:
        init = ' '*init_space
        tab = ' '*tab_space
        core = ''.join(f'{init}{tab}<{tag_item} {attr_time}="{t}" {attr_value}="{d}" />\n' for t,d in self)
        return f"{init}<{tag_name}>\n{core}{init}</{tag_name}>"
...
def SegFuncQuickSum(v:'list[SegFunc]')->SegFunc:
    ret = SegFunc()
    pq:'PQueue[tuple[int, int, int]]' = PQueue()
    sum = 0
    n = len(v)
    for i in range(n):
