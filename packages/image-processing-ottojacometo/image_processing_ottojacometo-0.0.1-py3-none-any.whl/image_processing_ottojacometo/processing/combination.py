...
def find_difference(image1, image2):
    assert image1.shape == image2.shape, "Specify 2 images with the same size."
    gray_image1= rgb2gray(image1)
    gray_image2= rgb2gray(image2)
    (score, difference_image)= structural_similarity(gray_image1, gray_image2, full= True)
    print(f"Similarity of the images: {score:.6f}")
...
def transfer_histogram(image1, image2):
    matched_image= match_histograms(image1, image2, mutlichannel= True)
