...
class TestRepValueReducer:
    ...
    @pytest.mark.parametrize("data", REP_VALUE_TEST_DATA)
...
class TestLpnRepValueReducer:
    @pytest.mark.parametrize(
        ...
        [
            ...
            (None, None),
            ([np.nan], [60]),
            # No snow points - see #41799
            ([110], [50]),
            # Lpn without variation
            ([110], [60]),
            ([100, 199], [60, 60]),
            ([100, 330, 450, 670], [60, 50, 50, 50]),
            # One variation
            ([100, 330, 450, 670], [60, 60, 60, 60]),
            ([560, 330, 45], [60, 60, 60]),
            # Two variations
            ([120, 220, 320, 420, 320, 220, 180], [60, 60, 60, 60, 60, 60, 60]),
            # More than 3 variations
            ([120, 500, 470, 460, 800, 820, 530], [60, 60, 60, 60, 60, 60, 60]),
        ],
    )
    def test_compute(self, lpn, wwmf, assert_equals_result):
        if lpn is None:
            params, levels = {}, []
        else:
            valid_time = [Datetime(2023, 3, 1, 3 * i) for i in range(len(lpn))]
            lpn = xr.DataArray(
...
class TestAltitudeRepValueReducer:
    ...
    @pytest.mark.parametrize("data", REP_VALUE_TEST_DATA)
    def test_compute(self, var_name, data, assert_equals_result):
        np.random.seed(1)
...
class TestRepValueBuilder:
    ...
    def test_compute_all_with_lpn(self, data, assert_equals_result):
        np.random.seed(1)
        valid_time = [Datetime(2023, 3, 1, 3 * i) for i in range(7)]
        lpn = xr.DataArray(
...
class TestAltitudeRepValueBuilder:
    ...
    @pytest.mark.parametrize("data", REP_VALUE_TEST_DATA_ALTITUDE)
    def test_compute(self, var_name, unit, data, assert_equals_result):
        np.random.seed(1)
        assert_equals_result(
            build_expected_result(
                data,
    ...
    @pytest.mark.parametrize(
        "data1,data2", product(REP_VALUE_TEST_DATA_ALTITUDE, repeat=2)
    )
    def test_compute_both(self, data1, data2, assert_equals_result):
        np.random.seed(1)
        assert_equals_result(
            build_expected_result(
