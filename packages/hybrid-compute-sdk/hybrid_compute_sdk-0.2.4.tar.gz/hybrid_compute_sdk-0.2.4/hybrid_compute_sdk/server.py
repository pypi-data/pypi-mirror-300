...
class HybridComputeSDK:
    ...
    def gen_response(self, req, err_code, resp_payload):
        ...
        p_enc1 = self.selector("PutResponse(bytes32,bytes)") + \
            ethabi.encode(['bytes32', 'bytes'], [req['skey'], resp2])

        p_enc2 = self.selector("execute(address,uint256,bytes)") + \
        ...
        oo_hash = Web3.keccak(ethabi.encode(['bytes32', 'address', 'uint256'], [
                             Web3.keccak(p), self.EntryPointAddr, self.HC_CHAIN]))
