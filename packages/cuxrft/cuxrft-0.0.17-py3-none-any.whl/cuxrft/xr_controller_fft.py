...
try:
    ...
    import sys
    spec = importlib.util.spec_from_file_location("cuxrft", "/home/tvo/cuxrft-dev/CuXRFT/src/cuxrft/byte_converter.py")
    foo = importlib.util.module_from_spec(spec)
    sys.modules["cuxrft.byte_converter"] = foo
...
def get_freeGPU(availableGPUs, filterList=['mumax3', 'mumax3me'], filterListCheckGPU=[], treshHoldGPUUsage=0.1):
    ...
    if (os.name == "nt"):
        ...
        upperIndex = None
        lowerIndex = None
        for i in range(len(nvidiaGPUAnswer)):
            if len(nvidiaGPUAnswer[i]) > 0:
                if '|====' in nvidiaGPUAnswer[i][0] and '====|' in nvidiaGPUAnswer[i][0] and '==+==' not in nvidiaGPUAnswer[i][0] and lowerIndex is None:
                    lowerIndex = i
                elif '|====' in nvidiaGPUAnswer[i][0] and '====|' in nvidiaGPUAnswer[i][0] and '==+==' in nvidiaGPUAnswer[i][0] and upperIndex is None:
                    upperIndex = i

        nvidiaGPUAnswerLower = [list(filter(lambda char: char != "|" and char != "N/A", char)) for char in nvidiaGPUAnswer[lowerIndex+1:-2]]
        nvidiaGPUAnswerUpper = [list(filter(lambda char: char != "|" and char != "N/A", char)) for char in nvidiaGPUAnswer[upperIndex+1:lowerIndex-2]]
        nvidiaGPUIndex = []
        hardwareIndexAvailable = []
        for upperIndex in list(range(0, len(nvidiaGPUAnswerUpper)-5, 4)):
            if (castable_int(nvidiaGPUAnswerUpper[upperIndex][0])):
        ...
        if (len(nvidiaGPUAnswerLower) > 0 and nvidiaGPUAnswerLower[0][0] != 'No'):
            for i in range(len(nvidiaGPUAnswerLower)):
                upperListConverter = list(range(1, len(nvidiaGPUAnswerUpper)-5, 4))
                if Path(nvidiaGPUAnswerLower[i][3]).stem.lower() in programsToFilterGPUs:
                    if (Path(nvidiaGPUAnswerLower[i][3]).stem.lower() in checkUsageFor and len(nvidiaGPUAnswerUpper[upperListConverter[int(nvidiaGPUAnswerLower[i][0])]]) > 9 and float(nvidiaGPUAnswerUpper[upperListConverter[int(nvidiaGPUAnswerLower[i][0])]][9].replace('%', ""))/100 > treshHoldGPUUsage or Path(nvidiaGPUAnswerLower[i][3]).stem.lower() not in checkUsageFor and Path(nvidiaGPUAnswerLower[i][3]).stem.lower() in programsToFilterGPUs):
                        try:
                            hardwareIndexAvailable.remove(int(nvidiaGPUAnswerLower[i][0]))
                        except ValueError:
                            pass
                    elif (Path(nvidiaGPUAnswerLower[i][3]).stem.lower() in checkUsageFor and len(nvidiaGPUAnswerUpper[upperListConverter[int(nvidiaGPUAnswerLower[i][0])]]) == 9 and float(nvidiaGPUAnswerUpper[upperListConverter[int(nvidiaGPUAnswerLower[i][0])]][7].replace('%', ""))/100 > treshHoldGPUUsage or Path(nvidiaGPUAnswerLower[i][3]).stem.lower() not in checkUsageFor and Path(nvidiaGPUAnswerLower[i][3]).stem.lower() in programsToFilterGPUs):
                        try:
                            hardwareIndexAvailable.remove(int(nvidiaGPUAnswerLower[i][0]))
                        except ValueError:
                            pass
...
@dask.delayed
def calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims, multiple_GPUs, useGPU, debugging=False):
    GPU_avail = {}
    if debugging == True:
    ...
    if (multiple_GPUs == False):
        ...
        for key in chunkedDims.keys():
            print('Got GPU.\nCalculating chunk ' + str(chunkedDims[key]['chunkIndex']) + ' for ' + key + '.')
    else:
        if (isinstance(useGPU, int)):
...
def sel_chunk(data, chunks, out, resultDim, fftAxis, fftDirection="forward", fftNorm=None, chunkedDims={}, useGPU=0, sc=None, multiple_GPUs=False, FFTOrderCoords={}, debugging=False):
    if (chunkedDims.keys() != chunks.keys()):
        if debugging == True:
            print(data[resultDim].shape)
        for key in chunks.keys():
            ...
            for i in range(0, data.dims[key], chunkLength): 
                ...
                if debugging == True:
                    print(chunkedDims)
                out[sc] = sel_chunk(data.isel({key: slice(i, i+chunkLength)}), chunks, out, resultDim, fftAxis, fftDirection, fftNorm, chunkedDims, useGPU=useGPU, sc=sc, multiple_GPUs=multiple_GPUs, debugging=debugging)
                if debugging == True:
        ...
        if (sc is not None):
            delayedObject = from_delayed(calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims, multiple_GPUs, useGPU, debugging=debugging), dtype=np.complex128, shape=out[sc].shape)
        else:
            delayedObject = from_delayed(calcFFT(data, resultDim, fftDirection, fftAxis, fftNorm, FFTOrderCoords, chunkedDims, multiple_GPUs, useGPU, debugging=debugging), dtype=np.complex128, shape=out.shape)
        return delayedObject

def fft_cellwise(data, chunks='auto', FFT_dims='', data_vars='', delayed=False, multiple_GPUs=False, GPUs=[0], keepGPUcontrollingServerRunning=False, sel=None, isel=None, debugging=False, splitComplex=False):
    """
    Performs FFT along FFT_dim for data_var
        ...
        splitComplex                        : bool, if the data is supposed to be stored in to a netcdf file directly an error can arise because no complex values can be stored. This flag splits the data and stores real and imaginary part into two different data_var
        Returns
        -------
    ...
    for data_var in data_vars:
        ...
        for FFT_dim in FFT_dimsTmp:
            ...
            cache.set_size(0)
            out = dask.array.rechunk(dask.array.squeeze(sel_chunk(data, chunks, out, data_var, fftAxis, "forward", useGPU=GPUs, multiple_GPUs=multiple_GPUs, debugging=debugging)), rechunkDict)
            if (delayed == False):
    ...
    if isinstance(chunksOrg, dict):
        rechunkDataset = {}
        for key in chunksOrg.keys():
            if key in FFT_dimsFlatten:
                rechunkDataset[key + '_freq'] = data.dims[key + '_freq']
            else:
                rechunkDataset[key] = chunksOrg[key]
        for FFT_dim in FFT_dimsFlatten:
            if FFT_dim + '_freq' not in list(rechunkDataset.keys()):
                rechunkDataset[FFT_dim + '_freq'] = data.dims[FFT_dim + '_freq']
        data = data.chunk(rechunkDataset)
    if (multiple_GPUs == True and keepGPUcontrollingServerRunning == False and delayed == False):
        GPU_client({'exit': 0})
    datasetDictSplit = {}
    if (splitComplex == True and wasDataArray == False):
        for data_var in data.data_vars.keys():
            datasetDictSplit[data_var + 'Real'] = data[data_var].real
            datasetDictSplit[data_var + 'Imag'] = data[data_var].imag
        return xr.Dataset(datasetDictSplit)
    else:
        return data

def ifft_cellwise(data, chunks='auto', FFT_dims='', data_vars='', delayed=False, multiple_GPUs=False, GPUs=[0], keepGPUcontrollingServerRunning=False, sel=None, isel=None, debugging=False, splitComplex=False):
    """
    ...
    for data_var in data_vars:
        ...
        for FFT_dim in FFT_dimsTmp:
            ...
            cache = cupy.fft.config.get_plan_cache() 
            cache.set_size(0)
            out = dask.array.rechunk(dask.array.squeeze(sel_chunk(data, chunks, out, data_var, fftAxis, "inverse", useGPU=GPUs, multiple_GPUs=multiple_GPUs, FFTOrderCoords=FFTOrderCoords, debugging=debugging)), rechunkDict)
            if (delayed == False):
