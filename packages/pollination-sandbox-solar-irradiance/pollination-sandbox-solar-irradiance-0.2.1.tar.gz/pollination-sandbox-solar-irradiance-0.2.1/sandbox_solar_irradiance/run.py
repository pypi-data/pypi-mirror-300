...
import flow.main as sandbox_solar_irradiance_workerbee


_recipe_default_inputs = {   'model_folder': None,
    'models': None,
    'north': 0.0,
...
class LetSandboxSolarIrradianceFly(luigi.WrapperTask):
    # global parameters
    _input_params = luigi.DictParameter()

    def requires(self):
        yield [sandbox_solar_irradiance_workerbee._Main_15a1447bOrchestrator(_input_params=self._input_params)]
...
def start(project_folder, user_values, workers):
    ...
    if 'simulation_folder' not in input_params or not input_params['simulation_folder']:
        if 'simulation_id' not in input_params or not input_params['simulation_id']:
            simulation_id = 'sandbox_solar_irradiance_%d' % int(round(time.time(), 2) * 100)
        else:
    ...
    artifacts = ['model_folder', 'models', 'wea']
    optional_artifacts = []
    ...
    luigi.build(
        [LetSandboxSolarIrradianceFly(_input_params=input_params)],
