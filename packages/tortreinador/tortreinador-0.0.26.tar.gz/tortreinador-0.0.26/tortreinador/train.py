...
class TorchTrainer:
    ...
    def fit(self, t_l, v_l, checkpoint_=None, **kwargs):
        ...
        if 'mix' in kwargs.keys():
            ...
            self.error = torch.Tensor(kwargs['mix']['error'])
        ...
        for e in range(START_EPOCH, self.epoch):
            ...
            if 'mix' in kwargs.keys():
                if kwargs['mix']['condition'] == 1 and e <= kwargs['mix']['warmup_error']:
                    self.current_error_rate = (FINAL_RATE - INIT_RATE) * (e / (kwargs['mix']['warmup_error']-1)) + INIT_RATE
...
def config_generator(model_save_path: str, warmup_epochs: int = None, lr_milestones: list = None,
                     ...
                     warmup_error: int = None, event_rate: float = None, error: object = None):
    """
    ...
    if if_mix and error is not None:
        config['mix'] = {}
        ...
        if initial_rate is not None and final_rate is not None and event_rate is None:
            ...
            if final_rate is None:
                ...
                print("Warning: The mix data training strategy 2 is turn on, however the final rate is unspecified, set it as 0.8")

            else:
                config['mix']['final_rate'] = final_rate

    elif if_mix and error is None:
        raise ValueError("Got Empty error list")
