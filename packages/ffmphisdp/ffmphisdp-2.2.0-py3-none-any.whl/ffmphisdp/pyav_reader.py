...
class ControlledVideoReader:
    def __init__(
        ...
        **kwargs,
        ...
        self.container_with_edit_list = av.open(self.video_path)
        self.defaut_start_time = self.container_with_edit_list.streams.video[self.video_stream_idx].start_time
    ...
    def read(self) -> typing.Tuple[bool, typing.Optional[np.ndarray]]:
        ...
        try:
            if not self.already_has_next:
                ...
                self._current_frame_index += 1
            self.already_has_next = False
            while not (self.frame_selection[self.current_frame_index]):
                self.current_frame = next(self.decoder)
                self._current_frame_index += 1
    ...
    def set_frame_idx(self, frame_idx: int):
        ...
        pts = self.frame_infos[frame_idx] + self.video_stream.start_time - self.defaut_start_time
