...
def S3Download(remote_path, local_path=None, \
    ...
    if not local_path is None:
        ...
        if local_exists:
            # local mtime
            local_mtime = datetime.fromtimestamp(os.path.getmtime(local_path),timezone.utc).timestamp()
    elif not local_mtime is None:
        local_mtime = datetime.fromtimestamp(local_mtime,timezone.utc).timestamp()
        local_exists = True
