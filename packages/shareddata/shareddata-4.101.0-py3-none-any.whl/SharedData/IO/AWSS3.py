...
def S3Download(remote_path, local_path=None, \
    ...
    if not local_path is None:
        ...
        if local_exists:
            ...
            local_mtime = local_mtime.astimezone(timezone.utc).replace(tzinfo=None).timestamp()
...
def UpdateModTime(local_path, remote_mtime):
    ...
    offset = remote_mtime_dt - datetime.fromtimestamp(remote_mtime, timezone.utc).replace(tzinfo=None)
...
def S3Upload(file_io, path, mtime, database_folder=None):
    ...
    while trials > 0:
        try:
            ...
            mtime_utc = datetime.fromtimestamp(mtime)
            mtime_utc = mtime_utc.astimezone(timezone.utc).replace(tzinfo=None).timestamp()
