...
from SharedData.TableIndexJit import get_symbol_loc, get_portfolio_loc, get_tag_loc
...
class SharedNumpy(np.ndarray):
    ...
    def get_tag_loc(self, tag, maxids=0):
        ...
        try:
            ...
            taghash = hash(tag)
            loc = get_tag_loc(
                self[:], self.index.portlastidx, self.index.portprevidx, tag, taghash, maxids)
            success = True
        except Exception as e:
