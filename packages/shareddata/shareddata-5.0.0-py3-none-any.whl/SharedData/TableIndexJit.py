...
@njit(cache=True)
def testhash(records,i,pkey):
    n = pkey.size-1
    h0 = np.int64(records['date'][i])
...
###################### DATE ########################
@njit(cache=True)
def create_pkey_symbol_jit(records, count, pkey, dateiniidx, dateendidx, dateunit,
...
def upsert_date_symbol_jit(records, count, new_records, pkey, dateiniidx, dateendidx, dateunit,
    ...
    for i in range(nrec):
        h0 = np.int64(new_records['date'][i])
        h1 = djb2_hash(new_records['symbol'][i])
        h = (h0 ^ h1) % n
        found = False
