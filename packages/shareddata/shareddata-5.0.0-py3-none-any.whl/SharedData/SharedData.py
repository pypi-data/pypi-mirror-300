...
class SharedData:
    ...
    def listall(self, keyword='', user='master', list_timeseries=True):
        ...
        if arrobj.size>0:
            ...
            dfremote['last_modified'] = pd.to_datetime(dfremote['last_modified']).dt.tz_convert(None)
            dfremote['size'] = dfremote['size'].astype(int)
        ...
        if len(records)>0:
            ...
            dflocal['size'] = dflocal['size'].astype(int)
        ...
        if len(ls)>0:
            if not 'last_modified_local' in ls.columns:
                ls['last_modified_local'] = pd.NaT
            
            if not 'size_local' in ls.columns:
                ls['size_local'] = 0

            if not 'files_local' in ls.columns:
                ls['files_local'] = 0

            ls = ls[['last_modified_local','size_local','files_local',
                    'last_modified_remote', 'size_remote', 'files_remote',
                    'user','database','period','source','container', 'tablename','partition']].copy()
