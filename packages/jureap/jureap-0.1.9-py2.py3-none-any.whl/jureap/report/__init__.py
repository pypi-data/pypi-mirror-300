...
def generate(report_type, input_dir, pipeline_array, prefix_array, output_dir, share_dir, tmp_dir):
    experiment_array = parse_experiments(input_dir, pipeline_array, prefix_array)
...
def parse_experiments(input_dir, pipeline_array, prefix_array):
    experiment_array = []
    for index, pipeline in enumerate(pipeline_array):
        ...
        experiment_array.append(experiment(pipeline_reldir, pipeline, prefix, csv_data, json_data))

    return experiment_array


class experiment:
    def __init__(self, pipeline_dir, pipeline, prefix, csv_data, json_data):
