...
def generate_plot_pdf_file(plotting_data, output_dir):
    ...
    ax = plot.subplots()
    ax.set_xlabel("Number of Nodes ($N_\\text{JEDI}$, $N_{4 \\mathrm{x} \\text{A100}}/2$; log-scale)")
    ...
    colorgen = iter(cmap(numpy.linspace(0, 1, len(plotting_data["experiment"]))))
    ...
    for pipeline_data in plotting_data["experiment"]:
        ...
        if pipeline_data["label"] == "jedi.evaluation.jedi":
            oldcolor=color
        ...
        ax.plot(xaxis_data, ideal_scaling_array, color=error_color, ls="--", lw=0.6, alpha=1)
        ax.plot(xaxis_data, low_scaling_array, color=error_color, ls="--", lw=0.6, alpha=1)
    ...
    yticklabels = sorted(list(set(yticklabels)))

    min_yticklabel = math.log(min(yticklabels))
    max_yticklabel = math.log(max(yticklabels))
