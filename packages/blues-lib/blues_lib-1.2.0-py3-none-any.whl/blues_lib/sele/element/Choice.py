...
class Choice():
  ...
  def select(self,target_CS_WE,parent_CS_WE=None):
    ...
    for target_CS_WE in target_CS_WEs:
      count += self.__toggle(target_CS_WE,True,parent_CS_WE)
    return count

  def deselect(self,target_CS_WE,parent_CS_WE=None):
    '''
    Support multi input parameters as selectors
    Parameter:
      target_CS_WE {str|WebElemnt|list<str>|list<WebElement>} : one or a list of target element
    ...
    for target_CS_WE in target_CS_WEs:
      count += self.__toggle(target_CS_WE,False,parent_CS_WE)
    return count

  def __toggle(self,target_CS_WE,checked=True,parent_CS_WE=None):
    '''
    ...
    '''
    count = 0
    web_elements = self.__querier.query_all(target_CS_WE,parent_CS_WE)
    if not web_elements:
      return count
