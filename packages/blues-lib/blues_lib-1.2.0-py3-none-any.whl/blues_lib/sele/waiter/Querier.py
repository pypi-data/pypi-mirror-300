...
class Querier():
  ...
  @QuerierDeco('query',timeout=5)
  def query(self,target_CS_WE,parent_CS_WE=None):
    '''
    ...
    if not parent_CS_WE:
      ...
      return self.__query(target_CS_WE)
    else:
      # Scenaria 2: with a the parent, wait the parent and find
      parent_element = self.__query(parent_CS_WE)
      return self.__finder.find(target_CS_WE,parent_CS_WE)

  @QuerierDeco('query_all',timeout=5)
  def query_all(self,target_CS_WE,parent_CS_WE=None):
    '''
    ...
    if not parent_CS_WE:
      # Scenaria 1: without the parent
      return self.__query_all(target_CS_WE)
    else:
      # Scenaria 2: with a the parent, wait the parent and find
      parent_element = self.__query(parent_CS_WE)
      return self.__finder.find_all(target_CS_WE,parent_CS_WE)

  def __query(self,target_CS_WE):
    '''
    ...
    return self.__ec.to_be_presence(target_CS_WE,self.__timeout)

  def __query_all(self,target_CS_WE):
    '''
    ...
    return self.__ec.all_to_be_presence(target_CS_WE,self.__timeout)
