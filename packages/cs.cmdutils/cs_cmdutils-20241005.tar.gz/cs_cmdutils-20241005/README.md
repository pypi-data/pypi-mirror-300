...
*`BaseCommand.cmd_shell(*a, upd: Optional[cs.upd.Upd] = <function uses_upd.<locals>.<lambda> at 0x1059877e0>, **kw)`*:
Usage: {cmd}
...
*`BaseCommand.handle_signal(self, sig, frame, *, runstate: Optional[cs.resources.RunState] = <function uses_runstate.<locals>.<lambda> at 0x105987060>)`*:
...
*`BaseCommand.run_context(*a, upd: Optional[cs.upd.Upd] = <function uses_upd.<locals>.<lambda> at 0x1059871a0>, **kw)`*:
...
## <a name="vprint"></a>`vprint(*print_a, options: Optional[inspect._empty] = <function uses_cmd_options.<locals>.<lambda> at 0x105987c40>, verbose=None, **print_kw)`
