...
Usage summary:
    ...
    Usage: base [-a] [-b bvalue] [-c] [--] arguments...
        -a    Do it all.
        -b    But using bvalue.
        -c    The 'c' option!
      """
...
*`BaseCommand.cmd_shell(*a, upd: Optional[cs.upd.Upd] = <function uses_upd.<locals>.<lambda> at 0x101357740>, **kw)`*:
Usage: {cmd}
Run a command prompt via cmd.Cmd using this command's subcommands.
...
*`BaseCommand.handle_signal(self, sig, frame, *, runstate: Optional[cs.resources.RunState] = <function uses_runstate.<locals>.<lambda> at 0x101356fc0>)`*:
The default signal handler, which cancels the default `RunState`.

*`BaseCommand.has_subcommands(self)`*:
...
*`BaseCommand.poparg(argv: List[str], *a, unpop_on_error=False, opt_spec_class=None)`*:
...
*`BaseCommand.popopts(argv, attrfor=None, **opt_specs)`*:
...
*`BaseCommand.run_context(*a, upd: Optional[cs.upd.Upd] = <function uses_upd.<locals>.<lambda> at 0x101357100>, **kw)`*:
...
*`BaseCommand.usage_text(self, *, cmd=None, format_mapping=None, short=False, show_subcmds=None)`*:
...
*`BaseCommandOptions.popopts(self, argv, **opt_specs)`*:
...
## <a name="extract_usage_from_doc"></a>`extract_usage_from_doc(doc: str | None, usage_marker='Usage:') -> Tuple[str, str]`
...
## <a name="OptionSpec"></a>Class `OptionSpec(cs.deco.Promotable)`
...
*`SubCommand.get_usage_format(self) -> str`*:
...
*`SubCommand.usage_text(self, *, short: bool, recurse: bool = False, show_subcmds: Union[bool, str, List[str], NoneType] = None, usage_mapping: Optional[Mapping] = None) -> str`*:
...
## <a name="vprint"></a>`vprint(*print_a, options: Optional[inspect._empty] = <function uses_cmd_options.<locals>.<lambda> at 0x101357ba0>, verbose=None, **print_kw)`
