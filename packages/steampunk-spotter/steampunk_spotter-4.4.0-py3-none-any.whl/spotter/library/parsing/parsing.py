...
def _parse_tasks(  # noqa: PLR0912  # TODO: oh dear
    tasks: List[Dict[str, Any]], file_name: str, parse_values: bool, play_id: Optional[str], block_id: Optional[str]
                ...
                if block_section in task:
                    ...
                    if isinstance(task[block_section], list):
                        block_tasks, block_blocks = _parse_tasks(
                            task[block_section], file_name, parse_values, play_id, task_block_id
                        )
...
def _parse_vars(variables: Dict[str, Any], file_name: str) -> List[Dict[str, Any]]:
    """
    ...
    for var_name in variables:
        _, hidden = _remove_secret_parameter_values(variables[var_name], secrets)
        obfuscated.extend(item.to_parent(var_name) for item in hidden)
    if not obfuscated:
        return []

    vars_meta = variables.pop("__meta__", None)
    meta = {
        "file": file_name,
        "line": vars_meta["__line__"],
        "column": vars_meta["__column__"],
        "start_mark_index": vars_meta["__start_mark_index__"],
        "end_mark_index": vars_meta["__end_mark_index__"],
    }
...
def _parse_playbook(
    ...
    for play in [p for p in playbook if p is not None]:
        ...
        item_tasks, item_blocks = _parse_tasks(all_tasks, file_name, parse_values, play_id, None)
...
def parse_unknown_ansible_artifact(  # noqa: PLR0915
    path: Path, exclude_paths: Set[str], parse_values: bool = False
) -> Tuple[
    List[Dict[str, Any]],
    List[Dict[str, Any]],
    List[Dict[str, Any]],
    ...
    if str(path.absolute()) in exclude_paths:
        ...
        if _is_playbook(loaded_yaml):
            parsed_tasks, parsed_blocks, parsed_playbooks = _parse_playbook(loaded_yaml, str(path), parse_values)
            ...
            parsed_vars = _parse_vars(loaded_yaml, str(path))
            parsed_ansible_artifacts_vars += parsed_vars
        elif isinstance(loaded_yaml, list):
            parsed_tasks, parsed_blocks = _parse_tasks(loaded_yaml, str(path), parse_values, None, None)
            parsed_ansible_artifacts_tasks += parsed_tasks
            parsed_ansible_artifacts_blocks += parsed_blocks
        ...
        for sub_path in sorted(path.iterdir()):
            ...
            ) = parse_unknown_ansible_artifact(sub_path, exclude_paths, parse_values)
            parsed_ansible_artifacts_tasks += parsed_tasks
            parsed_ansible_artifacts_playbooks += parsed_playbooks
            parsed_ansible_artifacts_blocks += parsed_blocks
...
def parse_ansible_artifacts(
    paths: List[Path], exclude_paths: Optional[List[Path]], parse_values: bool = False
) -> ParsingResult:
    """
    ...
    for path in paths:
        (
            ...
            yaml_error,
            (parsed_included_files_count, parsed_excluded_paths_count),
        ) = parse_unknown_ansible_artifact(path, paths_to_exclude, parse_values)
        parsed_ansible_artifacts_tasks += parsed_tasks
        parsed_ansible_artifacts_playbooks += parsed_playbooks
        parsed_ansible_artifacts_blocks += parsed_blocks
