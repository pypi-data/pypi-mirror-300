...
def run(
    ...
    # Invalid row ids will be stored at this location.
    if drop_invalid_rows:
        row_ids_path = os.path.join(temp_dir, "row_ids")
    else:
        row_ids_path = None

    # Create the same file but with line numbers as these are required
    # for reporting duplicate lines.
    input_path_with_line_numbers = os.path.join(temp_dir, "dataset_with_line_nrs.csv")
    with open(input_path_with_line_numbers, "w") as f:
        before = datetime.now()
        process = subprocess.run(
            ["nl", "-v0", "-w1", "-s,", input_path],
            stdout=f,
            text=True
        )
        after = datetime.now()
        print(f"Initial copy took {(after - before).total_seconds() / 60} min")
        if process.returncode != 0:
            raise Exception(f"Failed to add line numbers")

    before = datetime.now()
    validate.run_validation_program(
        input_path=input_path_with_line_numbers,
        config_path=config_path,
        output_path=report_path,
        types_path=types_path,
        row_ids_path=row_ids_path,
        drop_first_column=True,  # ignore the column containing line numbers
    )
    after = datetime.now()
    ...
    with spark_session(temp_dir, name="Validation", input_files=[input_path_with_line_numbers]) as ss:
        spark_schema = StructType(
            [StructField(ROW_NUM_COLUMN, IntegerType())]  # the row numbers we added artificially
        ...
        df = (
            ss.read
            .option("header", "false")
            .csv(input_path_with_line_numbers, schema=spark_schema)
            .withColumn(HAS_ERROR_COL, F.lit(False))
        ...
        if drop_invalid_rows:
            row_ids_path = os.path.join(temp_dir, "row_ids")
            erroneous_row_ids_df = (
                ss.read
                .option("header", "false")
                .csv(row_ids_path, schema=StructType(
                    [StructField(ROW_NUM_COLUMN, IntegerType())]
                ))
            )
            df = add_erroneous_row_ids(df, erroneous_row_ids_df)
        else:
            row_ids_path = None

        # TODO: Check that we don't run into duplication check issues if the file is malformed at this point
        if should_check_uniqueness:
            ...
            duplicate_row_nrs_df, num_duplication_errors_total, duplication_errors = \
                find_duplicates_spark(df, unique_keys, ROW_NUM_COLUMN)
            update_report_with_uniqueness_check_result(
        ...
        if is_passed:
            ...
            if should_write_parquet:
                temp_output_path = os.path.join(temp_dir, "temp-dataset-out.parquet")
                df.write.parquet(temp_output_path)
                shutil.copytree(temp_output_path, output_path)
            else:
                # TODO: Make sure this does not OOM
