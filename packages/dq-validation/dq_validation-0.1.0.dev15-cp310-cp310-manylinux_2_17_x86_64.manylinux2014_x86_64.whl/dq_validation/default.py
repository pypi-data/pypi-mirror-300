...
def create_pandas_schema(config: ValidationConfig):
    ...
    dtypes[HAS_ERROR_COL] = np.bool
    columns = [str(x) for x in range(len(config.root.config.columns))] + [ROW_NUM_COLUMN, HAS_ERROR_COL]
...
def find_duplicates_pandas(csv: pd.DataFrame, unique_keys: List[List[int]]):
    ...
    for subset_columns in unique_keys:
        ...
        duplicated_rows_subset = list(csv.loc[is_duplicated][ROW_NUM_COLUMN][:NUM_ERRORS_RECORD_BY_KEY_TUPLE])
...
def create_empty_df(config: ValidationConfig) -> pd.DataFrame:
    # Offset by one because there would be an additional column containing the row nr
    columns, dtype = create_pandas_schema(config)
    return pd.DataFrame(dtype=dtype, columns=columns)
...
def read_csv_from_parts(path: str, config: ValidationConfig) -> pd.DataFrame:
    valid_dataset_parts = sorted(glob.glob(os.path.join(path, "*.csv")))
    print(f"Found {len(valid_dataset_parts)} valid dataset parts")
    ...
    if valid_dfs:
        ...
        string_columns = [
            str(jx) for jx, col in enumerate(config.root.config.columns)
...
def run(
    ...
    print(f"Reading input file at {annotated_dataset_path}")
    before = datetime.now()
    df = read_csv_from_parts(annotated_dataset_path, config)
    if df.shape[0] == 0:
        print(f"Dataframe as output by validation pipeline is empty")
    after = datetime.now()
