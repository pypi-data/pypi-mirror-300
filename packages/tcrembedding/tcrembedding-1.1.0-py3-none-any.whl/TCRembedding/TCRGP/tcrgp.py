...
def subsmatFromAA2(identifier,data_file=pkg_resources.resource_filename('TCRembedding', 'TCRGP/data/aaindex2.txt')):
    ...
    with open(data_file,'r') as f:
        ...
        subsmat=np.zeros((21,21),dtype=float)
        i0=0
        for line in f:
...
def encode_with_pc_2d(seq_lists, lmaxes, pc):
    ...
    for i in range(len(seq_lists)):
        ...
        for j in range(len(seq_lists[i])):
            X[j,:,i_start:i_end] = np.reshape( pc[:,seq_lists[i][j][:lmaxes[i]]], (d, Di) )
        i_start=i_end
    return X
...
def get_sequence_lists(datafile,organism,epi,cdr_types,delimiter,clip,lmax3=None,
                       va=None,vb=None, cdr3a=None,cdr3b='cdr3b',epis='epitope',subs=None,
