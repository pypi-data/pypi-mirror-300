...
class GIANA_model():
    ...
    def EncodeRepertoire(self, data, ST=3, verbose=True):
        # No V gene version
        ...
        if data.empty:
            ...
            for seq in seqs:
                if len(seq) >= ST + 7:
                    #dM = np.array([self.EncodingCDR3(seq[ST:-2], M6, n0)])
                    #dM = dM.astype("float32")
                    dM = self.EncodingCDR3(seq[ST:-2], M6, n0)
                    vectors.append(dM)
                else:
