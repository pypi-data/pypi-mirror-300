...
def test_create_twice_still_same_hash(samples_dir):
    model_file_path = samples_dir

    # Create a first time the archive, retrieve its hash
    with create_archive(path=model_file_path) as (
        ...
        assert os.path.exists(zst_archive_path), "Auto removal failed"

    # Create the archive a second time, should return the same hash as before
    with create_archive(path=model_file_path) as (
        zst_archive_path,
        hash2,
        size,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "hash": ["A model version with this hash already exists."],
                "id": ["57b5ef16-f78c-4385-bfc1-74e7f5a74625"],
            },
            [
                (
                    "arkindex_cli.commands.models.utils",
                    40,
                    "An available model version already exists with this hash, with the ID (57b5ef16-f78c-4385-bfc1-74e7f5a74625).",
                )
            ],
        ),
        (
...
def test_validate_model_version_unhandled_error(api_client, caplog):
    caplog.set_level(logging.ERROR)
    validation_args = {
        "size": 8,
        "hash": "a" * 32,
