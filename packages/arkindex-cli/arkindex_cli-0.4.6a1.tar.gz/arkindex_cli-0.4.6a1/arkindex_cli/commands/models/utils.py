...
def create_model_version(
    ...
    try:
        ...
        if e.status_code == 400:
            ...
            elif "hash" in e.content:
                raise Exception(
                    f"An available model version already exists with this hash, with the ID ({e.content['id']})."
                )
        raise e
...
def validate_model_version(
    ...
    try:
        ...
        if "hash" in e.content:
            logger.error(
                f"An available model version already exists with this hash, with the ID ({e.content['id'][0]})."
            )
