...
#include "ClairQLLexer.h"
#include "ClairQLParser.h"
#include "ClairQLParserBaseVisitor.h"

#include "error.h"
#include "parser.h"
#include "string.h"

#define VISIT(RULE) any visit##RULE(ClairQLParser::RULE##Context* ctx) override
#define VISIT_UNSUPPORTED(RULE)                                \
  VISIT(RULE) {                                                \
    throw NotImplementedError("Unsupported rule: " #RULE); \
  }

#define HANDLE_CLAIRQL_ERROR(TYPE, OTHER_CLEANUP)                                                     \
  (const TYPE& e) {                                                                                     \
...
class ClairQLParseTreeConverter : public ClairQLParserBaseVisitor {
 private:
  parser_state* state;
  bool is_internal;
...
#define IS_AST_NODE_INSTANCE_IMPL(CLAIRQL_MODULE, TYPE_NAME)                               \
  PyObject* node_type = PyObject_GetAttrString(state->CLAIRQL_MODULE##_module, TYPE_NAME); \
...
#undef IS_AST_NODE_INSTANCE_IMPL
 ...
 public:
  ClairQLParseTreeConverter(parser_state* state, bool is_internal) : state(state), is_internal(is_internal) {}
  ...
  PyObject* visitAsPyObjectFinal(antlr4::tree::ParseTree* tree) {
    ...
    } catch HANDLE_CLAIRQL_ERROR(SyntaxError, ) catch HANDLE_CLAIRQL_ERROR(
        NotImplementedError,
    ) catch HANDLE_CLAIRQL_ERROR(ParsingError, ) catch (const PyInternalError& e) {
      return NULL;
  ...
  VISIT(Select) {
    ...
    return visit(ctx->clairqlxTagElement());
  }
...
#undef IS_NODE_A_CONCAT_CALL
  ...
  VISIT(ColumnExprTagElement) { return visit(ctx->clairqlxTagElement()); }
  ...
  VISIT(TableExprTag) { return visit(ctx->clairqlxTagElement()); }
  ...
  VISIT(ClairqlxTagAttribute) {
    string name = visitAsString(ctx->identifier());

    auto column_expr_ctx = ctx->columnExpr();
    if (column_expr_ctx) {
      RETURN_NEW_AST_NODE(
          "ClairQLXAttribute", "{s:s#,s:N}", "name", name.data(), name.size(), "value", visitAsPyObject(column_expr_ctx)
      );
    }
    ...
    if (string_ctx) {
      ...
      if (!value) throw PyInternalError();
      RETURN_NEW_AST_NODE("ClairQLXAttribute", "{s:s#,s:N}", "name", name.data(), name.size(), "value", value);
    }
  ...
  VISIT(ClairqlxTagElementClosed) {
    string kind = visitAsString(ctx->identifier());
    RETURN_NEW_AST_NODE(
        "ClairQLXTag", "{s:s#,s:N}", "kind", kind.data(), kind.size(), "attributes",
        visitPyListOfObjects(ctx->clairqlxTagAttribute())
    );
  }

  VISIT(ClairqlxTagElementNested) {
    string opening = visitAsString(ctx->identifier(0));
    string closing = visitAsString(ctx->identifier(1));
    if (opening != closing) {
      throw SyntaxError("Opening and closing ClairQLX tags must match. Got " + opening + " and " + closing);
    }

    auto tag_element_ctx = ctx->clairqlxTagElement();
    auto column_expr_ctx = ctx->columnExpr();
    auto tag_attribute_ctx = ctx->clairqlxTagAttribute();
    ...
    if (tag_element_ctx) {
      if (found_source) {
        Py_DECREF(attributes);
        throw SyntaxError("Nested ClairQLX tags cannot have a source attribute");
      }
      PyObject* source_attribute = build_ast_node(
          "ClairQLXAttribute", "{s:s#,s:N}", "name", "source", 6, "value", visitAsPyObject(ctx->clairqlxTagElement())
      );
      if (!source_attribute) {
        Py_DECREF(attributes);
        throw PyInternalError();
      ...
      PyObject* source_attribute = build_ast_node(
          "ClairQLXAttribute", "{s:s#,s:N}", "name", "source", 6, "value", visitAsPyObject(ctx->columnExpr())
      );
      if (!source_attribute) {
        Py_DECREF(attributes);
        throw PyInternalError();
    ...
    RETURN_NEW_AST_NODE("ClairQLXTag", "{s:s#,s:N}", "kind", opening.data(), opening.size(), "attributes", attributes);
  }
...
class ClairQLErrorListener : public antlr4::BaseErrorListener {
 public:
  string input;

  ClairQLErrorListener(string input) : input(input) {}
...
#define METHOD_PARSE_NODE(PASCAL_CASE, CAMEL_CASE, SNAKE_CASE)                                                         \
  static PyObject* method_parse_##SNAKE_CASE(PyObject* self, PyObject* args, PyObject* kwargs) {                       \
    ...
    auto lexer = new ClairQLLexer(input_stream);                                                                         \
    auto stream = new antlr4::CommonTokenStream(lexer);                                                                \
    auto parser = new ClairQLParser(stream);                                                                             \
    parser->removeErrorListeners();                                                                                    \
    auto error_listener = new ClairQLErrorListener(str);                                                                 \
    parser->addErrorListener(error_listener);                                                                          \
    ClairQLParser::PASCAL_CASE##Context* parse_tree;                                                                     \
    try {                                                                                                              \
      parse_tree = parser->CAMEL_CASE();                                                                               \
    } catch HANDLE_CLAIRQL_ERROR(SyntaxError, delete error_listener; delete parser; delete stream; delete lexer;         \
                                  delete input_stream;)                                                                \
    ...
    ClairQLParseTreeConverter converter = ClairQLParseTreeConverter(state, internal == 1);                                 \
...
static PyObject* method_parse_string_literal_text(PyObject* self, PyObject* args) {
  ...
  } catch HANDLE_CLAIRQL_ERROR(SyntaxError, );
...
static PyMethodDef parser_methods[] = {
    {.ml_name = "parse_expr",
     ...
     .ml_doc = "Parse the ClairQL expression string into an AST"},
    ...
    {.ml_name = "parse_select",
     ...
     .ml_flags = METH_VARARGS | METH_KEYWORDS,
     .ml_doc = "Parse the ClairQL SELECT statement string into an AST"},
...
static int parser_modexec(PyObject* module) {
  ...
  state->ast_module = PyImport_ImportModule("clairview.clairql.ast");
  if (!state->ast_module) {
    return -1;
  }
  state->base_module = PyImport_ImportModule("clairview.clairql.base");
  if (!state->base_module) {
    return -1;
  }
  state->errors_module = PyImport_ImportModule("clairview.clairql.errors");
...
static struct PyModuleDef parser = {
    ...
    .m_name = "clairql_parser",
    .m_doc = "ClairQL parsing",
...
PyMODINIT_FUNC PyInit_clairql_parser(void) {
