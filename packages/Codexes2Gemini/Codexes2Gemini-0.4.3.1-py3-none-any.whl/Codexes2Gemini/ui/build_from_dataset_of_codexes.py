...
def prompts_plan_builder_ui(user_space: UserSpace):
    ...
    with st.form("Select Data Set"):
        selection_strategy = st.radio("Choose context selection strategy source:", ["Random", "User Upload"], index=0)
        uploaded_file = st.file_uploader("Upload CSV file (PG19 format)", type=["csv"],
                                         help="This widget be ignored unless you choose Upload option. See link for example of the acceptable format.")
