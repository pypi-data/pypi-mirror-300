...
def extract_coords_func(uargs):
    ...
    head = os.path.splitext(uargs.output_file)[0]
    xyzp.save_xyz(
        f'{head}_coords.xyz',
        labels,
        coords,
...
def gen_job_func(uargs):
    ...
    n_procs = extractor.NProcsInputExtractor.extract(uargs.input_file)[0]
    maxcore = extractor.MaxCoreInputExtractor.extract(uargs.input_file)[0]
    ...
    if not uargs.no_sub:
        subprocess.call(
            '{} {}'.format(
                oj.scheduler_job_class.SUBMIT_COMMAND,
                oj.job_file
                ),
            shell=True
        )
...
def plot_uvvis_func(uargs):
    ...
    fig, ax = plotter.plot_uvvis(
        ...
        window_title=f'UV-Visible Spectrum from {uargs.output_file}',
...
def print_freq_func(uargs) -> None:
    ...
    if uargs.num is None:
        uargs.num = len(_fext.data[0]['frequency (cm^-1)'])

    for frq in _fext.data[0]['frequency (cm^-1)'][:uargs.num]:
        print(f'{frq:.2f}')
...
def read_args(arg_list=None):
    ...
    distort.add_argument(
        'output_file',
        type=str,
        help='Orca output file name(s) - must contain frequency section'
    )
    ...
    plot_uvvis = plot_parser.add_parser(
        'uvvis',
    ...
    print_freq.add_argument(
        'output_file',
        type=str,
        help='Orca output file name(s) - must contain Frequencies section'
    )
    ...
    print_natorbs.add_argument(
        'output_file',
        type=str,
        help='Orca output file name(s) - must contain LOEWDIN ORBITAL-COMPOSITIONS section' # noqa
    )
