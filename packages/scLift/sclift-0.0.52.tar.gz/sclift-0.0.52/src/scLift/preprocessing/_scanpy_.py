...
from scLift.file import save_h5ad
from scLift.util import add_cluster_info, matrix_data, set_inf_value

from ykenan_log import Logger

log = Logger("scLift_preprocessing_scanpy")
...
def filter_data(
    ...
    log.info(f"Size of filtered scATAC data: {filter_adata.shape}")
...
def get_difference_genes(
    ...
    if "log1p" not in adata.uns_keys():
        log.info("The `log1p` not detected in `adata.uns_keys`, `log1p` operation needs to be performed.")
    ...
    if "base" not in adata.uns["log1p"].keys():
        log.warn("`base` does not exist in `adata.uns[\"log1p\"]`, it defaults to `None`.")
        adata.uns["log1p"].update({"base": None})

    # gene
    log.info("Rank genes for characterizing groups.")
    ...
    for _cluster_ in cluster_list:
        log.info(f"Obtaining differentially expressed genes for `cluster` ({_cluster_}).")
        ...
        for _gene_name_, _score_, _p_value_, _adjusted_p_value_, _log2_fold_change_ in zip(
            ...
            diff_genes_p_value_matrix[_gene_index_, _cluster_index_] = 0 if np.isnan(_p_value_) else _p_value_
            diff_genes_adjusted_p_value_matrix[_gene_index_, _cluster_index_] = 0 if np.isnan(
                _adjusted_p_value_
            ) else _adjusted_p_value_
            diff_genes_log2_fold_change_matrix[_gene_index_, _cluster_index_] = 0 if np.isnan(
                _log2_fold_change_
            ) else _log2_fold_change_
