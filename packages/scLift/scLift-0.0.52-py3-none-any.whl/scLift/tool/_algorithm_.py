...
log = Logger("scLift_tool_algorithm")
lock = Lock()
...
class RandomWalk:
    ...
    def _get_seed_cell_core_(self, params) -> None:
        (init_data, i, seed_cell_threshold, seed_cell_weight, seed_cell_en_weight, seed_cell_matrix,
         seed_cell_en_matrix) = params
        ...
        if trait_value_max <= 0:
            lock.acquire()
            log.warn(
                f"There is no possibility of enrichment between scATAC-seq and {self.trait_list[i]} trait obtained "
                f"from the initialization score."
            )
            lock.release()
        else:
            ...
            if not self.is_simple:
                ...
                seed_cell_en_matrix[:, i] = seed_cell_en_value / (
                    1 if seed_cell_en_value.sum() == 0 else seed_cell_en_value.sum())

    def _get_seed_cell_(self, init_data: AnnData) -> Tuple[
        collection, matrix_data, matrix_data, matrix_data, matrix_data]:
        """
        ...
        pool.map(self._get_seed_cell_core_, params)
        pool.close()
        pool.join()
    ...
    def _run_base_(self, params):
        """
        Run a random walk on one of the traits or diseases
        :param params: params
        """
        (data_vector, seed_cell_threshold, i, score, info) = params
        # information output
        lock.acquire()
        log.info(
            f"Random walk on trait {self.trait_info[i]}: {i} ({'' if info is None else info}) ==> Threshold value: "
            f"{seed_cell_threshold[i]}"
        )
        lock.release()

        # Random walk
        score[:, i] = self._random_walk_core_(data_vector)
    ...
    def run_random(self) -> None:
        ...
        pool = Pool(self.thread)
        pool.map(self._run_random_, self.trait_range)
        pool.close()
        pool.join()
    ...
    def _run_(self, seed_cell_data: matrix_data, seed_cell_threshold: collection, label: str) -> matrix_data:
        ...
        params: list = []
        for i in self.trait_range:
            params.append((seed_cell_data[:, i], seed_cell_threshold, i, score, label))

        pool = Pool(self.thread)
        pool.map(self._run_base_, params)
        pool.close()
        pool.join()
    ...
    def _run_en_core_(self, params):
        (
            i,
            seed_cell_threshold,
            seed_cell_en_weight,
            score,
            trait_cell_enrichment,
            trait_cell_credible,
            label
        ) = params
        # information output
        lock.acquire()
        log.info(
            f"Start random walk on trait {self.trait_info[i]}: {i} ({label}) ==> Threshold value: "
            f"{seed_cell_threshold[i]}"
        )
        lock.release()
    ...
    def _run_enrichment_(self, seed_cell_en_weight: matrix_data, seed_cell_threshold: collection, label: str) -> None:
        ...
        for i in self.trait_range:
            params.append(
                (i, seed_cell_threshold, seed_cell_en_weight, score, trait_cell_enrichment, trait_cell_credible, label)
            )

        pool = Pool(self.thread)
        pool.map(self._run_en_core_, params)
        pool.close()
        pool.join()
...
def overlap(regions: DataFrame, variants: DataFrame) -> DataFrame:
    ...
    for index, chr_a, start, end in zip(
        regions_sort["index"], regions_sort["chr"], regions_sort["start"], regions_sort["end"]
    ):
            ...
            if start <= variants_chr_type_position_list[-1] and end >= variants_chr_type_position_list[0]:
                ...
                if start_index != end_index:
                    ...
                    variants_overlap_info.index = (
                        variants_overlap_info["variant_id"].astype(str)
                        + "_" + variants_overlap_info["index"].astype(str)
                    )
...
def overlap_sum_core(params) -> None:
    (variants, regions, trait_name, trait_names, label_all, matrix) = params

    variant: AnnData = variants[trait_name]
    index: int = trait_names.index(trait_name)
    lock.acquire()
    log.info(f"Start {index + 1} trait/disease {trait_name} (overlap variant information)")
    lock.release()
...
def overlap_sum(regions: AnnData, variants: dict, trait_info: DataFrame, thread: int = 16) -> AnnData:
    """
    ...
    for trait_name in trait_names:
        params.append((variants, regions, trait_name, trait_names, label_all, matrix))

    pool = Pool(thread)
    pool.map(overlap_sum_core, params)
    pool.close()
    pool.join()
...
def calculate_init_score_weight(
    ...
    is worth noting that when set to `True`, the `is_ablation` parameter will become invalid, and when set to `False`,
    `is_ablation` will only take effect;
    ...
    init_score_weight_adata = AnnData(
        to_sparse(_init_score_weight_), obs=adata.obs, var=overlap_adata.uns["trait_info"]
    )
...
def obtain_cell_cell_network(
    ...
    cell_mutual_knn_weight, cell_mutual_knn = semi_mutual_knn_weight(
        cell_affinity, neighbors=k, or_neighbors=or_k, weight=weight
    )
