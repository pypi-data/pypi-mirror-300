...
from scLift.file import read_sc_atac, read_h5ad, save_h5ad
from scLift.util import path, collection, set_inf_value, chrtype, add_cluster_info, project_cache_path, generate_str

file_method = StaticMethod("sclift_preprocessing_snapatac2")
log = Logger("sclift_preprocessing_snapatac2")
...
def handler_sc_atac(
    ...
    if isinstance(fragment_file, path):
        log.info(f"Read finish, shape: {data.shape}")
    else:
        log.info(f"Read finish")
...
def _get_handler_sc_atac_(
    ...
    if h5ad_file is not None and os.path.exists(h5ad_file):
        ...
        _cache_file_path_ = os.path.join(project_cache_path, generate_str())
        _is_cache_ = False

        if h5ad_file is None:
            _is_cache_ = True
            file_method.makedirs(_cache_file_path_)
...
def merge_sc_atac(
    ...
    :param harmony_groupby: If specified, split the data into groups and perform batch correction on each group
        separately.
...
def get_gene_expression(
    ...
    log.info("Obtain gene expression matrix.")
    ...
    log.info(f"Shape: {selected_adata.shape}")

    # normalize
    log.info(f"Filter genes. shape: {gene_matrix.shape}")
    sc.pp.filter_genes(gene_matrix, min_cells=min_cells)
    log.info(f"Gene matrix shape: {gene_matrix.shape}")

    log.info(f"Normalize, log1p and magic.")
...
def get_peak_matrix(
    ...
    log.info(f"Peak calling at the `{cluster}`-level.")
    try:
        snap.tl.macs3(selected_adata, groupby=cluster)
    except Exception as e:
        log.error(f"The `cluster`({cluster}) is likely to have `NaN` values, please check.")
        raise RuntimeError(f"The `cluster`({cluster}) is likely to have `NaN` values, please check.\n {e}")

    log.info("Obtain a unified, non-overlapping, and fixed-width peak list.")
    peaks = snap.tl.merge_peaks(selected_adata.uns['macs3'], genome_anno)

    log.info("Create a cell by peak matrix.")
...
def _handler_info_to_adata_(
    ...
    log.info(f"Create difference peaks data, shape {shape}")
    ...
    for _id_, log2_fold_change, p_value, adjusted_p_value, _cluster_ in zip(
        ...
        log2_fold_change_matrix[obs_unique.index(_id_), cluster_list.index(_cluster_)] = log2_fold_change
        p_value_matrix[obs_unique.index(_id_), cluster_list.index(_cluster_)] = p_value
        adjusted_p_value_matrix[obs_unique.index(_id_), cluster_list.index(_cluster_)] = adjusted_p_value
...
def get_tf_data(
    ...
    log.info("Finding marker regions.")
    marker_peaks = snap.tl.marker_regions(peak_mat, groupby=cluster, pvalue=p_value)

    if len(marker_peaks.keys()) == 0 and p_value < 0.05:
        p_value = 0.05
        log.warn(f"Due to the absence of marker regions, the `p_value` value is automatically changed to `0.05`.")
        marker_peaks = snap.tl.marker_regions(peak_mat, groupby=cluster, pvalue=p_value)

    if len(marker_peaks.keys()) == 0:
        log.error(f"No marker regions for `p_value` = {p_value}, you can try to increase the `p_value`.")
        raise ValueError(f"No marker regions for `p_value` = {p_value}, you can try to increase the `p_value`.")

    log.info("Motif enrichment.")
    ...
    log.info(f"Merge motif result for `{have_cluster_list}`")
...
def get_difference_peaks(
    ...
    for _cluster_ in cluster_list:
        log.info(f"Processing {cluster}: {_cluster_}/({cluster_str}).")
        ...
        log.info(f"Processing {cluster}: {_cluster_} difference peaks.")
