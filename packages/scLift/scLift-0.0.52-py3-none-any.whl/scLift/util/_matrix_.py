...
from ._constant_ import dense_data, sparse_data, sparse_matrix, matrix_data, number, collection

log = Logger("scLift_util_matrix")
...
def to_dense(sm: matrix_data, is_array: bool = False) -> dense_data:
    ...
    if sm is None:
        log.warn("The input matrix (sm parameter) is not feasible")
        return np.asmatrix([])
...
def to_sparse(dm: dense_data, way_callback=sparse.csr_matrix, is_matrix: bool = True) -> sparse_matrix:
    ...
    if dm is None:
        log.warn("The input matrix (dm parameter) is not feasible")
...
def get_sub_data(data: collection, size: int) -> collection:
    ...
    if size >= old_size:
        log.warn("The given size is greater than the original data size")
        return data
...
def split_matrix(data: matrix_data, axis: Literal[0, 1] = 0, chunk_number: int = 1000) -> list:
    ...
    log.info(f"Divide the matrix into {split_number} parts")
...
def merge_matrix(datas: list, axis: Literal[0, 1] = 0) -> list:
    ...
    log.info(f"Merge matrix {matrix_shape} shape")
