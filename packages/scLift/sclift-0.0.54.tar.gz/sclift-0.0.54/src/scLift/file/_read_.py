...
from scLift.util import path, to_sparse, list_duplicate_set, collection, project_name, project_version

file_method = StaticMethod()
log = Logger("scLift_file_read")


def read_h5ad(file: path) -> AnnData:
    """
    Read AnnData data
    :param file: file path
    :return: AnnData data
    """
    log.info("Reading AnnData file: {}".format(file))
...
def read_pkl(file: path):
    log.info("Reading pkl file: {}".format(file))
...
def barcodes_add_anno(annotation_file: path, cell_anno: DataFrame, clusters: str = None) -> DataFrame:
    ...
    if "barcodes" not in list(cell_annotation_file.columns):
        log.error("The annotation file must contain a column name with `barcodes`")
        raise ValueError("The annotation file must contain a column name with `barcodes`")

    if "barcodes" not in list(cell_anno.columns):
        log.error("The `cell_anno` must contain a column name with `barcodes`")
    ...
    if len(cell_anno["barcodes"]) > len(set(cell_annotation_file["barcodes"])):
        log.error("Insufficient number of barcodes in the annotation file.")
...
def _read_info_by_metadata_(
    ...
    :param clusters: Column for clusters processing.
...
def read_v3_10x_h5(filename: path):
    """
...
def read_sc_atac_10x_h5(
    ...
    Read hdf5 file from Cell Ranger v3 or later versions.
    """
    log.info("Reading scATAC-seq 10x data")
...
def read_variants(
    ...
    :param column_map: The mapping of column names facilitates mapping the corresponding column names in the mutation file to the specified column name information, i.e. `{0: "chr", 1: "position", 2: "rsId", 3: "pp"}`;
    :param is_verbose: If `True`, display the log of the read file.
    ...
    if base_path is None and file_size == 0:
        log.error("At least one of the `resource` and `files` parameters has a parameter")
        raise ValueError("At least one of the `resource` and `files` parameters has a parameter")

    new_files = []

    if isinstance(base_path, path):
        new_files = file_method.get_files_path(base_path)
    ...
    for variant_file in tqdm(new_files):
        ...
        if column_map is not None:
            ...
            if is_verbose:
                log.info(f"Read variant file ==> {variant_file}, key: {key}")
        ...
        if not set(need_variant_columns).issubset(set(columns)):
            log.error(f"The column name of the {variant_file} file needs to include {need_variant_columns}")
            raise ValueError(f"The column name of the {variant_file} file needs to include {need_variant_columns}")

        if variant.shape[0] == 0:
            log.warn(f"The content of file {variant_file} is empty")
            continue
        ...
        if key not in key_list:
            ...
            log.warn(f"The `{old_key}` already exists in the previous file, set this key to `{key}`")
            variant["id"] = key
