...
class Item(BaseModel):
    ...
    Item
    """ # noqa: E501
    rendered_body: StrictStr = Field(description="HTML形式の本文")
    body: StrictStr = Field(description="Markdown形式の本文")
    coediting: StrictBool = Field(description="この記事が共同更新状態かどうか (Qiita Teamでのみ有効)")
    comments_count: StrictInt = Field(description="この記事へのコメントの数")
    created_at: StrictStr = Field(description="データが作成された日時")
    group: Optional[Group] = None
    id: StrictStr = Field(description="記事の一意なID")
    likes_count: StrictInt = Field(description="この記事への「いいね」の数（Qiitaでのみ有効）")
    private: StrictBool = Field(description="限定共有状態かどうかを表すフラグ (Qiita Teamでは無効)")
    reactions_count: StrictInt = Field(description="絵文字リアクションの数（Qiita Teamでのみ有効）")
    stocks_count: StrictInt = Field(description="この記事がストックされた数")
    tags: List[ItemTag] = Field(description="記事に付いたタグ一覧")
    title: StrictStr = Field(description="記事のタイトル")
    updated_at: StrictStr = Field(description="データが最後に更新された日時")
    url: StrictStr = Field(description="記事のURL")
    user: User
    page_views_count: Optional[StrictInt] = Field(description="閲覧数")
    team_membership: Optional[ItemTeamMembership] = None
    slide: StrictBool = Field(description="スライドモードが有効を表すフラグ")
    organization_url_name: Optional[StrictStr] = Field(description="記事のOrganization の url_name を表します。")
    __properties: ClassVar[List[str]] = ["rendered_body", "body", "coediting", "comments_count", "created_at", "group", "id", "likes_count", "private", "reactions_count", "stocks_count", "tags", "title", "updated_at", "url", "user", "page_views_count", "team_membership", "slide", "organization_url_name"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if page_views_count (nullable) is None
        # and model_fields_set contains the field
        if self.page_views_count is None and "page_views_count" in self.model_fields_set:
            _dict['page_views_count'] = None

        # set to None if organization_url_name (nullable) is None
        # and model_fields_set contains the field
        if self.organization_url_name is None and "organization_url_name" in self.model_fields_set:
            _dict['organization_url_name'] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "tags": [ItemTag.from_dict(_item) for _item in obj["tags"]] if obj.get("tags") is not None else None,
            "title": obj.get("title"),
            "updated_at": obj.get("updated_at"),
            "url": obj.get("url"),
            "user": User.from_dict(obj["user"]) if obj.get("user") is not None else None,
            "page_views_count": obj.get("page_views_count"),
            "team_membership": ItemTeamMembership.from_dict(obj["team_membership"]) if obj.get("team_membership") is not None else None,
            "slide": obj.get("slide"),
