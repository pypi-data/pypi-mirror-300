...
class IssueAccessTokenRequest(BaseModel):
    ...
    """ # noqa: E501
    client_id: Annotated[str, Field(min_length=40, strict=True, max_length=40)] = Field(description="登録されたAPIクライアントを特定するためのID")
    client_secret: Annotated[str, Field(min_length=40, strict=True, max_length=40)] = Field(description="登録されたAPIクライアントを認証するための秘密鍵")
    code: Annotated[str, Field(min_length=40, strict=True, max_length=40)] = Field(description="リダイレクト用のURLに付与された、アクセストークンと交換するための文字列")
    __properties: ClassVar[List[str]] = ["client_id", "client_secret", "code"]
    ...
    def client_secret_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if not re.match(r"^[0-9a-f]{40}$", value):
            raise ValueError(r"must validate the regular expression /^[0-9a-f]{40}$/")
        return value
