...
def validate_output_to(path: str) -> None:
    """
    Validates the output_to param used for data splitting has format '<catalog>.<schema>'.
    Additionally, checks that the user has the necessary permissions to write to the schema.
    """
    if not re.match(r'^[^\.]+\.[^\.]+$', path):
        raise ValueError('output_to must be in the format "<schema>.<catalog>".')
    catalog, schema_name = path.split('.')
    client = WorkspaceClient()
    validate_uc_permissions(catalog,
                            'catalog', ['ALL_PRIVILEGES', 'USE_CATALOG'],
                            input_name='output_to',
                            client=client)
    validate_uc_permissions(f'{catalog}.{schema_name}',
                            'schema', ['ALL_PRIVILEGES', 'USE_SCHEMA'],
                            input_name='output_to',
...
def validate_experiment_path(experiment_path: str) -> None:
    try:
        ...
        if experiment:
            raise ValidationError(
                f'Experiment path {experiment_path} already exists. Please provide a new experiment name.')
    except Exception as e:
        raise ValidationError(f'Failed to validate experiment path {experiment_path}') from e
...
def validate_create_training_run_inputs(train_data_path: str,
    ...
    if experiment_name and experiment_directory:
        # if the user doesn't provide name or directory, then we will create a valid experiment path,
        # so we do not need to validate it
        experiment_path = f'{experiment_directory}/{experiment_name}'
