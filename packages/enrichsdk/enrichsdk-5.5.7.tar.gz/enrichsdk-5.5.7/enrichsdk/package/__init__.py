...
import inspect
import logging
import platform # WINDOWS PORTING
...
# OpenAI
############################################################
def customize_with_gpt(what, text):

    if 'OPENAI_API_KEY' not in os.environ:
        print("OpenAI not setup. So not customizing further")
        return content

    system_prompt1 = """\
You are expert in python programming. Here we are constructing a python module to do the following:
    """
    system_prompt2 = """\
1. ONLY extend the class given below. Dont replace
2. Use the same structure and style for the code generated
\n\n""" + text

    print(f"Please enter instructions for customizing the {what}. Ctrl-D to end it")
    lines = []
    while True:
        try:
            line = input()
    ...
    print("Got it. Customizing the code")
...
def prepare_workspace(target, force=False, auto=False):
    ...
    witems = [
        "pipelines",
        os.path.join("pipelines", "conf"),
        os.path.join("pipelines", "lib"),
        os.path.join("pipelines", "jobs"),
        "dashboard",
        "bin",
        "docs",
        "commands",
        ...
        "assets",
        "tasks",
        os.path.join("tasks", "conf"),
        os.path.join("tasks", "lib"),
        os.path.join("tasks", "jobs"),
        "workflows",
        os.path.join("workflows", "prefect"),
        os.path.join("workflows", "spark"),
    ]
...
def prepare_script_minimal(path, templatepath):
    ...
    # Now customize it..
    content = customize_with_gpt("Transform Script", content)
...
def prepare_singlepageapp(path, templatepath):
    ...
    # Now customize
    content = customize_with_gpt("Single Page App", content)
...
def get_sample_djangoenv():
    ...
    if "ENRICH_ROOT" in os.environ:
        ...
        if "enrich" not in cwd:
            # cwd += "/enrich"
            cwd += ("/enrich" if opsys != "Windows" else "\\enrich")
        enrich_root = cwd
...
""" if opsys != "Windows" else f"""\

REM Global settings
set "ENRICH_ROOT={cwd}"
set "ENRICH_DATA=%ENRICH_ROOT%\\data"
set "ENRICH_ETC=%ENRICH_ROOT%\\etc"
set "ENRICH_CUSTOMERS=%ENRICH_ROOT%\\customers"

set "DATABASE_URL=sqlite:///%ENRICH_ROOT%\\shared\\db.sqlite3"
set DJANGO_SETTINGS_MODULE=config.settings.local
set DJANGO_DEFAULT_FROM_EMAIL=support@scribbledata.io
set DJANGO_SECRET_KEY="hz[+T3&F8?bOM.cTkh2o!Ox#OI,QM8R4bGjR=O4e)@iJ[DV:M-"
set DJANGO_EXTRA_ALLOWED_HOSTS=
set DJANGO_UWSGI_MODE=production
set EMAIL_HOST=email-smtp.us-east-1.amazonaws.com
set EMAIL_HOST_USER=AKIAZYH5XXX
set EMAIL_HOST_PASSWORD=BJGTIp88VjXXX
set EMAIL_PORT=587
set EMAIL_USE_TLS=TRUE

REM Suppress any python warnings
...
def prepare_data_roots(context={}):
    """
    Create the output directories
    """

    enrich_data = os.path.join(os.environ['ENRICH_ROOT'], 'data')
    enrich_customers = os.path.join(os.environ['ENRICH_ROOT'], 'customers')
    customers = os.listdir(enrich_customers)

    for c in customers:

        customer_root = os.path.join(enrich_customers, c)
        appdirs = os.listdir(customer_root)
        for appdir in appdirs:
            apppath = os.path.join(customer_root, appdir)
            if os.path.isdir(apppath) and appdir[0].isupper():
                target = os.path.join(enrich_data, c, appdir)
                if not os.path.exists(target):
                    os.makedirs(target, exist_ok=True)
                    print(greenbg("[Created] {}".format(target)))
                else:
                    print(greenbg("[Exists] {}".format(target)))
...
def populate(context={}):
    ...
    if "djangoenv" in os.environ:
        ...
        djangoenv = os.path.join(os.environ["ENRICH_ETC"], "django-env" if opsys != "Windows" else "django-env.bat")
    else:
...
#workon {venv_name}
export ENRICH_ROOT={enrich_root}
source $ENRICH_ROOT/etc/django-env
"""
...
def checkenv(validate_only=False, context={}):
    ...
    enrich_root = os.environ['ENRICH_ROOT']
    sys.path.append(os.path.join(enrich_root, "lib", "python"))
    import sitelib
    ...
    try:
        status, content = sitelib.read_siteconf(siteconf)
        if content is not None:
            print(greenbg("Valid siteconf exists"))
        else:
            print(redbg("Siteconf missing or invalid"))
    except:
        print(redbg("Siteconf is missing"))
