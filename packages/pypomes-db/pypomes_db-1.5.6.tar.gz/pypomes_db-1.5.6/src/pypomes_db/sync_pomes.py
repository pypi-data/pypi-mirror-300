...
def db_sync_data(errors: list[str] | None,
    ...
    if curr_source_conn and curr_target_conn:
        ...
        target_sel_stmt: str = source_sel_stmt
        if batch_size:
            if source_engine == "postgres":
                source_sel_stmt += f" OFFSET @offset ROWS LIMIT {batch_size}"
            elif source_engine in ["oracle", "slqserver"]:
                source_sel_stmt += f" OFFSET @offset ROWS FETCH NEXT {batch_size} ROWS ONLY"
            if target_engine == "postgres":
                target_sel_stmt += f" OFFSET @offset ROWS LIMIT {batch_size}"
            elif target_engine in ["oracle", "slqserver"]:
                target_sel_stmt += f" OFFSET @offset ROWS FETCH NEXT {batch_size} ROWS ONLY"
        ...
        try:
            ...
            target_cursor.execute(statement=target_sel_stmt.replace("OFFSET @offset ROWS ", ""))
            ...
            while source_rows or target_rows:
                ...
                if source_offset and not source_rows:
                    source_cursor.execute(statement=source_sel_stmt.replace("@offset", str(source_offset)))
                ...
                if target_offset and not target_rows:
                    target_cursor.execute(statement=target_sel_stmt.replace("@offset", str(target_offset)))
