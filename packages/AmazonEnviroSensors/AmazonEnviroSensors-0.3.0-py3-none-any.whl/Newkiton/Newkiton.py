from bluepy3.btle import Scanner, DefaultDelegate, Peripheral
import struct
import datetime
import logging
    ...
    def handleNotification(self, cHandle, data):
        ...
        if read_response_len == 20:
            ...
            for addr_idx in range(7):
                ...
                logging.debug(
                    "CMD={:#0X} Addr={:#0X} Temp={}".format(
                        responses[RESP_OFFSET_CMD], responses[RESP_OFFSET_BADDR] + addr_idx, temperature
                    )
                )
        self.read_event.set()
        return True
