...
template <class Derived> class QuaternionBase : public RotationBase<Derived, 3>
{
  public:
    ...
    // disabled this copy operator as it is giving very strange compilation
    // errors when compiling test_stdvector with GCC 4.4.2. This looks like a
    // GCC bug though, so feel free to re-enable it if it's useful; however
    // notice that we already have the templated operator= above and e.g. in
    // MatrixBase we didn't have to add, in addition to templated operator=,
    // such a non-templated copy operator.
    ...
    /** \returns the quaternion which transform \a a into \a b through a
     * rotation */
...
class Quaternion : public QuaternionBase<Quaternion<_Scalar, _Options>>
{
  public:
    ...
    FromTwoVectors(const MatrixBase<Derived1> &a,
                   const MatrixBase<Derived2> &b);
...
struct traits<Map<Quaternion<_Scalar>, _Options>>
    : traits<
          Quaternion<_Scalar, (int(_Options) & Aligned) == Aligned ? AutoAlign
                                                                   : DontAlign>>
{
    ...
    : public QuaternionBase<Map<const Quaternion<_Scalar>, _Options>>
{
  public:
     ...
     * in the following order: \code *coeffs == {x, y, z, w} \endcode
     *
...
QuaternionBase<Derived>::toRotationMatrix(void) const
{
    ...
    // gcc 4.3 !!) if not inlined then the cost of the return by value is huge ~
    // +35%, however, not inlining this function is an order of magnitude
    // slower, so it has to be inlined, and so the return by value is not an
    // issue
    Matrix3 res;
...
QuaternionBase<Derived>::slerp(const Scalar &t,
                               const QuaternionBase<OtherDerived> &other) const
{
