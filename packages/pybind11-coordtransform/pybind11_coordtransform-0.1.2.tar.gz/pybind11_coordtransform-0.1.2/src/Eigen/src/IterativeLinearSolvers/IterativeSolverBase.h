...
template <typename MatrixType> struct is_ref_compatible
{
    ...
    {
        value =
            is_ref_compatible_impl<typename remove_all<MatrixType>::type>::value
    };
};
...
template <typename MatrixType> class generic_matrix_wrapper<MatrixType, false>
{
  public:
    ...
    {
        typedef
            typename ActualMatrixType::template ConstSelfAdjointViewReturnType<
                UpLo>::Type Type;
    };
...
class IterativeSolverBase : public SparseSolverBase<Derived>
{
  ...
  public:
    ...
    /** Initializes the iterative solver for the sparsity pattern of the matrix
     * \a A for further solving \c Ax=b problems.
     *
     * Currently, this function mostly calls analyzePattern on the
     * preconditioner. In the future we might, for instance, implement column
    ...
    /** Initializes the iterative solver with the matrix \a A for further
     ...
     * decomposition of A and \a x0 as an initial solution.
     *
     * \sa solve(), compute()
     */
                                ...
                                SparseMatrixBase<DestDerived> &aDest) const
    {
        ...
        // We do not directly fill dest because sparse expressions have to be
        // free of aliasing issue. For non square least-square problems, b and
        // dest might not have the same size whereas they might alias
        // each-other.
        ...
        for (Index k = 0; k < rhsCols; ++k) {
            ...
            // The call to _solve_vector_with_guess_impl updates m_info, so if
            // it failed for a previous column we need to restore it to the
            // worst value.
    ...
    _solve_with_guess_impl(const Rhs &b, MatrixBase<DestDerived> &aDest) const
    {
        ...
        for (Index k = 0; k < rhsCols; ++k) {
            ...
            // The call to _solve_vector_with_guess updates m_info, so if it
            // failed for a previous column we need to restore it to the worst
            // value.
            if (m_info == NumericalIssue)
                global_info = NumericalIssue;
            else if (m_info == NoConvergence)
