...
{
    return umfpack_di_symbolic(n_row, n_col, Ap, Ai, Ax, Symbolic, Control,
                               Info);
}
...
inline int umfpack_symbolic(int n_row, int n_col, const int Ap[],
                            ...
                            double Info[UMFPACK_INFO])
{
    return umfpack_zi_symbolic(n_row, n_col, Ap, Ai, &numext::real_ref(Ax[0]),
...
{
    return umfpack_dl_symbolic(n_row, n_col, Ap, Ai, Ax, Symbolic, Control,
                               Info);
}
...
umfpack_symbolic(SuiteSparse_long n_row, SuiteSparse_long n_col,
                 ...
                 double Info[UMFPACK_INFO])
{
    return umfpack_zl_symbolic(n_row, n_col, Ap, Ai, &numext::real_ref(Ax[0]),
                               0, Symbolic, Control, Info);
}
...
inline int umfpack_get_lunz(int *lnz, int *unz, int *n_row, int *n_col,
                            int *nz_udiag, void *Numeric, std::complex<double>)
{
...
inline int umfpack_get_numeric(int Lp[], int Lj[], double Lx[], int Up[],
                               ...
                               void *Numeric)
{
    return umfpack_di_get_numeric(Lp, Lj, Lx, Up, Ui, Ux, P, Q, Dx, do_recip,
                                  Rs, Numeric);
}
...
{
    return umfpack_dl_get_numeric(Lp, Lj, Lx, Up, Ui, Ux, P, Q, Dx, do_recip,
                                  Rs, Numeric);
}
                                   ...
                                   double User_Info[UMFPACK_INFO], int)
{
    ...
    return umfpack_zi_get_determinant(&mx_real, 0, Ex, NumericHandle,
                                      User_Info);
}
...
{
    double &mx_real = numext::real_ref(*Mx);
    return umfpack_zl_get_determinant(&mx_real, 0, Ex, NumericHandle,
                                      User_Info);
}
...
class UmfPackLU : public SparseSolverBase<UmfPackLU<_MatrixType>>
{
  ...
  public:
     ...
     *  Note that the matrix should be column-major, and in compressed format
     * for best performance. \sa SparseMatrix::makeCompressed().
     */
    template <typename InputMatrixType>
  ...
  protected:
    ...
    void factorize_impl()
    {
        ...
        m_fact_errorCode = umfpack_numeric(
            mp_matrix.outerIndexPtr(), mp_matrix.innerIndexPtr(),
            mp_matrix.valuePtr(), m_symbolic, &m_numeric, m_control.data(),
            m_umfpackInfo.data());
...
template <typename MatrixType> void UmfPackLU<MatrixType>::extractData() const
{
    if (m_extractedDataAreDirty) {
        ...
        umfpack_get_lunz(&lnz, &unz, &rows, &cols, &nz_udiag, m_numeric,
                         Scalar());
                                        ...
                                        MatrixBase<XDerived> &x) const
{
    ...
    for (int j = 0; j < rhsCols; ++j) {
        ...
        StorageIndex errorCode =
            umfpack_solve(UMFPACK_A, mp_matrix.outerIndexPtr(),
                          mp_matrix.innerIndexPtr(), mp_matrix.valuePtr(),
