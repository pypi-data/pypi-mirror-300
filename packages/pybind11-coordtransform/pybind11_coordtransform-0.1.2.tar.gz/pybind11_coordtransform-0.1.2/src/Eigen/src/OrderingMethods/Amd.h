    ...
    PermutationMatrix<Dynamic, Dynamic, StorageIndex> &perm)
{
    ...
    StorageIndex d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3,
                              jlast, ln, dense, nzmax, mindeg = 0, nvi, nvj,
                              nvk, mark, wnvi, ok, nel = 0, p, p1, p2, p3, p4,
                              pj, pk, pk1, pk2, pn, q, t, h;
    ...
    StorageIndex *last =
        perm.indices().data(); /* use P as workspace for last */
    ...
    for (i = 0; i < n; i++) {
        ...
        if (d == 1 && has_diag) /* node i is empty */
        {
            ...
            w[i] = 0;
        } else if (d > dense || !has_diag) /* node i is dense or has no
                                              structural diagonal element */
        {
    ...
    while (nel < n) /* while (selecting pivots) do */
    {
        ...
        for (p = pk1, pk = pk1; pk < pk2; pk++) /* finalize Lk */
        {
            ...
            head[d] = i;
            mindeg =
                std::min<StorageIndex>(mindeg, d); /* find new minimum degree */
            degree[i] = d;
