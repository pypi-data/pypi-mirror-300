...
template <typename IndexType> struct ColStructure
{
    ...
    {
        IndexType
            thickness; /* number of original columns represented by this */
    ...
    {
        IndexType
            headhash; /* head of a hash bucket, if col is at the head of */
        /* a degree list */
...
template <typename IndexType> struct RowStructure
{
    ...
    {
        IndexType
            degree;  /* number of principal & non-principal columns in row */
    ...
    )
{
    ...
    for (k = 0; k < n_col2; /* 'k' is incremented below */) {
        ...
        if (pfree + needed_memory >= Alen) {
            pfree = Colamd::garbage_collection(n_row, n_col, Row, Col, A,
                                               &A[pfree]);
            ngarbage++;
        ...
        Colamd::detect_super_cols(Col, A, head, pivot_row_start,
                                  pivot_row_length);
    ...
    while (rp < rp_end) {
        ...
        for (super_c = first_col; super_c != Empty;
            ...
            for (c = Col[super_c].shared4.hash_next; c != Empty;
                ...
                COLAMD_ASSERT(Col[c].shared2.score ==
                              Col[super_c].shared2.score);
