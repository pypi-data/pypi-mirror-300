  ...
  public:
    ...
    template <typename Stream> void dumpMemory(Stream &s)
    {
        ...
        s << "  nonzeros: "
          ...
          << "\n";
        s << "  perm:     " << ((total += m_P.size() * sizeof(int)) >> 20)
          << "Mb"
          << "\n";
  ...
  protected:
    ...
    template <bool DoLDLT> void factorize(const MatrixType &a)
    {
        ...
        if (m_P.size() == 0 && (int(UpLo) & int(Upper)) == Upper) {
            // If there is no ordering, try to directly use the input matrix
            // without any copy
            internal::simplicial_cholesky_grab_input<CholMatrixType,
                                                     MatrixType>::run(a, pmat,
                                                                      tmp);
        } else {
