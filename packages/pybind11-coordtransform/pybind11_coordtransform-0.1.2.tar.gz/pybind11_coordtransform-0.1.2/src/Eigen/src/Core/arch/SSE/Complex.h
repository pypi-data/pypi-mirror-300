...
EIGEN_STRONG_INLINE void pstoreu<std::complex<float>>(std::complex<float> *to,
                                                      const Packet2cf &from)
{
    EIGEN_DEBUG_UNALIGNED_STORE pstoreu(&numext::real_ref(*to),
                                        Packet4f(from.v));
}
                                              ...
                                              const Packet1cd &b)
{
    ...
    return Packet1cd(
        _mm_add_pd(_mm_mul_pd(vec2d_swizzle1(a.v, 0, 0), b.v),
                                              ...
                                              const Packet1cd &b)
{
    ...
    return Packet1cd(
        _mm_div_pd(res.v, _mm_add_pd(s, _mm_shuffle_pd(s, s, 0x1))));
}
