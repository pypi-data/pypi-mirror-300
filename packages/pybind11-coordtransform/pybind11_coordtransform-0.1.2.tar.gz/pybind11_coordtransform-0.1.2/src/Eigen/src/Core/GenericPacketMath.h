...
template <typename T>
struct ptrue_impl<
    T, typename internal::enable_if<is_scalar<T>::value &&
...
template <typename Packet> EIGEN_DEVICE_FUNC inline Packet pnot(const Packet &a)
{
...
template <> EIGEN_DEVICE_FUNC inline unsigned long pabs(const unsigned long &a)
{
    return a;
}
template <>
...
template <int N> EIGEN_DEVICE_FUNC inline int plogical_shift_right(const int &a)
{
...
template <int N> EIGEN_DEVICE_FUNC inline int plogical_shift_left(const int &a)
{
    return a << N;
}
template <int N>
...
predux_mul(const Packet &a)
{
    ...
    return predux_helper(
        a, EIGEN_BINARY_OP_NAN_PROPAGATION(Scalar, (pmul<Scalar>)));
}
...
EIGEN_DEVICE_FUNC inline bool predux_any(const Packet &a)
{
    ...
    // For all these cases, taking the sum is just fine, and this boils down to
    // a no-op for scalars.
    typedef typename unpacket_traits<Packet>::type Scalar;
