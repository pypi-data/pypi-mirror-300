...
EIGEN_STRONG_INLINE Packet8s pcast<Packet4f, Packet8s>(const Packet4f &a,
                                                       const Packet4f &b)
{
    return vcombine_s16(vmovn_s32(vcvtq_s32_f32(a)),
                        vmovn_s32(vcvtq_s32_f32(b)));
}
template <>
...
EIGEN_STRONG_INLINE Packet8us pcast<Packet4f, Packet8us>(const Packet4f &a,
                                                         const Packet4f &b)
{
    return vcombine_u16(vmovn_u32(vcvtq_u32_f32(a)),
...
template <>
EIGEN_STRONG_INLINE Packet16uc pcast<Packet8us, Packet16uc>(const Packet8us &a,
                                                            const Packet8us &b)
{
...
template <>
EIGEN_STRONG_INLINE Packet16uc pcast<Packet4ui, Packet16uc>(const Packet4ui &a,
                                                            const Packet4ui &b,
                                                            const Packet4ui &c,
                                                            const Packet4ui &d)
{
...
EIGEN_STRONG_INLINE Packet8us pcast<Packet2ul, Packet8us>(const Packet2ul &a,
                                                          ...
                                                          const Packet2ul &d)
{
    const uint16x4_t ab_u16 =
        vmovn_u32(vcombine_u32(vmovn_u64(a), vmovn_u64(b)));
    const uint16x4_t cd_u16 =
        vmovn_u32(vcombine_u32(vmovn_u64(c), vmovn_u64(d)));
...
EIGEN_STRONG_INLINE Packet4i pcast<Packet2d, Packet4i>(const Packet2d &a,
                                                       const Packet2d &b)
{
    return vcombine_s32(vmovn_s64(vcvtq_s64_f64(a)),
                        vmovn_s64(vcvtq_s64_f64(b)));
}
...
EIGEN_STRONG_INLINE Packet4ui pcast<Packet2d, Packet4ui>(const Packet2d &a,
                                                         const Packet2d &b)
{
    return vcombine_u32(vmovn_u64(vcvtq_u64_f64(a)),
                        vmovn_u64(vcvtq_u64_f64(b)));
}
