...
template <typename Func, typename Evaluator> struct packetwise_redux_traits
{
    enum
    {
        ...
        Unrolling =
            Cost <= EIGEN_UNROLLING_LIMIT ? CompleteUnrolling : NoUnrolling
    };
};
    ...
    : evaluator_base<PartialReduxExpr<ArgType, MemberOp, Direction>>
{
    ...
    enum
    {
        TraversalSize = Direction == int(Vertical)
    ...
    enum
    {
        ...
        Alignment = 0 // FIXME this will need to be improved once
                      // PartialReduxExpr is vectorized
    };
    ...
    {
        return m_functor(
            m_arg.template subVector<DirectionType(Direction)>(index));
    }
    ...
    EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC PacketType packet(Index idx) const
    {
        ...
        PanelType panel(
            m_arg, Direction == Vertical ? 0 : idx,
        ...
        PacketType p =
            internal::packetwise_redux_impl<BinaryOp, PanelEvaluator>::
                template run<PacketType>(panel_eval, m_functor.binaryFunc(),
                                         m_arg.outerSize());
        return p;
    }
