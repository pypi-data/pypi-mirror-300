...
prsqrt<Packet4f>(const Packet4f &_x)
{
    ...
    Packet4f y_newton = pmul(y_approx, pmadd(y_approx, pmul(neg_half, y_approx),
                                             p4f_one_point_five));

    // Select the result of the Newton-Raphson step for positive normal
    // arguments. For other arguments, choose the output of the intrinsic. This
    // will return rsqrt(+inf) = 0, rsqrt(x) = NaN if x < 0, and rsqrt(x) = +inf
    // if x is zero or a positive denormalized float (equivalent to flushing
    // positive denormalized inputs to zero).
