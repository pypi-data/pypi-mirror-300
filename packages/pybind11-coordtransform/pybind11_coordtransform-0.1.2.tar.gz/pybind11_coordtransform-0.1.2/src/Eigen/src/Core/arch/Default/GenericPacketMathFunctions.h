                                                            ...
                                                            Packet &exponent)
{
    ...
    // (technically the exponent is unspecified for inf/NaN, but GCC/Clang set
    // it to zero)
    const Scalar scalar_non_finite_exponent =
        Scalar((ScalarUI(1) << int(ExponentBits)) - ScalarUI(1)); // 255
    const Packet non_finite_exponent =
        pset1<Packet>(scalar_non_finite_exponent);
...
pldexp_generic(const Packet &a, const Packet &exponent)
{
    ...
    const PacketI bias = pset1<PacketI>(
        (ScalarI(1) << (int(ExponentBits) - 1)) - ScalarI(1)); // 127
...
template <typename Packet> struct pldexp_fast_impl
{
    ...
    {
        const Packet bias = pset1<Packet>(Scalar(
            (ScalarI(1) << (int(ExponentBits) - 1)) - ScalarI(1))); // 127
...
plog_impl_float(const Packet _x)
{
    ...
    // Add the logarithm of the exponent back to the result of the
    // interpolation.
    if (base2) {
...
plog_impl_double(const Packet _x)
{
    ...
    // Evaluate the polynomial approximant , probably to improve
    // instruction-level parallelism. y = x - 0.5*x^2 + x^3 * polevl( x, P, 5 )
    // / p1evl( x, Q, 5 ) );
    Packet y, y1, y_;
    y = pmadd(cst_cephes_log_p0, x, cst_cephes_log_p1);
    y1 = pmadd(cst_cephes_log_p3, x, cst_cephes_log_p4);
    ...
    // interpolation.
    if (base2) {
        const Packet cst_log2e =
            pset1<Packet>(static_cast<double>(EIGEN_LOG2E));
        x = pmadd(x, cst_log2e, e);
    } else {
...
// using "Extended precision modular arithmetic"
    ...
    x = pmadd(y, pset1<Packet>(-3.1391647326017846353352069854736328125e-07f),
              x);
    x = pmadd(
        y,
        pset1<Packet>(
...
// The coefficients are: 0xbfc90f80, 0xb7354480, 0x2e74b9ee
    ...
    Packet sign_bit =
        ComputeSine
            ...
            : preinterpret<Packet>(
                  plogical_shift_left<30>(padd(y_int, csti_1)));
...
psqrt_complex(const Packet &a)
{
    ...
    // equation for a single complex square root of the number x + i*y. We want
    // to find real numbers u and v such that
    //    (u + i*v)^2 = x + i*y  <=>
    ...
    // To avoid over- and underflow, we use the stable formula for each
    // hypotenuse
    //    l0 = (min0 == 0 ? max0 : max0 * sqrt(1 + (min0/max0)**2)),
    // where max0 = max(|x0|, |y0|), min0 = min(|x0|, |y0|), and similarly for
    // l1.
    ...
    const RealScalar neg_zero =
        RealScalar(numext::bit_cast<float>(0x80000000u));
    ...
    real_inf_result.v = pselect(
        negative_real_mask.v, pcplxflip(real_inf_result).v, real_inf_result.v);
    // prepare packet of (+∞,+∞) or (+∞,-∞), depending on the sign of the
...
{
    typedef typename unpacket_traits<Packet>::type Scalar;
    // 1. Approximate the reciprocal as the reciprocal of the high order
    // element.
...
template <> struct accurate_log2<float>
{
                                        ...
                                        Packet &log2_x_lo)
    {
        ...
        // while the remaining 4 terms of Q(x), as well as the final
        // multiplication by x to reconstruct log(1+x) are evaluated in extra
        // precision using double word arithmetic. C0 through C3 are extra
        // precise constants stored as double words.
        //
...
template <> struct fast_accurate_exp2<float>
{
    ...
    EIGEN_STRONG_INLINE Packet operator()(const Packet &x)
    {
        // This function approximates exp2(x) by a degree 6 polynomial of the
        // form Q(x) = 1 + x * (C + x * P(x)), where the degree 4 polynomial
        // P(x) is evaluated in single precision, and the remaining steps are
        // evaluated with extra precision using double word arithmetic. C is an
        // extra precise constant stored as a double word.
        //
        // The polynomial coefficients were calculated using Sollya commands:
        // > n = 6;
        // > f = 2^x;
        // > interval = [-0.5;0.5];
        // > p =
        // fpminimax(f,n,[|1,double,single...|],interval,relative,floating);
...
template <> struct fast_accurate_exp2<double>
{
    ...
    EIGEN_STRONG_INLINE Packet operator()(const Packet &x)
    {
        // This function approximates exp2(x) by a degree 10 polynomial of the
        // form Q(x) = 1 + x * (C + x * P(x)), where the degree 8 polynomial
        // P(x) is evaluated in single precision, and the remaining steps are
        // evaluated with extra precision using double word arithmetic. C is an
        // extra precise constant stored as a double word.
        //
...
generic_pow(const Packet &x, const Packet &y)
{
    ...
    const Packet invalid_negative_x = pandnot(
        pandnot(pandnot(x_is_neg, abs_x_is_inf), y_is_int), abs_y_is_inf);
    ...
    // General computation of pow(x,y) for positive x or negative x and integer
    // y.
                ...
                pselect(pow_is_nan, cst_nan,
                        pselect(pow_is_inf, cst_pos_inf,
                                pselect(pow_is_zero, cst_zero,
                                        pselect(negate_pow_abs,
                                                pnegate(pow_abs), pow_abs))))));
}
