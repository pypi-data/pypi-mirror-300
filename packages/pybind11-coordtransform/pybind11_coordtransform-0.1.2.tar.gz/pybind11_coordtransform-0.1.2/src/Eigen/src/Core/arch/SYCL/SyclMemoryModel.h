  ...
  public:
    ...
    typename pointerMap_t::iterator get_node(const virtual_pointer_t ptr)
    {
        if (this->count() == 0) {
            m_pointerMap.clear();
            EIGEN_THROW_X(
                std::out_of_range("There are no pointers allocated\n"));
        }
        if (is_nullptr(ptr)) {
            m_pointerMap.clear();
        ...
        if (node == std::end(m_pointerMap)) {
            ...
            if (node == std::begin(m_pointerMap)) {
                m_pointerMap.clear();
                EIGEN_THROW_X(std::out_of_range(
    ...
    get_buffer(const virtual_pointer_t ptr)
    {
        ...
        // get_node() returns a `buffer_mem`, so we need to cast it to a
        // `buffer<>`. We can do this without the `buffer_mem` being a pointer,
        // as we only declare member variables in the base class (`buffer_mem`)
        // and not in the child class (`buffer<>).
        ...
        : m_pointerMap{}, m_freeList{}, m_baseAddress{baseAddress}
    {
        if (m_baseAddress == 0) {
            EIGEN_THROW_X(
                std::invalid_argument("Base address cannot be zero\n"));
        }
    };
...
template <cl::sycl::access::mode AcMd, typename T> struct RangeAccess
{
    ...
    typedef cl::sycl::accessor<scalar_t, 1, AcMd, global_access,
                               is_place_holder>
        accessor;
