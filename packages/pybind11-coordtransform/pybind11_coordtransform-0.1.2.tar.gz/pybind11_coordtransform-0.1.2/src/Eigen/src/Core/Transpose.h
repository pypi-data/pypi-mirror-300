...
struct traits<Transpose<MatrixType>> : public traits<MatrixType>
{
    ...
    enum
    {
        ...
        Flags0 =
            traits<MatrixTypeNestedPlain>::Flags & ~(LvalueBit | NestByRefBit),
        Flags1 = Flags0 | FlagsLvalueBit,
        Flags = Flags1 ^ RowMajorBit,
        InnerStrideAtCompileTime =
            inner_stride_at_compile_time<MatrixType>::ret,
    ...
    : public internal::TransposeImpl_base<MatrixType>::type
{
  public:
    ...
    typedef
        typename internal::conditional<internal::is_lvalue<MatrixType>::value,
...
{ // PacketSize x PacketSize
    static void run(MatrixType &m)
    {
        ...
        typedef
            typename internal::packet_traits<typename MatrixType::Scalar>::type
                Packet;
            ...
            if (!NumTraits<Scalar>::IsComplex && m.rows() >= PacketSize) {
                if ((m.rows() % PacketSize) == 0)
                    BlockedInPlaceTranspose<
                        MatrixType, internal::evaluator<MatrixType>::Alignment>(
                        m);
                else
    ...
    Scalar, DestIsTransposed, CwiseBinaryOp<BinOp, DerivedA, DerivedB>>
{
                    ...
                    const CwiseBinaryOp<BinOp, DerivedA, DerivedB> &src)
    {
        return ((blas_traits<DerivedA>::IsTransposed != DestIsTransposed) &&
                (dest != 0 &&
                 dest == (const Scalar *)extract_data(src.lhs()))) ||
