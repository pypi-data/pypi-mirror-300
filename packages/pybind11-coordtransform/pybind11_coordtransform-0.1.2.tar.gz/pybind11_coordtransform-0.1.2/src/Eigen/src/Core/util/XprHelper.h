...
#else
            // without C++11, we use is_convertible to Index instead of
            // is_integral in order to treat enums as Index.
                                        ...
                                        true> // Try-half
{
    ...
    enum
    {
        value = compute_default_alignment_helper<ArrayBytes,
...
class compute_matrix_flags
{
  ...
  public:
    ...
    // the evaluator's flags. However, I (Gael) think that DirectAccessBit
...
{
    typedef typename conditional<bool(traits<T>::Flags &NestByRefBit),
                                 T const &, const T>::type type;
    ...
    enum
    {
        ...
        CoeffReadCost = evaluator<T>::
            CoeffReadCost, // NOTE What if an evaluator evaluate itself into a
                           ...
                           //      the parent evaluators. Another solution could
                           //      be to count the number of temps?
...
{
    /* dense_xpr_base should only ever be used on dense expressions, thus
     * falling either into the MatrixXpr or into the ArrayXpr cases */
};
...
template <typename XprType, typename CastType> struct cast_return_type
{
    ...
    typedef
        typename conditional<is_same<CurrentScalarType, NewScalarType>::value,
...
struct plain_diag_type
{
    ...
    {
        diag_size =
            EIGEN_SIZE_MIN_PREFER_DYNAMIC(ExpressionType::RowsAtCompileTime,
