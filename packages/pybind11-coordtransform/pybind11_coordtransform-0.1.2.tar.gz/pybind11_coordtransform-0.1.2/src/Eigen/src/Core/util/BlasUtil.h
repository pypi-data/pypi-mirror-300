...
class blas_data_mapper
{
  public:
    ...
    EIGEN_ALWAYS_INLINE Scalar &operator()(Index i, Index j) const
    {
        return m_data[StorageOrder == RowMajor
                          ? j * m_incr.value() + i * m_stride
    ...
    struct storePacketBlock_helper
    {
        ...
        EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
        store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType,
                                     Incr> *sup,
    ...
    {
        storePacketBlock_helper<SubPacket, std::complex<float>, n, idx - 1>
            spbh;
        EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
        store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType,
                                     Incr> *sup,
    ...
    {
        storePacketBlock_helper<SubPacket, std::complex<double>, n, idx - 1>
            spbh;
        EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
        store(const blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType,
                                     Incr> *sup,
    ...
    // This function stores a PacketBlock on m_data, this approach is really
    // quite slow compare to Incr=1 and should be avoided when possible.
    template <typename SubPacket, int n>
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
    storePacketBlock(Index i, Index j,
                     const PacketBlock<SubPacket, n> &block) const
...
{
    return combine_scalar_factors_impl<ResScalar, Lhs, Rhs>::run(alpha, lhs,
                                                                 rhs);
}
template <typename ResScalar, typename Lhs, typename Rhs>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE ResScalar
