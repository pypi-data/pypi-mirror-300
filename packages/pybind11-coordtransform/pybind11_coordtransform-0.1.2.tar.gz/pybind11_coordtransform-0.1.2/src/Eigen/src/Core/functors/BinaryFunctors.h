...
struct functor_traits<scalar_min_op<LhsScalar, RhsScalar, NaNPropagation>>
{
    ...
    {
        Cost =
            (NumTraits<LhsScalar>::AddCost + NumTraits<RhsScalar>::AddCost) / 2,
        PacketAccess = internal::is_same<LhsScalar, RhsScalar>::value &&
...
struct scalar_hypot_op<Scalar, Scalar> : binary_op_base<Scalar, Scalar>
{
    ...
    {
        // This functor is used by hypotNorm only for which it is faster to
        // first apply abs on all coefficients prior to reduction through hypot.
        // This way we avoid calling abs on positive and real entries, and this
        // also permits to seamlessly handle complexes. Otherwise we would have
        // to handle both real and complexes through the same functor...
...
{
    typedef
        typename ScalarBinaryOpTraits<Scalar, Exponent,
