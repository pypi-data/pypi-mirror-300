                  ...
                  GEBPPacketFull>
{
    EIGEN_STRONG_INLINE void acc(const AccPacket &c, const ResPacket &alpha,
                                 ResPacket &r) const
    {
        // This volatile inline ASM both acts as a barrier to prevent
        // reordering, as well as enforces strict register use.
                  ...
                  GEBPPacketFull>
{
  ...
  private:
    ...
    madd_helper(const LhsPacket &a, const RhsPacketx4 &b, AccPacket &c) const
    {
        ...
        // workaround gcc issue
        // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=89101 vfmaq_laneq_f32 is
        // implemented through a costly dup
        if (LaneID == 0)
...
#if EIGEN_COMP_GNUC_STRICT && !(EIGEN_GNUC_AT_LEAST(9, 0))
        // workaround gcc issue
        // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=89101 vfmaq_laneq_f64 is
        // implemented through a costly dup
        if (LaneID == 0)
