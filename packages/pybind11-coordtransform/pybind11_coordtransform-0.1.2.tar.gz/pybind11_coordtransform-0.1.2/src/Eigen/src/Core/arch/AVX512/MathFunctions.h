...
pexp<Packet16f>(const Packet16f &_x)
{
    ...
    // Get r = x - m*ln(2). Note that we can do this without losing more than
    // one ulp precision due to the FMA instruction.
...
EIGEN_STRONG_INLINE Packet16h pfrexp(const Packet16h &a, Packet16h &exponent)
{
    Packet16f fexponent;
    const Packet16h out =
        float2half(pfrexp<Packet16f>(half2float(a), fexponent));
...
{
    Packet16f fexponent;
    const Packet16bf out =
        F32ToBf16(pfrexp<Packet16f>(Bf16ToF32(a), fexponent));
...
psqrt<Packet16f>(const Packet16f &_x)
{
    ...
    return _mm512_mask_blend_ps(denormal_mask, pmul(_x, x),
                                _mm512_setzero_ps());
}
...
psqrt<Packet8d>(const Packet8d &_x)
{
    ...
    return _mm512_mask_blend_pd(denormal_mask, pmul(_x, x),
                                _mm512_setzero_pd());
}
#else
...
template <> EIGEN_STRONG_INLINE Packet16f prsqrt<Packet16f>(const Packet16f &x)
{
...
prsqrt<Packet16f>(const Packet16f &_x)
{
    ...
    // Do a single step of Newton-Raphson iteration to improve the
    // approximation. This uses the formula y_{n+1} = y_n * (1.5 - y_n * (0.5 *
    // x) * y_n). It is essential to evaluate the inner term like this because
    // forming y_n^2 may over- or underflow.
    Packet16f y_newton =
        pmul(y_approx,
             pmadd(y_approx, pmul(neg_half, y_approx), p16f_one_point_five));

    // Select the result of the Newton-Raphson step for positive finite
    // arguments. For other arguments, choose the output of the intrinsic. This
    // will return rsqrt(+inf) = 0, rsqrt(x) = NaN if x < 0, and rsqrt(0) =
    // +inf.
...
prsqrt<Packet8d>(const Packet8d &_x)
{
    ...
    // whether AVX512ER is available). The Newton-Raphson algorithm has
    // quadratic convergence and roughly doubles the number of correct digits
    // for each step. This uses the formula y_{n+1} = y_n * (1.5 - y_n * (0.5 *
    // x) * y_n). It is essential to evaluate the inner term like this because
    // forming y_n^2 may over- or underflow.
    Packet8d y_newton = pmul(y_approx, pmadd(neg_half, pmul(y_approx, y_approx),
                                             p8d_one_point_five));
#if !defined(EIGEN_VECTORIZE_AVX512ER)
    y_newton = pmul(y_newton, pmadd(y_newton, pmul(neg_half, y_newton),
                                    p8d_one_point_five));
#endif
    // Select the result of the Newton-Raphson step for positive finite
    // arguments. For other arguments, choose the output of the intrinsic. This
