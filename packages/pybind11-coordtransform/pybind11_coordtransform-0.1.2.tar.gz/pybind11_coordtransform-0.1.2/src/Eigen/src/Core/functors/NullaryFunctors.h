...
template <typename Scalar> struct linspaced_op_impl<Scalar, /*IsInteger*/ false>
{
    ...
    operator()(IndexType i) const
    {
        if (m_flip)
            ...
            return (i == m_size1) ? m_high
                                  : Scalar(m_low + RealScalar(i) * m_step);
    }
    ...
    {
        // Principle:
        // [low, ..., low] + ( [step, ..., step] * ( [i, ..., i] + [0, ...,
        // size] ) )
        if (m_flip) {
            Packet pi = plset<Packet>(Scalar(i - m_size1));
            Packet res =
                padd(pset1<Packet>(m_high), pmul(pset1<Packet>(m_step), pi));
            ...
            Packet res =
                padd(pset1<Packet>(m_low), pmul(pset1<Packet>(m_step), pi));
            if (EIGEN_PREDICT_TRUE(i !=
                                   m_size1 - unpacket_traits<Packet>::size + 1))
                return res;
            Packet mask =
                pcmp_lt(plset<Packet>(0),
...
template <typename Scalar> struct linspaced_op_impl<Scalar, /*IsInteger*/ true>
{
    EIGEN_DEVICE_FUNC linspaced_op_impl(const Scalar &low, const Scalar &high,
                                        Index num_steps)
        : m_low(low),
          m_multiplier((high - low) / convert_index<Scalar>(
                                          num_steps <= 1 ? 1 : num_steps - 1)),
          m_divisor(convert_index<Scalar>(
                        (high >= low ? num_steps : -num_steps) + (high - low)) /
...
template <typename Scalar> struct linspaced_op
{
    ...
    // This proxy object handles the actual required temporaries and the
    // different implementations (integer vs. floating point).
