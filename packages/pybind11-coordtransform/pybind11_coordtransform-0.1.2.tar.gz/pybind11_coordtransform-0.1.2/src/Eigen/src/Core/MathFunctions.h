...
template <typename Scalar> struct round_using_floor_ceil_impl
{
    ...
    static inline Scalar run(const Scalar &x)
    {
        ...
        // If not enough precision to resolve a decimal at all, return the
        // input. Otherwise, adding 0.5 can trigger an increment by 1.
...
template <typename Scalar> struct random_default_impl<Scalar, false, true>
{
    static inline Scalar run(const Scalar &x, const Scalar &y)
    {
        if (y <= x)
            return x;
        // ScalarU is the unsigned counterpart of Scalar, possibly Scalar
        // itself.
        typedef typename make_unsigned<Scalar>::type ScalarU;
        // ScalarX is the widest of ScalarU and unsigned int.
        // We'll deal only with ScalarX and unsigned int below thus avoiding
        // signed types and arithmetic and signed overflows (which are undefined
        // behavior).
        typedef typename conditional<(ScalarU(-1) > unsigned(-1)), ScalarU,
                                     unsigned>::type ScalarX;
        // The following difference doesn't overflow, provided our integer types
        // are two's complement and have the same number of padding bits in
        // signed and unsigned variants. This is the case in most modern
...
template <>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE long double absdiff(const long double &x,
                                                          const long double &y)
{
    return fabsl(x - y);
}
...
template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double log1p(const double &x)
{
    return ::log1p(x);
}
#endif
...
template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double floor(const double &x)
{
    return ::floor(x);
}
#endif
...
template <typename T> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE T rsqrt(const T &x)
{
...
template <> EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE double expm1(const double &x)
{
    return ::expm1(x);
}
#endif
