                                            ...
                                            true /* vectorizable */>
{
                           ...
                           Index size, OtherScalar c, OtherScalar s)
    {
        ...
        if (SizeAtCompileTime == Dynamic &&
            ...
            conj_helper<OtherPacket, Packet, NumTraits<OtherScalar>::IsComplex,
                        false>
                pcj;
            ...
            if (internal::first_default_aligned(x, size) == alignedStart) {
                ...
                Index peelingEnd = alignedStart + ((size - alignedStart) /
                                                   (Peeling * PacketSize)) *
                                                      (Peeling * PacketSize);
                for (Index i = alignedStart; i < peelingEnd;
                    ...
                    pstoreu(px + PacketSize,
                            padd(pm.pmul(pc, xi1), pcj.pmul(ps, yi1)));
                    pstore(py, psub(pcj.pmul(pc, yi), pm.pmul(ps, xi)));
                    pstore(py + PacketSize,
                           psub(pcj.pmul(pc, yi1), pm.pmul(ps, xi1)));
                ...
                if (alignedEnd != peelingEnd) {
                    ...
                    pstoreu(x + peelingEnd,
                            padd(pm.pmul(pc, xi), pcj.pmul(ps, yi)));
                    pstore(y + peelingEnd,
                           psub(pcj.pmul(pc, yi), pm.pmul(ps, xi)));
                }
            }
                     ...
                     0) // FIXME should be compared to the required alignment
        {
            const OtherPacket pc = pset1<OtherPacket>(c);
            const OtherPacket ps = pset1<OtherPacket>(s);
            conj_helper<OtherPacket, Packet, NumTraits<OtherPacket>::IsComplex,
                        false>
                pcj;
            conj_helper<OtherPacket, Packet, false, false> pm;
