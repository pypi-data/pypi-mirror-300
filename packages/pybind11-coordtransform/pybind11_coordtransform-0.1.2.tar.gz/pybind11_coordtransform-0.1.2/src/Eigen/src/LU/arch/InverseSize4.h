                             ...
                             ResultType>
{
    enum
    {
        ...
        StorageOrdersMatch = (MatrixType::Flags & RowMajorBit) ==
                             (ResultType::Flags & RowMajorBit)
    };
    ...
    static void run(const MatrixType &mat, ResultType &result)
    {
        ...
        // determinant of the input matrix, det = |A||D| + |B||C| -
        // trace(A#*B*D#*C)
        ...
        iD = padd(iD,
                  pmul(vec4f_swizzle2(C, C, 1, 1, 3, 3), vec4f_movehl(AB, AB)));
        iD = psub(pmul(D, vec4f_duplane(dA, 0)), iD);

        // iA = A*|D| - B*D#*C
        iA = pmul(vec4f_swizzle2(B, B, 0, 0, 2, 2), vec4f_movelh(DC, DC));
        iA = padd(iA,
                  pmul(vec4f_swizzle2(B, B, 1, 1, 3, 3), vec4f_movehl(DC, DC)));
                             ...
                             ResultType>
{
    ...
    static void run(const MatrixType &mat, ResultType &result)
    {
        ...
        // Four 2x2 sub-matrices of the input matrix, each is further divided
        // into upper and lower row e.g. A1, upper row of A, A2, lower row of A
        // input = [[A, B],  =  [[[A1, [B1,
        //          [C, D]]        A2], B2]],
        //                       [[C1, [D1,
        //                         C2], D2]]]
        ...
        iB1 = psub(
            iB1, pmul(vec2d_swizzle2(D1, D1, 1), vec2d_swizzle2(AB2, AB1, 2)));
        iB2 = psub(
            iB2, pmul(vec2d_swizzle2(D2, D2, 1), vec2d_swizzle2(AB2, AB1, 2)));
        ...
        iC1 = psub(
            iC1, pmul(vec2d_swizzle2(A1, A1, 1), vec2d_swizzle2(DC2, DC1, 2)));
        iC2 = psub(
            iC2, pmul(vec2d_swizzle2(A2, A2, 1), vec2d_swizzle2(DC2, DC1, 2)));
