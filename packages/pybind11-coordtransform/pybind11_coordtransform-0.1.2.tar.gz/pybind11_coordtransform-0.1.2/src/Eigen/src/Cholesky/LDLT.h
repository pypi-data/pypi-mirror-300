...
class LDLT : public SolverBase<LDLT<_MatrixType, _UpLo>>
{
  public:
    ...
    explicit LDLT(const EigenBase<InputType> &matrix)
        : m_matrix(matrix.rows(), matrix.cols()),
          m_transpositions(matrix.rows()), m_temporary(matrix.rows()),
          m_sign(internal::ZeroSign), m_isInitialized(false)
    {
    ...
    {
        eigen_assert(m_isInitialized && "LDLT is not initialized.");
        return m_sign == internal::PositiveSemiDef ||
    ...
    {
        eigen_assert(m_isInitialized && "LDLT is not initialized.");
        return m_sign == internal::NegativeSemiDef ||
               m_sign == internal::ZeroSign;
    }
     ...
     *
     * More precisely, this method solves \f$ A x = b \f$ using the
     * decomposition \f$ A = P^T L D L^* P \f$ by solving the systems \f$ P^T
     * y_1 = b \f$, \f$ L y_2 = y_1 \f$, \f$ D y_3 = y_2 \f$, \f$ L^* y_4 = y_3
     * \f$ and \f$ P x = y_4 \f$ in succession. If the matrix \f$ A \f$ is
     * singular, then \f$ D \f$ will also be singular (all the other matrices
     * are invertible). In that case, the least-square solution of \f$ D y_3 =
     * y_2 \f$ is computed. This does not mean that this function computes the
     * least-square solution of \f$ A x = b \f$ if \f$ A \f$ is singular.
     *
     ...
     * decompositions, thus enabling generic code such as: \code x =
  ...
  protected:
     ...
     * U. The strict upper part is used during the decomposition, the strict
     * lower part correspond to the coefficients of L (its diagonal is equal to
     * 1 and is not stored), and the diagonal entries correspond to D.
     */
    MatrixType m_matrix;
...
template <> struct ldlt_inplace<Lower>
{
                          ...
                          Workspace &temp, SignMatrix &sign)
    {
        ...
        for (Index k = 0; k < size; ++k) {
            ...
            // omitted if the pivot was smaller than the cutoff value. However,
            // since LDLT is not rank-revealing we should only make sure that we
            // do not introduce INF or NaN values. Remark that LAPACK also uses
            // 0 as the cutoff value.
            RealScalar realAkk = numext::real(mat.coeffRef(k, k));
            bool pivot_is_valid = (abs(realAkk) > RealScalar(0));

            if (k == 0 && !pivot_is_valid) {
                // The entire diagonal is zero, there is nothing more to do
                // except filling the transpositions, and checking whether the
                // matrix is zero.
                sign = ZeroSign;
                for (Index j = 0; j < size; ++j) {
                    transpositions.coeffRef(j) = IndexType(j);
                    ret = ret &&
                          (mat.col(j).tail(size - j - 1).array() == Scalar(0))
                              .all();
                }
                return ret;
            }
                              ...
                              const typename MatrixType::RealScalar &sigma = 1)
    {
        ...
        // Apply the update
        for (Index j = 0; j < size; j++) {
            // Check for termination due to an original decomposition of
            // low-rank
            if (!(isfinite)(alpha))
                break;
...
template <> struct ldlt_inplace<Upper>
{
    ...
    {
        Transpose<MatrixType> matt(mat);
        return ldlt_inplace<Lower>::update(matt, transpositions, tmp,
                                           w.conjugate(), sigma);
    }
};
...
LDLT<MatrixType, _UpLo>::compute(const EigenBase<InputType> &a)
{
    ...
    for (Index col = 0; col < size; ++col) {
        ...
        if (_UpLo == Lower)
            abs_col_sum =
                m_matrix.col(col).tail(size - col).template lpNorm<1>() +
                m_matrix.row(col).head(col).template lpNorm<1>();
        else
            abs_col_sum =
                m_matrix.col(col).head(col).template lpNorm<1>() +
    ...
    const typename LDLT<MatrixType, _UpLo>::RealScalar &sigma)
{
    ...
    if (m_isInitialized) {
        ...
        m_temporary.resize(size);
        m_sign =
            sigma >= 0 ? internal::PositiveSemiDef : internal::NegativeSemiDef;
        m_isInitialized = true;
    }

    internal::ldlt_inplace<UpLo>::update(m_matrix, m_transpositions,
                                         m_temporary, w, sigma);
                                                      ...
                                                      DstType &dst) const
{
    ...
    // NumTraits<RealScalar>::highest()); However, LDLT is not rank revealing,
    // and so adjusting the tolerance wrt to the highest diagonal element is not
    // well justified and leads to numerical issues in some cases. Moreover,
    // Lapack's xSYTRS routines use 0 for the tolerance. Using
