...
template <typename Derived> class SparseMatrixBase : public EigenBase<Derived>
{
  public:
     ...
     * numbers (e.g. int, float or double) then \a RealScalar is just the same
     * as \a Scalar. If \a Scalar is \a std::complex<T> then RealScalar is \a T.
     *
     * \sa class NumTraits
     */
...
#undef EIGEN_DOC_BLOCK_ADDONS_INNER_PANEL_IF
  ...
  public:
    friend std::ostream &operator<<(std::ostream &s, const SparseMatrixBase &m)
    {
        ...
        if (Flags & RowMajorBit) {
            ...
            for (Index row = 0; row < nm.outerSize(); ++row) {
                Index col = 0;
                for (typename internal::evaluator<NestedCleaned>::InnerIterator
                         it(thisEval, row);
                     it; ++it) {
    ...
    /** \returns an expression of P H P^-1 where H is the matrix represented by
     * \c *this */
