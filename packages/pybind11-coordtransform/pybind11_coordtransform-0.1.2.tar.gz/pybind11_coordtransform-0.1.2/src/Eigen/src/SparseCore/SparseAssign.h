...
void assign_sparse_to_sparse(DstXprType &dst, const SrcXprType &src)
{
    ...
    if ((!transpose) && src.isRValue()) {
        ...
        for (Index j = 0; j < outerEvaluationSize; ++j) {
            dst.startVec(j);
            for (typename SrcEvaluatorType::InnerIterator it(srcEvaluator, j);
                 it; ++it) {
                Scalar v = it.value();
...
struct Assignment<DstXprType, SrcXprType, Functor, Sparse2Dense, Weak>
{
    ...
    {
        if (internal::is_same<Functor, internal::assign_op<
        ...
        const Index outerEvaluationSize =
            (internal::evaluator<SrcXprType>::Flags & RowMajorBit) ? src.rows()
                                                                   : src.cols();
        for (Index j = 0; j < outerEvaluationSize; ++j)
            for (typename internal::evaluator<SrcXprType>::InnerIterator i(
                     srcEval, j);
                 i; ++i)
