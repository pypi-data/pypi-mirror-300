      ...
      public:
        ...
        EIGEN_STRONG_INLINE InnerIterator &operator++()
        {
            if (m_lhsIter && m_rhsIter &&
                ...
                ++m_lhsIter;
                ++m_rhsIter;
            } else if (m_lhsIter && (!m_rhsIter ||
                                     (m_lhsIter.index() < m_rhsIter.index()))) {
                m_id = m_lhsIter.index();
                m_value = m_functor(m_lhsIter.value(), Scalar(0));
                ++m_lhsIter;
            } else if (m_rhsIter && (!m_lhsIter ||
                                     (m_lhsIter.index() > m_rhsIter.index()))) {
    ...
    class InnerIterator
    {
      ...
      public:
        ...
        EIGEN_STRONG_INLINE InnerIterator &operator++()
        {
            ...
            if (m_id < m_innerSize) {
                Scalar lhsVal =
                    m_lhsEval.coeff(IsRowMajor ? m_rhsIter.outer() : m_id,
    ...
    class InnerIterator
    {
      ...
      public:
        ...
        EIGEN_STRONG_INLINE InnerIterator &operator++()
        {
            ...
            if (m_id < m_innerSize) {
                Scalar rhsVal =
                    m_rhsEval.coeff(IsRowMajor ? m_lhsIter.outer() : m_id,
              ...
              const SparseMatrixBase<SparseDerived> &b)
{
    return CwiseBinaryOp<
        internal::scalar_sum_op<typename DenseDerived::Scalar,
                                typename SparseDerived::Scalar>,
