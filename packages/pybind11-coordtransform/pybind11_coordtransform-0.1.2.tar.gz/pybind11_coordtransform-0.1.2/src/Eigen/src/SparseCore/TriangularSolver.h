    ...
    static void run(const Lhs &lhs, Rhs &other)
    {
        ...
        for (Index col = 0; col < other.cols(); ++col) {
            for (Index i = 0; i < lhs.cols(); ++i) {
                ...
                if (tmp !=
                    Scalar(0)) // optimization when other is actually sparse
                {
                    LhsIterator it(lhsEval, i);
                    while (it && it.index() < i)
                        ++it;
    ...
    static void run(const Lhs &lhs, Rhs &other)
    {
        ...
        for (Index col = 0; col < other.cols(); ++col) {
            for (Index i = lhs.cols() - 1; i >= 0; --i) {
                ...
                {
                    if (!(Mode & UnitDiag)) {
                        // TODO replace this by a binary search. make sure the
                        // binary search is safe for partially sorted elements
                        LhsIterator it(lhsEval, i);
                        while (it && it.index() != i)
                            ++it;
                        eigen_assert(it && it.index() == i);
