...
class SparseCompressedBase : public SparseMatrixBase<Derived>
{
  ...
  protected:
    ...
    /** \internal return the index of the coeff at (row,col) or just before if
     * it does not exist. This is an analogue of std::lower_bound.
     */
...
template <typename Derived> class SparseCompressedBase<Derived>::InnerIterator
{
  ...
  private:
    // If you get here, then you're not using the right InnerIterator type,
    // e.g.:
...
struct evaluator<SparseCompressedBase<Derived>> : evaluator_base<Derived>
{
    ...
    typedef
        typename DenseCoeffsBase<Derived, ReadOnlyAccessors>::CoeffReturnType
            CoeffReturnType;
