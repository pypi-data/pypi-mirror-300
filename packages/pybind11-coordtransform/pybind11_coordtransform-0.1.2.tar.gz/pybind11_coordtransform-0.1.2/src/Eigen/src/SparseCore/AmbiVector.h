...
{
    m_mode = mode;
    // This is only necessary in sparse mode, but we set these unconditionally
    // to avoid some maybe-uninitialized warnings if (m_mode==IsSparse)
    {
        m_llSize = 0;
        m_llStart = -1;
    }
...
_Scalar &AmbiVector<_Scalar, _StorageIndex>::coeffRef(Index i)
{
    if (m_mode == IsDense)
        ...
        ListEl *EIGEN_RESTRICT llElements =
            reinterpret_cast<ListEl *>(m_buffer);
        ...
        if (m_llSize == 0) {
            ...
            eigen_assert(i >= llElements[m_llCurrent].index &&
                         "you must call restart() before inserting an element "
