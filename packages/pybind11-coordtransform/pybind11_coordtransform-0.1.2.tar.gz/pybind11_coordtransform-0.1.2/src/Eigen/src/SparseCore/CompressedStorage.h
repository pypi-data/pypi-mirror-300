...
template <typename _Scalar, typename _StorageIndex> class CompressedStorage
{
  ...
  public:
    ...
    /** \returns the largest \c k such that for all \c j in [0,k) index[\c
     * j]\<\a key */
                            ...
                            const Scalar &defaultValue = Scalar(0)) const
    {
        ...
        return ((id < end) && (m_indices[id] == key)) ? m_values[id]
                                                      : defaultValue;
    }
                                   ...
                                   const Scalar &defaultValue = Scalar(0))
    {
        ...
        if (id >= m_size || m_indices[id] != key) {
            if (m_allocatedSize < m_size + 1) {
                ...
                internal::scoped_array<StorageIndex> newIndices(
                    m_allocatedSize);

                // copy first chunk
                internal::smart_copy(m_values, m_values + id, newValues.ptr());
                internal::smart_copy(m_indices, m_indices + id,
                                     newIndices.ptr());
    ...
    void moveChunk(Index from, Index to, Index chunkSize)
    {
        ...
        if (to > from && from + chunkSize > to) {
            // move backward
            internal::smart_memmove(m_values + from,
                                    m_values + from + chunkSize, m_values + to);
  ...
  protected:
    inline void reallocate(Index size)
    {
        ...
        if (copySize > 0) {
            internal::smart_copy(m_values, m_values + copySize,
                                 newValues.ptr());
            internal::smart_copy(m_indices, m_indices + copySize,
                                 newIndices.ptr());
        }
        std::swap(m_values, newValues.ptr());
        std::swap(m_indices, newIndices.ptr());
