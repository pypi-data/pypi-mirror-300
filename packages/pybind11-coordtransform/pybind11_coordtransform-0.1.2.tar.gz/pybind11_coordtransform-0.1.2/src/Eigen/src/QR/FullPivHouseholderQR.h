  ...
  public:
    ...
    FullPivHouseholderQR(Index rows, Index cols)
        : m_qr(rows, cols), m_hCoeffs((std::min)(rows, cols)),
          ...
          m_cols_permutation(cols), m_temp(cols), m_isInitialized(false),
          m_usePrescribedThreshold(false)
    {
    }
     ...
     * matrix of which \c *this is the QR decomposition.
     *
     * \param b the right-hand-side of the equation to solve.
     *
     * \returns the exact or least-square solution if the rank is greater or
     * equal to the number of columns of A, and an arbitrary solution otherwise.
     *
    ...
    /** \returns a const reference to the vector of indices representing the
     * rows transpositions */
...
void FullPivHouseholderQR<MatrixType>::computeInPlace()
{
    ...
    for (Index k = 0; k < size; ++k) {
        ...
        // if the corner is negligible, then we have less than full rank, and we
        // can finish early
        if (internal::isMuchSmallerThan(biggest_in_corner, biggest,
                                        m_precision)) {
            m_nonzero_pivots = k;
    ...
    : public ReturnByValue<FullPivHouseholderQRMatrixQReturnType<MatrixType>>
{
  public:
    typedef typename FullPivHouseholderQR<MatrixType>::IntDiagSizeVectorType
        IntDiagSizeVectorType;
    typedef typename internal::plain_diag_type<MatrixType>::type HCoeffsType;
    typedef Matrix<typename MatrixType::Scalar, 1,
                   MatrixType::RowsAtCompileTime, RowMajor, 1,
