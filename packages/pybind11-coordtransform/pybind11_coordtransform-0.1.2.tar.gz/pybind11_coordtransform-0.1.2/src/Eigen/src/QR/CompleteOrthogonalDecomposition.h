    ...
    : public SolverBase<CompleteOrthogonalDecomposition<_MatrixType>>
{
  ...
  public:
    ...
    /** \returns the rank of the matrix of which *this is the complete
    ...
    /** \returns true if the matrix of which *this is the decomposition
     ...
     * greater than
     *  \f$ \vert pivot \vert \leqslant threshold \times \vert maxpivot \vert
     * \f$ where maxpivot is the biggest pivot.
     *
  ...
  protected:
    ...
    void _check_solve_assertion(const Rhs &b) const
    {
        ...
        eigen_assert((Transpose_ ? derived().cols() : derived().rows()) ==
                         b.rows() &&
                     "CompleteOrthogonalDecomposition::solve(): invalid number "
...
void CompleteOrthogonalDecomposition<MatrixType>::computeInPlace()
{
    ...
    if (rank < cols) {
        ...
        for (Index k = rank - 1; k >= 0; --k) {
            if (k != rank - 1) {
                // Given the API for Householder reflectors, it is more
                // convenient if we swap the leading parts of columns k and r-1
                // (zero-based) to form the matrix X_k = [X(0:k, k), X(0:k,
                // r:n)]
                m_cpqr.m_qr.col(k).head(k + 1).swap(
                    m_cpqr.m_qr.col(rank - 1).head(k + 1));
            }
            // Construct Householder reflector Z(k) to zero out the last row of
            // X_k, i.e. choose Z(k) such that [X(k, k), X(k, r:n)] * Z(k) =
            // [beta, 0, .., 0].
            RealScalar beta;
            m_cpqr.m_qr.row(k)
                .tail(cols - rank + 1)
