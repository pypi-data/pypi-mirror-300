...
class ColPivHouseholderQR : public SolverBase<ColPivHouseholderQR<_MatrixType>>
{
  ...
  public:
     ...
     * perform decompositions via ColPivHouseholderQR::compute(const
     * MatrixType&).
     */
    ColPivHouseholderQR()
    ...
    /** \returns a reference to the matrix where the Householder QR
     ...
     * values. Only the upper triangular part should be referenced. To get it,
     * use \code matrixR().template triangularView<Upper>() \endcode For
     * rank-deficient matrices, use \code matrixR().topLeftCorner(rank(),
     * rank()).template triangularView<Upper>() \endcode
     */
    ...
    /** \returns the inverse of the matrix of which *this is the QR
     * decomposition.
     *
...
void ColPivHouseholderQR<MatrixType>::computeInPlace()
{
    ...
    for (Index k = 0; k < size; ++k) {
        ...
        // Track the number of meaningful pivots but do not stop the
        // decomposition to make sure that the initial matrix is properly
        // reproduced. See bug 941.
        ...
        for (Index j = k + 1; j < cols; ++j) {
            // The following implements the stable norm downgrade step discussed
            // in http://www.netlib.org/lapack/lawnspdf/lawn176.pdf and used in
            // LAPACK routines xGEQPF and xGEQP3. See lines 278-297 in
            ...
            if (m_colNormsUpdated.coeffRef(j) != RealScalar(0)) {
                ...
                RealScalar temp2 = temp * numext::abs2<RealScalar>(
                                              m_colNormsUpdated.coeffRef(j) /
                                              m_colNormsDirect.coeffRef(j));
                if (temp2 <= norm_downdate_threshold) {
                    // The updated norm has become too inaccurate so re-compute
