...
class CholmodBase : public SparseSolverBase<Derived>
{
  ...
  public:
    ...
    /** Returns a reference to the Cholmod's configuration structure to get a
     * full control over the performed operations. See the Cholmod user guide
     * for details. */
    ...
    void _solve_impl(const MatrixBase<Rhs> &b, MatrixBase<Dest> &dest) const
    {
        eigen_assert(
            m_factorizationIsOk &&
            "The decomposition is not in a valid state for solving, you must "
        ...
        Ref<const Matrix<typename Rhs::Scalar, Dynamic, Dynamic, ColMajor>>
            b_ref(b.derived());
        ...
        dest =
            Matrix<Scalar, Dest::RowsAtCompileTime, Dest::ColsAtCompileTime>::
                Map(reinterpret_cast<Scalar *>(x_cd->x), b.rows(), b.cols());
                     ...
                     SparseMatrixBase<DestDerived> &dest) const
    {
        ...
        // TODO optimize this copy by swapping when possible (be careful with
        // alignment, etc.) NOTE cholmod_spsolve in fact just calls the dense
        // solver for blocks of 4 columns at a time (similar to Eigen's sparse
        // solver)
    ...
    Scalar logDeterminant() const
    {
        ...
        if (m_cholmodFactor->is_super) {
            // Supernodal factorization stored as a packed list of dense
            // column-major blocs, as described by the following structure:

            // super[k] == index of the first column of the j-th super node
            StorageIndex *super =
                static_cast<StorageIndex *>(m_cholmodFactor->super);
