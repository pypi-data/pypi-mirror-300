...
class WdaUI(object):
    ...
    def deactivate(self, duration):
        self.client.deactivate(duration)
    ...
    def click_on_partial_text(self, partial_text, class_name=None, index=0):
        self.session(partial_text=partial_text, class_name=class_name, index=index).tap()
        time.sleep(sleep_time)
    ...
    def long_click_view(self, class_name, text=None, index=0, duration=3):
        self.session(class_name=class_name, text=text, index=index).tap_hold(duration=duration)
        time.sleep(sleep_time)
    ...
    def screenshot_view(self, filename, class_name=None, text=None, partial_text=None, index=0):
        self.session(
            class_name=class_name, text=text, partial_text=partial_text, index=index
        ).screenshot(filename)
        time.sleep(sleep_time)
    ...
    def scroll_then_click(self, text=None, partial_text=None, index=0, direction="visible"):
        if direction == "visible":
            if not self.elem(class_name="WebView").exists:
                scrollcount = 0
                scrollmaxcount = 15
                eletext = self.session(text=text, partial_text=partial_text, index=index)
    ...
    def scroll_only(self, text=None, text_contains=None, index=0, direction="visible"):
        if direction == "visible":
            if not self.elem(class_name="WebView").exists:
                scrollcount = 0
                scrollmaxcount = 15
                eletext = self.session(text=text, partial_text=text_contains, index=index)
                while not eletext.displayed and scrollcount < scrollmaxcount:
                    scrollcount += 1
                    self.scroll_height()
            else:
            ...
            self.session(text=text, partial_text=text_contains, index=index).scroll(direction)
    ...
    def click_if_exists(self, class_name=None, text=None, index=0, timeout=10):
        self.elem(class_name=class_name, text=text, index=index).click_if_exists(timeout=timeout)
    ...
    def click_on_righttop(self, view_elem):
        if view_elem is None:
            raise NoSuchElementError()
        x = view_elem.bounds.left + (view_elem.bounds.right - view_elem.bounds.left) * 7 / 8
    ...
    def parent_view(self, child_elem, parent_class_name):
        ...
        for index in range(count):
            ...
            if parent_elem.subelems(class_name=child_class_name, text=child_text).exists:
                return parent_elem
...
if __name__ == "__main__":
    c = WdaUI("http://localhost:8100", "com.tencent.qy.xin")
    c.session(class_name="Button", partial_text="讯录").click(timeout=1.0)
