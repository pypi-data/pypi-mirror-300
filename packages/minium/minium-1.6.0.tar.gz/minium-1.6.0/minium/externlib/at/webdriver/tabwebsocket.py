...
class TabWebSocket(object):
    ...
    def _on_open(self, ws=None):
        ...
        """
        logger.debug('%s, %s, %s, opened', ws, self.url, self._thread.ident)
        self._status = "open"
    ...
    def _on_message(self, ws=None, message=None):
        ...
        :type ws: websocket.WebSocketApp
        """
        self.logger.debug("RECV > %s: %.1024s", self._thread.ident, message)
    ...
    def _on_error(self, ws=None, error=None, *args):
        logger.error("%s, %s, %s, error %s", ws, self.url, self._thread.ident, error)

    def _on_close(self, ws=None, *args):
    ...
    def _send_request(self, params):
        ...
        self.logger.debug('SEND > %s', serialize)
        self.ensure_opened()
