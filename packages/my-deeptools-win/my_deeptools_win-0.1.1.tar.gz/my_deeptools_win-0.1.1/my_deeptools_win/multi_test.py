...
# thread = 10
import time
def convert_bam_to_bigbed(bam_file_path, output_file_path,thread,binsize):
    ...
    for index_num,chr_name in enumerate(chr_length_dt):
        ...
        for i in range(thread):
            ...
            bam_file_path,[chr_name, int(i * chr_bins / thread) * binsize, int((i + 1) * chr_bins / thread) * binsize],binsize)))
...
def main(bam_file_path,
    ...
    with console.status("[bold green]Working on tasks...") as status:
        convert_bam_to_bigbed(bam_file_path,tmp_path,thread=threads,binsize = binsize)
    log.info("start convert bigbed to bigwig")
    covert_bigbed_to_bigwig(tmp_path, bw_filw_path)
    import os
    os.remove(tmp_path)
    log.info("end")
