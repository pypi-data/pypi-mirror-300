...
def is_official_image() -> bool:
    """Return True if Vioneta Agro is running in an official container."""
...
async def async_get_system_info(hass: HomeAssistant) -> dict[str, Any]:
    ...
    if info_object["docker"]:
        if info_object["user"] == "root" and is_official_image():
            info_object["installation_type"] = "Vioneta Agro Container"
        else:
            info_object["installation_type"] = "Unsupported Third Party Container"

    elif is_virtual_env():
        info_object["installation_type"] = "Vioneta Agro Core"

    # Enrich with Supervisor information
    if is_hassio:
        if not (info := hassio.get_info(hass)):
            _LOGGER.warning("No Vioneta Agro Supervisor info available")
            info = {}
        ...
        if info.get("hassos") is not None:
            info_object["installation_type"] = "Vioneta Agro OS"
        else:
            info_object["installation_type"] = "Vioneta Agro Supervised"
