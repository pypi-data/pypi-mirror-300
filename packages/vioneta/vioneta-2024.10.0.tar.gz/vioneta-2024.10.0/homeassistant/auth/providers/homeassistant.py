        ...
        for user in data["users"]:
            ...
            if self.normalize_username(username, force_normalize=True) != username:
                logging.getLogger(__name__).warning(
                    (
                        "Vioneta Agro auth provider is running in legacy mode "
...
class HassAuthProvider(AuthProvider):
    """Auth provider based on a local storage of users in Vioneta Agro config dir."""

    DEFAULT_TITLE = "Vioneta Agro Local"

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize an Vioneta Agro auth provider."""
