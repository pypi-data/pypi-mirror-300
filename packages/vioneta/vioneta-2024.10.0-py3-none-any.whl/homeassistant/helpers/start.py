...
def _async_at_core_state(
    ...
    """Execute a job at_start_cb when Vioneta Agro has the wanted state.

    The job is executed immediately if Vioneta Agro is in the wanted state.
    ...
    def _matched_event(event: Event) -> None:
        """Call the callback when Vioneta Agro started."""
        hass.async_run_hass_job(at_start_job, hass)
        nonlocal unsub
        unsub = None
...
def async_at_start(
    ...
    """Execute a job at_start_cb when Vioneta Agro is starting.

    The job is executed immediately if Vioneta Agro is already starting or started.
...
def async_at_started(
    ...
    """Execute a job at_start_cb when Vioneta Agro has started.

    The job is executed immediately if Vioneta Agro is already started.
