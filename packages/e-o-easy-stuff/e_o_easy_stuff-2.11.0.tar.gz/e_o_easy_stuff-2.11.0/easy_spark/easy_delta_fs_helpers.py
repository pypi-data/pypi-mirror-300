...
class EasyDeltaFSHelpers:
    ...
    @staticmethod
    def get_fs_path_from_status(status):
        return status.getPath()

    @staticmethod
    def get_file_name_from_status(status) -> str:
        return status.getPath().getName()

    @staticmethod
    def get_file_full_path_from_status(status) -> str:
        return status.getPath().toString

    @staticmethod
    def get_file_modification_time_from_status(status):
        return status.getModificationTime()

    @staticmethod
    def get_readable_file_modification_time_from_status(status) -> datetime:
        return datetime.fromtimestamp(EasyDeltaFSHelpers.get_file_modification_time_from_status(status) / 1000.0)

    @staticmethod
    def is_directory_from_status(status) -> bool:
    ...
    def list_path_details_from_fs_path(self, fs_path) -> enumerate[EasyFileDetails]:
        statuses = self.list_statuses_from_fs_path(fs_path)
        for status in statuses:
            yield self.get_file_details_from_status(status)

    def list_path_details_from_path(self, path: str, is_relative: bool) -> enumerate[EasyFileDetails]:
        fs_path = self.get_fs_path(path, is_relative)
