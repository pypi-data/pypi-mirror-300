...
class TransferFunctionCalculator:
    ...
    def _load_data(self) -> pd.DataFrame:
        ...
        """
        tmp = pd.read_csv(self.file_path, header=None, nrows=1, skiprows=0)
        header = tmp.loc[tmp.index[0]]
        df = pd.read_csv(self.file_path, header=None, skiprows=1)
        df.columns = header
        return df
    ...
    def process_data(self, reference_key: str, target_key: str) -> pd.DataFrame:
        ...
        """
        key_freq: str = self.key_freq

        # データ型の確認と変換
        self.df[key_freq] = pd.to_numeric(self.df[key_freq], errors="coerce")
        ...
        # NaNを含む行を削除
        self.df = self.df.dropna(subset=[key_freq, reference_key, target_key])

        # グループ化と中央値の計算
        grouped = self.df.groupby(key_freq)
    ...
    def plot_cospectra(
        ...
        """
        data1 = self.df[self.df[key1] > 0].groupby(self.key_freq)[key1].median()
        data2 = self.df[self.df[key2] > 0].groupby(self.key_freq)[key2].median()
