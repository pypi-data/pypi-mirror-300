...
def patch_setup_ssl_cert():
	...
	def A(setup_community_ssl_cert):
		...
		match get_licensed_environment():
			case DevLocalstackEnvironment():LOG.debug('Developer credentials detected, falling back to fetching the community certificate');B();return
		LOG.debug('Attempting to download pro TLS certificate file');G=get_auth_headers();C=requests.Session();D=get_proxies()
		if D:C.proxies.update(D)
		def H():
			A=C.get(PLATFORM_CERTIFICATE_ENDPOINT_URL,timeout=PLATFORM_CERTIFICATE_ENDPOINT_TIMEOUT,headers=G)
			if not A.ok:raise Exception(f"Failed to download certificate, response code: {A.status_code}")
			return A.json()['url']
		try:I=retry(H,**PLATFORM_RETRY_CONFIG);retry(download,url=I,path=A,timeout=CERTIFICATE_DOWNLOAD_TIMEOUT,quiet=True,**CERTIFICATE_DOWNLOAD_RETRY_CONFIG);LOG.debug('TLS certificate downloaded successfully to %s',A)
