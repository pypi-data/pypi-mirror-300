...
from ..entities.handlers import orjson_exists, cache_exists, fetch_cache
...
class RequestHandler:
    ...
    def handler(
        ...
        if all(
            ...
            if cache_exists(email=self.email, KEY=self.__key__):
                SID, DEVICEID = fetch_cache(email=self.email, KEY=self.__key__)
                self.sid = SID
                self.device = DEVICEID
            else:
                self.bot.run(self.email, self.password, use_cache=False)
                return self.handler(method, url, data, content_type, is_login_required)
        ...
        response = self.handle_response(status_code=status_code, response=content, url=url)
    ...
    def fetch_signature(
        ...
        headers.update({
            ...
            "NDC-MESSAGE-SIGNATURE": (
                self.generate.NdcMessageSignature(data, self.sid, self.device)
            )
        })
        return headers, data
