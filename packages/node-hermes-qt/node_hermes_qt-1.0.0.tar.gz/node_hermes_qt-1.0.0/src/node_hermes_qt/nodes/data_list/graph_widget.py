...
# 15 distinct colors
PLOT_COLORS = [[0, 0, 0], [230, 25, 75], [60, 180, 75], [255, 225, 25], [0, 130, 200], [245, 130, 48], [145, 30, 180],
...
class TimeseriesTrace:
    ...
    def update_data(self):
        ...
        self.plot_trace.setData(self.datapoint.timestamps-self.start_time, self.datapoint.series)
...
class TimeseriesGraphDisplay(QtWidgets.QWidget):
    ...
    def add_datapoint(self, data: DataPointTracker):
        config = PlotTraceConfig(
            line=PlotLineConfig(color=PLOT_COLORS[len(self.active_series)]),
            marker=PlotMarkerConfig(color=PLOT_COLORS[len(self.active_series)]),
        )
