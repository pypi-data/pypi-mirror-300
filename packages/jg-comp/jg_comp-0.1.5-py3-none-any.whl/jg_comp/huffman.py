...
class HuffmanEncoder:
    ...
    def huffman_encode(self, data: List[int]) -> Tuple[bytes, Dict[int, str]]:
        if not data:
            return b'', {}
        ...
        huffman_dict = {element[0]: element[1] for element in heap[0][1:]} if heap else {}
    ...
    def huffman_decode(self, encoded_data: bytes, huffman_dict: Dict[int, str]) -> List[int]:
        if not encoded_data:
            raise ValueError("Encoded data is empty, cannot decode.")
