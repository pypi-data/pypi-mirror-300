...
class GkeStatefulSet(pulumi.ComponentResource):
    ...
    def __init__(self, resource_name: str,
                 ...
                 statefuleset_spec_kwargs: dict = None,
        ...
        super().__init__('sane:gke:SetefulSet', resource_name, None, opts)
        ...
        # PVC
        if statefuleset_spec_kwargs is None:
            statefuleset_spec_kwargs = {}

        if app_container_pvc_mounts is None:
            app_container_pvc_mounts = {}
        for _name, _data in app_container_pvc_mounts.items():
            statefuleset_spec_kwargs.setdefault("volume_claim_templates", []).append(
        ...
        self.statefulset = kubernetes.apps.v1.StatefulSet(
            f"{resource_name}_statefulset",
            metadata=kubernetes.meta.v1.ObjectMetaArgs(
                name=resource_name,
                namespace=namespace,
                labels={
