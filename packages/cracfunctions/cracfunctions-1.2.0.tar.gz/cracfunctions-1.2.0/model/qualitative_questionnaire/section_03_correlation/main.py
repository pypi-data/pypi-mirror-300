...
def main(
    ...
    univ_list_filt = sorted(
        (
            univ_report.loc[
                (
                    (univ_report.soglia_ar != "Unpredictive")
                    & (univ_report.soglia_iv != "Unpredictive")
                ),
                "indicator_name",
            ]
            .drop_duplicates()
    ...
    univ_list_filt_woe = [i + "_woe" for i in univ_list_filt]
    correlation_matrix_pearson, correlation_matrix_spearman = main_corr(
        perim=perim_processed, var_list=univ_list_filt_woe
    )
