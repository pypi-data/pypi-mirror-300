                        ...
                        int version)
{
    ...
    millisecond = convert_BCDs(&data[offset + 3], 3);
    ...
    */
    ...
    // 累計成交數量
    offset = 25;
    quote.volume_acc = convert_BCDs(&data[offset], 4);
    ...
    if (flag & 0x80) {
        ...
        // 成交價
        quote.close_price = (double)convert_BCDs(&data[offset], 5) / 10000;
        offset += 5;

        // 成交量
        quote.close_volume = convert_BCDs(&data[offset], vol_len);
        offset += vol_len;
    ...
    */
    ...
    for(int i=0; i<quote.number_best_bid; i++) {
        quote.bid_price[i] = (double) convert_BCDs(&data[offset], 5) / 10000;
        offset += 5;
        quote.bid_volume[i] = convert_BCDs(&data[offset], vol_len);
        offset += vol_len;
    ...
    for(int i=0; i<quote.number_best_ask; i++) {
        quote.ask_price[i] = (double) convert_BCDs(&data[offset], 5) / 10000;
        offset += 5;
        quote.ask_volume[i] = convert_BCDs(&data[offset], vol_len);
        offset += vol_len;
                          ...
                          const struct timeval *tv)
{
    ...
    while(len > 0) {
        ...
        // 1, 2: Length
        offset = 1;
        data_len = convert_BCDs(&data[offset], 2);
        if (data_len == 0)
            break;
        ...
        // 6: Serial number
        offset = 6;
        serial_number = convert_BCDs(&data[offset], 4);
