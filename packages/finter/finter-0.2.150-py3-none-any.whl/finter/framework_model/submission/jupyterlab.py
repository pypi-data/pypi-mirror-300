...
UNIVERSE_OPTIONS = ["kr_stock", "us_stock", "us_etf", "vn_stock", "id_stock"]
...
class JupyterLabSubmission(NotebookExtractor):
    ...
    def submit(self, model_name=None, model_universe=None, docker=None):
        self.reinitialize(
            model_name=model_name, model_universe=model_universe, docker=docker
        )
        with self.status_output:
            try:
    ...
    def reinitialize(
        ...
        self.__init__(
            ...
            docker if docker is not None else self.docker_checkbox.value,
        )
        self.status_output.clear_output()
