...
def steam_analyze_lhc_event(
    ...
    Parameters:
    - path_settings_file (str): Absolute path to the settings file (e.g., `settings.username.yaml`).
    - input_parameter_dictionary (dict): Dictionary specifying the parameters needed for the function call.
      The parameters are validated using a Pydantic data model. The required keys in the dictionary are:
      ...
      - metrics_to_calculate (Optional[List[str]]): List of error metrics to calculate for all signals specified in the
      configuration file. Defaults to an empty list if not provided.
      - timeout_s (int): Timeout in seconds after which a frozen simulation will be aborted.
      - flag_run_software (bool): Flag indicating whether to execute the simulation software or only create simulation
    ...
    try:
        ...
        if circuit_type == "RCD_AND_RCO":
            ...
            _analyze_circuit_event(
                ...
                input_parameter_dictionary=validated_params.dict(),
                circuit_type="RCD",
            ...
            # TODO: remove these double logic from AnalysisSteam and put it into the above if-statement just like for
            #  RCD-RCO events so that the code is more consistent.
            _analyze_circuit_event(
                path_settings_file=path_settings_file,
                input_parameter_dictionary=validated_params.dict(),
...
def _analyze_circuit_event(
    ...
    This function is intended for internal use and is called by `steam_analyze_lhc_event`.

    Parameters:
    - path_settings_file (str): Absolute path to the settings file (e.g., `settings.username.yaml`).
    - input_parameter_dictionary (dict): Dictionary containing the parameters needed for the function call.
      This dictionary is validated with Pydantic within the `steam_analyze_lhc_event` function.
    ...
    aSTEAM.data_analysis.AnalysisStepDefinition = {
        ...
        "CalculateMetrics": CalculateMetrics(type="CalculateMetrics",
                                             ...
                                             metrics_to_calculate=input_parameter_dictionary['metrics_to_calculate'],
                                             variables_to_analyze=variables_to_analyze,
                                             metrics_output_filepath = os.path.join(local_software_folder,circuit_type,
                                                                                    str(file_counter),
                                                                                    "output_metrics.yaml")
                                             ),
    }
    ...
    if len(input_parameter_dictionary['metrics_to_calculate']) > 0:
            AnalysisStepSequence.extend(["RunViewer","CalculateMetrics"])
