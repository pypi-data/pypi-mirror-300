...
def  read_file_from_gcs_extended(storage_client:GCSClient, bucket_name:str, file_name:str, file_extension:DataSourceType=None, pipelinemon:Pipelinemon=None, logger=None, print_out=False):
    """Helper function to read a JSON or CSV file from Google Cloud Storage with optional Pipelinemon monitoring."""
    try:
        # Determine the file extension
        base_file_name, ext = os.path.splitext(file_name) ## ext is the file extension with the dot (.) included
        ext = ext.lower()
        if not ext:
            if file_extension:
                ...
                raise ValueError(f"File '{file_name}' has no extension and no file_extension parameter provided.")
        else:
            if file_extension:
                if ext != file_extension.value:
                    raise ValueError(f"File extension '{ext}' does not match the expected extension '{file_extension.value}'")
        ...
        if not blob.exists():
            ...
            if pipelinemon:
                pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED,
                                               subject=file_name,
                                               description=f"File not found in GCS: {file_name} in bucket {bucket_name}"))
            return None
        ...
        if blob.size == 0:
            ...
            if pipelinemon:
                pipelinemon.add_log(ContextLog(level=LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED,
                                               subject=f"blob {file_name} in {bucket_name}",
                                               description="blob.size == 0, meaning empty file"))
            return None
        ...
        if ext == ".json":
            try:
                ...
                msg = f"Error decoding JSON from GCS: {file_name} in bucket {bucket_name}: {e}"
                log_error(msg=msg, logger=logger, print_out=print_out)
                if pipelinemon:
                    pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED,
                                                   subject=file_name, description=msg))
                return None
        elif ext == ".csv":
            try:
                ...
                msg = f"Error reading CSV from GCS: {file_name} in bucket {bucket_name}: {e}"
                log_error(msg=msg, logger=logger, print_out=print_out)
                if pipelinemon:
                    pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED,
                                                   subject=file_name, description=msg))
...
def write_file_to_gcs_extended(storage_client: GCSClient,
                               data: dict | list | str, bucket_name: str, file_name: str,
                                duplication_handling_enum: DuplicationHandling,
                                duplication_match_condition_type_enum: MatchConditionType,
    ...
    try:
        ...
        if ext.lower() == '.json':
            ...
            if isinstance(data, (list, dict)):
                ...
                data_str = data  # Assuming data is already a CSV-formatted string
        ...
        error_during_operation = f"Error occurred while writing file to GCS path: {saved_to_path} ; Error details: {type(e).__name__} - {str(e)}"
...
def query_sql_bigquery_table(project_id: str,
                             query: str,
                             bigquery_client: Optional[bigquery.Client] = None,
                             pipelinemon: Optional[Pipelinemon] = None,
...
def query_existing_dates_from_timeseries_bigquery_table(
    project_id: str,
    data_table_full_path: str,
    data: Union[Dict[str, Any], List[Dict[str, Any]]],
    date_column: str,
