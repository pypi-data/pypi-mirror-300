...
def  read_json_from_gcs_extended(storage_client:GCSClient, bucket_name:str, file_name:str, pipelinemon:Pipelinemon=None,  logger=None, print_out=False):
    ...
    try:
        ...
        if blob.size == 0:
            ...
            logger.warning(msg)
            if pipelinemon:
                pipelinemon.add_log(ContextLog(level=LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED, subject=f"blob {file_name} in {bucket_name}", description="blob.size == 0, meaning empty file"))
            return None
        if pipelinemon:
            pipelinemon.add_log(ContextLog(level=LogLevel.INFO_READ_FILE_FROM_CLOUD_STORAGE_COMPLETE, subject=f"blob {file_name} in {bucket_name}", description="File read from GCS"))
        return data
    except NotFound:
        log_warning(msg=f"Warning: The file {file_name} was not found in the bucket {bucket_name}.", logger=logger, print_out=print_out)
        if pipelinemon:
            pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED, subject=file_name, description=f"File not found in GCS: {file_name} in bucket {bucket_name}"))
        return None
    except json.JSONDecodeError:
        msg=f"Error decoding JSON from GCS: {file_name} in bucket {bucket_name}"
        log_error(msg=msg, logger=logger, print_out=print_out)
        if pipelinemon:
            pipelinemon.add_log(ContextLog(LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED, subject=file_name, description=msg))
        return None
    except Exception as e:
...
def write_file_to_gcs_extended(storage_client: GCSClient, data: dict | list | str, bucket_name: str, file_name: str,
