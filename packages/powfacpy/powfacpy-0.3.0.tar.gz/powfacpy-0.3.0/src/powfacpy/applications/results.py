...
class Results(ApplicationBase):
    ...
    def _set_comres_settings_for_csv_export(
        ...
        comres.iopt_locn = (
            2  # column header includes path relative to network data folder
        )
        ...
        with open(file_path) as read_file, open(file_path + ".temp", "w") as write_file:
            ...
            for col, path in enumerate(full_paths):
                ...
                if col > 0:
                    ...
                    path = self._format_path_inside_network_data_folder(path)
        ...
        if self.multi_index_labels:
            ...
            for n, col in enumerate(df.columns[1:], start=1):
                ...
                if self.pf_objects_in_labels:
                    obj = parent_folder.GetContents(col[0])[0]
                else:
                    path = col[num_header_rows - 2]
                    path = path[: path.index(".")]
                    obj = self._format_path_inside_network_data_folder(path)
                headers[n] = (obj, var)
            ...
            for n, col in enumerate(df.columns[1:], start=1):
                ...
                path = col[num_header_rows - 2]
                path = path[: path.index(".")]
                obj_path = self._format_path_inside_network_data_folder(path)
                headers[n] = obj_path + "\\" + var
            df.columns = headers

    def _format_path_of_obj_inside_active_project(
        self, path: str, network_data_folder_path: str
    ) -> str:
        """Format the path inside active project folder.

        Args:
             path (str): path inside network data folder (without class names)
            network_data_folder_path (str): without class names

        Returns:
            str: path truncated until 'self.truncate_paths_until'
        """
        truncate = f"{network_data_folder_path}\\{self.truncate_paths_until}"
        path = PFStringManipulation.truncate_until(path, truncate)
        return path
