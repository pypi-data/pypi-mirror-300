...
import powfacpy.applications.plots
...
The column headers are the path of the objects relative to the network data folder (by default) together with the result variable name. You can change the relative path by setting the attribute `pfres.truncate_paths_until` (relative to network data folder):  

```{python}
pfres.multi_index_labels = True
pfres.pf_objects_in_labels = False
pfres.truncate_paths_until = "Grid\\"
df_sim_res = pfres.export_to_pandas()
df_sim_res.head(3)
```
