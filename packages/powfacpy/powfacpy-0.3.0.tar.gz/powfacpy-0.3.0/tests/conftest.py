    ...
    if settings["PowerFactory username"]:
        if settings["PowerFactory command line arguments for GetApplication"]:
            return powerfactory.GetApplicationExt(
                ...
                settings[
                    "PowerFactory command line arguments for GetApplication"
                ]
            )
        else:
            return powerfactory.GetApplicationExt(
...
def create_copy_of_test_project(
    ...
    project_name_in_powfacpy_folder: str
):
    user = act_prj.app.GetCurrentUser()
    powfacpy_folder_path = settings[
        "path to powfacpy folder in PowerFactory database"
    ]
    ...
    folder_of_project_for_testing = act_prj.get_single_obj(
        powfacpy_folder_path,
        parent_folder = user, 
        include_subfolders=False)   
    project_copy = act_prj.copy_single_obj(project_for_testing,
