...
class FileParseResult(object):
    ...
    def __init__(self, response: Dict, client: HttpClient, lazy_load_content=False, document: Dict = None):
    ...
    def __convert_document_json(self, doc_json: Dict) -> Union[RawDocument, CsvDocument, DocxDocument, XlsxDocument, PdfDocument]:        
    ...
    def get_markdown(
        ...
        response = self.client.http_post(
            ...
            data={"knownEntities": utf_compatible_filtered_entities, "text": rawtext, "generatorConfig": generator_config, "generatorDefault": generator_default},
        )
    ...
    def get_tables(self) -> List[Table]:
        ...
        if self.file.fileType not in (FileTypeEnum.csv, FileTypeEnum.xlsx,FileTypeEnum.pdf,FileTypeEnum.png,FileTypeEnum.tif,FileTypeEnum.jpg):
            return []
    ...
    def get_chunks(
        ...
        for chunk in split_markdown(text, max_chars):
            ...
            is_sensitive = self.is_sensitive(
                sensitive_entity_types, start, end  # type: ignore
            )
