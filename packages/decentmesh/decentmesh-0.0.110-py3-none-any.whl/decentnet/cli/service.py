...
async def __generate_keys():
    async def handle_async(session):
        ...
        if owned_key is None:
            ...
            await generate_keys(async_mode=True)
    ...
    def handle_sync(session):
        ...
        if owned_key is None:
            print("Generating first keys for communication")
            generate_keys(async_mode=False)
...
async def generate_keys(async_mode):
    """Generate keys based on the async or sync mode."""
    if async_mode:
        await generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=True)
        await generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=True)
        await generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=True)
        await generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=False)
    else:
        generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=True)
        generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=True)
        generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=True)
        generate_impl(private_key_file=None, public_key_file=None, description="First Key", sign=False)
