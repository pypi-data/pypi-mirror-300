...
from neomodantic.exceptions import DeflateError, InflateError
...
class Property:
    ...
    :param label: Optional, used by ``django_neodantic``.
    :type label: :class:`str`
    :param help_text: Optional, used by ``django_neodantic``.
    ...
    def __init__(self, base_property=None, **kwargs):
        ...
        if base_property is not None:
            if not isinstance(base_property, Property):
                raise TypeError("Expecting neomodantic Property")
...
class AliasProperty(Property):
    ...
    #     return AliasProperty(self.target)
