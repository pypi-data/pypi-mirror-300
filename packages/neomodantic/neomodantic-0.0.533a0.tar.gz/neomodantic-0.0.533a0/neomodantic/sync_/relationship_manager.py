...
from neomodantic.exceptions import NotConnected, RelationshipClassRedefined
from neomodantic.sync_.core import db
from neomodantic.sync_.match import NodeSet, Traversal, _rel_helper, _rel_merge_helper
...
RelType = TypeVar("RelType", bound=StructuredRel)

class RelationshipManager(BaseModel, Generic[RelType, TargetNode]):
    """
    Base class for all relationships managed through neomodantic.
    ...
    @check_source
    def connect(self, node: TargetNode, properties: Optional[RelType] | dict = None):
        """
        Connect a node
    ...
    @check_source
    def replace(self, node: TargetNode, properties: Optional[RelType] | dict = None):
        """
    ...
    @check_source
    def relationship(self, node: TargetNode) -> Optional[RelType]:
        """
    ...
    @check_source
    def all_relationships(self, node: TargetNode) -> list[RelType]:
        """
    ...
    @check_source
    def reconnect(self, old_node: TargetNode, new_node: TargetNode):
        """
    ...
    @check_source
    def disconnect(self, node: TargetNode):
        """
        Disconnect a node
    ...
    def get(self, **kwargs) -> TargetNode:
        """
    ...
    def get_or_none(self, **kwargs) -> Optional[TargetNode]:
        """
    ...
    def filter(self, *args, **kwargs) -> NodeSet:
        """
...
class RelationshipDefinition(Generic[RelType, TargetNode]):
    def __new__(cls, *args, **kwargs) -> RelationshipManager[RelType, TargetNode]:
...
class ZeroOrMore(RelationshipManager):
    ...
    description: str = "zero or more relationships"


class RelationshipTo(RelationshipDefinition, Generic[RelType, TargetNode]):
    def __new__(cls, *args, **kwargs) -> RelationshipManager[RelType, TargetNode]:
        return super().__new__(cls)
...
class RelationshipFrom(RelationshipDefinition, Generic[RelType, TargetNode]):
    def __new__(cls, *args, **kwargs) -> RelationshipManager[RelType, TargetNode]:
        return super().__new__(cls)
...
class Relationship(RelationshipDefinition, Generic[RelType, TargetNode]):
    def __new__(cls, *args, **kwargs) -> RelationshipManager[RelType, TargetNode]:
        return super().__new__(cls)
