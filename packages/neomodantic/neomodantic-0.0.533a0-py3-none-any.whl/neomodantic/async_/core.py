...
class AsyncDatabase(local):
    ...
    async def _create_node_fulltext_index(
        ...
        if await self.version_is_higher_than("5.16"):
            ...
            raise FeatureNotSupported(
                f"Creation of full-text indexes from neomodantic is not supported for Neo4j in version {await self.database_version}. Please upgrade to Neo4j 5.16 or higher."
            )
    ...
    async def _create_node_vector_index(
        ...
        if await self.version_is_higher_than("5.15"):
            ...
            raise FeatureNotSupported(
                f"Creation of vector indexes from neomodantic is not supported for Neo4j in version {await self.database_version}. Please upgrade to Neo4j 5.15 or higher."
            )
    ...
    async def _create_relationship_vector_index(
        ...
        if await self.version_is_higher_than("5.18"):
            ...
            raise FeatureNotSupported(
                f"Creation of vector indexes for relationships from neomodantic is not supported for Neo4j in version {await self.database_version}. Please upgrade to Neo4j 5.18 or higher."
            )
...
class AsyncStructuredNode(NodeBase):
    ...
    def nodes(cls):
        ...
        :rtype: NodeSet
        """
        from neomodantic.async_.match import AsyncNodeSet
    ...
    async def _build_merge_query(
        ...
        if relationship is None:
            ...
            from neomodantic.async_.match import _rel_helper
