...
from neomodantic import config
...
from neomodantic.properties import FulltextIndex, Property, VectorIndex
from neomodantic.sync_.property_manager import PropertyManager
from neomodantic.util import (
    _UnsavedNode,
    classproperty,
    deprecated,
...
class Database(local):
    """
    A singleton object via which all operations from neomodantic to the Neo4j backend are handled with.
    """
    ...
    def set_connection(self, url: str = None, driver: Driver = None):
        ...
        Args:
            ...
            When provided, a Neo4j driver instance will be created by neomodantic.

            driver (neo4j.Driver): Optionally, a pre-created driver instance.
            When provided, neomodantic will not create a driver instance but use this one instead.
        """
        if driver:
            self.driver = driver
    ...
    def cypher_query(
        ...
        If you use neomodantic with your own driver, you must catch SessionExpired exceptions yourself and retry with a new driver instance.
        :type: bool
    ...
    def _create_node_fulltext_index(
        ...
        if self.version_is_higher_than("5.16"):
            ...
            raise FeatureNotSupported(
                f"Creation of full-text indexes from neomodantic is not supported for Neo4j in version {self.database_version}. Please upgrade to Neo4j 5.16 or higher."
            )
    ...
    def _create_node_vector_index(
        ...
        if self.version_is_higher_than("5.15"):
            ...
            raise FeatureNotSupported(
                f"Creation of vector indexes from neomodantic is not supported for Neo4j in version {self.database_version}. Please upgrade to Neo4j 5.15 or higher."
            )
    ...
    def _create_relationship_vector_index(
        ...
        if self.version_is_higher_than("5.18"):
            ...
            raise FeatureNotSupported(
                f"Creation of vector indexes for relationships from neomodantic is not supported for Neo4j in version {self.database_version}. Please upgrade to Neo4j 5.18 or higher."
            )
...
class NodeMeta(ModelMetaclass):
    def __new__(mcs, name, bases, namespace) -> type[BaseModel]:
        ...
        namespace['__module__'] = 'neomodantic.sync_'
        if '__annotations__' not in namespace:
            namespace['__annotations__'] = {}
        for key, value in namespace.items():
            if key.startswith("_"):
                continue
            if isinstance(value, property) and key not in ('deleted', 'id', 'element_id'):
                namespace[key] = computed_field(value, return_type=Any)
            elif isinstance(value, Property):
                namespace['__annotations__'][key] = value.python_type
            elif not do_not_check_relationship_definition and isinstance(value, RelationshipDefinition):
        ...
        if not hasattr(cls, '__private_attributes__'):
            cls.__private_attributes__ = {}
        
        for key, value in namespace.items():
            if isinstance(value, Property):
                cls.__private_attributes__[key] = value
        ...
        if hasattr(cls, "__abstract_node__"):
            ...
            if "deleted" in namespace:
                raise ValueError(
                    "Property name 'deleted' is not allowed as it conflicts with neomodantic internals."
                )
...
class StructuredNode(NodeBase, BaseModel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if "deleted" in kwargs:
            raise ValueError("deleted property is reserved for neomodantic")
    ...
    def nodes(cls):
        ...
        :rtype: NodeSet
        """
        from neomodantic.sync_.match import NodeSet
    ...
    def _build_merge_query(
        ...
        if relationship is None:
            ...
            from neomodantic.sync_.match import _rel_helper
    ...
    def inflate(cls, node):
        """
        Inflate a raw neo4j_driver node to a neomodantic node
        :param node:
        :return: node object
        """
