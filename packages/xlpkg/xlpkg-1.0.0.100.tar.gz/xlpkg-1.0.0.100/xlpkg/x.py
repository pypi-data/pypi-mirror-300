...
def f_release_port(port):
    if f_currenv()=='Linux':
        ...
        if result.stdout:
            ...
            # 结束进程
            subp.run(['kill', '-9', pid])
            x.f_pc(42,f"Port {port} has been released.")
        else:
            x.f_pc(41,f"Port {port} is not being used.")
...
def f_getfields(es,index,query,fields):
    ...
    q2=eval(q1)
    ret=x.f_getalldoc1(es,index,q2,1)
    return(ret)
...
def f_createindex(es,index):
    #索引已存在
    if es.indices.exists(index=index):
        x.f_pc(31,f"The index {index} already exist.")
        return(-1)
    else:
        es.indices.create(index=index)
        x.f_pc(32,f"The index {index}  creation OK.")
        return(0)
    pass
...
def f_delindex(es,index):
    ...
    if es.indices.exists(index=index):
        ...
        x.f_pc(32,f"The index {index} has been deleted.")
        return(0)
    else:
        x.f_pc(31,f"The index {index} does not exist.")
        return(-1)
    pass
...
def f_add_content(self,tw,n=0,cur=None,sql=None):
    ...
    for i in range(len(rows)):
        for j in range(len(cols)):
            item = QTableWidgetItem('{}'.format(rows[i][j]))
...
def f_res2dict(cur):
    ...
    """
    data = cur.fetchall()
    dic = x.f_col2dict(cur)  # 获取字段名和索引的映射关系
