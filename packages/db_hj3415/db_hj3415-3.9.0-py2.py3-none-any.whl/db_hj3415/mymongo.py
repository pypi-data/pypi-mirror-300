...
# 기본 설정 (로그 레벨과 출력 형식 설정)
logging.basicConfig(level=logging.INFO, format='%(module)s / %(levelname)s / func(%(funcName)s) / %(message)s')
...
class Base:
    ...
    def __init__(self, db: str, table: str):
        ...
        try:
            ...
            logging.warning(f"{table} collection이 존재하지 않아 table을 None으로 설정합니다.")
            self._col = None
    ...
    @db.setter
    def db(self, db: str):
        logging.info(f'Change db : {self.db} -> {db}')
        self._db = self.mongo_client[db]
        try:
            self._col = self._db[self.table]
        except AttributeError:
            logging.warning(f"{self.table} collection이 존재하지 않아 table을 None으로 설정합니다.")
            # 클래스 생성시 아직 테이블 설정이 안되었을 때
            self._col = None
    ...
    def table(self, table: str):
        ...
        logging.info(f'Change table : {self.table} -> {table}')
        self._col = self._db[table]
    ...
    def drop(cls, db=None, table=None) -> bool:
        ...
        if not table:
            if Base.is_there(db=db):
                logging.info(f"'{db}' 데이터베이스 삭제 중...")
                cls.mongo_client.drop_database(db)
                return True
            else:
                logging.warning(f"{db} 데이터베이스가 존재하지 않습니다.")
                return False

        # 데이터베이스명과 테이블명을 모두 입력한 경우
        if Base.is_there(db=db, table=table):
            logging.info(f"'{db}' 데이터베이스의 '{table}' 테이블 삭제 중...")
            cls.mongo_client[db].drop_collection(table)
            return True
        else:
            logging.warning(f"{db} 데이터베이스 또는 {table} 테이블이 존재하지 않습니다.")
            return False
    ...
    def is_there(cls, db=None, table=None) -> bool:
        ...
        if not table:
            ...
            if db in dbs:
                return True
            logging.warning(f"{db} 데이터베이스가 서버에 없습니다.")
            return False
        ...
        logging.warning(f"{db} 데이터베이스의 {table} 테이블이 서버에 없습니다.")
        return False
    ...
    def clear_table(self):
        ...
        logging.info(f"{self.db}/{self.table}의 모든 도큐먼트 삭제하는 중..")
...
class Corps(Base):
    ...
    def get_name(cls, code: str) -> Optional[str]:
        ...
        if name is None:
            logging.warning(f"코드 {code}에 해당하는 종목명이 없습니다.")
        return name
    ...
    def _save_df(cls, code: str, page: str, df: pd.DataFrame, clear_table: bool):
        ...
        if df.empty:
            logging.warning(f'입력된 {code}/{page} 데이터프레이 비어서 저장은 하지 않습니다.')
            return

        if clear_table:
            # 페이지가 비어 있지 않으면 먼저 지운다.
            logging.warning(f"데이터를 저장하기 전, {code} / {page} 데이터베이스를 삭제합니다.")
    ...
    def _load_df(self) -> pd.DataFrame:
        try:
            ...
            logging.warning(f"{self.db} / {self.table}에 데이터가 없습니다.")
            df = pd.DataFrame()
        return df
...
class C101(Corps):
    ...
    def get_recent(self, merge_intro=False) -> dict:
        ...
        try:
            ...
            logging.warning("There is no data on C101")
            d = {}
        return d
    ...
    def find(self, date: str, merge_intro=False) -> dict:
        ...
        try:
            ...
            logging.warning(f"There is no data (date:{date}) on C101")
            return {}
...
class C106(Corps):
    ...
    def find(self, row_title: str) -> dict:
        ...
        if data is None:
            logging.warning(f'{row_title} is not in {self.table}')
            data = {}
    ...
    def get_rivals(self) -> list:
        ...
        if data is None:
            logging.warning(f'rival data is none.{self.table} seems empty.')
            return []
...
class C104(C1034):
    ...
    def modify_stamp(self, days_ago: int):
        ...
        logging.info(f"Stamp changed: {before} -> {after}")
...
class MI(Base):
    ...
    def get_recent(self) -> Tuple[str, float]:
        ...
        try:
            ...
            logging.warning(f"There is no data on {self.index}")
            return '', float('nan')
...
class Logs(Base):
    """
    database명 logs, 컬렉션 COL_TITLE로 각 컬렉션에 해당하는 로그(날짜, 텍스트)를 기록한다.
    """
