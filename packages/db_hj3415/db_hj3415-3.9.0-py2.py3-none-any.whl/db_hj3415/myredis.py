...
logging.basicConfig(level=logging.WARNING, format='%(module)s / %(levelname)s / func(%(funcName)s) / %(message)s')
...
class Base:
    ...
    def delete(cls, redis_name: str):
        ...
        :return:
        """
        logging.debug(Base.list_redis_names())
    ...
    def get_cached_data(cls, redis_name: str) -> str:
        ...
        if cached_data is None:
            ...
            logging.debug(type(cached_data.decode('utf-8')))
            return json.loads(cached_data.decode('utf-8'))

    @classmethod
    def set_cached_data(cls, redis_name: str, data: Any, expiration_sec: int = DEFAULT_CACHE_EXPIRATION_SEC) -> None:
        """Redis에 데이터를 캐싱하고 만료 시간을 설정합니다."""
        cls.redis_client.setex(redis_name, expiration_sec, json.dumps(data))
        logging.info(f"Redis 캐시에 저장 (만료시간: {expiration_sec}초) - redis_name : {redis_name}")
    ...
    def fetch_and_cache_data(cls, redis_name: str, refresh: bool, fetch_function: Callable, *args) -> Any:
        ...
        if not refresh:
            ...
            if cached_data:
                ttl_hours = round(cls.redis_client.ttl(redis_name) / 3600, 1)
                logging.info(f"Redis 캐시에서 데이터 가져오기 (남은시간: {ttl_hours} 시간) - redis_name : {redis_name}")
                logging.debug(type(cached_data))
                logging.debug(cached_data)
        ...
        if data:
            cls.set_cached_data(redis_name, data)
        return data
...
class DartToday(Base):
    ...
    def save(self, data: List[dict]):
        ...
        self.set_cached_data(self.redis_name, json.dumps(data), 3600)
    ...
    def get(self) -> List[dict]:
        ...
        logging.debug(cached_data)
        if cached_data is None:
            logging.debug(f"dart today data : []")
            return []
        else:
            # rcept_no를 기준으로 정렬한다.
            sorted_list = sorted(json.loads(cached_data), key=lambda x: x['rcept_no'])
            logging.debug(f"dart today data(total:{len(sorted_list)}) : {sorted_list[:1]}..")
            return sorted_list
...
class Corps(Base):
    ...
    def code(self, code: str):
        ...
        logging.info(f'Change code : {self.code} -> {code}')
        self._code = code
    ...
    def page(self, page: str):
        ...
        logging.info(f'Change page : {self.page} -> {page}')
        self._page = page

    def get_name(self, refresh=False) -> Optional[str]:
        """
        종목명을 반환한다.
        :return:
        """
        ...
        def fetch_get_name(code: str) -> str:
            return mymongo.Corps.get_name(code)

        return self.fetch_and_cache_data(redis_name, refresh, fetch_get_name, self.code)
    ...
    def _list_rows(cls, func: mymongo.Corps, redis_name: str, refresh=False) -> list:
        ...
        def fetch_list_rows(func_in: mymongo.Corps) -> list:
            logging.debug(func_in.list_rows())
...
class C1034(Corps):
    ...
    def find(self, row_title: str, remove_yoy=False, refresh=False) -> Tuple[int, dict]:
        if remove_yoy:
        ...
        def fetch_find(row_title_in: str, remove_yoy_in) -> Tuple[int, dict]:
            return self.mymongo_c1034.find(row_title_in, remove_yoy_in)

        return self.fetch_and_cache_data(redis_name, refresh, fetch_find, row_title, remove_yoy)
...
class C108(Corps):
    ...
    def get_recent_date(self, refresh=False) -> datetime.datetime | None:
...
class MI(Base):
    ...
    def index(self, index: str):
        assert index in mymongo.MI.COL_TITLE, f'Invalid value : {index}({mymongo.MI.COL_TITLE})'
        logging.info(f'Change index : {self.index} -> {index}')
