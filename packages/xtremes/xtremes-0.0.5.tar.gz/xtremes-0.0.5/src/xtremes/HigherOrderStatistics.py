...
def log_likelihood(high_order_statistics, gamma=0, mu=0, sigma=1, r=None):
    ...
    if np.abs(gamma) < 0.0001:
        out = - r * np.log(sigma) - np.exp(-unique_hos[0]) - np.sum(unique_hos[-r:,], axis=0)
    else:
        ...
        out[mask] = -r * np.log(sigma) - (1+gamma*unique_hos[0][mask])**(-1/gamma) - np.sum(np.apply_along_axis(f, 1, unique_hos[-r:,]), axis=0)
...
def Frechet_log_likelihood(high_order_statistics, alpha=1, sigma=1, r=None):
    ...
    if alpha < 0.00001:
        out = -1000 * np.ones_like(unique_hos[0])
    else:
        mask = (unique_hos[0]>0)
        f = lambda x: np.log(x[mask])
        out[mask] = r * np.log(alpha) + r * alpha * np.log(sigma) - (alpha+1)*np.sum(np.apply_along_axis(f, 1, unique_hos[-r:,]), axis=0) - (unique_hos[0][mask]/sigma)**(-alpha)
        out[np.invert(mask)] = -1000 # ln(0)
    joint_ll = np.dot(out, counts)
...
def run_ML_estimation(file, corr='IID', gamma_true=0, block_sizes = [5, 10, 15, 20, 25, 30, 35, 40, 45, 50], stride='SBM', option=1, estimate_pi=False):
    ...
    :param option: int, optional
        Option for ML estimation (default is 'max-only').
...
class HighOrderStats:
    ...
    def get_ML_estimation(self, initParams = 'auto', FrechetOrGEV = 'Frechet', r=None):
        r"""
