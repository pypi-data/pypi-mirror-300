    ...
    try:
        ...
        if current_version != latest_version:
            ...
            if result == QMessageBox.Yes:
                ...
                restart_msg.setText("The plugin has been updated. To apply the changes, Napari needs to be restarted.\n"
        ...
        if response.status_code == 404:
            QMessageBox.critical(None, "Error", f"An error occurred while checking for updates: it"
                                                f" seems the plugin cannot be found on PyPi")
        else:
            QMessageBox.critical(None, "Error", f"An error occurred while checking for updates: {e}")


def restart_napari(viewer=None):
    # Close Napari
    if viewer:
        viewer.window.close()
    ...
    napari_script = os.path.join(os.path.dirname(python_executable), "napari.exe")  # or just "napari" based on your installation
    subprocess.Popen([python_executable, "-m", "napari", "--with", "napari_svetlana"])
    ...
    if "parent_path" in globals():
        ...
        if "image_path" in b.keys() and "labels_path" in b.keys() and "regionprops" in b.keys() and "labels_list" \
            ...
            if os.path.isabs(b["image_path"][0]):
                ...
                labels_path_list = b["image_path"]
            else:
                image_path_list = [os.path.join(parent_path, path) for path in b["image_path"]]
                labels_path_list = [os.path.join(parent_path, path) for path in b["image_path"]]
