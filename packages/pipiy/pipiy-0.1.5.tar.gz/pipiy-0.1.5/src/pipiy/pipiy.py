...
def show_version():
    # pyproject.toml에서 버전 정보를 가져오는 함수
    try:
        pyproject_data = toml.load("pyproject.toml")
        version = pyproject_data.get("tool", {}).get("poetry", {}).get("version", "Unknown version")
        print(f"Pipiy version: {version}")
    except (FileNotFoundError, toml.TomlDecodeError):
        print("Pipiy version: Unknown (pyproject.toml not found or invalid)")
