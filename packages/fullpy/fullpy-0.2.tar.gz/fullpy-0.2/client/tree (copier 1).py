...
class Node(HTML):
  ...
  #def get_tree(self): return self.tree or self.parent.get_tree()
  
  def get_data(self):
    """Return a (icon, label, color, has_children) triple."""
    return "icon", "node's label", None, False
  ...
  def build(self, builder):
    icon, label, color, has_children = self.get_data()
    depth = 0
    node = self.parent
    while node:
      node = node.parent
    ...
    if has_children and self.expanded:
      if self.children is None: self.children = self.get_children()
  ...
  def update(self, update_chidlren = True):
    if update_chidlren:
...
class FixedNode(Node):
  ...
  def get_data    (self): return self.icon, self.label, bool(self.children)
