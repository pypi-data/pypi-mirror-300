...
def coerce_request(
    query: str | QueryRequest, named_template: str | None = None
...
async def get_query_request(name: str) -> QueryRequest:
    with requests.Session() as session:
        ...
        response.raise_for_status()
        return QueryRequest(**response.json())
...
def agent_query(
    ...
    response = requests.post(
        ...
        json=coerce_request(query, named_template).model_dump(),
...
def submit_agent_job(
    ...
    response = requests.post(
        ...
        json=coerce_request(query, named_template).model_dump(),
        params={"agent_type": agent_type, "timeout": timeout},
        timeout=timeout,
        headers={"Authorization": f"Bearer {get_pqa_key()}"},
    )
    response.raise_for_status()
