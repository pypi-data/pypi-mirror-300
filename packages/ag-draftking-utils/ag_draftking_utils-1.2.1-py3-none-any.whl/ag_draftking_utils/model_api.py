...
class Model:
    ...
    def __init__(self, model_id, model_name, model_description, df, x_vars, y_var,
    ...
    def create_model_cv_scores(self):
        ...
        cv = cross_val_score(self.model, X=self.df[self.x_vars], y=self.df[self.y_var],
                             cv=self.n_folds, scoring=self.metric_name)
        self.metric_value = cv.mean()
        e = time.time()
        t = '{:.2f}'.format(e-s)
        print(f'Done creating CV scores, took {t} seconds')
    ...
    def train_model(self):
        ...
        e = time.time()
        t = '{:.2f}'.format(e-s)
        print(f'Done training full model, took {t} seconds')
    ...
    def cache_model(self):
        ...
        e = time.time()
        t = '{:.2f}'.format(e-s)
        print(f'Saved model to {file}, took {t} seconds')
    ...
    def create_cv_predictions_deprecated(self, clean_before_running=True):
        ...
        e = time.time()
        t = '{:.2f}'.format(e-s)
        print(f'Done creating CV predictions, took {t} seconds')
    ...
    def save_model_metrics_to_db(self, conn):
        ...
        try:
            ...
            n_rows = df.shape[0]
            print(f'Saved {n_rows} rows to {model_metadata_path}')
    ...
    @staticmethod
    def create_model_obj_deprecated(model_id, df, x_vars, conn):
        """
        Note that need to pass in x_vars, since MySQL might truncate it
        :param model_id: int: which specific model version
        :param df: full dataframe with X and Y variables
        :param x_vars: variables used for prediction
        ...
        select 
            model_name, model_description, y_var, path_to_cached_model, n_folds
        from TrainingModelResults 
