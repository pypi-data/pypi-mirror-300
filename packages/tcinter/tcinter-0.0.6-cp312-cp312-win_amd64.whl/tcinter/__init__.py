...
tk = tcinter.Tk()
frame = tcinter.Frame(tk, relief=RIDGE, borderwidth=2)
frame.pack(fill=BOTH,expand=1)
label = tcinter.Label(frame, text="Hello, World")
label.pack(fill=X, expand=1)
button = tcinter.Button(frame,text="Exit",command=tk.destroy)
...
WRITABLE = _tcinter.WRITABLE
EXCEPTION = _tcinter.EXCEPTION
...
try: _flatten = _tcinter._flatten
...
try: _cnfmerge = _tcinter._cnfmerge
except AttributeError: pass
...
def _get_default_root(what=None):
    if not _support_default_root:
        raise RuntimeError("No master specified and tcinter is "
...
class Variable:
    ...
    def __init__(self, master=None, value=None, name=None):
        ...
        # raised from Modules/_tcinter.c:SetVar like:
...
class Misc:
    ...
    def _gridconvvalue(self, value):
        if isinstance(value, (str, _tcinter.Tcl_Obj)):
            try:
                svalue = str(value)
                if not svalue:
                    return None
...
class Tk(Misc, Wm):
    ...
    def __init__(self, screenName=None, baseName=None, className='Tk',
        ...
        interactive = False
        self.tk = _tcinter.create(screenName, baseName, className, interactive, wantobjects, useTk, sync, use)
        if useTk:
            self._loadtk()
    ...
    def _loadtk(self):
        ...
        if tk_version != _tcinter.TK_VERSION:
            raise RuntimeError("tk.h version (%s) doesn't match libtk.a version (%s)"
                               % (_tcinter.TK_VERSION, tk_version))
        # Under unknown circumstances, tcl_version gets coerced to float
        tcl_version = str(self.tk.getvar('tcl_version'))
        if tcl_version != _tcinter.TCL_VERSION:
            raise RuntimeError("tcl.h version (%s) doesn't match libtcl.a version (%s)" \
                               % (_tcinter.TCL_VERSION, tcl_version))
...
def Tcl(screenName=None, baseName=None, className='Tk', useTk=False, init='puts ""'):
