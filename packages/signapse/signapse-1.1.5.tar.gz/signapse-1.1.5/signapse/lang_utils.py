    ...
    def sam_masks(self,img_centre):
        if (self.args.opt.face_crop) or (self.args.opt.hand_crop):
            if (self.args.opt.hand_crop):
                if  self.args.sign_request_id+".pkl"  in os.listdir("./inputs/hand_masks"):
                    mask = load_zipped_pickle(f"./inputs/hand_masks/{self.args.sign_request_id}.pkl")
                    if self.args.stop > 0:
                        mask = mask[:self.args.stop]
                else:
