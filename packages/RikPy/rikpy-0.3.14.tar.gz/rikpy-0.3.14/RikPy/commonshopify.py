...
def Shopify_bulk_archive_products(shop, access_token, api_version, product_ids):
    """
    Bulk archive Shopify products.

    :param shop: The Shopify store's domain, e.g., 'your-shop.myshopify.com'
    ...
    # STEP 1: CREATE THE JSONL FILE
    # Create and write to the JSONL file
    file_path = 'archive_products.jsonl'
    with open(file_path, 'w') as file:
        for product_id in product_ids:
            # Construct the line as a dictionary
            line_dict = {
    ...
    variables = {
        "input": [
            {
                "filename": "archive_products.jsonl",
                "mimeType": "text/jsonl",
                "httpMethod": "POST",
    ...
    # Upload JSONL file using multipart data
    print(f"Uploading JSONL...")
    multipart_data = MultipartEncoder(
        fields={**{param['name']: param['value'] for param in upload_parameters}, 'file': ('archive_products.jsonl', open(file_path, 'rb'), 'text/jsonl')}
    )
    ...
    mutation_string = """
    mutation publishableUpdate($id: ID!, $input: ProductInput!) {
    ...
    print("Bulk archive operation initiated successfully.")
