...
def validate_inputs(inputs):
    ...
    schema = {
        'client_id': {'type': 'string', 'minlength': 36, 'maxlength': 40, 'required': True},
        'client_secret': {'type': 'string', 'minlength': 36, 'maxlength': 64, 'required': True},
        'api_url': {'type': 'string', 'regex': r'^(https?|http)://[^\s/$.?#].[^\s]*$', 'required': True}
    }
    ...
    if v.validate(inputs):
        if ("/BeyondTrust/api/public/v" not in inputs['api_url']):
            raise exceptions.OptionsError("invalid API URL, it must contains /BeyondTrust/api/public/v as part of the path")
