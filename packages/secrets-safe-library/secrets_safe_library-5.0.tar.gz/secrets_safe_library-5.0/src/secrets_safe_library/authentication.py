...
class Authentication:
    ...
    def oauth(self):
        ...
        response = self._req.post(endpoint_url, auth_info, header, verify=self._verify_ca, timeout=(self._timeout_connection_seconds, self._timeout_request_seconds))
    ...
    def send_post_sign_app_in(self):
        ...
        if self._certificate and self._certificate_key:
            with tempfile.NamedTemporaryFile(delete=True, mode='w', suffix='.pem') as cert_file:
                cert_file.write(self._certificate)
                cert_file.flush()

                with tempfile.NamedTemporaryFile(delete=True, mode='w', suffix='.pem') as key_file:
                    key_file.write(self._certificate_key)
                    key_file.flush()

                    response = self._req.post(self._sig_app_in_url, headers=headers, cert=(cert_file.name, key_file.name))
        else:
