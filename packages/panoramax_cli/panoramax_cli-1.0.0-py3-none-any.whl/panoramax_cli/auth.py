...
from panoramax_cli.utils import test_panoramax_url, createSessionWithRetry
import os
...
def login(s: Session, panoramax: Panoramax) -> bool:
    """
    Login to panoramax and store auth cookie in session
    """
    if panoramax.token is not None:
        return _login_with_token(s, panoramax)

    return _login_with_stored_credentials(s, panoramax)


def _login_with_token(s: Session, panoramax: Panoramax) -> bool:
    s.headers.update({"Authorization": f"Bearer {panoramax.token}"})
    return True


def _login_with_stored_credentials(s: Session, panoramax: Panoramax):
    creds = _read_existing_credentials(panoramax)

    instance_cred = creds.get_instance_credentials(panoramax.url)
    if not instance_cred:
        _generate_and_update_credentials(s, creds, panoramax)
        return False

    account_name = _check_if_associated(s, panoramax, instance_cred)
    if not account_name:
...
def create_auth_credentials(
    ...
    Login to panoramax and store auth cookie in session
    """
    with createSessionWithRetry(disable_cert_check) as s:
        test_panoramax_url(s, panoramax.url)
        creds = _read_existing_credentials(panoramax)
        ...
        return _generate_and_update_credentials(s, creds, panoramax)


def _generate_and_update_credentials(
    session: requests.Session, creds: Credentials, panoramax: Panoramax
) -> InstanceCredential:
    i = _generate_new_instance_credentials(session, panoramax)
...
def _generate_new_instance_credentials(
    session: requests.Session, panoramax: Panoramax
) -> InstanceCredential:
    token_response = session.post(f"{panoramax.url}/api/auth/tokens/generate")
...
def _check_if_associated(
    session: requests.Session, panoramax: Panoramax, creds: InstanceCredential
) -> Optional[str]:
