...
def get_uploadset_files(
    panoramax: Panoramax, session: requests.Session, uploadSet: UploadSet
) -> UploadSet:
...
def info(
    ...
    s = session.get(url, timeout=REQUESTS_TIMEOUT)
    if s.status_code == 404:
        raise CliException(f"Upload Set {uploadSet.id} not found")
    if s.status_code >= 400:
        raise CliException(
...
def wait_for_upload_sets(
    ...
    uploadSets = [info(panoramax, session, us) for us in uploadSets]
...
def display_upload_sets_statuses(
    panoramax: Panoramax, session: requests.Session, uploadSets: List[UploadSet]
):
    table = Table()
