...
def create_upload_set(
    ...
    if uploadSet.id is None:
        ...
        post_response = session.post(url, json=post_data, timeout=REQUESTS_TIMEOUT)
...
You can either:
  ...
  - If you are the API administrator or can contact them,
    ...
    # Synchronize existing upload set
        ...
        info(gvs, session, uploadSet)
        print(f"🌐 Resume processing Upload Set: {uploadSet.id}")
        get_uploadset_files(gvs, session, uploadSet)
...
def upload_single_file(
    ...
    try:
        picture_response = session.post(
            url,
            files={"file": open(uploadFile.path, "rb")},
            data=post_data,
            timeout=(REQUESTS_CNX_TIMEOUT, uploadTimeout),
        )
...
def complete_upload_set(
    gvs: Panoramax, session: requests.Session, uploadSet: UploadSet
):
    ...
    if not uploadSet.completed:
        ...
        post_response = session.post(url, timeout=REQUESTS_TIMEOUT)
        raise_for_status(post_response, "Impossible to close Upload Set")


def _login_if_needed(session: requests.Session, panoramax: Panoramax) -> bool:
    # Check if API needs login
    apiConf = session.get(f"{panoramax.url}/api/configuration")
    if apiConf.json().get("auth", {}).get("enabled", False):
        logged_in = login(session, panoramax)
        if not logged_in:
            return False
    return True
...
def upload_path(
    ...
    with createSessionWithRetry(disableCertCheck) as s:
        test_panoramax_url(s, panoramax.url)

        # early test login
        if not _login_if_needed(s, panoramax):
            raise CliException(
        ...
        for uploadSet in uploadSets:
            if title is not None:
                ...
                uploadSet.title = uploadSet.path.stem
        ...
        with Live(progress_group) as live_render:
            ...
            for uploadSet in uploadSets:
                ...
                create_upload_set(panoramax, s, uploadSet)
                ...
                try:
                    complete_upload_set(panoramax, s, uploadSet)
...
def local_checks(
    ...
    with Live(progress_group):
        ...
        def onPicError(short, long):
            ...
            error_progress.update(
                error_task,
                description=f"[bold red]⚠️ {len(allErrors)} picture(s) with metadata issues",
            )
