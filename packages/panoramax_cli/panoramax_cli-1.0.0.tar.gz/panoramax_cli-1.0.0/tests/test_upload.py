...
def test_upload_set_creation_failure(requests_mock, datafiles):
...
def test_upload_set_creation_failure_incompatible(requests_mock, datafiles):
    requests_mock.get(MOCK_API_URL + "/api", status_code=200)
    requests_mock.get(MOCK_API_URL + "/api/configuration", json={}, status_code=200)
    requests_mock.post(MOCK_API_URL + "/api/upload_sets", status_code=404)
...
def test_upload_with_invalid_file(requests_mock, datafiles):
    ...
    requests_mock.post(
        f"{MOCK_API_URL}/api/upload_sets",
        json={"id": usId},
    )

    # the body will contains the name of the file in the multipart form, we can match on it
    requests_mock.post(
        f"{MOCK_API_URL}/api/upload_sets/{usId}/files",
        status_code=202,
        json={"picture_id": "bla"},
        additional_matcher=lambda r: b"e1.jpg" in r.body,
    )
    requests_mock.post(
        f"{MOCK_API_URL}/api/upload_sets/{usId}/files",
        status_code=400,
        additional_matcher=lambda r: b"e2.jpg" in r.body,
    )
    requests_mock.post(
        f"{MOCK_API_URL}/api/upload_sets/{usId}/files",
        status_code=409,
        additional_matcher=lambda r: b"e3.jpg" in r.body,
    )
    requests_mock.get(
        f"{MOCK_API_URL}/api/upload_sets/{usId}",
        json={
            "completed": False,
            "dispatched": False,
            "ready": False,
    ...
    requests_mock.post(f"{MOCK_API_URL}/api/upload_sets/{usId}/complete", json={})
