...
import requests
from panoramax_cli import status, upload, exception, model
...
def test_valid_csv_upload(panoramax_with_token, datafiles, user_credential):
    ...
    with requests.session() as s:
        ...
        status.wait_for_upload_sets(
            panoramax_with_token, s, [us], timeout=timedelta(minutes=1)
        )
        us = status.get_uploadset_files(panoramax_with_token, s, us)
        us = status.info(panoramax_with_token, s, us)
    ...
    c1.raise_for_status()
    c2 = requests.get(
        f"{panoramax_with_token.url}/api/collections/{us.associated_collections[1].id}/items"
    )
    c2.raise_for_status()
    c3 = requests.get(
        f"{panoramax_with_token.url}/api/collections/{us.associated_collections[2].id}/items"
    )
    c3.raise_for_status()
    ...
    assert datetime.fromisoformat(
        itemByFilename["e1.jpg"]["properties"]["datetime"]
    ) == datetime.fromisoformat("2018-01-22T02:52:09+03:00")
    assert (
        itemByFilename["e1.jpg"]["properties"]["exif"]["Exif.Image.Software"]
        == "MS Paint"
    )
    assert (
        itemByFilename["e1.jpg"]["properties"]["exif"]["Exif.Image.Artist"]
        == "A 2 years old"
    )
    ...
    assert datetime.fromisoformat(
        itemByFilename["e3.jpg"]["properties"]["datetime"]
