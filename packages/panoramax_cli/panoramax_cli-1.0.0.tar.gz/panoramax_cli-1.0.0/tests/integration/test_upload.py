...
def test_valid_upload(panoramax_with_token, datafiles, user_credential):
    ...
    with requests.session() as s:
        ...
        wait_for_upload_sets(
            panoramax_with_token, s, report.upload_sets, timeout=timedelta(minutes=1)
        )
        us = model.UploadSet(id=report.upload_sets[0].id)
        us = info(panoramax_with_token, s, us)
        us = get_uploadset_files(panoramax_with_token, s, us)
