...
def test_CsvMetadataHandler(datafiles):
    mtd = csv.CsvMetadataHandler(Path(datafiles / "metadata.csv"))
    assert mtd.data == {
...
def test_CsvMetadataHandler_exif(datafiles):
    mtd = csv.CsvMetadataHandler(Path(datafiles / "metadata_exif.csv"))
    assert mtd.data == {
        "e1.jpg": PartialGeoPicTags(
            lat=50.5151,
            lon=3.265,
...
""".replace(
        ...
        f.write(r)
    mtd = csv.CsvMetadataHandler(Path(p))
    assert mtd.data == {
        "e1.jpg": PartialGeoPicTags(
            lat=50.5151,
            lon=3.265,
