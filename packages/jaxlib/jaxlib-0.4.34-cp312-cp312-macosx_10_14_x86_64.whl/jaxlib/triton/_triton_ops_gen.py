...
class FuncOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_633')) else
      _ods_ir.AttrBuilder.get('anonymous_633')(function_type, context=_ods_context))
...
def assert_(condition, message, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(AssertOp(condition=condition, message=message, loc=loc, ip=ip))
...
def experimental_tensormap_create(desc_ptr, global_address, box_dim, global_dim, global_stride, element_stride, elem_type, interleave_layout, swizzle_mode, fill_mode, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ExperimentalTensormapCreateOp(desc_ptr=desc_ptr, global_address=global_address, box_dim=box_dim, global_dim=global_dim, global_stride=global_stride, element_stride=element_stride, elem_type=elem_type, interleave_layout=interleave_layout, swizzle_mode=swizzle_mode, fill_mode=fill_mode, loc=loc, ip=ip))
...
def experimental_tensormap_fenceproxy_acquire(desc_ptr, *, loc=None, ip=None) -> _ods_ir.Operation:
  return _get_op_result_or_op_results(ExperimentalTensormapFenceproxyAcquireOp(desc_ptr=desc_ptr, loc=loc, ip=ip))
