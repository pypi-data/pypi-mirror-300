...
from .constants import tick_fields , bar_fields , bid_ask_fields , greek_fields , full_feed_fields, min_fields
...
class TouchlineData:
    ...
    def __post_init__(self):
        ...
        self.timestamp = datetime.strptime(self.raw_data[2], '%Y-%m-%dT%H:%M:%S')
...
class bar_feed:
    ...
    def __repr__(self):
        bar_tuple = namedtuple('bar_feed', field_names= bar_fields)
        attributes = list(map(lambda x: getattr(self , x), bar_fields))
        return str(bar_tuple(*attributes))    
    
    def to_dict(self):
        return dict(map(lambda x: (x , getattr(self , x) ), bar_fields))
