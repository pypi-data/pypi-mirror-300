...
Scope : {self.find_scope_name(self._data[row].default_scope)} \n\n"
    ...
    def remove(self, index):
        try:
            with Session() as session:
                dao_sync_items.delete_sync_items(self._data[index].id, session)
                dao_sync_sources.delete_source(self._data[index].id, session)
    ...
    def update_data(self):
        ...
        self._data = self.__get_data()
        if current_len != len(self._data):
            self.beginInsertRows(QModelIndex(), current_len, current_len)
            self.endInsertRows()
        
        self.dataChanged.emit(self.index(0,0), self.index(len(self._data)-1, 0), [self.name, self.sourceType, self.status, self.item_remaining, self.total_items, self.SourceInfo, self.LastUpdate, self.items_failed, self.items_skipped])
    ...
    def evaluateQuantifyData(self, name, set_up, scope_uuid, path):
        ...
        for data in self.__get_data():
            ...
            if data.type is SyncSourceTypes.quantify:
                if data.config_data['quantify_directory'] == str(pathlib.Path(QUrl(path).path())):
