...
class dao_dataset(dao_base):
    ...
    def __search(stmt : Select, scope_ids : List[int], datasetSearch:DatasetSearch):
        ...
        if datasetSearch.search_query:
            stmt = stmt.where(Datasets.search_helper.ilike(f"%{datasetSearch.search_query}%"))
    ...
    def __get_scope_ids(uuid_list, session):
        if uuid_list:
            ...
            scope_ids = [_get_scope_raw(scope.uuid, session, True).id for scope in dao_scope.read_all(username= user_settings.user_name , session=session)]
        return scope_ids
