...
from PyQt6.QtWidgets import QMessageBox, QLineEdit, QGridLayout, QCheckBox, QVBoxLayout, QHBoxLayout, QWidget, QPushButton, QFileDialog
...
class AmplitudeMaskTab(QWidget):
    ...
    def Init_GUI(self):
        ...
        self.selectfile = QPushButton("Open Amplitude Correction Image")
        self.selectfile.clicked.connect(self.loadAmplitudeFromFile)

        self.loadimage = QPushButton("Load Amplitude Correction Image")
        ...
        self.image_loading_layout.addWidget(self.selectfile)
    ...
    def loadAmplitudeFromFile(self):
        """Loads a mesh from a file in gmsh .msh format.
        """
        # Opens a prompt for the user to select a mesh file
        name = QFileDialog.getOpenFileName(self,"Open File","${HOME}","BMP (*.bmp);;",)
        self.image_path.setText(name[0])
        outcome = self.update_pattern()
        if not outcome:
    ...
    def update_pattern(self):
        ...
        if self.loaded_file != self.image_path.text():
            ...
            # Normalize the pattern (as it's an amplitude correction [0,1])

            self.pattern = (self.pattern-np.min(self.pattern))/(np.max(self.pattern-np.min(self.pattern)))
            return True
    ...
    def is_amplitude(self):
        """Checks if the flatness correction element is an amplitude correction.

        Returns:
            bool: True if the flatness correction element is an amplitude correction, False otherwise.
        """
        return True
