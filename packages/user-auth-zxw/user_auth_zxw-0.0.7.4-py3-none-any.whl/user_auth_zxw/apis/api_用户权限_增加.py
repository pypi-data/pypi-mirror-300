...
async def delete_role(user_id: int, role_name: str, db: AsyncSession = Depends(get_db)):
    # 1. 查询用户
...
async def add_new_role(request: 请求_分配或创建角色, db: AsyncSession = Depends(get_db)):
    """
    分配或创建角色
                如果用户不存在则返回404,
                如果角色不存在则创建角色,
    ...
    app = result_app.scalar_one_or_none()  # scalar_one_or_none()作用是返回一个结果，如果没有结果则返回None
...
@router.post("/assign-role/", response_model=返回_分配或创建角色)
async def __分配或创建角色__(request: 请求_分配或创建角色):
    return await add_new_role(request)
