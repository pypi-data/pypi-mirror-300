...
class PandasFeatureViewPipelineNode(PandasExecNode):
    ...
    def to_arrow(self, expected_output_schema: Optional[Schema] = None) -> pyarrow.Table:
    ...
    def _node_to_value(
        ...
        if pipeline_node.HasField("transformation_node"):
            ...
            resolved_secrets = {}
            secrets = self.feature_definition_wrapper.fv_spec.secrets
            if secrets:
                if not self.secret_resolver:
                    msg = "No secret resolver was provided."
                    raise TectonValidationError(msg)
                resolved_secrets = self.secret_resolver.resolve_map(secrets)
            return BoundMaterializationContext.create(
                self.feature_time_limits.start, self.feature_time_limits.end, secrets=resolved_secrets
            )
        else:
...
class PandasDataNode(PandasExecNode):
    ...
    def to_arrow(self, expected_output_schema: Optional[Schema] = None) -> pyarrow.RecordBatchReader:
