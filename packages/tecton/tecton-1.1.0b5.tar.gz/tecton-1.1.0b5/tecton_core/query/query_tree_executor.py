...
class QueryTreeExecutor:
    ...
    _dialect_to_compute_map: Dict[Dialect, SQLCompute] = {}
    ...
    def _execute_data_source_step(self, qt_root: NodeRef) -> QueryTreeOutput:
        ...
        for name, staging_node in staging_nodes_to_process.items():
            ...
            with self._monitor_stage("Reading Data Source", dialect=dialect) as monitor:
                compute = self._get_or_create_compute_by_dialect(
                    dialect,
                    qt_root,
                )
                ...
                if not isinstance(data_source_node, MockDataSourceScanNode):
                    ...
                    self._register_user_input_tables(qt_root=qt_root, compute=compute)
    ...
    def _execute_pipeline_step(self, output: QueryTreeOutput, qt_node: NodeRef) -> QueryTreeOutput:
        ...
        compute = self._get_or_create_compute_by_dialect(pipeline_dialect, qt_node)
        data_source_readers = output.data_source_readers
        with self._monitor_stage("Evaluating Feature View pipelines", dialect=pipeline_dialect) as monitor, compute:
            if pipeline_dialect == Dialect.PANDAS:
                ...
                staged_tables = rewriter.rewrite(
                    qt_node, output.data_source_readers, secret_resolver=self.secret_resolver
                )
                # Override data_source_readers and compute -
                # the rest of the PIPELINE subtree should be executed by DuckDB
                data_source_readers = dict(staged_tables)
                compute = self._get_or_create_compute_by_dialect(Dialect.DUCKDB, qt_node)

            for table_name, pa_reader in data_source_readers.items():
                if not pa_reader:
            ...
            # tmp table for user provided data (e.g spine)
            self._register_user_input_tables(qt_root=qt_node, compute=compute)

            staging_nodes_to_process = get_staging_nodes(qt_node, QueryTreeStep.PIPELINE)
            if len(staging_nodes_to_process) == 0:
                # ToDo (oleksii): check if this branch is redundant
                return QueryTreeOutput(data_source_readers={}, feature_view_readers={})
    ...
    def _execute_agg_step(self, output: QueryTreeOutput, qt_node: NodeRef) -> QueryTreeOutput:
        compute = self._get_or_create_compute_by_dialect(Dialect.DUCKDB)
    ...
    def _execute_odfv_step(self, prev_step_output: QueryTreeOutput, qt_node: NodeRef) -> QueryTreeOutput:
        ...
        if has_odfvs:
            compute = self._get_or_create_compute_by_dialect(Dialect.PANDAS)
    ...
    def _process_offline_scan_node(self, node: OfflineStoreScanNode, staged_table_name: str) -> None:
        ...
        compute = self._get_or_create_compute_by_dialect(Dialect.DUCKDB)
        self._register_user_input_tables(node.as_ref(), compute)
    ...
    def _register_user_input_tables(self, qt_root: NodeRef, compute: SQLCompute) -> None:
        ...
        def register_user_input_tables(node):
    ...
    def _get_or_create_compute_by_dialect(
        ...
        compute = SQLCompute.for_dialect(dialect, qt_root, self.secret_resolver, self.offline_store_options_providers)
