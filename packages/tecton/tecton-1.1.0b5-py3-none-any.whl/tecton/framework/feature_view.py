...
from tecton.framework.configs import _convert_secret_to_sanitized_reference
        ...
        *,
        ...
        if aggregation_leading_edge == AggregationLeadingEdge.UNSPECIFIED:
            ...
            if aggregation_leading_edge_default == configs.LatestEventTime:
                ...
                AggregationLeadingEdge.WALL_CLOCK_TIME
...
class RealtimeFeatureView(_RealtimeOrPromptBase):
    ...
    :param mode: The transformation mode. Valid values are "python", "pandas", or "no_transformation". Defaults to "no_transformation" if no transformation is defined.
    ...
    def __init__(
        ...
        *,
        ...
        prevent_destroy: bool = False,
        schema: Optional[List[types.Field]] = None,
        features: Optional[List[Union[feature.Attribute, feature.Calculation]]] = None,
        feature_view_function: Optional[Callable] = None,
        ...
        self._validate_parameters(name, features, schema, mode, feature_view_function, sources, _context_parameter_name)
        ...
        if feature_view_function:
            ...
            include_realtime_context_in_pipeline = _should_include_realtime_context(
                features=features, context_parameter_name=_context_parameter_name
            )
            pipeline_root = _build_join_inputs_pipeline(
                sources=sources,
                context_parameter_name=_context_parameter_name,
                include_realtime_context=include_realtime_context_in_pipeline,
            )
        ...
        if schema:
            ...
            for feature in features:
                if isinstance(feature, Attribute):
                    realtime_args.attributes.append(feature._to_proto())
                elif isinstance(feature, Calculation):
                    realtime_args.calculations.append(feature._to_proto())
                else:
                    # @realtime_realtime_feature_view_typechecked on __init__ should prevent this branch.
                    msg = f"Feature '{feature.name}' is {feature.__class__}, which is not compatible with Realtime Feature View. "
                    raise TectonValidationError(msg)
        else:
        ...
        if secrets:
            for secret_name, secret in secrets.items():
                secret_references[secret_name] = _convert_secret_to_sanitized_reference(secret)
    ...
    def _validate_parameters(
        self, name, features, schema, mode, feature_view_function, sources, context_parameter_name
    ):
        ...
        if not conf.get_bool("ALLOW_REALTIME_FEATURE_VIEW_NO_TRANSFORMATION") and (
...
def _build_join_inputs_pipeline(
    sources: List[SourceType], context_parameter_name: str, include_realtime_context: bool
) -> PipelineNodeWrapper:
    input_nodes = [_source_to_pipeline_node(source, _get_name_for_source(source)).node_proto for source in sources]

    if include_realtime_context:
...
def _get_name_for_source(source: SourceType) -> str:
    # use getattr instead of hasattr because we only want to use source.name if the name is set, not if the name is None.
...
def _should_include_realtime_context(features: List[Attribute], context_parameter_name: str) -> bool:
    request_timestamp_feature_name = f"{context_parameter_name}__{REQUEST_TIMESTAMP_FIELD_NAME}"
    return request_timestamp_feature_name in [_feature.name for _feature in features]
