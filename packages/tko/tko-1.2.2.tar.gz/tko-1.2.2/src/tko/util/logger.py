...
class ActionData:
    def __init__(self, timestamp: str, action_value: str, rep: str = "", task: str = "", payload: str = ""):
    ...
    def action_self_same_task(self, other: ActionData):
        ...
        if other.action_value != LogAction.SELF.value:
            return False
        return self.rep_alias == other.rep_alias and self.task_key == other.task_key
...
class LoggerStore(ABC):
    ...
    @abstractmethod
    def set_rep(self, rep_alias: str) -> None:
        pass
...
class LoggerFS(LoggerStore):
    ...
    def row_to_action_data(self, row: list[str]) -> ActionData:
        ...
        task = row[3]
        payload = row[4]
        action_data = ActionData(timestamp, action_value, self.rep_alias, task, payload)
    ...
    def push_to_file(self, action_data: ActionData, last_hash: str):
        ...
        if not os.path.exists(os.path.dirname(self.get_log_file())):
...
class Logger:
    ...
    def record_event(self, action: LogAction, task_key: str = "", payload: str = ""):
        if self.rep_alias == "":
            return
        action_data = ActionData(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), action.value, self.rep_alias, task_key, payload)
