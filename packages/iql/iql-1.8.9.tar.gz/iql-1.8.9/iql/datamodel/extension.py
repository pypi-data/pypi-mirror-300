    ...
    def execute_batch(
        ...
        if self.is_async:
            ...
            try:
                ...
                logger.info("No loop, created one %s %s", e, loop)
                asyncio.set_event_loop(loop)

            g = asyncio.gather(*(self.executeasync(sq) for sq in subqueries_flat))
            results = loop.run_until_complete(g)  # asyncio.run(g)
