...
class SubQuery:
    ...
    def populate_from_cache(self) -> bool:
        ...
        logger.debug("Checking cache for %s, %s", cache_val, type(self))
        ...
        if df is not None:
            ...
            logger.debug("Not found in cache %s", self.subquery)
            return False
...
def harmonize_and_concat_arrow_tables(tables: Iterable):
    ...
    for column, types in columns.items():
        ...
        if len(t) > 1 and len([t for t in types if not pa.types.is_null(t)]) > 1:  # type: ignore
