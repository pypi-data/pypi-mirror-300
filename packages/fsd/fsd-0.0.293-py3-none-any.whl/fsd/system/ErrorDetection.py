...
class ErrorDetection:
    ...
    def initial_setup(self):
        ...
        system_prompt = (
            "You're a DevOps expert analyzing run, build, or compile errors. Determine if it's a code issue (type 1) or configuration/dependency issue (type 2). Provide a clear analysis.\n\n"
            "Instructions:\n"
            "1. Examine the error message.\n"
            "2. Categorize as type 1 or 2.\n"
            ...
            "5. {'error_type': 1, 'error_message': 'IndentationError: unexpected indent (script.py, line 15)'}\n"
            "6. {'error_type': 2, 'error_message': 'psycopg2.OperationalError: could not connect to server: Connection refused'}\n"
    ...
    async def get_task_plan(self, error):
        ...
        try:
            response = await self.repo.ai.prompt(self.conversation_history, self.max_tokens, 0.2, 0.1)
    ...
    async def get_task_plans(self, error):
        ...
        """
        logger.debug(f"\n #### The `ErrorDetection` agent is analyzing the error and generating a task plan")
        plan = await self.get_task_plan(error)
        logger.debug(f"\n #### The `ErrorDetection` agent has completed the error analysis and produced a plan")
        return plan
