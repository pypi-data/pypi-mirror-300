...
class FileManagerAgent:
    ...
    def read_file_content(self, file_path):
        ...
        if not os.path.exists(file_path):
            logger.debug(f"\n #### The `FileManagerAgent` has detected that the file does not exist: `{file_path}`")
            return ""
        ...
        for encoding in encodings:
            try:
                ...
                logger.debug(f"\n #### The `File Manager Agent` encountered an issue while reading `{file_path}` with `{encoding}` encoding: `{e}`")
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` was unable to read `{file_path}` in binary mode: `{e}`")
    ...
    async def get_file_planning(self, idea):
        ...
        prompt = (
                ...
                "Adding_new_files must include only new files explicitly mentioned in the instruction that need to be created, including all .md files if specified. IMPORTANT: Never skip .md or documentation files like README.md etc. Strictly exclude ALL dependency configuration files (such as requirements.txt, package.json, Podfile, yarn.lock, Gemfile, etc.) from Adding_new_files. These will be handled separately later. For image files, only include .svg files if mentioned. Ignore all other image file types for adding new files. "
                "Existing_files must include only the full paths for file names explicitly mentioned in the instruction. "
                "Context_files must include only the full path context file names explicitly mentioned in the instruction. Exclude all dependency files, configuration files, and non-code files such as .gitignore, package.json, Podfile, etc. Focus on including only relevant code files. "
                ...
                "    \"Context_files\": [\"/full/path/to/file1.extension\", \"/full/path/to/file2.extension\"],\n"
                "    \"Adding_new_files\": []\n"
                "}\n\n"
                ...
                "            \"Parameters\": {\n"
                "                \"full_path\": \"" + self.repo.get_repo_path() + "/full/correct/path/from/development/plan/example.extension\"\n"
                "            }\n"
                "        }\n"
                "    ]\n"
                "}\n\n"
                "Existing_files, Context_files, and Adding_new_files must include only files explicitly mentioned in the instruction.\n"
                "full_path must be the complete path from the project root to the file, including the file name.\n"
                ...
                "IMPORTANT: Strictly adhere to the development plan. For Adding_new_files, never create new paths or folders not explicitly mentioned in the plan. Do not invent or hallucinate any file paths or names. Only include files and paths that are exactly as specified in the development plan."
            )
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` encountered an unexpected error: `{e}`")
            return {
                "reason": str(e)
            }
    ...
    async def get_adding_file_planning(self, idea, tree):
        ...
        prompt = (
            ...
            "            \"Pipeline\": \"1 or 2\",\n"
            "            \"Parameters\": {\n"
            "                \"full_path\": \"" + self.repo.get_repo_path() + "/path/to/example.extension\"\n"
            "            }\n"
            "        }\n"
            "    ]\n"
            "}\n"
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` encountered an unexpected error during adding file planning: `{e}`")
            return {
                "reason": str(e)
            }
    ...
    async def get_moving_file_planning(self, idea, tree):
        ...
        try:
            ...
            logger.debug(f"\n #### The `FileManagerAgent` encountered an unexpected error during moving file planning: `{e}`")
            return {
                "reason": str(e)
            }
