...
class CodingAgent:
    ...
    async def get_coding_request(self, is_first, file, techStack, crawl_logs, prompt):
        ...
        if is_svg:
            user_prompt = (
                f"You are a world-class SVG designer with unparalleled expertise. Create the following SVG with exceptional precision and sophistication: "
                f"{'Utilize the provided data above to meticulously complete the task' if crawl_logs else ''}:\n"
                f"Implementing a high-quality, professional SVG image on {file_name}:\n"
                f"Image description: {prompt}\n"
                f"Strictly adhere to SVG best practices and create exemplary, efficient SVG code that showcases your unmatched expertise.\n"
                f"{lazy_prompt}\n"
                "Craft a sleek, luxurious, and impeccably clean SVG with flawless proportions, demonstrating your EXPERT-level design prowess.\n"
                "Remember: You are the pinnacle of SVG design excellence. Your work must exude unparalleled expertise, meticulous attention to detail, and a touch of elegance.\n"
                "Aim for a visually striking result that embodies sophistication while maintaining a refined, minimalist aesthetic.\n"
                "Ensure the final SVG is cool, modern, and radiates a sense of luxury and exclusivity.\n"
            ...
            user_prompt = (
                ...
                "If the original instruction involves images or icons, incorporate them flawlessly to achieve an impeccable design that will exceed customer expectations. For image.icon, select and implement the most appropriate, elegant, and professional-looking icon that best fits the context and styling requirements.\n"
                ...
                "When working with icons, ensure they are styled to perfectly match and enhance the overall design aesthetic.\n"
                "IMPORTANT: Always review the instruction carefully. Do not overlap or duplicate work unnecessarily. Focus on efficiently implementing the required changes without redundancy.\n"
