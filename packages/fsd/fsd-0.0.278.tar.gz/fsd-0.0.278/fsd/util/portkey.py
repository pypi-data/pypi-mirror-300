...
class AIGateway:
    ...
    def __init__(self):
        ...
        """
        self.models = ["azure", "bedrock"]  # Exclude DALL-E 3 and Gemini from general text models
    ...
    async def prompt(self, 
        ...
        """
        models_to_try = list(self.models)  # Create a copy to avoid modifying the original list
        while models_to_try:
    ...
    async def stream_prompt(self, 
        ...
        Raises:
            Exception: If there's an error in making the API call after trying all models.
        """
        models_to_try = list(self.models) + ["gemini"]  # Include Gemini as an option for streaming
        while models_to_try:
            model = self._get_next_model() if model != "gemini" else "gemini"
            models_to_try.remove(model)
            ...
            try:
                ...
                chat_completion_stream = self.portkey_clients[model].chat.completions.create(**common_params)
                ...
                self._update_stats(success=True)
                logger.info(f"Completed streaming from {model} model.")
                return final_response
            except Exception as e:
                self._update_stats(success=False)
                logger.warning(f"Error in streaming prompt from {model} model: {str(e)}. Trying next model.")
                if "Invalid bedrock provider" in str(e) and model == "bedrock":
                    logger.error("Bedrock configuration error detected. Removing Bedrock from available models.")
                    self.models.remove("bedrock")
                    del self.model_usage["bedrock"]
