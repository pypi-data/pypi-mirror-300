...
class AzureModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        logger.info("Using AzureModel for prompt")
        common_params = {
            "messages": messages,
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        logger.info("Using AzureModel for stream_prompt")
        common_params = {
            "messages": messages,
            "temperature": temperature,
            "max_tokens": 4096,
...
class BedrockModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        logger.info("Using BedrockModel for prompt")
        common_params = {
            "messages": messages,
            "temperature": temperature,
            "max_tokens": 4096,
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        logger.info("Using BedrockModel for stream_prompt")
        common_params = {
            "messages": messages,
            "temperature": temperature,
            "max_tokens": 4096,
...
class GeminiModel(BaseModel):
    async def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        logger.info("Using GeminiModel for prompt")
        common_params = {
            "messages": messages,
            "temperature": temperature,
            "max_tokens": 4096,
    ...
    async def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        logger.info("Using GeminiModel for stream_prompt")
        common_params = {
            "messages": messages,
            "temperature": temperature,
            "max_tokens": 4096,
...
class AIGateway:
    ...
    async def prompt(self, 
                     ...
                     top_p: float = 0) -> Dict:
        logger.info("Starting prompt method")
        tried_models = set()
        while len(tried_models) < len(self.MODEL_WEIGHTS):
            ...
            try:
                ...
                completion = model.prompt(messages, max_tokens, temperature, top_p)
                logger.info(f"Successfully received response from {model_type} model")
    ...
    async def coding_prompt(self, 
                     ...
                     temperature: float = 0.2, 
                     top_p: float = 0) -> Dict:
        logger.info("Starting coding_prompt method")
        tried_models = set()
        while len(tried_models) < len(self.CODING_MODEL_WEIGHTS):
            ...
            tried_models.add(model_type)
            try:
                logger.info(f"Attempting to use {model_type} model for coding")
                model = getattr(self, f"{model_type}_model")
                completion = model.prompt(messages, max_tokens, temperature, top_p)
                logger.info(f"Successfully received response from {model_type} model for coding")
                logger.info(completion)
                return completion
            except Exception as e:
                logger.error(f"Error in prompting {model_type} model for coding: {str(e)}")
        
        logger.error("All models failed to respond for coding prompt")
        raise Exception("All models failed to respond")
    ...
    async def stream_prompt(self, 
                     ...
                     temperature: float = 0.2, 
                     top_p: float = 0):
        logger.info("Starting stream_prompt method")
        tried_models = set()
        while len(tried_models) < len(self.STREAM_MODEL_WEIGHTS):
            ...
            tried_models.add(model_type)
            try:
                logger.info(f"Attempting to use {model_type} model for streaming")
                model = getattr(self, f"{model_type}_model")
                chat_completion_stream = model.stream_prompt(messages, max_tokens, temperature, top_p)
                logger.info(f"Successfully received streaming response from {model_type} model")
                final_response = ""
    ...
    def generate_image(self, prompt: str, size: str = "1024x1024"):
        logger.info("Starting image generation with DALL-E 3")
