...
class ImageAnalysAgent:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        prompt = (
            ...
            f"For each image mentioned in the user prompt:\n\n"
            f"1. **Title**: Use a clear #### heading with a concise title\n"
            f"2. **Description**: Provide a detailed, well-organized, and concise description\n"
            f"3. **Style Details**: Include specific information about:\n"
            f"   - Style\n"
            f"   - Dimensions\n"
            f"   - Colors\n"
            f"   - Elements\n"
            f"4. **Save Path**: MUST ALWAYS specify the full path for saving the image within {self.repo.get_repo_path()}\n"
            f"5. **Format**: Indicate the image format (PNG, JPG, or JPEG)\n\n"
            f"## Response Format\n\n"
            f"- Use clear headings (maximum size #### for h4) to organize your response\n"
            f"- Separate each image section clearly using a line of dashes (---------------------)\n"
            f"- Use appropriate spacing and markdown for readability\n"
