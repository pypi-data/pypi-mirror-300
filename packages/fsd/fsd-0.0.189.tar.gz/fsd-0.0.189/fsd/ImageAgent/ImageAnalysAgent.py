...
class ImageAnalysAgent:
    ...
    async def get_idea_plan(self, user_prompt):
        prompt = (
            ...
            f"1. Use a clear #### heading with a concise title\n"
            f"2. Provide a detailed description in a well-organized, concise manner\n"
            f"3. Include specific details about style, colors, and elements\n"
            f"4. MUST ALWAYS specify the full path for saving the image within {self.repo.get_repo_path()}\n"
            f"5. Indicate the image format (PNG, JPG, or JPEG)\n\n"
            f"Use clear headings (maximum size #### for h4) to organize your response. "
            f"Separate each image section clearly. Use appropriate spacing and markdown for readability."
            f"Only describe images explicitly mentioned in the user prompt. Do not add or invent additional images."
        )
