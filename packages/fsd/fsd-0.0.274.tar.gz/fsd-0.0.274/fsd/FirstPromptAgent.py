...
class FirstPromptAgent:
    ...
    async def get_prePrompt_plans(self, user_prompt):
        ...
        try:
            ...
            response = await self.repo.ai.prompt(messages, 4096, 0, 0)
            return self._parse_response(response)
        except Exception as e:
            logger.error(f"\n #### The `FirstPromptAgent` encountered an error during pre-prompt planning:\n{str(e)}")
            return {"reason": str(e)}

    def _create_messages(self, user_prompt):
        logger.debug("\n #### The `FirstPromptAgent` is constructing messages for the AI gateway")
        system_content = (
            ...
            "Return only a valid JSON response without any additional text, symbols, markdown formatting, or extra explanation. "
    ...
    def _parse_response(self, response):
        logger.debug("\n #### The `FirstPromptAgent` is parsing the AI gateway response")
        try:
            return json.loads(response.choices[0].message.content)
        except json.JSONDecodeError:
            logger.error("\n #### The `Receptionist Agent` encountered an error and is attempting to repair.")
            logger.info(f"DAMAGE RESPOND: {response.choices[0].message.content}")
