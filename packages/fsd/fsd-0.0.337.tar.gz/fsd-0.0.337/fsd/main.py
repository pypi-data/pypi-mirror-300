...
from fsd.Deployment import DeploymentCheckAgent  # Ensure this module is correctly imported and available
    ...
    try:
        ...
        while True:
            ...
            if user_prompt_json.startswith('/rollback'):
                ...
                continue
            user_prompt, name_subdomain, file_attachments, focused_files = parse_payload(user_prompt_json, project_path)

            if user_prompt == "deploy_to_server":
                check_result = await deploy.get_dependency_check_plans()
                result = check_result.get('result')
                if result == "0" or result == 0:
                    logger.info("\n #### This project is not supported to deploy now!")
                elif result == "1" or result == 1:
                    path = check_result.get('path')
                    if path != "null":
                        repo.deploy_to_server(path, name_subdomain)
                    else:
...
def parse_payload(user_prompt_json, project_path):
    try:
        ...
        return user_prompt, name_subdomain, file_path, tracked_file
    except:
