...
class GitRepo:
    ...
    def deploy_to_server(self, domain=DEFAULT_DOMAIN):
        try:
            # Zip all files in repo and upload to S3
            s3_key = deploy_zip_to_s3(self.repo_path)
            ...
            response = send_message_to_sqs(action, type, s3_uri, domain)

            if response.get('MessageId'):
                logger.info(f"Deployment message sent to SQS. MessageId: {response['MessageId']}")
            else:
                logger.warning("Message sent to SQS, but no MessageId was returned.")
