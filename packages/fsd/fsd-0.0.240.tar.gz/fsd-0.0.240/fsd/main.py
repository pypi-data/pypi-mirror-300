    ...
    try:
        ...
        while True:
            ...
            if user_prompt_json.startswith('/rollback'):
                ...
                continue
            user_prompt, file_attachments, focused_files = parse_payload(user_prompt_json)
            logger.info(user_prompt)
            logger.info(file_attachments)
            logger.info(focused_files)
            result = await get_prePrompt(user_prompt)
            pipeline = result['pipeline']
            if pipeline == "1":
                await explainer_controller.get_started(user_prompt, file_attachments)
