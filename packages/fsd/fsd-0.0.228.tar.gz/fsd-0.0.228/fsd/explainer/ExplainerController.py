...
class ExplainerController:
    ...
    async def get_prePrompt(self, user_prompt, file_attachments):
        """Generate idea plans based on user prompt and available files."""
        return await self.preprompt.get_prePrompt_plans(user_prompt, file_attachments)

    async def get_normal_answer(self, user_prompt, language, role,file_attachments):
        """Generate idea plans based on user prompt and available files."""
        return await self.normalExplainer.get_normal_answer_plans(user_prompt, language, role, file_attachments)

    async def get_file_answer(self, user_prompt, language, files, role, file_attachments):
        """Generate idea plans based on user prompt and available files."""
        return await self.mainExplainer.get_answer_plans(user_prompt, language, files, role, file_attachments)

    async def get_explaining_files(self, prompt, file_attachments):
        """Generate idea plans based on user prompt and available files."""
        return await self.fileFinder.get_file_plannings(prompt, file_attachments)
    ...
    async def get_started(self, user_prompt, file_attachments):
        logger.info("\n #### The `Director Support Agent` will now begin processing your request.")
        logger.info("\n-------------------------------------------------")

        prePrompt = await self.get_prePrompt(user_prompt, file_attachments)
        ...
        if pipeline == "1":
            ...
            file_result = await self.get_explaining_files(finalPrompt, file_attachments)
            working_files = file_result.get('working_files', [])
            await self.get_file_answer(finalPrompt, language, working_files, role, file_attachments)
        elif pipeline == "2":
            logger.debug("\n #### The `General Explainer Agent` is presently engaged in processing your query and formulating a comprehensive response.")
            await self.get_normal_answer(finalPrompt, language, role, file_attachments)
