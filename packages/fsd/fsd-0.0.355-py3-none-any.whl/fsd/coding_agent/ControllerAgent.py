...
class ControllerAgent:
        ...
        for step in plan.get('steps', []):
            ...
            if self.is_coding_file(file_name):
                ...
                if main_path:
                    ...
                    logger.info("-------------------------------------------------")
                    logger.info(f"#### The `{techStack.upper()} Agent` is processing file: `{os.path.basename(file_name)}`.")
    ...
    async def regular_code_task_pipeline(self, finalPrompt, role, original_prompt_language):
        ...
        if crawl_plan:
            for step in crawl_plan.get('crawl_tasks', []):
                ...
                if crawl_url:
                    logger.debug(f" #### The `crawler agent` is reading: `{crawl_url}`")
                    result = self.crawler.process(crawl_url, crawl_format)
                    logger.debug(f" #### The `crawler agent` has finished reading: `{crawl_url}`")
                    crawl_logs.append({
                        'url': crawl_url,
                        'result': result
                    })
    ...
    async def get_started(self, user_prompt):
        ...
        logger.info(" #### The `Director Action Agent` reported that all tasks are completed!")
        logger.info("-------------------------------------------------")
