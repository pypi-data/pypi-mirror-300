...
class ImageAnalysAgent:
    ...
    async def get_idea_plan(self, user_prompt, original_prompt_language):
        prompt = (
            ...
            f"For each image mentioned in the user prompt, respond using a nicely formatted markdown table:\n\n"
            f"| Attribute | Details |\n"
            f"|-----------|--------|\n"
            f"| Title | Use a clear, concise title |\n"
            f"| Description | Provide a detailed, well-organized, and concise description |\n"
            f"| Style | Specify the image style |\n"
            f"| Dimensions | Indicate the image dimensions |\n"
            f"| Colors | List the main colors used |\n"
            f"| Elements | Describe key elements in the image |\n"
            f"| Save Path | MUST ALWAYS specify the full path for saving the image within {self.repo.get_repo_path()} |\n"
            f"| Format | Indicate the image format (PNG, JPG, or JPEG) |\n\n"
            f"## Response Format\n\n"
            f"- Use the table format for each image\n"
