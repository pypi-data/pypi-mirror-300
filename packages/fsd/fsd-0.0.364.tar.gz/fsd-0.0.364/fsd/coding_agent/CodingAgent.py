...
class CodingAgent:
    ...
    def initial_setup(self, context_files, instructions, context, role):
        ...
        self.conversation_history = [
            ...
            {"role": "user", "content": f"These are your high-level instructions: {instructions}"},
            {"role": "assistant", "content": "Understood. I'll proceed with the given instructions."},
    ...
    async def get_coding_request(self, is_first, file, techStack, crawl_logs, prompt):
        ...
        if is_svg:
            user_prompt = (
                f"As a world-class SVG designer, create the following SVG with utmost precision: "
                f"Implement the SVG image on {file_name}:\n"
                f"Image description: {prompt}\n"
                "NOTICE: Your response should ONLY contain SEARCH/REPLACE blocks for SVG code changes. Nothing else is allowed."
            )
        else:
            user_prompt = (
                f"As a world-class, highly experienced {techStack} developer, implement the following task with utmost efficiency and precision: "
                f"{lazy_prompt}\n"
                f"Work on {file_name}:\n{prompt}\n"
            )

            if crawl_logs:
                user_prompt += f"Additional context to support this task:\n{crawl_logs}\n"
