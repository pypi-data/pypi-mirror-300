...
class BaseModel:
    ...
    def prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float) -> Dict:
        raise NotImplementedError

    def stream_prompt(self, messages: List[Dict[str, str]], max_tokens: int, temperature: float, top_p: float):
        raise NotImplementedError
...
class AIGateway:
    ...
    def prompt(self, 
        ...
        while len(tried_models) < len(self.MODEL_WEIGHTS):
            ...
            try:
                ...
                completion = self._execute_with_timeout(model.prompt, messages, max_tokens, temperature, top_p)
                ...
                time.sleep(1)  # Add a small delay before trying the next model
    ...
    def coding_prompt(self, 
        ...
        raise Exception("All models failed to respond for coding prompt")
    ...
    def stream_prompt(self, 
        ...
        while len(tried_models) < len(self.STREAM_MODEL_WEIGHTS):
            ...
            try:
                model = getattr(self, f"{model_type}_model")
                chat_completion_stream = self._execute_with_timeout(model.stream_prompt, messages, max_tokens, temperature, top_p)
        ...
        raise Exception("All models failed to respond for streaming")

    def generate_image(self, prompt: str, size: str = "1024x1024"):
        dalle_model = Portkey(api_key=self.API_KEY, virtual_key=self.VIRTUAL_KEYS["dalle3"], config=self.CONFIG_ID)
        try:
            image = self._execute_with_timeout(dalle_model.images.generate, prompt=prompt, size=size)
