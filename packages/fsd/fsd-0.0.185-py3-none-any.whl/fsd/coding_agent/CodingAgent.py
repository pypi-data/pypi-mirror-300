...
class CodingAgent:
    ...
    async def get_coding_request(self, is_first, file, techStack, crawl_logs, prompt):
        ...
        if is_svg:
            ...
            user_prompt = (
                f"You're a top-notch {techStack} dev with tons of experience. Your job is to:\n\n"
                f"{'Start' if is_first else 'Keep working'} on {file_name}, showing off your {techStack} skills:\n"
                f"1. Stick to {techStack} best practices, write great code.\n"
                "2. Follow instructions carefully to nail the main goal.\n"
                f"{lazy_prompt}\n"
                "3. Use existing code patterns to keep things consistent and scalable.\n"
                "4. For UI stuff, make awesome layouts that are easy to use.\n"
                "5. Create a design that wows users and gives them a great experience.\n"
                "6. Handle errors well, log everything, and validate inputs for a stable system.\n"
                "7. Make it fast and efficient, avoiding memory leaks.\n"
                "8. Write clean, easy-to-read code that follows style guides.\n"
                "9. Add tests to make sure everything works right.\n"
                "10. Think about edge cases and future needs when designing.\n\n"
                "Remember: Your code should be top-notch, showing your skills and attention to detail.\n\n"
                "IMPORTANT: Only use SEARCH/REPLACE blocks for code changes. No extra text. Make sure your code works and is ready to go."
            )
