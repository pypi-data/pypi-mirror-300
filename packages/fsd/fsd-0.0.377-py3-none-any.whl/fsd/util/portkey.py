...
class AIGateway:
    ...
    async def coding_prompt(self, 
        ...
        try:
            ...
            completion = await self.bedrock_model.prompt(messages, max_tokens, temperature, top_p)
            logger.debug("Successfully received response from bedrock model for coding")
            return completion
        except Exception as e:
            logger.error(f"Error in prompting bedrock model for coding: {str(e)}")
            raise Exception("Bedrock model failed to respond for coding prompt")
