# Script for plotting the final results of the double mutant cycle analysis
...
def make_graph(df, m_name="G", s_name=None):
    ...
    print(df)
    els = {}
    edges = [("WT", "R224A"), ("WT", "R75A"), ("WT", "DM"), ("R224A", "DM"), ("R75A", "DM")]
    G = nx.Graph()
    weights = []
    for i in range(0, 5):
        e1 = edges[i][0]
...
path = "Z:\\Group Share\\Hiruni Jayasekera\\HSJ_MAE AqpZ CL\\Final2\\"
os.chdir(path)
...
for name in names:
    ...
    for j in range(0, len(temps)):
        ...
        for i in range(0, 6):
            ...
            p = {"WT": [0, 1], "DM": [1, 0], "R224A": [1, 1], "R75A": [0, 0]}
