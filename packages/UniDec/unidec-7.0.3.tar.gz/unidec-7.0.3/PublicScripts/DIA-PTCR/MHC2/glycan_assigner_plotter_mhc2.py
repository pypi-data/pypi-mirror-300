...
pfile = "MHC2_DPA_Combined"

peaks = np.loadtxt(pfile + "_peaks.txt", delimiter=",", usecols=(0, 1))
peaks[:, 1] = peaks[:, 1] / np.sum(peaks[:, 1])

efile = pfile + "_tryp_glycan_list_abridged_matches_brute_force0.xlsx"
statsfile = pfile + "_tryp_glycan_list_abridged.csv"

sdf = pd.read_csv(statsfile)

sites = np.arange(0, 4, 1) + 1
sitelabels = sites.astype(str)
...
df2.to_excel(pfile+"_stats_array_barcode0.xlsx")

df2 = pd.DataFrame(parray.transpose(), columns=unigly)
df2.to_excel(pfile+"_prob_array_barcode0.xlsx")

df2 = pd.DataFrame(earray.transpose(), columns=unigly)
df2.to_excel(pfile+"_equal_array_barcode0.xlsx")

#np.savetxt(pfile+"_stats_array_barcode3.txt", sarray)
#np.savetxt(pfile+"_prob_array_barcode3.txt", parray)

plt.savefig(pfile + "_barcode_triple_0.pdf")
plt.savefig(pfile + "_barcode_triple_0.png")
plt.show()
