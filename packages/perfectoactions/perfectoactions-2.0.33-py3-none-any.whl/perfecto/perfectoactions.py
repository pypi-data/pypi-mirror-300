...
def get_url(resource, resource_id, operation):
    ...
    if "eyJhb" in token:
        query = urllib.parse.urlencode(
            {"operation": operation, "securityToken": token})
    else:
        if ":" not in token:
            raise Exception(
...
def getTimeofExecution(response, pattern1, pattern2):
    ...
    for match in matches:
        match_item = str(re.findall(pattern2, match.group()))
        match_item = match_item.replace('timestamp[0]=', "").replace(
...
def start_exec():
    ...
    exec_id = getregex_output(
        response, r"executionId\"\:\"[\w\d@.-]+\"", ':".*$')
    return exec_id
...
def get_reservations_list_response(resource, command, end, admin):
    ...
    url += "&startTime=" + \
        str(int((time.mktime(datetime.datetime.now().timetuple()))) * 1000)
    if end != "":
        endTime = datetime.datetime.fromtimestamp(int(time.mktime(
            datetime.datetime.now().timetuple()))) + datetime.timedelta(days=int(end))
...
def create_reservation(resource, resource_id, operation):
    ...
    endTime = int((datetime.datetime.fromtimestamp(startTime) +
                  timedelta(minutes=reserve_time)).timestamp())
    if "eyJhb" in token:
        query = urllib.parse.urlencode({"operation": operation, "securityToken": token,
                                       "resourceIds": resource_id, "startTime": startTime*1000, "endTime": endTime*1000})
    else:
        if ":" in token:
            user = token.split(":")[0]
            pwd = token.split(":")[1]
    ...
    try:
        ...
        if "green" in color:
            ...
            if "True" in reserve:
                ...
                try:
                    response = send_request(create_reservation(
                        RESOURCE_TYPE_RESERVATIONS, device_id, "create"))
                    decoded = response.read().decode("utf-8")
                    # print(decoded)
                    desc = "Reserved"
                    status += "RS:OK;"
        ...
        if "true" in start_execution.lower():
            ...
            if "True" in cleanup:
                if not "iOS" in osDevice:
                    print("cleaning up: " + model +
                          ", device id: " + device_id)
                    try:
                        output, temp = exec_command(
                            EXEC_ID, device_id, "device", "clean")
                        status += "clean:" + output
                    except Exception as e:
                        print(e)
                        status += "clean:Failed!"
            ...
            if "True" in reboot:
                if all(
                    ...
                    try:
                        output, timeofExec = exec_command(
                            EXEC_ID, device_id, "device", "reboot")
                ...
                try:
                    ...
                    if tempstatus.count(",") == 2 or tempstatus.count("  ") == 2:
                        if (tempstatus.count("  ") == 2):
                            networkstatus = ', e'.join(
                                str(tempstatus).split("  "))
                        else:
    ...
    if get_handset_count(RESPONSE) > 0:
        ...
        if len(device_list) > 0:
            ...
            pool = multiprocessing.Pool(
                processes=pool_size, maxtasksperchild=2)
            try:
                print(
                    "\nFound " + str(len(device_list)) +
                    " devices with status: " + desc
                )
                sys.stdout.flush()
...
def fetch_details(i, exp_number, result, exp_list):
    ...
    if i == exp_number:
        if "=" in result:
            exp_list = exp_list.append(result.split(
                "=", 1)[1].replace("'", "").strip())
        else:
...
def create_summary(df, title, column, name):
    ...
    try:
        ...
        ax1.legend(labels=status, prop={'size': 7},
                   bbox_to_anchor=(1, 0.5), loc="best")
        # plot table
        ...
        tbl.set_fontsize(8)
        encoded = fig_to_base64(os.path.join(
            TEMP_DIR, "results", name + ".png"))
        summary = '<img src="data:image/png;base64, {}"'.format(
            encoded.decode("utf-8"))
    except Exception as e:
        print("API: " + df.url + " throws " + df.msg +
              ". Raise a support case to get API access for fetching user list/ device list.")
        summary = '<div style="color: white;">' + df.msg + \
            '. Raise a support case to get API access for fetching user list/ device list.</div><img '
    return summary
...
def prepare_graph(df, column):
    ...
    if column in df.columns:
        ...
        ax = (
            ...
            .value_counts()
            .sort_index()
            .plot(kind="bar", fontsize=12, stacked=False, colormap='Paired', figsize=(20, 10), ylim=(0, 2))
        )
    ...
    if "true" in os.environ["PREPARE_ACTIONS_HTML"]:
        ...
        <html lang="en">
            ...
            * {{
            ...
            #download, #download2 {{
                background-color: #333333;
                border: 3px solid  rgba(82, 198, 156, 0.68);
                color: white;
                font-size: 13px;
                cursor: pointer;
                display:none;
                    ...
                    <div>
                        <div class="tabbed">
                            ...
                            <div  align="center">
                                ...
                                <a href="https://""" + cloudname.upper() + """.perfectomobile.com" target="_blank" class="site-logo">
                                <img id="logo" src=""" + os.environ["company_logo"] + """ style="margin:1%;" alt="Company logo" ></a> """
            + summary + """ alt='summary' id='summary' onClick='zoom(this)'></img> </br></p></div>
            ...
            + """ alt="Description" class="mySlides"  onClick='zoom(this)' id="slide">
                    ...
                    <a target="_blank" style="font-size:12;font-family:sans-serif;color:powderblue;" href="https://clearbit.com">Logos provided by Clearbit</a>
                </div>
                <script>
                var myIndex = 0;
                carousel();
...
def send_request_repo(url, content):
    try:
        ...
        response = urllib.request.urlopen(quote(url, safe=":/?=&").replace(" ", "%20"), timeout=30)
    except Exception as e:
        return e
    sys.stdout.flush()
    return response
    ...
    if not (str(actualDate) == ""):
        ...
        if actualDate < expectedDate:
            ...
            if DELETE.lower() == "true":
                map = sendAPI(
                    os.environ["repo_resource_type"], value, "delete")
                print("Delete API output of item: " +
                      str(value) + ":\n " + str(map))
                status = map["status"]
                if status != "Success":
                    FINAL_LIST.append(
                    ...
                    raise RuntimeError(
                        "Repository item " + value + " was not deleted. API Response:" + str(map))
                    sys.exit(-1)
                else:
                    FINAL_LIST.append(
                        "File:"
...
def manage_repo(resource_key):
    ...
    try:
        itemList = map["items"]
        if os.environ["repo_build_only"] == "true":
            itemList = [x for x in itemList if x.startswith(
                resource_key) and re.compile(".ipa$|.apk$|.zip$|.apks$").search(x)]
        else:
    ...
    repo_folder_pool = multiprocessing.Pool(
        processes=pool_size, maxtasksperchild=2)
    try:
...
def deleteOlderFiles(resource_type, delete, admin, repo_paths, days, build_only):
    ...
    os.environ["repo_build_only"] = build_only
        ...
        if lists is not None:
            if len(lists) > 0:
                ...
                for result in new_result:
                    ...
                    if "File:" in result:
                        fetch_details_repo(
                            i, new_result.index(result, i), result, file)
                    ...
                    if "Days:" in result:
                        fetch_details_repo(
                            i, new_result.index(result, i), result, days)
                    i = i + 1
    pandas.set_option("display.max_columns", None)
    pandas.set_option("display.max_colwidth", 100)
    pandas.set_option("colheader_justify", "center")
...
def reserve_limiter(resource_type, limit, list, end, admin, skip_list, include_list, delete):
    ...
    if count > 0:
        ori_df = pandas.DataFrame([flatten_json(
            x) for x in executions['reservations']], index=range(count), columns=cols)
        ori_df[startTime] = pandas.to_datetime(ori_df[startTime], unit="ms")
        ori_df[startTime] = (
            ori_df[startTime].dt.tz_localize(
                "utc").dt.tz_convert(tzlocal.get_localzone())
        )
        ori_df[startTime] = ori_df[startTime].dt.strftime("%d/%m/%Y %H:%M:%S")
        ori_df[endTime] = pandas.to_datetime(ori_df[endTime], unit="ms")
        ori_df[endTime] = (
            ori_df[endTime].dt.tz_localize(
                "utc").dt.tz_convert(tzlocal.get_localzone())
        )
                        ...
                        if (temp_df['counter'].iloc[0] > (int(str(limit)))):
                            ...
                            while (new_df.shape[0] > 0):
                                time_range = DateTimeRange(
                                    new_df[startTime].iloc[0],  new_df[endTime].iloc[0])
                                count = 0
                                if (new_df.shape[0] > 0):
                                    df = df.reset_index(drop=True)
                                    for index, row in df.iterrows():
                                        x = DateTimeRange(
                                            df[startTime].iloc[index], df[endTime].iloc[index])
                                ...
                                if (count > (int(str(limit)))):
                                    reservation_id = str(
                                        new_df[reservationid].iloc[0])
                                    if (new_df[user].iloc[0] not in skip_list.split(",")):
                                        if (str(delete).lower() == "true"):
                                            # delete each reservation
                                            print("id: " + reservation_id +
                                                  " will be deleted now.")
                                            map = deleteReserveAPI(
                                                RESOURCE_TYPE_RESERVATIONS, reservation_id, "delete", "true")
                                            print("Delete API output of id: " +
                                                  reservation_id + "\n " + str(map))
                                            try:
                                                ...
                                                if ("Success" in delete_status):
                                                    ...
                                                    raise Exception(
                                                        "Unable to delete reservation: " + reservation_id + " Response: " + map.items())
                                            except Exception as e:
                                                raise Exception(
                                                    "Unable to delete reservation: " + reservation_id + " Response: " + map['errorMessage'])
                                        deleted_df.loc[i] = new_df.iloc[0]
                                        i = i+1
                                        ori_df.drop(ori_df[ori_df[reservationid] == reservation_id].head(
                                            1).index, inplace=True)
                                    df.drop(df[df[reservationid] == reservation_id].head(
                                        1).index, inplace=True)
                                    new_df.drop(new_df[new_df[reservationid] == reservation_id].head(
                                        1).index, inplace=True)
                                else:
                                    break
                            temp_df.drop(temp_df[temp_df['time'] == new_startTime].head(
                                1).index, inplace=True)

        print("\n remaining  reservations:\n")
        ori_df[endTime] = pandas.to_datetime(
            ori_df[endTime], format="%d/%m/%Y %H:%M:%S")
        # increasing a second to match as per reservation UI
        ...
        if (deleted_df.shape[0] > 0):
            ...
            deleted_df.columns = ['Reserved To', 'Device ID',
                                  'Reservation Id',	'Start time', 'End time']
    ...
    try:
                ...
                try:
                    ...
                    try:
                        if (int(reserve.split(":")[0].split(";")[0])):
                            os.environ["RESERVE_TIME"] = reserve.split(":")[
                                0].split(";")[0]
                    ...
                    raise Exception(
                        "Begin with reserve parameter with a colon seperator followed by reservation time in minutes. Minimum default time is 15 minutes. E.g. reserve:15 ")
            if "reserve_limit" in args["actions"]:
                ...
                try:
                    ...
                    if (int(reserve_limit.split(":")[0].split("|")[0].split(";")[0])):
                        os.environ["reserve_limit_user"] = reserve_limit.split(":")[0].split("|")[
                            0].split(";")[0]
                        print("reserve limit user: " +
                              os.environ["reserve_limit_user"])
                except:
                    raise Exception(
                        "Begin with reserve_limit parameter with a colon seperator followed by reservation limit. E.g: reserve_limit:2 will limit reservations to 2 per user at any point in time.")
                try:
                    if (len(reserve_limit.split(":")[0].split("|")) > 1):
                        if (str(reserve_limit.split(":")[0].split("|")[1].split(";")[0]).lower() == "true"):
                            delete = reserve_limit.split(":")[0].split("|")[
                                1].split(";")[0]
                        else:
                            delete = "false"
                    else:
                    ...
                    raise Exception(
                        "Provide true to delete the reservations. default is false. E.g: reserve_limit:2|true will delete reservations to 2 per user at any point in time.")
                try:
                    if (len(reserve_limit.split(":")[0].split("|")) > 2):
                        os.environ["reserve_limit_skip"] = reserve_limit.split(":")[0].split("|")[
                            2].split(";")[0]
                        print("reserve limit skip users: " +
                ...
                try:
                    if (len(reserve_limit.split(":")[0].split("|")) > 3):
                        include_list = reserve_limit.split(":")[0].split("|")[
                            3].split(";")[0]
                ...
                try:
                    if (len(reserve_limit.split(":")[0].split("|")) > 4):
                        os.environ["reserve_limit_days"] = reserve_limit.split(":")[0].split("|")[
                            4].split(";")[0]
                        print("reserve limit days: " +
                              os.environ["reserve_limit_days"])
                    else:
                        os.environ["reserve_limit_days"] = ""
                except:
                    raise Exception(
                        "Provide optional reserve limit in days. E.g: reserve_limit:2|true|||4 will limit reservations to 2 per user beginning now to within the next 4 days")

                delete_reserve_df = reserve_limiter(RESOURCE_TYPE_RESERVATIONS, os.environ["reserve_limit_user"], "list", os.environ[
                                                    "reserve_limit_days"], "true", os.environ["reserve_limit_skip"], include_list, delete)
        ...
        if "NA" != clean_repo:
            try:
                ...
                # This is to check if extra arguments with ; are added to the -a command
                if ";" in str(clean_repo_var[4]):
                    day = str(clean_repo_var[4]).split(";")[0]
                else:
                    day = str(clean_repo_var[4])
                build_only = "false"  # By default, don't delete build files
                if len(clean_repo_var) > 5:
                    if ";" in str(clean_repo_var[5]):
                        build_only = str(clean_repo_var[5]).split(";")[0]
                    else:
                        build_only = clean_repo_var[5]
        ...
        try:
            result = devlist.apply_async(
                get_xml_to_xlsx, [RESOURCE_TYPE,
                                  "list", "get_devices_list.xlsx"]
            )
        except Exception:
            devlist.close()
            print(traceback.format_exc())
        ...
        user_html = get_json_to_xlsx(
            RESOURCE_TYPE_USERS, "list", 'get_users_list.xlsx')
        get_dev_list = []
        if args["device_status"]:
            os.environ["device_status"] = " with " + \
                args["device_status"].lower() + " status"
        ...
        if (bool):
            if str(args["device_status"]).lower() in ["disconnected", "notavailable", "unavailable"]:
                print("There are some devices which are " +
                      args["device_status"].lower())
                sys.exit(-1)
