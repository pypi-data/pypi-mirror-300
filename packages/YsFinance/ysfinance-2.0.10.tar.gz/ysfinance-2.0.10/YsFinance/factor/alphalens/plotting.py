...
def sd_plot_quantile_statistics_table(factor_data):
    quantile_stats = factor_data.groupby('factor_quantile') \
        .agg(['min', 'max', 'mean', 'std', 'count'])['factor']
    quantile_stats['daily_count'] = np.floor(quantile_stats['count']
                                             / len(factor_data.index.get_level_values('date').unique()))
    quantile_stats['count %'] = np.floor(quantile_stats['count']
                                         / quantile_stats['count'].sum() * 10000)/100

    print("FactorValues Quantiles Statistics")
    utils.print_table(quantile_stats)


def sd_plot_quantile_returns_bar(mean_ret_by_q,
                                 by_group=False,
                                 ylim_percentiles=None,
                                 ax=None):
    mean_ret_by_q = mean_ret_by_q.copy()
    ...
    if ylim_percentiles is not None:
        ymin = (np.nanpercentile(mean_ret_by_q.values,
                                 ylim_percentiles[0]) * DECIMAL_TO_PCT)
        ymax = (np.nanpercentile(mean_ret_by_q.values,
                                 ylim_percentiles[1]) * DECIMAL_TO_PCT)
    else:
        ymin = None
        ymax = None
    ...
    if by_group:
        ...
        for a, (sc, cor) in zip(ax, mean_ret_by_q.groupby(level='group')):
            (cor.xs(sc, level='group')
                .multiply(DECIMAL_TO_PCT)
                .plot(kind='bar', title=sc, ax=a))

            a.set(xlabel='', ylabel='Mean Return (%)',
                  ylim=(ymin, ymax))
        ...
        (mean_ret_by_q.multiply(DECIMAL_TO_PCT)
            .plot(kind='bar',
                  title="Annualized Return By Factor Quantile", ax=ax))
        ax.set(xlabel='', ylabel='Annualized Return (%)',
               ylim=(ymin, ymax))

        return ax


def sd_plot_cumulative_returns_by_quantile(quantile_returns,
                                           period,
                                           longshort=True,
                                           ax=None):
    if ax is None:
    ...
    if longshort:
        cum_ret = ret_wide.cumsum()
        cum_ret = cum_ret * DECIMAL_TO_PCT
    else:
        cum_ret = ret_wide.apply(perf.cumulative_returns)
    ...
    ax.legend()
    ymin, ymax = cum_ret.min().min(), cum_ret.max().max()
    ax.set(ylabel=('Cumulative Returns %' if longshort else 'Cumulative NetValue'),
           title=('''LongShort Cumulative Return by Quantile
                    ({} Period Forward Return)'''.format(period)
                  if longshort else
                  '''LongOnly Cumulative Return by Quantile
                           ({} Period Forward Return)'''.format(period)),
           xlabel='',
           #          yscale='linear' if longshort else 'symlog',
           yticks=np.linspace(ymin, ymax, 5),
           ylim=(ymin, ymax))
...
def sd_plot_cumulative_returns_spread(factor_returns,
                                      period,
                                      title=None,
                                      ax=None):
    if ax is None:
        f, ax = plt.subplots(1, 1, figsize=(18, 6))
        factor_returns = factor_returns.cumsum()

        factor_returns.plot(ax=ax, lw=3, alpha=0.6)
        ax.set(ylabel='Cumulative Returns',
               title=("Spread Cumulative Return and ReturnAE ({} Fwd Period)".format(period)
                      if title is None else title),
               xlabel='')
    return ax

def sd_plot_cumulative_excessreturn_by_quantile(quantile_returns,
                                                   period,
                                                   ax=None):
    if ax is None:
        f, ax = plt.subplots(1, 1, figsize=(18, 6))
    ...
    ax.legend()
    ymin, ymax = cum_ret.min().min(), cum_ret.max().max()
    ax.set(ylabel=('Cumulative ExcessReturns %'),
           title=('''Cumulative ExcessReturns by Quantile
                    ({} Period Forward Return)'''.format(period)
                ),
           xlabel='',
           #          yscale='linear' if longshort else 'symlog',
...
def sd_plot_cumulative_excessreturn(factor_returns,
                                    period,
                                    title=None,
                                    ax=None):
    if ax is None:
        ...
        factor_returns.plot(ax=ax, lw=3, alpha=0.6)
        ax.set(ylabel='Cumulative Returns',
               title=("Cumulative ExcessReturn and ReturnAE ({} Fwd Period)".format(period)
                      if title is None else title),
               xlabel='')
    return ax