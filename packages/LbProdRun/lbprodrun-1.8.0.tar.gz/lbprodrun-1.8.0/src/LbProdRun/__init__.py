...
def execute(job_spec, dry_run=False, interactive=False, prmon=False):
    ...
    if isinstance(job_spec.application, JobSpecV1.FullDevApplication):
        ...
        and _is_lbconda_app(job_spec.application.name, job_spec.application.version)
        is not None
    ):
        env_name, env_version = _is_lbconda_app(
            job_spec.application.name, job_spec.application.version
        )
...
def _is_new_style(name: str, version: str) -> bool:
    ...
    if _is_lbconda_app(name, version) is not None:
        return True
