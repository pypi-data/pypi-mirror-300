...
class SearchSettingsDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent, session):
        ...
        self.import_dialog.setFileMode(QtWidgets.QFileDialog.ExistingFile)
        self.import_dialog.setNameFilter('JSON file (*.json, *.JSON)')
        self.import_dialog.setViewMode(QtWidgets.QFileDialog.List)
        self.export_dialog = QtWidgets.QFileDialog()
        self.export_dialog.setWindowTitle('Save filter template')
        self.export_dialog.setFileMode(QtWidgets.QFileDialog.AnyFile)
        self.export_dialog.setAcceptMode(QtWidgets.QFileDialog.AcceptSave)
        self.export_dialog.setNameFilter('JSON file (*.json, *.JSON)')
        self.export_dialog.setViewMode(QtWidgets.QFileDialog.List)
    ...
    def _on_build_index_button_clicked(self, checked=False):
        if self.confirm_dialog.exec_() == QtWidgets.QMessageBox.Yes:
            self.session.cmds.Search.rebuild_project_index(self.project_combobox.currentText())
