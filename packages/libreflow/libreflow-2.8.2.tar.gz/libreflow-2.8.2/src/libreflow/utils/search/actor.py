...
class Rebuild_Project_Index(Cmd):
    ...
    '''
    def _decode(self, project_name):
        self._project_name = project_name

    def _execute(self):
        self.actor().crawl_project(self._project_name)
...
class Search(Actor):
    ...
    def crawl_project(self, project_name, filters=None):
        '''
        ...
        for f in filters:
            for oid in self.glob_project(project_name, f['filter'], f.get('start_oid'), f.get('max_depth', -1)):
                try:
                    self._create_entry(oid)
                except pymongo.errors.DuplicateKeyError:
                    self.session().log_debug(f'Search: entry {oid} already exists')
                else:
                    self.session().log_info(f'Search: new entry {oid} added to index')
    
    def glob_project(self, project_name, oid_filter, start_oid=None, max_depth=-1):
        '''
        ...
        if start_oid is None:
            ...
            oid_filter = start_oid+'/'+oid_filter
        
        return self._glob(start_oid, oid_filter, 0, max_depth)
    ...
    def _glob(self, oid, oid_filter, depth, max_depth=-1):
        matches = []

        if re.fullmatch(oid_filter, oid) is not None:
            matches.append(oid)
        ...
        for child_oid in self._ls(oid):
            matches += self._glob(child_oid, oid_filter, depth + 1, max_depth)
    ...
    def _ls(self, oid):
        ...
        try:
            ...
            child_oids  = [
                child_oid for child_oid, _, _, _, _, _ in from_relations[0]
