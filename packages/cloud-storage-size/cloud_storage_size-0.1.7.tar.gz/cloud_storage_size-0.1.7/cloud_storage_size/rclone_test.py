...
def __check_installed(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not is_installed():
            logger.debug("rclone is not installed. Installing...")
            __install_rclone()
        logger.debug("rclone is installed.")
...
def ls(remote: str) -> str:
    ...
    return _run_rclone_cmd(command)

if __name__ == "__main__":
    logging.basicConfig(level=logging.DEBUG)
    r = ls(":google cloud storage,service_account_file=/data1/yingcai/video_data/credentials/gcs.json,project_number=tputryout,location=us-central1,object_acl=private,bucket_acl=private:haiper_dataset_youtube")
    print(r)
