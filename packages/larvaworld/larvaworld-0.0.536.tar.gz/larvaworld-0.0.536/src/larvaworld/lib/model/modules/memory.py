...
class RLmemory(Memory):
    ...
    state_spacePerSide = PositiveInteger(0,
    ...
    gain_space = param.List(default=[-300.0, -50.0, 50.0, 300.0], item_type=float,
    ...
    def __init__(self, **kwargs):
        ...
        self.state_space = np.array(
            [ii for ii in itertools.product(range(2 * self.state_spacePerSide + 1), repeat=n)])
    ...
    def update_q_table(self, state, reward):
        self.q_table[self.lastState, self.lastAction] = (1 - self.alpha) * self.q_table[
            self.lastState, self.lastAction] + self.alpha * (reward + self.gamma * np.max(self.q_table[state]))
