...
def cdc_replication_process(dlt, spark,  table_name: str, files_path: str, bucket_name: str, cdc_data_list_maps, bronze_directory: str,time_diff_history_cdc_timestamp=30):

    spark = SparkSession.builder.appName(f"DLT Pipeline {cdc_data_list_maps[0]['source_schema']}").getOrCreate()
    spark.conf.set("pipelines.tableManagedByMultiplePipelinesCheck.enabled", "false")

    # Base code pipeline: streaming process
    for table in cdc_data_list:
...
def create_bronze_table_definition(spark, dlt, table_name: str, files_path: str, file_format: str, partitions: dict, schema_exclude_columns: list, keys: list, time_diff_history_cdc_timestamp: int):
    @dlt.table(
