...
def create(config, plandir, cluster, overrides, dnsconfig=None):
    ...
    if dedicated_etcd:
        ...
        if not extra_disks:
            warning("Not setting dedicated_etcd because no extra disks were specified for ctlplane nodes")
        else:
            ...
            if config.type == 'vsphere' or isinstance(extra_disk, dict) and extra_disk.get('interface', '') == 'scsi':
                ...
                disk = 'vdb'
            etcdfile = config.process_inputfile(cluster, f"{plandir}/98-var-etcd.yaml", overrides={'disk': disk})
            with open(f"{clusterdir}/openshift/98-var-etcd.yaml", 'w') as _f:
                _f.write(etcdfile)
