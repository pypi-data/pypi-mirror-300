...
async def draw_image(scores_total,player_total,old_data,cache_dir,cache_file,SS = False):
    ...
    start_x_offset = background_offset_x = start_background_offset_x = 100
    start_y_position = background_offset_y = 500
    ...
    for total_song in song_image_urls:
        ...
        if song_image:
            ...
            background_offset_x += song_background_length + 200
    ...
    for name in scores_total['song_name']:
        ...
        bs_draw.text(((position[0] + image_size[0] + 18), position[1] - 25), name, font=font_song, fill=(144, 238, 255))
        i += 1
    ...
    for song_pp in scores_total['song_pp']:
        ...
        background_image.paste(copy_pp_icon, (position[0] + image_size[0] + 10, position[1] + pp_icon_size[1] + 15), copy_pp_icon)
        # PP信息
        pp_size = 85
        font_pp = loading_font.font_loader(font_size = pp_size)
        bs_draw.text((position[0] + image_size[0] + pp_icon_size[0] + 10, position[1] + 50), str(f'{song_pp:.2f}'), font=font_pp, fill=(255, 105, 180))
        i += 1
    ...
    for song_weight in scores_total['song_weight']:
        ...
        bs_draw.text((position[0] + image_size[0] + 20, (int(position[1]) + (int(name_size) + int(pp_size)) // 2) + 40), str(pp_weight), font=font_weight, fill=(255, 255, 255))
        i += 1
...
#    open_acc_rank_s = Image.open(f'{Path(__file__).parent}/static/rank/s.png').convert('RGBA')
    ...
    for accuracy in scores_total['song_accuracy']:
        ...
        background_image.paste(copy_acc_icon, (position[0] + image_size[0] + 10, (int(position[1]) + (int(name_size) + int(pp_size) + int(weight_size)) // 2) + 70), copy_acc_icon)
        # 准度
        accuracy = round(float(accuracy * int(100)), 2)
        accuracy_size  = 80
        accuracy_font = loading_font.font_loader(font_size = accuracy_size)
        bs_draw.text((position[0] + image_size[0] + acc_icon_size[0] + 10, (int(position[1]) + (int(name_size) + int(pp_size) + int(weight_size)) // 2) + 50), (str(accuracy) + '%'), font=accuracy_font, fill=(255, 255, 255))
        i += 1
        '''
        # 准度评级
    ...
    if SS != True:
        for improve_acc in scores_total['improve_acc']:
            ...
            bs_draw.text((position[0] + image_size[0] + 20, (int(position[1]) + (int(name_size) + int(pp_size) + int(weight_size) + int(accuracy_size)) // 2) + 80), str( '(+' + str(improve_acc) + '%)'), font=improve_acc__font, fill=(144, 238, 144))
            i += 1
    else:
    ...
    for song_id in scores_total['song_id']:
        position = calculation.calculate_position(start_y_position = start_y_position + (crop_size[3] - crop_size[1]),record = i,change = crop_size[2])
        # id图标
        copy_id_icon = open_id_icon.copy()
        id_icon_size = (80, 80)
        copy_id_icon = copy_id_icon.resize(id_icon_size)
        background_image.paste(copy_id_icon, (position[0], int(position[1]) + (image_size [1] - (crop_size[3] - crop_size[1]))), copy_id_icon)
        # 删除掉api响应中莫名其妙的xxx,确保map id是正确的
        song_id = song_id.replace('x', '')
        id_size = 90
        id_font = loading_font.font_loader(font_size = id_size)
        bs_draw.text((position[0] + id_icon_size[0], int(position[1]) + (image_size [1] - (crop_size[3] - crop_size[1])) - 20), str(song_id), font=id_font, fill=(123, 209, 225))
        i += 1
    ...
    for difficulty in song_difficulty:
        ...
        background_image.paste(copy_difficulty_icon, (position[0] + image_size[0] - difficulty_icon_size[0],  position[1] + image_size[1] - difficulty_icon_size[1]), copy_difficulty_icon)

        # 绘制星评
        difficulty_size = 50
        font_difficulty = loading_font.font_loader(font_size = difficulty_size)
        difficulty_star = song_stars
        bs_draw.text((position[0] + image_size[0] - difficulty_size // 2 - 80, position[1] + image_size [1] - difficulty_size - 55), str(f'{difficulty_star[i]:.2f}'), font=font_difficulty, fill=(255, 255, 225))
        i += 1
