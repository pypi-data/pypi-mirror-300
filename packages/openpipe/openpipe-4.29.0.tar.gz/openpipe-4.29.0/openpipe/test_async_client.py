...
async def test_async_content_mistral():
    completion = await client.chat.completions.create(
        ...
        messages=[{"role": "system", "content": "count to 3"}],
        metadata={"prompt_id": "test_async_content_mistral"},
    )
...
async def test_async_function_call_mistral():
    completion = await client.chat.completions.create(
        ...
        function_call=function_call,
        functions=[function],
        metadata={"prompt_id": "test_async_function_call_mistral"},
    )
...
async def test_async_tool_calls_mistral():
    completion = await client.chat.completions.create(
        ...
        metadata={"prompt_id": "test_async_tool_calls_mistral"},
    )
...
async def test_async_caching():
    models = ["gpt-3.5-turbo", "openpipe:test-content-35"]
    ...
    for model in models:
        ...
        completion = await client.chat.completions.create(
            model=model,
            messages=messages,
            metadata={"prompt_id": "test_async_caching"},
        )
        await asyncio.sleep(0.1)
        completion = await client.chat.completions.create(
            model=model,
...
async def test_async_bad_openai_call():
    ...
    assert (
        ...
        == "The model `gpt-3.5-turbo-blaster` does not exist or you do not have access to it."
    )
