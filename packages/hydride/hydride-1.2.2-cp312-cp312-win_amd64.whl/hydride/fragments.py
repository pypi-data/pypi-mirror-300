...
class FragmentLibrary:
    ...
    def add_molecule(self, molecule):
        ...
        for i, fragment in enumerate(fragments):
            ...
            self._frag_dict[
                ...
                centered_heavy_coord,
                centered_hydrogen_coord,
            )
            if stereo != 0:
                ...
                self._frag_dict[
                    ...
                    molecule.res_name[i],
                    molecule.atom_name[i],
                    refl_centered_heavy_coord,
                    refl_centered_hydrogen_coord,
                )
    ...
    def calculate_hydrogen_coord(self, atoms, mask=None, box=None):
        ...
        # for each fragment
        tar_frag_center_coord = np.zeros((atoms.array_length(), 3), dtype=np.float32)
        tar_frag_heavy_coord = np.zeros((atoms.array_length(), 3, 3), dtype=np.float32)
        ref_frag_heavy_coord = np.zeros((atoms.array_length(), 3, 3), dtype=np.float32)
        ...
        tar_frag_heavy_coord = displacement(
            tar_frag_center_coord[:, np.newaxis, :], tar_frag_heavy_coord, box
        )
...
def _rotate(coord, matrices):
    ...
    """
    return np.transpose(
        np.matmul(matrices, np.transpose(coord, axes=(0, 2, 1))), axes=(0, 2, 1)
    )
