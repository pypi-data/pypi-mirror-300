...
class Api(object):
    ...
    def _get_object(self, object_id, custom_endpoint=None):
        url = f"{self.base_url}/{self.endpoint}/{object_id}/" + (f"{custom_endpoint}/" if custom_endpoint else "")
        response = self.session.get(url)
    ...
    def _set_object(self, data, custom_endpoint=None):
        url = f"{self.base_url}/{self.endpoint}/" + (f"{custom_endpoint}/" if custom_endpoint else "")
