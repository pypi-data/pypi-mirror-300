...
class Organization(ParladataObject):
    ...
    def __init__(self, name: str, id: int, parser_names: str, is_new: bool, gov_id: str=None) -> None:
        self.id = id
        self.name = name
...
class OrganizationStorage(Storage):
    ...
    def get_or_add_object(
        ...
        if not self.organizations:
            self.load_data()
        organization = self.get_object_by_parsername("organizations", organization_data["name"])
        if organization:
            return organization
        elif not add:
            return None
