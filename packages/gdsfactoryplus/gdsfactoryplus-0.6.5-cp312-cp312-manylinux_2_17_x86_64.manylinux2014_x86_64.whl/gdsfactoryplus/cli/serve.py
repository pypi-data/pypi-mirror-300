...
from gdsfactoryplus.communication import send_message
from gdsfactoryplus.show import send_error

from ..pdk import PDK_CELL_NAMES
from ..settings import SETTINGS as s
from ..shared import import_pdk, maybe_find_docode_project_dir, register_cells
...
def patch_netlist_get(path: str, schemapath: str = ""):
    print(os.path.join(PROJECT_DIR, s.name))
    register_cells(os.path.join(PROJECT_DIR, s.name))
...
if __name__ == "__main__":
    print(merge_rdb_strings(open("xml1.lyrdb").read(), open("xml2.lyrdb").read()))
