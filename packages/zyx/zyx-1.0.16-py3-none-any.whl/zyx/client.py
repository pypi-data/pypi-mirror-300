...
class Client:
    ...
    def format_to_openai_tools(tools: List[ToolType]) -> List[Tool]:
        ...
        return [Tool(name=convert_to_openai_tool(tool)["function"]["name"], tool=tool, openai_tool=convert_to_openai_tool(tool)) for tool in tools]
    ...
    def __init__(
        ...
        self.config = ClientConfig(api_key=api_key, base_url=base_url, organization=organization, verbose=verbose)
    ...
    def __init_client__(self):
        ...
        if self.provider == "openai":
            ...
            client = OpenAI(api_key=self.config.api_key, base_url=self.config.base_url, organization=self.config.organization)
        elif self.provider == "litellm":
            import litellm
            from litellm import LiteLLM
            litellm.drop_params = True
            client = LiteLLM(api_key=self.config.api_key, base_url=self.config.base_url, organization=self.config.organization)
    ...
    def __patch_client__(self, mode: Optional[InstructorMode] = "tool_call"):
        ...
        if not self.clients.client:
            raise ValueError("Client not initialized. Please specify a valid provider.")

        if self.provider == "openai":
            from instructor import from_openai
            patched_client = from_openai(self.clients.client, mode=get_mode(mode) if mode else None)
        else:
            from instructor import patch
            patched_client = patch(self.clients.client, mode=get_mode(mode) if mode else None)
    ...
    def chat_completion(self, args: CompletionArgs):
        ...
        if args.model.startswith("o1-"):
            ...
            exclude_params.update({"max_tokens", "temperature", "top_p", "frequency_penalty", "presence_penalty", "stop", "tools", "tool_choice", "parallel_tool_calls"})

        if args.stream:
            if self.config.verbose:
                logger.info(f"Streaming completion... with {args.model} model")
            stream = self.clients.client.chat.completions.create(**args.model_dump(exclude=exclude_params))
            return (chunk.choices[0].delta.content for chunk in stream if chunk.choices[0].delta.content)

        if self.config.verbose:
            logger.info(f"Generating completion... with {args.model} model")
        exclude_params.add("stream")
        return self.clients.client.chat.completions.create(**args.model_dump(exclude=exclude_params))
    ...
    def instructor_completion(self, args: CompletionArgs):
        ...
        if args.model.startswith("o1-"):
            logger.warning("OpenAI O1- model detected. Removing all non-supported parameters.")
            exclude_params.update({"max_tokens", "temperature", "top_p", "frequency_penalty", "presence_penalty", "tools", "parallel_tool_calls", "tool_choice", "stop"})

        if args.stream:
            if self.config.verbose:
                logger.info(f"Streaming Instructor completion... with {args.model} model")
            exclude_params.add("stream")
            return self.clients.instructor.chat.completions.create_partial(**args.model_dump(exclude=exclude_params))

        if self.config.verbose:
            logger.info(f"Generating Instructor completion... with {args.model} model")
        exclude_params.add("stream")
        return self.clients.instructor.chat.completions.create(**args.model_dump(exclude=exclude_params))
    ...
    def run_completion(
        ...
        Args:
            messages (Union[str, list[dict]]): The messages to pass to the LLM.
            model (str): The model to use for the completion.
            response_model (Optional[Type[BaseModel]]): The Pydantic model to use for the response.
            mode (Optional[InstructorMode]): The mode to use for the Instructor completion.
            ...
            tool_choice (Optional[Literal["none", "auto", "required"]]): Whether to run the tools.
        ...
        Returns:
            CompletionResponse: The response from the completion.
        """
        formatted_tools = self.format_to_openai_tools(tools) if tools else None
    ...
    def completion(
        ...
        if recommended_provider != self.provider:
            self.__init__(api_key=recommended_api_key or api_key or self.config.api_key,
        ...
        return self.run_completion(messages=messages, model=model, response_model=response_model,
                                    mode=mode, max_retries=max_retries, run_tools=run_tools,
                                    ...
                                    presence_penalty=presence_penalty, stop=stop, stream=stream)
    ...
    def _completion(
        ...
        """Runs a completion with the specified arguments."""
        client = Client(api_key=api_key, base_url=base_url, organization=organization, provider=provider, verbose=verbose) if provider else Client(api_key=api_key, base_url=base_url, organization=organization)
        ...
        return client.run_completion(messages=messages, model=model, api_key=api_key, base_url=base_url,
                                      organization=organization, response_model=response_model, mode=mode,
                                      max_retries=max_retries, run_tools=run_tools, tools=tools,
...
def completion(
    ...
    Args:
        ...
        verbose (Optional[bool]): Whether to print the verbose output.

    Returns:
        CompletionResponse: The response from the completion.   
    """

    provider, model, base_url, api_key = Client.recommend_client_by_model(model, base_url, api_key)
    
    client = Client(api_key=api_key, base_url=base_url, organization=organization, provider=provider, verbose=verbose)
    ...
    if response_model:
        ...
        if verbose:
            logger.info(f"Instructor Mode: {client.clients.instructor.mode}")

    return client.run_completion(messages=messages, model=model, response_model=response_model, mode=mode,
                                  max_retries=max_retries, run_tools=run_tools, tools=tools,
                                  parallel_tool_calls=parallel_tool_calls, tool_choice=tool_choice,
                                  max_tokens=max_tokens, temperature=temperature, top_p=top_p,
                                  frequency_penalty=frequency_penalty, presence_penalty=presence_penalty,
...
if __name__ == "__main__":
    ...
    print(completion(messages="Who is SpiderMan", response_model=PersonModel, verbose=True, base_url="http://localhost:11434/v1", model="llama3.2"))