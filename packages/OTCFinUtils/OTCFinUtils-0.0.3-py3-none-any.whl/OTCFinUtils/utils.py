...
from data_structs import Mapping, Condition, Operator, Category
...
def extract_lookup_column(mapping: dict) -> str:
    ...
    """
    try:
        column_string: str = mapping["new_lookupcolumn"]
...
def extract_file_data(row: pd.Series, mapping: dict):
    ...
    try:
        if data_equals_exclude_value(row, mapping):
            alternative_file_column: str = mapping["new_alternativefilecolumnname"]
            file_data = row[alternative_file_column]
        else:
            file_column: str = mapping["new_filecolumnname"]
...
def data_equals_exclude_value(row: pd.Series, mapping: dict) -> bool:
    ...
    try:
        file_column: str = mapping["new_filecolumnname"]
        exclude_values_list: str = mapping["new_excludevalue"]
...
def choice_value(file_data: str, mapping: dict) -> int | bool:
    ...
    """
    choice_string: str = mapping["new_choicedictionary"]
...
def evaluate_condition(row: pd.Series, mappping: dict) -> bool:
    ...
    """
    try:
        condition_string = mappping[Mapping.CONDITION]
...
def group_mappings(mappings: list[dict]) -> dict[Category, list[dict]]:
    ...
    for mapping in mappings:
        group: int = mapping["new_mapgrouporder"]
        table: str = mapping["new_dataversetablenametext"]
        sheet: str = mapping["new_sheetname"]
