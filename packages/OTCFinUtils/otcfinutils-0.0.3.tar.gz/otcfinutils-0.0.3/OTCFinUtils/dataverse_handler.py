...
import urllib.parse
import requests
from security import get_dataverse_token
...
class DVHandler:
    ...
    def __new__(cls, dataverse_url: Optional[str] = None) -> "DVHandler":
        if dataverse_url is None:
            load_dotenv()
            dataverse_url = getenv("DEFAULT_DATAVERSE_URL", "")

        if not cls._instance:
            cls._instance = super(DVHandler, cls).__new__(cls)
            cls._instance.__init__(dataverse_url)
        
        return cls._instance # type: ignore
    

    def __init__(self, dataverse_url: str) -> None:
        self.dv = dataverse_url
    ...
    def get_entities_list(self, table: str, filter: Optional[str] = None, select: Optional[str] = None, expand: Optional[str] = None) -> list[dict]:
        ...
        if not response.status_code == HTTPStatus.OK.value:
    ...
    def _create_url(self, domain: str|None=None, select: str|None=None, filter: str|None=None, expand: str|None=None) -> str:
        """
        A general helper function. Can be used extensively.
        """
        url = f"{self.api_url}/{domain}?"
        params = []
        params.append(f"$select={select}") if select else None
    ...
    def get_table_definition(self, table: str) -> dict:
        ...
        """
        dataverse_url = f"{self.api_url}/EntityDefinitions(LogicalName='{table}')"
        response = requests.get(url=dataverse_url, headers=self.headers)
    ...
    def get_lookup_details(self, table_name: str, logical_name: str) -> dict:
        ...
        if key_response.status_code != HTTPStatus.OK.value:
    ...
    def _execute_batch_operation(self, batch_params: list[APIParams]) -> str:
        ...
        api_url = f"{self.dv}/api/data/v9.2/$batch"
    ...
    # TODO: MOVE FILE MAPPER SPECIFIC LOADING FUNCTIONS TO SEPARATE FILE

    def get_buffer_ids(self, id_params_buffer: list[IDParams]) -> list[str]:
        """
        ...
        for id_params in id_params_buffer:
            api_params = self._extract_key_api_params(id_params)
    ...
    def _extract_key_api_params(self, id_params: IDParams) -> APIParams:
        """
    ...
    def get_buffer_lookups(self, lookup_params_buffer: list[list[IDParams]]) -> list[EntityLookups]:
        """
        ...
        for entity_lookup_params in lookup_params_buffer:
            for lookup_params in entity_lookup_params:
                api_params = self._extract_key_api_params(lookup_params)
                batch_params.append(api_params)
    ...
    def get_header_row_number(self, file_template_id: str, sheet: str) -> int:
        """
        ...
        dv_url = f"{self.dv}/api/data/v9.2/{table_name}?{filter_str}&{select_str}"
        response = requests.get(url=dv_url, headers=self.headers)
