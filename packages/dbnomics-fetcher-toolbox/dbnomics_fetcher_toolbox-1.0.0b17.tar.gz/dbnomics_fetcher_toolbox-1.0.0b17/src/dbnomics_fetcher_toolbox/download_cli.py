...
class DownloadCLIArgs(BaseCLIArgs):
    ...
    def as_downloader_helper_kwargs(self) -> DownloaderHelperKwargsFromCli:
        return {
            ...
            "updates_file": self.updates_file,
        }
...
class DownloadCLIArgsParser(BaseCLIArgsParser):
    def setup_argparse_parser(self, argparse_parser: argparse.ArgumentParser) -> None:
        ...
        argparse_parser.add_argument(
            ...
            help="only download sections newer than specified in updates file",
        )
        argparse_parser.add_argument(
        ...
        argparse_parser.add_argument(
            "--updates-file",
            default=os.getenv("TOOLBOX_DOWNLOAD_UPDATES_FILE"),
            help="output file to write the updates to (used for incremental mode)",
            type=Path,
        )
