...
from model.semseg.GCM import Global_Context_Module
...
class Decoder(nn.Module):
    ...
    def forward(self, res1, res2, res3, res4, h, w):
        if self.training:
            ...
            ah = h4 + h3 + h2
            ah = self.aux_head(ah, h, w)  # TODO deleted
...
class UNetFormer(BaseNet):
    def __init__(self, backbone,
        ...
        super(UNetFormer, self).__init__(backbone)
        # TODO
        self.isContext = isContext
        print(f"isContext = {self.isContext}")
        if self.isContext:
            self.GCM = Global_Context_Module(channels=self.backbone.channels)
        self.decoder = Decoder(self.backbone.channels, decode_channels, dropout, window_size, nclass)
    ...
    def base_forward_context(self, xs):
        ...
        h, w = x1.size()[-2:]
        res1, res2, res3, res4 = self.GCM(f1, f2, f3)
        if self.training:
            x, ah = self.decoder(res1, res2, res3, res4, h, w)
            # return x, ah
        else:
