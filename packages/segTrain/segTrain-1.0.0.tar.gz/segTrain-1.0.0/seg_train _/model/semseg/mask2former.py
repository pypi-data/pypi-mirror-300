...
class Mask2Former(BaseNet):
    def __init__(self, backbone, nclass: int = 25, isContext: bool = True):
        ...
        if self.isContext:
            ...
            for k, v in self.backbone_feature_shape.items():
                if k == "res2": continue
                self.attention12.append(nn.MultiheadAttention(embed_dim=v['channel'], num_heads=2))  # 4,2
                self.attention13.append(nn.MultiheadAttention(embed_dim=v['channel'], num_heads=2))  # 4,2
    ...
    def base_forward_context(self, xs):
        ...
        for i, x in enumerate(zip(f1, f2, f3)):
            ...
            Fx = torch.cat((fx1,
                           ...
                           dim=1)
            features[f'res{i + 2}'] = self.Squeue[i - 1](Fx)
