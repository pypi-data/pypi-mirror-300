...
from pyeqx.core import Configuration, Operation, configuration_from_dict, gen_name

TResultData = TypeVar("TResultData")


class FunctionExecuteResultBase(Generic[TResultData]):
    def __init__(self, data: TResultData, is_success: bool, error: Exception = None):
...
class FunctionExecuteResult(FunctionExecuteResultBase[TResultData]):
    def __init__(self, data: TResultData | None, error: Optional[Exception] = None):
        if error is None:
