...
from .vector_db import ChromaDB, LlamaIndexDB, LlamaIndexWithChromaDB, PostgresVectorDB
...
class DynamoFL:
    ...
    def create_rag_hallucination_test(  # pylint: disable=dangerous-default-value
        ...
        vector_db: Optional[
            Union[ChromaDB, LlamaIndexDB, LlamaIndexWithChromaDB, PostgresVectorDB]
        ] = None,
        grid: List[Dict[str, List[Union[str, float, int]]]] = [{}],
    ) -> TestEntity:
    ...
    def create_guardrail_evaluation_test(  # pylint: disable=dangerous-default-value
        ...
        **kwargs
        ...
        Args:
            name (str): Name of the test
            guardrail_model_key (str): Key of the guardrail model
            target_model_key (str): Key of the target model
            dataset_id (str): Id of the dataset to be used.
                Dataset must have the following columns with the following names:
                - "prompt" (string)
                - "isCompliant" (boolean)
                Optional columns:
                - "topic_name" (optional, string)
                - "topic_description" (optional, string)
                - "trickiness" (optional, string)
            ...
            text_variation_methods (List[Literal["original", "extra_spacing", "word_substitution"]]):
                ...
                is not perturbed.
        Policy Args: Only required if benchmark_type is "guardrail" or "guardrail_and_target":
            policy_name (Optional[str]): Name of the policy. Defaults to None.
            policy_description (Optional[str]): Description of the policy. Defaults to None.
            allowed_behaviors (Optional[List[str]]): List of allowed behaviors. Defaults to None.
            disallowed_behaviors (Optional[List[str]]): List of disallowed behaviors. Defaults to None.
        ...
        if benchmark_type == "guardrail":
            if guardrail_model_key is None:
                raise ValueError("guardrail_model_key is required for guardrail test")
            if target_model_key is not None:
                raise ValueError("target_model_key is not required for guardrail test")
        elif benchmark_type == "target":
            if target_model_key is None:
                raise ValueError("target_model_key is required for target test")
            if guardrail_model_key is not None:
                raise ValueError("guardrail_model_key is not required for target test")
        elif benchmark_type == "guardrail_and_target":
            if guardrail_model_key is None:
                raise ValueError("guardrail_model_key is required for guardrail_and_target test")
            if target_model_key is None:
                raise ValueError("target_model_key is required for guardrail_and_target test")
        if benchmark_type != "guardrail":
            if (
                policy_name is None
                or policy_description is None
                or allowed_behaviors is None
                or disallowed_behaviors is None
            ):
                raise ValueError(
                    "policy_name, policy_description, allowed_behaviors, and disallowed_behaviors are required for any test with a target model"
                )
        fast_mode = kwargs.get("fast_mode", False)
        common_attack_config = {
            "attack": Helpers.construct_dict_filtering_none_values(
        ...
        if gpu is None:
            if self.get_model(guardrail_model_key).type == "REMOTE":
                gpu = GPUConfig(gpu_type=GPUType.A10G, gpu_count=1)
            else:
                raise ValueError(
                    "Guardrail evaluation tests on local models require specifying a GPUConfig that matches the model."
                )
