...
class BaseEventClassForm(hawat.forms.BaseItemForm):
    ...
    displayed_source = wtforms.SelectMultipleField(
        ...
        choices=[(a, a) for a, b in (mentat.const.REPORT_FIELDS_SOURCE_LIST + mentat.const.REPORT_FIELDS_SOURCE_NUMBER)],
    ...
    displayed_target = wtforms.SelectMultipleField(
        ...
        choices=[(a, a) for a, b in (mentat.const.REPORT_FIELDS_TARGET_LIST + mentat.const.REPORT_FIELDS_TARGET_NUMBER)],
        default=[a for a, b in mentat.const.REPORT_FIELDS_TARGET_LIST[:1]],
        filters=[lambda x: x or []],
        description=lazy_gettext(
    ...
    def is_multivalue(field_name):
        ...
        :rtype: bool
        """
        return field_name in ('displayed')
...
class EventClassSearchForm(hawat.forms.BaseSearchForm):
    ...
    subclassing = wtforms.SelectField(
        lazy_gettext('Subclassing:'),
        choices=[
        ...
        default='',
        description=lazy_gettext('Search for event classes with subclassing enabled or disabled.')
    )
