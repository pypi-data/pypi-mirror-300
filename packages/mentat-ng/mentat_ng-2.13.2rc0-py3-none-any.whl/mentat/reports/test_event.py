...
from mentat.datatype.sqldb import GroupModel, FilterModel, NetworkModel, \
    SettingsReportingModel, EventReportModel, EventClassModel, DetectorModel
...
class TestMentatReportsEvent(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sqlstorage.session.add(ec1)
        self.sqlstorage.session.add(ec2)
    ...
    def test_03_filter_events(self):
        ...
        aggr_result = self.reporter.aggregate_events(aggr_value)
    ...
    def _generate_mock_report(self, abuse_group, severity, rtype, events=None):
        if not events:
