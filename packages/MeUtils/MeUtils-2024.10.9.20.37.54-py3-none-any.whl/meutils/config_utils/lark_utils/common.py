...
async def get_next_token(
    ...
    for i in range(10):  # 避免死循环
        ...
        if token := await redis_aclient.lpop(feishu_url):
            ...
            logger.debug("从redis中获取token")

            # 不加check会重复从头开始轮询，也不能保证key的有效性
            if check_token is None or await check_token(token):
...
async def get_next_token_for_polling(
    ...
    if from_redis:
        return await get_next_token(feishu_url, check_token, ttl=7 * 24 * 3600)
