...
BASE_URLS = [
    "https://api.textin.com/home",
    "http://textin.chatfire.cc/home",
    "http://textin.ffire.cc/home",
]
...
async def document_process(data: bytes,
    ...
    :return:
    """
    BASE_URL = np.random.choice(BASE_URLS)
    ...
    headers = {
        ...
        "token": np.random.choice(["", "821480df15b637852f7e4a0dabb8a156"]),  # 观察有效期
    }
    ...
    for i in range(5):
        try:
            ...
            if i > 2:
                request_kwargs = {
                    "proxies": await ips.get_proxies(i=i)
                }
