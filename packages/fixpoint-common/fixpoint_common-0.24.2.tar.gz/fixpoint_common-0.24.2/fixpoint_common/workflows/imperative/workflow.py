...
class Workflow(BaseModel):
    ...
    def _store_run(
        ...
        if isinstance(storage_config.workflow_storage, InMemWorkflowStorage):
            ...
            return
        elif isinstance(storage_config.workflow_storage, OnDiskWorkflowStorage):
            ...
            return
        elif isinstance(storage_config.workflow_storage, PostgresWorkflowStorage):
            data = WorkflowRunAttemptData(
                attempt_id=workflow_run.attempt_id,
                workflow_id=workflow_run.workflow_id,
                workflow_run_id=workflow_run.id,
            ...
            raise ValueError(
                f"Unsupported workflow storage: {storage_config.workflow_storage}"
            )
    ...
    def _load_run(
        ...
        if isinstance(storage_config.workflow_storage, InMemWorkflowStorage):
            try:
                ...
                raise NotFoundError(f"WorkflowRun {workflow_run_id} not found") from exc
        elif isinstance(storage_config.workflow_storage, OnDiskWorkflowStorage):
