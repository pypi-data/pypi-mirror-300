...
from fixpoint_common.workflows.human.human import PostgresHumanInTheLoop
...
class StorageConfig:
    ...
    def with_postgres(
        ...
        human_storage = _create_human_postgres_storage(pg_pool, apg_pool)
...
def _create_workflow_postgres_storage(
    ...
    """Create a Postgres storage driver for workflows"""
    return PostgresWorkflowStorage(pg_pool, apg_pool)


def _create_human_postgres_storage(
    pg_pool: ConnectionPool, apg_pool: Optional[AsyncConnectionPool]
) -> HumanInTheLoop:
    """Create a Postgres storage driver for human-in-the-loop"""
    return PostgresHumanInTheLoop(pg_pool, apg_pool)
