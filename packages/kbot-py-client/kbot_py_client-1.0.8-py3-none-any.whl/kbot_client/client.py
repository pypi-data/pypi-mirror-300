...
class Client:
    ...
    def __init__(self, server, port=443, proto='http', api_key=None, verify=True):
    ...
    def impersonate(self, username, usertype='local', external_auth='', timeout=5):
        ...
        if r.status_code != 200:
            print("ERROR. Failed to impersonate user! %s" % (r.text))
            raise RuntimeError("ERROR. Failed to impersonate user. Error: %s" % r.text)
    ...
    def __request(self, method: str, unit: str, data: dict = None, params: dict = None):
        ...
        r = requests.request(method.upper(), self.url + '/api/%s/' % (unit), params=params, data=dump_data, headers=self._headers, verify=self.verify)
        return r

    def request(self, method: str, uri: str, data: dict = None, params: dict = None):
        dump_data = json.dumps(data or {})
        r = requests.request(method.upper(), self.url + f'/api/{uri}', params=params, data=dump_data, headers=self._headers, verify=self.verify)
        return r

    # ! DEPRICATED: Since Kbot 2021.04 API authentication does not require Application Id
    def _get_application(self, slug='api-access'):
        ...
        r = requests.get(self.url + '/api/application/current', headers=headers, verify=self.verify)
    ...
    def message(self, cid: int, message: str, timeout: int=60) -> list:
        ...
        if r.status_code != 201:
            print("Error sending message: %s" % (r.text))
        ...
        while timeout > 0:
            ...
            if r.status_code == 200:
                ...
                print("Error getting response : %s" % (r.text))
                raise RuntimeError("Error getting response")
    ...
    def get(self, unit, params=None):
        return requests.get(self.url + '/api/%s/' % (unit), params=params, headers=self._headers, verify=self.verify)

    def put(self, unit, data, params=None):
        return requests.put(self.url + '/api/%s/' % (unit), params=params, data=json.dumps(data), headers=self._headers, verify=self.verify)

    def post(self, unit, data, params=None):
        return requests.post(self.url + '/api/%s/' % (unit), params=params, data=json.dumps(data), headers=self._headers, verify=self.verify)

    def delete(self, unit, params=None):
        return requests.delete(self.url + '/api/%s/' % (unit), params=params, headers=self._headers, verify=self.verify)

    def post_file(self, unit, data, params=None, files=None, headers=None):
        ...
        headers['Accept'] = "*/*"
        return requests.post(self.url + '/api/%s/' % (unit), params=params, data=data, headers=headers, files=files, verify=self.verify)
