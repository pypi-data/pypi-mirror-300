...
class PSS1830 :
    ...
    def _connect(self , mode='cli' , neip=None , port=22 , rootpw='QUx1MTIj' , tryNumber=0) :
        ...
        try :
            if mode == 'direct_cli' :
                self.client.connect(neip , port , "cli" , None , sock=jhostserver)
    ...
    def connect(self , mode='cli' , neip=None , port=22  , user=None , pw=None , rootpw=None,  jumpserver = None , resetRequired = False, tmpPassword = None , maxReconnectAttemp=40) :
        if not user :
            user = 'admin'
        if not pw :
            pw = 'YWRtaW4='
        ...
        if mode == 'ssh' : 
            self.connected = True
            return self.client
        isRequired = self.isRequireAknowledge(neVersionResponse)
        if isRequired :
    ...
    def isRequireAknowledge(self , newRelease=None) : 
        if not newRelease :
            newRelease = self.get_version()
    ...
    def wait_result(self , command,expect,expectError=None,errorMessage=None,autoError = True,return_untouched_buffer=False) :
    ...
    def quit_gmre(self) :
        if self.gmre and self.isGmreLogin :
            self.cli_execute('quit' , expect='Have a nice day')
