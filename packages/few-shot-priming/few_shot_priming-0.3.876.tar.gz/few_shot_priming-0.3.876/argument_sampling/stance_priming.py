...
def generate_most_similar_stance_arguments(experiment, path_similarities, path_samples, different_topics, debug, threshold=None, drop_duplicate_perspectives=False):

    similarities = load_similarities(path_similarities)
    dataset =  load_splits(experiment, oversample=False, drop_duplicate_perspectives=drop_duplicate_perspectives, debug=debug)
        ...
        for _, test_record in tqdm(dataset["test"].iterrows()):
            ...
            if threshold:
                for key in instances_scores_map:
                    if instances_scores_map[key] >threshold:
    ...
    if threshold:
        path_samples = path_samples.replace(".tsv", f"-{threshold}.tsv")
...
def get_thresholds_for_percentiles(path_similarities, percentile_num):
    ...
    indices = range(percentile_size,len(similarity_list),percentile_size)
    print(indices)
    sorted(similarity_list)
    return [similarity_list[i] for i in indices]
