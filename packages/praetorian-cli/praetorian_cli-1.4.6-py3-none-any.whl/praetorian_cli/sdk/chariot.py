...
class Chariot:
    ...
    def put(self, type: str, params: dict) -> {}:
        resp = requests.put(f'{self.keychain.api}/{type}',
                            json=params, headers=self.keychain.headers)
        process_failure(resp)
        return resp.json()
    ...
    def link_account(self, username: str, value: str = '', config: dict = {}):
        resp = requests.post(f'{self.keychain.api}/account/{username}', json=dict(config=config, value=value),
                             headers=self.keychain.headers)
        process_failure(resp)
        return resp.json()
    ...
    def _upload(self, chariot_filepath: str, content: str):
        ...
        presigned_url = requests.put(f'{self.keychain.api}/file', params=dict(name=chariot_filepath),
                                     headers=self.keychain.headers)
    ...
    def download(self, name: str, download_directory: str = ''):
        resp = requests.get(f'{self.keychain.api}/file', params=dict(name=name), allow_redirects=True,
                            headers=self.keychain.headers)
        process_failure(resp)
