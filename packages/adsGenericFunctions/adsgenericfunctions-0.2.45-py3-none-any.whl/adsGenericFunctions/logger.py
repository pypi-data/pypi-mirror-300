...
class CustomFormatter(logging.Formatter):
    ...
    bold_red = "\x1b[31;1m"
    reset = "\x1b[0m"
    #format = '{asctime:30}{levelname:10}{name:20}{message}'
...
class Logger:
    ...
    def __setup_logger(self, logger_name):
        ...
        formatter = logging.Formatter('{asctime:30}{levelname:10}{name:20}{message}', style='{')
    ...
    def log_to_db(self, start_time: datetime, end_time: datetime, status: str, message: str):
        ...
        if not self.connection:
            self.error("Aucune connexion à une base de données, pour le my_package.")
            return
        if not self.db_logging_enabled:
            self.info("L'insertion dans la base de données est désactivé.")
            return
        job_key = uuid.uuid4()
    ...
    def _insert_logs_tables(self, job_key: uuid, start_time: datetime, end_time: datetime, full_path: str, status: str, message: str):
        with self.connection.cursor() as cursor:
            cursor.execute(
                f"""
    ...
    def enable_logging(self, logLevel=logging.INFO):
