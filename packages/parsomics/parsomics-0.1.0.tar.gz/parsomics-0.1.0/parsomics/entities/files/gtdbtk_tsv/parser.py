...
from parsomics.entities.files.drep.entry.models import DrepEntry
from parsomics.entities.files.gtdbtk_tsv.classification_method import (
    GTDBTkClassificationMethod,
)
...
class GTDBTkTsvParser(BaseModel):
    ...
    def _add_genome_key_to_df(self, df, engine):
        ...
        df = df.with_columns(
            [
                (
                    pl.col("genome_name")
                    .map_elements(lambda x: self._genome_name_to_genome_key(x, engine))
                    .alias("new_col")
                )
            ]
        )
    ...
    def parse(self, engine) -> None:
        ...
        with Session(engine) as session:
            try:
                session.bulk_insert_mappings(GTDBTkTsvEntry, mappings)
                session.commit()
                logging.info(
