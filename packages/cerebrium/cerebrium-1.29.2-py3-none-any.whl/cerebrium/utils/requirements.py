...
def strip_delimiters(dep_list_str: str) -> dict[str, str]:
    if dep_list_str == "" or dep_list_str == "[]":
        return {}
    dep_list = dep_list_str.strip("[]").split(",")
    dep_list = [d.strip().strip("'").strip('"') for d in dep_list]
    return req_list_to_dict(dep_list)
...
def req_dict_to_str_list(
    requirements: dict[str, str], for_display: bool = False, is_conda: bool = False
) -> list[str]:
...
def shell_commands_to_file(
    ...
    with open(shell_file, "w", newline="\n") as f:
        f.write("set -e \n")
