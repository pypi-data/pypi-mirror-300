...
def test_iter(services, temp_service, session, imagerepo, shared_compute_pool, database, schema):
    ...
    try:
        ...
        expected_spec_for_new_service = '---\nspec:\n  containers:\n  - name: "hello-world"' \
                                        ...
                                        'limits:\n        memory: "6Gi"\n        cpu: "8"\n      requests:\n        ' \
                                        'memory: "0.5Gi"\n        cpu: "0.5"\n'
        assert_service_output(result[0], service_name, shared_compute_pool, expected_spec_for_new_service,
                              database.name, schema.name, 1, 1)
        ...
        expected_spec_for_temp_service = '---\nspec:\n  containers:\n  - name: "hello-world"' \
                                         ...
                                         '     memory: "0.5Gi"\n        cpu: "0.5"\n  endpoints:\n  - name: ' \
                                         '"default"\n    port: 8080\n'
        assert_service_output(result[0], temp_service.name, shared_compute_pool, expected_spec_for_temp_service,
                              database.name, schema.name, 1, 5, "created by temp_service")
    finally:
        s.drop()


def assert_service_output(service, expected_service_name, expected_compute_pool_name, expected_spec_text,
                          expected_database_name, expected_schema_name, expected_min_instances, expected_max_instances,
