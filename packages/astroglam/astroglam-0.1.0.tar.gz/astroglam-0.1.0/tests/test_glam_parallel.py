...
if __name__ == '__main__':
	...
	Sigma_CIIcell=10**7
	Sigma_OIII5007cell=10**7
	Sigma_SFRcell=10.0
            
	# run mcmc for oiii
	filename_backendoiii='oiii.h5'
	galaxy_exampleOIII.set_data(Sigma_SFR=Sigma_SFRcell, Sigma_CII=Sigma_CIIcell, Sigma_ion=Sigma_OIII5007cell, ion_tracer='OIII5007')
	...
	flat_samplesOIII = mcr.run_model_parallel()
            
	fig = corner.corner(flat_samplesOIII, labels=["log(n)", r"log(Z/Z$_{\odot}$)", r"log($\kappa_s$)"])

	fig.savefig("test_parallel.png")

	print('EXITED OK! GLAM PARALLEL SHOULD WORK FINE.')
