...
namespace uns {
  ...
  // ============================================================================
  // parseConfig
 template <class T>  std::string CSnapshotInterfaceIn<T>::parseConfig(std::string req)
  {
  ...
  // ============================================================================
  // parseSelectTime                                                             
template <class T> void CSnapshotInterfaceIn<T>::parseSelectTime()
  {
  ...
  // ============================================================================
  // nextFrame 
template <class T> int CSnapshotInterfaceIn<T>::nextFrame(std::string bits)
  {
    int ret=0;
    computeBits(bits);
    if (isNewFrame()) {
  ...
  // ============================================================================
  // nextFrame 
 template <class T> int CSnapshotInterfaceIn<T>::nextFrameSelect(ComponentRangeVector * crvs)
  {  
  ...
  // ============================================================================
  // computeBits 
template <class T> void CSnapshotInterfaceIn<T>::computeBits(std::string bits)
  {
    req_bits = 0;
    if (verbose) std::cerr << "BITS ="<<bits<<"\n";
    if (bits=="") { // all the bits
      req_bits = (unsigned int) ((unsigned int)( 1 << 31 )<<2)-1;
...
template <class T> bool CSnapshotInterfaceIn<T>::checkRangeTime(const float current_time)
  {
    assert(stv.size()>0);
