...
int UserSelection::isComponent(const std::string comp)
{
  ...
  if (match == -1) { // not match
    ...
    if (icrv != -1 && !nodata) {
      assert((unsigned int) icrv<crv->size());
      ...
      if (nodata) { // no data known yet (like ramses)
        ...
        CunsIn2<float>::initMap();
        if (std::find(select_order.begin(), select_order.end(),
                      CunsIn2<float>::s_mapCompInt[comp]) == select_order.end()) { // item not found
          select_order.push_back(CunsIn2<float>::s_mapCompInt[comp]); // save order
        }
      } else {
