...
class IAPListener():
    ...
    def __init__(self,
        ...
        self._links = self._dag.outputs.split(",")
        ...
        for entity in self._dag.pipeline:
            ...
            for item in entity.output_def:
                if "name" in item and item["name"] in self._links:
                    self.outputs[item["name"]] = item["link"]
                    self.encoders[item["name"]] = DataEncoder(item)
        if outputs and outputs in self.outputs:
            self.default_output = self.outputs[outputs]
        else:
            self.default_output = self.outputs[self._links[0]]
    ...
    async def _run_async(self):
        ...
        for q_name in self.inputs_name:
            ...
            nc_in = await self.nc.subscribe("nc."+queue_in+".*.*", queue=self.queue+"-"+item["link"])
