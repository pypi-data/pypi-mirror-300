...
class MySQLDatabase(object):
			...
			for r in records_insert:
				...
				for f in field_names:
					if f not in r or not r[f]:
						...
						# 直接转换成字符串
						values.append(f"'{str(r[f])}'")
					elif isinstance(r[f], datetime.datetime):
						# 日期时间处理为标准格式
						values.append(f"'{r[f]:%Y-%m-%d %H:%M:%S}'")
					else:
						# 其他数据类型都按JSON来处理
						values.append(f"'{json.dumps(r[f], ensure_ascii=False)}'")
	...
	def update(self, table, records):
		...
		for r in records:
			...
			SQL_string	= f"UPDATE {table} SET "
			field_value_pairs	= []
			for f in r:
				if f!='ID':
					# 注意值要用单引号包起来，且内容需要处理
					v	= f"'{self.replace(r[f])}'" if r[f] else 'NULL'
					field_value_pairs.append(f"`{f}`={v}")
			SQL_string += f"{','.join(field_value_pairs)} WHERE ID={ID}"
