...
class DataLakeClient:
    ...
    @contextmanager
    def get_object_as_file(self, remote_file_path:str, dest_file_path:str, f:BinaryIO):
        """Retrieves an object from the remote bucket using the `obj_name` and 
        writes to `f`. This will set the file cursor to 0 and is ready to use."""
        
        yield self.client.download_object_as_file(self.bucket, remote_file_path, dest_file_path, f)
        f.close()