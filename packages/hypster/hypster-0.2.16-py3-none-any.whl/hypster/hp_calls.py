...
class HPCall(ABC):
    def __init__(
        self, hp_instance: "HP", name: Optional[str] = None, default: Any = None
    ):
        if name is None:
            raise ValueError(
                "`name` argument is missing and must be provided explicitly."
            )
        self.hp = hp_instance
    ...
    def _validate_dict_keys(self, options: Dict[str, Any]):
        if not all(isinstance(k, (str, int, bool, float)) for k in options.keys()):
            ...
            raise ValueError(
                f"Dictionary keys must be str, int, bool, float. Got {bad_keys} instead."
            )
    ...
    def _validate_default(self, default: Any, options: Dict[str, Any]):
        if default is not None and isinstance(default, list):
            ...
            default is not None and default not in options
        ):  # TODO: add var/param names to error
    ...
    def handle_overrides_selections(self) -> Any:
        if self.name in self.hp.overrides:
            ...
            raise ValueError(
                f"`{self.name}` has no selections, overrides or defaults provided."
            )
...
class SelectCall(HPCall):
    def execute(
        self, options: Optional[Union[Dict[str, Any], List[Any]]] = None
    ) -> Any:
...
class PropagateCall(HPCall):
    ...
    def execute(
        ...
        self.hp.name_prefix = (
            self.name
            if original_name_prefix is None
            else f"{original_name_prefix}.{self.name}"
        )
    ...
    def extract_nested_config(config: Dict[str, Any], prefix: str) -> Dict[str, Any]:
        ...
        if discrepancies:
            raise ValueError(
                f"Discrepancies found in nested configuration for '{prefix}': {discrepancies}"
            )
    ...
    def process_final_vars(final_vars: List[str], name: str) -> List[str]:
        dot_notation_vars = [
            var[len(name) + 1 :] for var in final_vars if var.startswith(f"{name}.")
        ]
        dict_style_vars = list(
            PropagateCall.extract_nested_config(
                dict.fromkeys(final_vars, None), name
            ).keys()
        )
        dict_style_vars = [
            var for var in dict_style_vars if var not in dot_notation_vars
        ]  # remove dups
