...
from everysk.core.fields import StrField, ChoiceField, ListField, BoolField, DictField, IntField, DateTimeField, FloatField
...
class Result(BaseDict):
    status = ChoiceField(default='ERROR', choices=('OK', 'ERROR', 'UNKNOW'))
...
class InputParams(BaseDict):
    ...
    worker_type = StrField(empty_is_none=True)
...
class WorkerExecution(BaseEntity):
    ...
    input_params = InputParamsField()
    storage_input_params = BoolField(default=False)

    parallel_info = ParallelInfoField()
    status = StrField(empty_is_none=True)
    execution_type = StrField(empty_is_none=True)
    ...
    workflow_name = StrField(empty_is_none=True)
    worker_id = StrField(regex=settings.WORKER_ID_REGEX, empty_is_none=True)
    worker_name = StrField(empty_is_none=True)
    worker_type = StrField()
