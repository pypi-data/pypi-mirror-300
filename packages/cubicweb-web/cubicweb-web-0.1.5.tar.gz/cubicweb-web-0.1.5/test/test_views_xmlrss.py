    ...
    def test(self):
        ...
        with self.admin_access.web_request(relation=rels) as req:
            self.assertMultiLineEqual(
                req.user.view("xml"),
                f"""\
<CWUser eid="6" cwuri="{BASE_URL}6" cwsource="system">
  ...
  <in_group role="subject">
    <CWGroup eid="%(group_eid)s" cwuri="{BASE_URL}%(group_eid)s"/>
  </in_group>
  <in_state role="subject">
    <State eid="%(state_eid)s" cwuri="{BASE_URL}%(state_eid)s" name="activated"/>
  </in_state>
