[![Test & Deploy fake-usragent](https://github.com/fake-useragent/fake-useragent/actions/workflows/action.yml/badge.svg?branch=main)](https://github.com/fake-useragent/fake-useragent/actions/workflows/action.yml?query=branch%3Amain)
...
Or if that isn't working, try to install the latest package version like this (`1.5.1` is an example, check what the [latest version is on PyPi](https://pypi.org/project/fake-usragent/#history)):
...
Since GitHub Actions is unable to reach willshouse.com and has Cloudflare protection. We can run the script below to automatically scrape the user-agent strings from the external data source. The script will copy the [JSONlines](https://jsonlines.org/) file to the `src/fake_usragent/data` directory. Execute:
...
- 0.1.5 February 28, 2017
  ...
  - Increased `fake_usragent.settings.HTTP_TIMEOUT` to 5 seconds
...
- 0.1.4 December 14, 2016
  ...
  - Added alias `fake_usragent.FakeUserAgent` for `fake_usragent.UserAgent`
  - Added alias `fake_usragent.UserAgentError` for `fake_usragent.FakeUserAgentError`
  - Reduced `fake_usragent.settings.HTTP_TIMEOUT` to 3 seconds
...
- 0.1.3 November 24, 2016
  ...
  - Raises `fake_usragent.errors.FakeUserAgentError` in case when there is not way to download data
  - Raises `fake_usragent.errors.FakeUserAgentError` instead of `None` in case of unknown browser
