    ...
    def __create_sql_engine(self):
        ...
        try:
            if self.db_type == "oracle":
                ...
                return create_engine(f'oracle+oracledb://{self._user}:{self._password}@{self._host}:{self._port}/{self.db_name}')
            elif self.db_type == "sql_server":
                return create_engine(f'mssql+pyodbc://{self._user}:{self._password}@{self._host}:{self._port}/{self.db_name}?driver=ODBC+Driver+17+for+SQL+Server')
            elif self.db_type == "postgresql":
                return create_engine(f'postgresql://{self._user}:{self._password}@{self._host}:{self._port}/{self.db_name}')
            else:
