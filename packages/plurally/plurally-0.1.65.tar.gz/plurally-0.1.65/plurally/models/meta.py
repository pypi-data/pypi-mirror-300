...
```
        ...
        delay: int = Field(
            ...
            description="The delay (in seconds) in to wait before handling a message. This is useful in case someone sends multiple messages in a row. Delay is ignored when testing.",
            json_schema_extra={
                "advanced": True,
            },
        )
    ...
    class OutputSchema(Node.OutputSchema):
        ...
        sender_username: str = Field(
            ...
            json_schema_extra={
                ...
                "example-desc-force": "Username is fixed when testing.",
            },
        )
        sender_id: int = Field(
    ...
    def forward(self, _):
        ...
        history, last_message, last_processed_timestamp, was_escalated_previously = (
            self._fetch_new_dms()
        )
...
```
    ...
    class InputSchema(MetaAuth.InputSchema):
        ...
        sender_username_: str = Field(
            description="The username of the sender.",
        )
        history_: Table = Field(
            title="Conversation History",
            description="The messages that were received in the past in this conversation, from oldest to newest. \n\nThe columns are: \n- from_user: The username of the sender. \n- to_user: The username of the recipient. \n- message: The message content. \n- message_type: The type of message. \n- date_received: The date and time the message was received, e.g. 2024-09-16T09:15:18Z.",
            format="table",
        )
    ...
    def _process_escalation_if_needed(self, history, recipient_id, sender_username):
        ...
        for dm in history.data:
            recv = datetime.fromisoformat(dm["date_received"])
            ...
            if (
                fuzz.partial_ratio(
                    self.human_escalation_trigger.lower(), new_message_content
                )
                > 80
            ):
                logger.debug(
                    f"Detected human escalation trigger: {new_message_content}"
                )
                self.outputs = None
    ...
    def forward(self, node_input):
        if node_input.was_escalated_previously_:
            return
        if self._process_escalation_if_needed(
            node_input.history_, node_input.recipient_id, node_input.sender_username_
        ):
            return
        ...
        self._send_message(
            page_access_token,
            recipient_id,
            self.human_escalation_template.format(
                trigger=self.human_escalation_trigger
            ),
        )
