...
# try:
#     from settings.settings import * # type: ignore
...
def render(request, template_name, context={}, templates='templates'):
    ...
    """
    if templates == 'dark_fream':
        templates = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))), 'DarkFream', 'templates')
...
# Добавляем текущую директорию в список импортируемых модулей
