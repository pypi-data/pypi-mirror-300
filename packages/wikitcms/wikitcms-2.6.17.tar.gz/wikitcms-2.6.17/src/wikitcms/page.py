...
class Page(mwp.Page):
    ...
    def creation_date(self):
        ...
        """
        try:
            revs = self.revisions(max_items=1, dir="newer", prop="timestamp")
        except TypeError:
            # mwclient < 0.11.0
...
class ResultPage(Page):
    """A page containing result sections. This will usually be a
    ValidationPage (see below), but can also be a matrix template
    page, and it's occasionally useful to be able to inspect the
    result sections of those.
    """
...
class ValidationPage(ResultPage):
        ...
        for img in self.event.ff_release_images.all_images:
            ...
            for values, score in loadscores:
                if img["subvariant"].lower() in values:
                    img["score"] += score
            for values, score in typescores:
                if img["type"].lower() in values:
                    img["score"] += score
...
class AMIPage(Page):
    ...
    def seedtext(self):
        ...
        ciddic = fedfind.helpers.parse_cid(self.event.ff_release.cid, dic=True)
        start = datetime.datetime.strptime(ciddic["date"], "%Y%m%d").replace(
            tzinfo=datetime.timezone.utc
        )
        end = start + datetime.timedelta(days=2)
        topics = []
        topics.extend([f"aws.branched.Cloud_Base.{arch}" for arch in arches])
        topics.extend([f"aws.rawhide.Cloud_Base.{arch}" for arch in arches])
        url = "https://apps.fedoraproject.org/datagrepper/raw?"
        for i, topic in enumerate(topics):
            if i > 0:
                url += "&"
            url += f"topic=org.fedoraproject.prod.fedora_image_uploader.published.v1.{topic}"
        ...
        ours = [msg["msg"] for msg in msgs if msg["msg"]["compose_id"] == self.event.ff_release.cid]

        def _table_line(region, ami):
            """Convenience function for generating a table line."""
            url = "https://redirect.fedoraproject.org/console.aws.amazon.com/ec2/v2/home?"
            url += f"region={region}#LaunchInstanceWizard:ami={ami}"
            return f"| {region}\n| {ami}\n| [{url} Launch in EC2]\n|-\n"
        ...
        def _table(arch):
            ...
            # find the right messages for this arch
            relevants = [msg for msg in ours if msg["architecture"] == arch]
            if not relevants:
                return ""
            for msg in relevants:
                for region, ami in msg["regions"].items():
                    ret += _table_line(region, ami)
            ret += "|}\n\n"
            return ret
