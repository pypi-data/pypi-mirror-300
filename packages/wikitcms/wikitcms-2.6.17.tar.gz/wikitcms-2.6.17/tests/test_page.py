...
class TestAMIPage:
    ...
    # we have to patch these to avoid network round trips in fedfind
    @mock.patch("fedfind.release.Compose.cid", "Fedora-41-20240912.n.0")
    @mock.patch("fedfind.release.Compose.exists", True)
    def test_ami_page(self):
        ...
        """
        site = wikitcms.wiki.Wiki("fjajah", do_init=False, force_login=False)
        event = wikitcms.event.ComposeEvent(site, "41", "Beta", "1.2", cid="Fedora-41-20240912.n.0")
        page = event.ami_page
        assert page.checkname == "Template:Fedora 41 Beta 1.2 AMI"
        assert page.summary == "Relval bot-created AMI page for Fedora 41 Beta 1.2"
        assert page.event is event
        ...
        def fakedownloadjson(url):
            ...
            that backs this is the real data, but we hack up splitting
            it across two 'pages' to test the pagination handling.
            """
            page = 1
            if "page=2" in url:
                # dumb but enough
                page = 2
            path = os.path.join(DATAPATH, f"Fedora-41-20240912.0.ami.dgdata.{page}.json")
            with open(path, "r") as amifh:
                amidata = json.load(amifh)
            return amidata

        # again, read expected seedtext from a file
        exptextpath = os.path.join(DATAPATH, "Template:Fedora 41 Beta 1.2 AMI.seedtext")
        with open(exptextpath, "r") as exptextfh:
            exptext = exptextfh.read()
