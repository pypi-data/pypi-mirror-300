...
def main(rename, remap, strip, lower, help, filepath, output, inplace):
    try:
        if not rename and not remap and not strip and not lower and not help:
            click.echo("No actions executed, please select an available action for execution", err=True)
        ...
        if not filepath:
            file_path = input("Enter csv or xlsx path: ")

        else:
            file_path = filepath

        if not Path(file_path).exists():
            click.echo(f"File not found at {file_path}.")
            filepath = False
            main()
        ...
        click.echo(f"Unhandled Error Occurred: {e}")
        exit()
