...
from generated.python import InputParamsRef
from planqk.service.auth import PlanqkServiceAuth
from planqk.service.client import PlanqkServiceClient
from planqk.service.sdk import JobStatus, InputDataRef
...
class IntegrationTestSuite(unittest.TestCase):
    ...
    def test_should_execute_with_datapool_input(self):
        client = PlanqkServiceClient(service_endpoint, consumer_key, consumer_secret)

        health = client.health_check()
        assert health.status == "Service is up and running"

        data_ref= InputDataRef(data_pool_id = "e3859bb2-1959-4d3d-852b-62ee81b28a72", data_source_descriptor_id = "1c8d341e-1c86-4e19-acae-2f84bcab55ca", file_id="f5251675-49c6-49f3-a3a6-de75b8335d76")

        params_ref = InputParamsRef()

        job = client.start_execution(data_ref=data_ref, params_ref=params_ref)
        assert job.id is not None
        assert job.status == JobStatus.PENDING
