...
from hermes_core.links.queued_link import QueuedLink
...
class DependencyManager:
    ...
    def add(self, dependency: NodeDependency):
        ...
        for dependency in self.nodes:
            if isinstance(dependency.config, GenericNode.Config):
                ...
                self.parent.managed_child_nodes[dependency.name] = get_class_handle(dependency.config)(
                    dependency.config
                )
...
class GenericNode:
    ...
    def link_dependencies(self, root_node: "GenericNode"):
        ...
        # Link dependencies
        for dependency in self.dependency_manager.nodes:
            if isinstance(dependency.config, GenericNode.Config):
                assert dependency.name in self.managed_child_nodes, f"Node {dependency.name} not found in child nodes"
    ...
    def init(self):
        self.log.info("Initializing component")

    def deinit(self):
        self.log.info("Deinitializing component")
    ...
    def collect_depependencies(self) -> dict:
        deps = {}
        for dep in self.dependency_manager.nodes:
            deps[dep.name] = dep.get_dependency()
        return deps

    def attempt_init(self, **kwargs):
    ...
    def attempt_deinit(self, error=False):
        try:
