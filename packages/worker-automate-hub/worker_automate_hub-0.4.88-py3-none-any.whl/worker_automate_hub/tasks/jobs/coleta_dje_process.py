...
def fechar_aplicativo(nome_processo):
...
def report_to_user(filename,report,path_directory):
    ...
    headers = ["Número do processo", "Assunto", "Tipo de Comunicação", "Data Final Ciência", "Forma Ciência", "Detalhamento", "Inteiro Teor"]
...
async def coleta_dje_start_update(task):
    ...
    try:
        ...
        if os.path.exists("assets/dje"):
            ...
            return {"sucesso": False, "retorno": f"Não foi possivel encontrar o diretório com os locators para continuidade do processo, diretório: 'assets/dje'"}
        ...
        for index, row in certificate_files.iterrows():
            try:
                nome_aplicativo = "pjeoffice-pro.exe"
                fechar_aplicativo(nome_aplicativo)

                # Abre o PJeOffice
                app = Application().start("C:\\Program Files\\PJeOffice Pro\\pjeoffice-pro.exe")
                ...
                console.print(f'Certificado a ser executado: {certificado} para a empresa {empresa}...\n')
                ...
                if items:
                    ...
                    certificados_disponiveis = f'{locator_path}/CertificadosDisponiveis.png'
                    engrenagem_certificado = f'{locator_path}/Engrenagem_Configuracao.png'
                    ...
                    async with async_playwright() as p:
                        ...
                        try:
                            while True:
                                solicitacao_senha = pyautogui.locateOnScreen(informar_senha_existe, confidence=0.8)
                                if solicitacao_senha:
                                    pyautogui.click(pyautogui.locateCenterOnScreen(cancelar, confidence=0.8))
                        ...
                        certificados_pje = pyautogui.locateOnScreen(certificados_disponiveis, confidence=0.8)
                        if certificados_pje:
                            find_and_click(engrenagem_certificado, "Engrenagem, menu de configuração")
                            await asyncio.sleep(3)
                        ...
                        try:
                            while True:
                                remover_certificado_exist = pyautogui.locateOnScreen(remover_certificado, confidence=0.7)
                                if remover_certificado_exist:
                                    find_and_click(remover_certificado, "Remover Certificados")
                                    console.print('Certificado ja importado, removendo...\n')
                                    await asyncio.sleep(2)
                                else:
                                    console.print('Certificado ja importado, removidos...\n')
                                    break
                        except Exception as e:
                            pass

                        find_and_click(localizar, "Importar certificado..")
                        find_and_click(cert_fullpath, "Inserir Caminho do Certificado", write_text=file_path)
                        ...
                        if informar_senha_existe:
                            find_and_click(senha_certificado, "Inserir Senha do certificado", write_text=senha)
                            await asyncio.sleep(1)
                            find_and_click(ok_confirmar_certificado, "Confirmar seleção")
                        
                        await page.wait_for_load_state("networkidle")
                        await page.wait_for_selector("text=Comunicação Processual Acompanhe as citações, intimações e ofícios direcionados", timeout=15000)
                        await page.get_by_text("Comunicação Processual Acompanhe as citações, intimações e ofícios direcionados").click()
                        ...
                        for linha in linhas:
                            ...
                            dados_linha = [coluna.inner_text().strip() for coluna in colunas]
                    ...
                    result = {"error": True, "message": 'Certificado não encontrado no Google Drive', "certificado": certificado}
        ...
        if count_error == len(certificate_files):
            log_msg = f"Erro Processo do DJE, não obtivemos exito em nenhum certificado executado, erro: {feedback_cnpj}"
            ...
            try:
                send_email(smtp_server,smtp_port,smtp_user,smtp_pass,corpo_email,'RPA - DJE',emails_to,temp_dir)
