...
async def get_xml(xml_file):
    ...
    if arquivo_xml_button.is_enabled():
        arquivo_xml_button.click()
        console.print("Arquivo XML selecionado com sucesso... \n")
    else:
        raise {"sucesso": False, "retorno": f"Arquivo XML não foi encontrado."}
    
    console.print("Clicando em OK... \n")
    ok_button = main_window.child_window(title="OK", class_name="TBitBtn")
    if ok_button.is_enabled():
        ok_button.click()  # Clique no botão "OK"
        console.print("Botão OK clicado com sucesso... \n")
    else:
        raise {"sucesso": False, "retorno": f"Botão OK não foi encontrado."}
    
    await worker_sleep(3)

    console.print("Verificando a existencia da tela para importar o xml...\n")
...
async def import_nfe():
    await worker_sleep(2)
    importar_nfe = pyautogui.locateOnScreen(ASSETS_PATH + "\\entrada_notas\\ImportarNF-e.png", confidence=0.8)
    if importar_nfe:
    ...
    while(aguardando_nota == False or timeout >= timeout_try):
        ...
        if target:
            ...
            if target_nota_lancada:
                ...
                console.print("Nota ainda não lançada", style="bold green")
                logger.log("Nota ainda não lançada")
                timeout_try += 1
