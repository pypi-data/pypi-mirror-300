  ...
  <div class="order-wrapper">
    ...
    <OrderForm :is-loading="isLoading" add-default-classes>
        ...
        <div class="content-wrap flex flex-col">
          ...
          <div class="w-full flex">
            <MultiselectField class="w-11/12" v-model="airGroupModel" group-values="data" group-label="group"
              ...
              label="full_repr" :options="airGroups" track-by="full_repr" :close-on-select="false"
          ...
          <div class="w-11/12 mb-4" v-show="showFlightType">
            <Label label-text="Flight Type" :required="false" />
            <Toggle v-model="formModel.is_private" false-value="Commercial" true-value="Private" />
          </div>
          <SelectField class="w-11/12" v-model="formModel.currency" label-text="Currency"
            placeholder="Please select currency" :disabled="(!meta && !isAdmin) || !formModel?.type?.is_fuel"
            :errors="validationInfo?.currency?.$errors" :is-validation-dirty="validationInfo?.$dirty"
            label="display_name" :options="currencies" :loading="false" :smallWidth="true" />
        </div>
      </template>
      <template #content class="grid-cols-1" v-if="!isFirstStep && formModel?.type?.is_fuel">
        ...
        <div class="w-full flex flex-col" v-if="releaseType === 'Standard'">
          <div class="flex items-start w-full mb-4">
            <div class="w-11/12 flex gap-x-3">
              ...
              <div class="w-4/12">
                <Label :required="false" label-text="Time:" class="whitespace-nowrap text-disabled" />
                <FlatPickr v-if="formModel.fuel_order!.arrival_time_tbc" v-model="arrivalDateTime.time"
                  placeholder="Time" :errors="validationInfo?.fuel_order?.arrival_datetime_utc?.$errors"
          ...
          <div class="flex items-start w-11/12" v-if="airGroupModel?.length === 1">
            <div class="w-full flex gap-x-3">
              <InputField class="w-6/12" v-model="formModel.fuel_order!.fuel_quantity"
                ...
                :errors="validationInfo?.fuel_order?.fuel_quantity?.$errors" label-text="Fuel Uplift Quantity"
          ...
          <div class="flex flex-col items-start w-11/12" v-else>
            ...
            <div class="flex flex-col w-full items-start" v-for="(el, index) in orderStore.fuelGroupModel">
              <div class="w-full flex gap-x-3">
                ...
                <SelectField class="w-4/12" :model-value="el!.fuel_uom" placeholder="Please select fuel uplift units"
                  :disabled="!meta && !isAdmin || index > 0" label="description_plural" :options="fuelQuantityUnits"
                  :loading="false" @update:model-value="changeUom($event, index)" />
              </div>
            </div>
          </div>
          <div class="flex items-start w-full">
            <div class="w-11/12 flex gap-x-3">
              <div class="w-6/12">
                <InputField v-model="formModel.fuel_order!.post_pre_minutes"
                  :is-validation-dirty="validationInfo?.$dirty"
        ...
        <div class="w-full flex flex-col" v-if="releaseType === 'Open'">
          <div class="flex items-start w-full mb-4">
            <div class="w-11/12 flex gap-x-3">
              ...
              <div class="w-6/12" v-if="ufn">
                <Label :required="false" label-text="End Date:" class="whitespace-nowrap text-disabled" />
                <FlatPickr ref="departureDateRef" :is-disabled="true" :placeholder="'&nbsp;'" :config="{
                  allowInput: true,
                  altInput: true,
                  altFormat: 'Y-m-d',
                  dateFormat: 'Y-m-d',
            ...
            <div class="flex items-center justify-center mt-[2.5rem] w-1/12">
              <CheckboxField class="mb-0 mr-1" v-model="ufn" />
...
const {
  callFetch: fetchCurrencies,
} = useFetch<any[], (pageNumber: number) => void>(async (pageNumber: number) => {
  const { currencies: res, meta } = await OrderReferences.fetchCurrencies(pageNumber)
  if (pageNumber === 1 && res) {
    currencies.value = [...res];
  } else { currencies.value = [...currencies.value, ...res]; }
  if (meta.pagination.page !== meta.pagination.pages) {
...
const {
  data: fuelCapacity,
  callFetch: fetchFuelCapacity,
} = useFetch<any[], (pageNumber: number) => any>(async (typeId: number) => {
  return await OrderReferences.fetchFuelCapacity(typeId);
})
...
const updateReleaseType = async (release: string) => {
  ...
  if (release === 'Open') {
    if (formModel.value.is_any_aircraft) {
      ...
      const data = await fetchFuelCapacity(airGroupModel.value[0]?.type?.id ?? formModel.value.aircraft_type?.id);
      if (typeof data === 'object') {
        console.log(data);
...
const changeUom = (value: string, index: number) => {
