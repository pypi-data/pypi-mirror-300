<template>
  <div class="order-sidebar flex bg-white rounded-md" :class="{ 'is-sidebar-closed': isSidebarClosed }">
    ...
    <div class="order-sidebar-content w-full flex flex-col">
      <div class="order-sidebar-content-header px-[0.75rem] py-[0.5rem] flex justify-between items-center">
        {{ activeTab.name }}
      ...
      <ScrollBar class="my-[0.75rem]">
        <div class="order-sidebar-content-data px-[0.75rem] flex flex-col gap-3 max-h-full" v-if="activeTab.name === 'Notes'">
          <div class="order-sidebar-content-data-note border-0 rounded-lg p-[0.5rem] flex flex-col" v-for="note in orderNotes"
            :key="note.id">
        ...
        <div class="order-sidebar-content-data" v-else-if="activeTab.name === 'Activity'">
          <div class="order-sidebar-content-data-activity border-0 rounded-lg pl-[0.75rem] pt-0 flex"
            ...
            <div class="order-activityp-info-wrap p-[0.75rem] flex flex-col w-full">
    ...
    <div class="order-sidebar-menu w-2/12 py-[1rem] px-[0.75rem] flex flex-col gap-4">
      <div class="order-sidebar-menu-el flex flex-col items-center" :class="{ 'active': activeTab.name === el.name }"
        ...
        <div class="img-wrap flex items-center justify-center p-[0.5rem] border-0 rounded-lg"><img
            :src="getImageUrl(`assets/icons/${el.icon}.svg`)" :alt="el.icon"></div>
        {{ el.name }}
      </div>

      <div class="close-sidebar img-wrap cursor-pointer flex items-center justify-center p-[0.5rem]"
