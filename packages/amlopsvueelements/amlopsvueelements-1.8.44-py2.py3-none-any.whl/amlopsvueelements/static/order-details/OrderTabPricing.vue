<template>
  <div class="w-full h-full flex flex-col gap-2">
    <SupplyFuelDetailsModal :is-open="isModalOpened" ref="clientInput" @modal-close="closeModal"
      :supply-fuel="supplyFuel" :result-index="selectedModalSupplier!"
      :is-open-release="order?.type.is_fuel && order?.fuel_order?.is_open_release" name="supply-modal" />
    <div class="pricing-step bg-white w-full border border-transparent rounded-md">
      ...
      <div class="pricing-step-content w-full flex flex-col"
        ...
        <div class="pricing-step-content-header-wrap w-full flex items-center">
          ...
          <div class="pricing-step-content-col w-2/12">
            <div class="pricing-step-content-col-header px-[0.75rem] py-[0.5rem]">Total Uplift Cost</div>
          </div>
          <div class="pricing-step-content-col w-1/12">
        ...
        <div class="pricing-step-content-data-wrap w-full flex items-center"
          ...
          <div class="pricing-step-content-col w-1/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{ supplier.fuel_type.name }}</div>
          </div>
          <div class="pricing-step-content-col w-2/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{ supplier.supplier.full_repr }}</div>
          </div>
          <div class="pricing-step-content-col w-2/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{ supplier.ipa?.full_repr ?
              supplier.ipa?.full_repr : selectedSupplier !== null ? '-' : 'TBC' }}</div>
          </div>
          <div class="pricing-step-content-col w-1/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{ supplier.delivery_method?.name ??
              'All' }}</div>
          </div>
          <div class="pricing-step-content-col w-1/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{ supplier.apron?.name ?? 'All' }}
            </div>
          </div>
          <div class="pricing-step-content-col w-1/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{ supplier.terminal?.name ?? 'All' }}
            </div>
          </div>
          <div class="pricing-step-content-col w-2/12">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem]">{{
              formatNumber(supplier.total_uplift_cost) }} {{
                supplier.currency.code }}</div>
          </div>
          <div class="pricing-step-content-col w-1/12 relative">
            <div class="pricing-step-content-col-data px-[0.75rem] py-[0.5rem] flex gap-2"
      ...
      <div class="pricing-step-content-none w-full flex py-[1rem] pr-[0.75rem] pl-[2.5rem] flex flex-col"
      ...
      <div class="pricing-step-content w-full flex py-8 px-[0.75rem] flex flex-col" v-if="isLoadingSupplyFuel">
        <Loading />
      </div>
    </div>
    <div class="pricing-step bg-white w-full border border-transparent rounded-md">
      ...
      <div class="pricing-step-content" v-if="orderPricing && order && orderPricing.supplier_id">
        ...
        <div class="pricing-step-content-header-sub flex">
          <div class="pricing-step-content-header-sub-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] gap-2">
        ...
        <div class="pricing-step-content-element flex">
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light gap-2">
            ...
            <div class="pricing-step-content-element-el flex justify-start items-center w-4/12">{{
              addThousandSeparators(order?.fuel_order?.fuel_quantity) }} ({{
                order?.fuel_order?.fuel_uom?.description_plural }})</div>
          </div>
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light">
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">
              <div class="input-wrap flex pr-[0.75rem]">
                <InputField :model-value="removeTrailingZeros(orderPricing.fuel_pricing?.client?.unit_price_amount)"
                  class="w-6/12 mb-0" :is-white="true" :is-half="true" placeholder=" "
            ...
            <div class="pricing-step-content-element-el flex w-full justify-start items-center pr-[0.75rem]">
              <InputField :model-value="orderPricing.fuel_pricing?.client?.amount_total" class="roi-input w-full"
                ...
                <template #suffix>{{ orderPricing.fuel_pricing?.client?.amount_currency?.code
                  }}</template>
              </InputField>
            </div>
          </div>
        </div>
        <div class="pricing-step-content-divider flex w-full py-[0.5rem] px-[0.75rem]"
        ...
        <div class="pricing-step-content-element flex" v-for="(fee, key) in orderPricing.fees" :key="key"
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light gap-2">
            <div class="pricing-step-content-element-el-name flex justify-start items-center w-8/12">{{
              fee.supplier?.suppliers_fuel_fees_rates_row?.supplier_fuel_fee?.fuel_fee_category?.name ?? 'Fee' }}</div>
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light">
            ...
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">{{
              formatNumber(parseFloat(fee.supplier?.amount_total).toFixed(2)) }} {{
                fee.supplier?.unit_price_pricing_unit?.currency?.code
              }}</div>
          </div>
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">
              <div class="input-wrap flex pr-[0.75rem]">
                <InputField :model-value="removeTrailingZeros(fee.client?.unit_price_amount)" class="w-6/12 mb-0"
                  :is-white="true" :is-half="true" placeholder=" " @update:model-value="onFeeChange($event, key)" />
                <SelectField class="w-6/12 mb-0" :is-white="true" :is-half="true" placeholder=" "
            ...
            <div class="pricing-step-content-element-el flex w-full justify-start items-center pr-[0.75rem]">
              <InputField class="roi-input w-full" :model-value="fee?.client?.amount_total" :is-white="true"
                ...
                <template #suffix>{{ fee?.client?.unit_price_pricing_unit?.currency?.code }}</template>
              </InputField>
            </div>
          </div>
        </div>
        ...
        <div class="pricing-step-content-element flex" v-for="(tax, key) in orderPricing.taxes" :key="key"
          v-if="orderPricing && orderPricing.taxes.length > 0">
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light gap-2">
            <div class="pricing-step-content-element-el-name flex justify-start items-center w-8/12 gap-1 relative">{{
              ...
              <div class="pricing-step-content-block-body-note hover-wrap contents flex items-center">
                ...
                <div class="pricing-step-tooltip right-tooltip">
                  Applies on: {{ tax.supplier.applies_on?.fuel ? 'Fuel' : tax.supplier.applies_on?.fees ?
                    Object.values(tax.supplier.applies_on?.fees)[0] : tax.supplier.applies_on?.taxes ?
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem] el-border-light">
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">{{
              tax?.supplier?.tax_percentage ? `${tax?.supplier?.tax_percentage}%` : '-' }} </div>
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">
          ...
          <div class="pricing-step-content-element-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">{{
              tax?.client?.tax_percentage ? `${tax?.client?.tax_percentage}%` : '-' }}</div>
            <div class="pricing-step-content-element-el flex w-full justify-start items-center">
        ...
        <div class="pricing-step-content-results flex">
          <div class="pricing-step-content-results-wrap flex w-4/12 py-[0.5rem] pl-[0.75rem]">
          </div>
      ...
      <div class="pricing-step-content-missing flex items-center justify-center py-[1.25rem]" v-else>
    ...
    <div class="pricing-step bg-white w-full border border-transparent rounded-md">
      <div class="pricing-step-header flex justify-between py-[1rem] px-[0.75rem]">
        <div class="pricing-step-header-name">{{ order?.fuel_order?.is_open_release ? 'Indicative' : '' }} ROI
          Calculation
        </div>
        <div class="loading-wrap">
      ...
      <div class="pricing-step-content roi flex flex-col" v-if="orderPricing && order && orderPricing.supplier_id">
        <div class="roi-inputs flex">
          <div class="roi-inputs-wrap w-6/12 flex items-center p-3">
            ...
            <InputField class="roi-input w-6/12" :is-white="true" placeholder=" " :disabled="true"
        ...
        <div class="roi-results flex py-[0.75rem]">
          <div class="roi-results-wrap w-6/12 flex items-center px-[0.75rem]">
            ...
            <div class="roi-results-value w-6/12">{{ formatNumber(orderRoi?.calculated_roi_value) }} {{
              orderPricing?.fuel_pricing?.client?.amount_currency?.code }}</div>
          </div>
          <div class="roi-results-wrap w-6/12 flex items-center px-[0.75rem]">
            <div class="roi-results-label w-6/12 ">ROI</div>
            <div class="roi-results-value-green"
              :style="{ 'background-color': orderRoi?.roi_parameters?.background_fill_hex ?? 'rgb(58, 176, 80)' }">{{
                orderRoi?.calculated_roi === '1000000.00' ? 'Ꝏ' :
                  formatNumber(orderRoi?.calculated_roi) }} %</div>
          </div>
        </div>
      </div>
      <div class="pricing-step-content-missing flex items-center justify-center py-[1.25rem]" v-else>
