...
class LumensparkModel(PreTrainedModel):
    ...
    def from_pretrained(cls, model_id, cache_dir=None, **kwargs):
        """
        Downloads the pretrained weights from the Hugging Face model hub, and loads them.
        This method handles the Hugging Face download API and stores the weights locally.
        """
        # Set cache directory for storing models
        cache_dir = cache_dir or os.path.join(os.getcwd(), "lumenspark_weights")

        # Download model weights from Hugging Face
        weight_path = hf_hub_download(repo_id=model_id, filename="pytorch_model.bin", cache_dir=cache_dir)
