...
)
{
    ...
    algo_settings_t& settings = *settings_inp;
    ...
    fp_t log_epsilon_bar = std::log(1.0); // \bar{\epsilon}_0
    fp_t log_step_size   = std::log(settings.nuts_settings.step_size); // \bar{\epsilon}_0
    ...
    fp_t& step_size = settings.nuts_settings.step_size;
    step_size = nuts_find_initial_step_size(first_draw, mntm_vec, inv_precond_matrix, box_log_kernel_fn, leap_frog_fn, target_data, step_size);
    ...
    for (size_t draw_ind = 0; draw_ind < n_total_draws; ++draw_ind) {
        ...
        if (draw_ind < n_adapt_draws) {
            ...
            log_step_size = mu_val - h_val * std::sqrt(draw_ind + 1) / gamma_val;

            step_size = std::exp(log_step_size);

            auto diff = std::pow(draw_ind + 1, - kappa_val) * (log_step_size - log_epsilon_bar);

            log_epsilon_bar += diff;
        } else {
            step_size = std::exp(log_step_size);
        }
        ...
        if (step_size > std::pow(10, 10)) { // reset if the step sizes gets out of hand
            step_size = 1;
            log_step_size = 0;
            log_epsilon_bar = 0;
            h_val = 0;
