...
inline fp_t gaussian_log_density(const ColVec_t& x, const Mat_t& chol_cov) {
    size_t d = x.size();
    fp_t log_det = 2*chol_cov.diagonal().array().log().sum();
    ColVec_t Lz = chol_cov.triangularView<Eigen::Lower>().solve(x);
    return -.5 * (Lz.transpose() * Lz + d + std::log(2 * M_PI) + log_det);
}

inline fp_t gaussian_log_density(const ColVec_t& x, const ColVec_t& mean, const Mat_t& chol_cov) {
    return gaussian_log_density(x - mean, chol_cov);
}
