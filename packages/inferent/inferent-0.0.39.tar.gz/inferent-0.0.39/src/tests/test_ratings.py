...
class TestTSRating(unittest.TestCase):
    ...
    # TODO: this test is not working, because the calc_ratings method within
    # the pd.DataFrame.apply() call is not patching properly (closure?)
    @patch.object(TSRating, "calc_ratings")
    def test_enrich_update(self, mock_calc_ratings):
        df = pd.DataFrame(
            {"teams": [[["Player1"], ["Player2"]]], "ranks": [[0, 1]]}
        )

        mock_calc_ratings = MagicMock()
