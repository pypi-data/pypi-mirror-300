...
def compareWithSavedData(regression_filename, base_regression_filename, threshold=0):
    ...
    if not os.path.isfile(base_regression_filename):
        ...
        regression_writer = regressionTestWriter(
            _getTempRegressionPath(base_regression_filename)
        )
        regression_writer.write(_getTempRegressionPath(regression_filename))
        _printCDashRegressionNotFoundError(base_regression_filename)
        msg = (
            "Missing baseline regression: "
            + base_regression_filename
            + "\nTest regression created: "
            + _getTempRegressionPath(base_regression_filename)
        )
