...
"""
This program will generate .pyi files for all the XSIGMA modules
in the "xsigmamodules" package (or whichever package you specify).
These files are used for type checking and autocompletion in
some Python IDEs.

The XSIGMA modules must be in Python's path when you run this script.
Options are as follows:

 -p PACKAGE    The package to generate .pyi files for [xsigmamodules]
...
are put inside the existing xsigmamodules package).  This is equivalent
to the following:

    python -m xsigmamodules.generate_pyi -p xsigmamodules

To put the pyi files somewhere else, perhaps with a different suffix:

    python -m xsigmamodules.generate_pyi -o /path/to/xsigmamodules -e .pyi

To generate pyi files for just one or two modules:

    python -m xsigmamodules.generate_pyi -p xsigmamodules Core

To generate pyi files for your own modules in your own package:

    python -m xsigmamodules.generate_pyi -p mypackage mymodule [mymodule2 ...]
...
# list expected non-xsigma type names
types = set()
...
# XSIGMA methods have a special type
xsigmamethod = ""
template = ""


def isxsigmamethod(m):
    """Check for XSIGMA's custom method descriptor"""
    return type(m) == xsigmamethod


def isnamespace(m):
    """Check for namespaces within a module"""
    # until xsigmamodules.Core.namespace is directly accessible
    return str(type(m)) == "<class 'xsigmamodules.Core.namespace'>"
    ...
    for signature in signatures:
        if signature.startswith("def " + name + "("):
            ...
            if signature.startswith("def " + name + "()"):
                ...
                constructors.append(
                    re.sub(name + r"\(", "__init__(self, ", signature, 1)
                )
    return constructors
...
def handle_static(o, signature):
    ...
    if isxsigmamethod(o) and not has_self.search(signature):
        return "@staticmethod\n" + signature
    else:
        return signature
...
def namespace_pyi(c, mod):
    ...
    base = "namespace"
    if mod.__name__ != "xsigmamodules.Core":
        base = "xsigmamodules.Core." + base
...
def module_pyi(mod, output):
    """Generate the contents of a .pyi file for a XSIGMA module."""
    ...
    depends = set(["xsigmamodules.Core"])
...
def main(argv=sys.argv):
    ...
    parser = argparse.ArgumentParser(
        ...
        description="A .pyi generator for the XSIGMA python wrappers.",
    )
    parser.add_argument(
        "-p",
        "--package",
        type=str,
        default="xsigmamodules",
        help="Package name [xsigmamodules].",
    )
    parser.add_argument("-i", "--importer", type=str, help="Static module importer [].")
    parser.add_argument(
        "-o", "--output", type=str, help="Output directory [package directory]."
    )
    parser.add_argument(
        "-e", "--ext", type=str, default=".pyi", help="Output file suffix [.pyi]."
    )
    parser.add_argument(
        "--test",
        action="count",
    ...
    parser.add_argument(
        "modules", type=str, nargs="*", help="Modules to process [all]."
    )
        ...
        for modname in mod.__all__:
            ...
            try:
                ...
                if not errflag:
                    errflag = True
                    sys.stderr.write(
                        progname + ": couldn't get loader for " + modname + "\n"
                    )
            if spec is None:
                continue
