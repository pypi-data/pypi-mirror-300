...
from channels.models import Credential, Channel, Provider , Messagge  # Asegúrate de que esté bien importado el modelo
...
class Messagebird:
    ...
    def __init__(self, *args, **kwargs):
        ...
        credential_instance = Credential.objects.get(channel=self.__channel_instance, provider=self.__provider_instance)
    ...
    def send_template(self, customer_id, key, value):
        ...
        payload = json.dumps({
            "receiver": {
                "contacts": [
                    {"identifierKey": "phonenumber", "identifierValue": phone_number}
                ]
            },
            "template": {
    ...
    def get_contact(self, customer_id):
        ...
        """
        url = f"{self.__base_url}workspaces/{self.__workspace_id}/contacts/{customer_id}"
