...
class WeReadApi:
    ...
    def retry_if_not_specific_errcode(exception):
        """Return True if we should retry (in this case when errcode is not -2021), False otherwise"""
        if isinstance(exception, Exception) and hasattr(exception, 'errcode'):
            return exception.errcode != -2021
        return True
    
    @retry(stop_max_attempt_number=3, wait_fixed=5000,retry_on_exception=retry_if_not_specific_errcode)
