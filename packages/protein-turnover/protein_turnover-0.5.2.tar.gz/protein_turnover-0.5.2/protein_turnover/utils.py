...
class PeptideInfo:
    ...
    def getEnrichments(self) -> np.ndarray:
        """elementCount + 1 array of enrichments"""
        naturalLabelledElement = self.abundance

        elementCount = self.elementCount

        de = (
            self.settings.maximumLabelEnrichment - naturalLabelledElement
        ) / elementCount
        return np.array(
            [naturalLabelledElement + i * de for i in range(elementCount + 1)],
            dtype=np.float32,
        )
...
def getEnrichments(peptide: str, settings: PeptideSettings) -> np.ndarray:
    """elementCount + 1 array of enrichments"""
    naturalLabelledElement = settings.abundance

    elementCount = settings.getElementCountFromPeptide(peptide)

    de = (settings.maximumLabelEnrichment - naturalLabelledElement) / elementCount
    return np.array(
        [naturalLabelledElement + i * de for i in range(elementCount + 1)],
        dtype=np.float32,
    )
