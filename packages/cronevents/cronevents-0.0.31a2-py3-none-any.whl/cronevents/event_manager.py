...
def query_syntax_checker(query: str) -> None:
    ...
    query = query.lower()
    if not any(query.strip().startswith(token) for token in STARTING_TOKENS):  # not query.strip().startswith('every'):
        raise CronEventSyntaxError('Query must start with one: ' + ', '.join(STARTING_TOKENS))

    for token in STARTING_TOKENS:
        if query.strip().startswith(token):
    ...
    try:
        ...
        if '@' not in query:
            ...
            time_to = parse_time(query.split('@')[0]) if any(
                query.lower().startswith(token) for token in STARTING_TOKENS
