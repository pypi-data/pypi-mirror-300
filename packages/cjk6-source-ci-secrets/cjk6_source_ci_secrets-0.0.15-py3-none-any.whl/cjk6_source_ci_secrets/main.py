...
def main():
	...
	if args.use_svn:
		...
		svn_repo = svn.local.LocalClient(p)
		print(svn_repo.info())
		print(commit)
		# CHOOSE EITHER PREVIOUS COMMIT OR 0 IF FIRST COMMIT (MAY NEED TO EDIT THIS BASED ON PASSED IN SINCE_COMMIT)
		prev_commit = max(0,int(commit)-1)
		print(prev_commit)
		print(svn_repo.diff(commit, str(prev_commit)))
	else:
