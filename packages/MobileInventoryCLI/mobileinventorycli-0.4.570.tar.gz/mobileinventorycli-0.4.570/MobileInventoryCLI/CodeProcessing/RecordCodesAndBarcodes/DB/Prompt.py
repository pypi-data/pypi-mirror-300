...
{Fore.light_salmon_1}System Version: {Back.grey_70}{Style.bold}{Fore.red}{VERSION}{Style.reset}'''.replace('#','\n')
            ...
            def lineTotal():
                ...
                return total
            m="DUR="+str(datetime.now()-datetime.fromtimestamp(lastTime)).split(".")[0]
...
{Fore.black}{Back.grey_50}TmInShl:{str(InShellElapsed).split(".")[0]}|DT:{now.ctime()}{Style.reset}
...
{Fore.green}q={Fore.green_yellow}quit|{Fore.light_sea_green}qb={Fore.green_yellow}backup & quit{Fore.cyan}|{Fore.light_salmon_1}c2c=calc2cmd={Fore.sky_blue_2}clctr rslt to inpt{Style.reset}
...
'''
            ...
            if cmd.endswith("#c2cbe"):
                ...
                print(helpText)
                extra=f'''{Fore.light_yellow}The Recommended Scanners, currently
...
{Fore.light_steel_blue}if 'b' returns to previous menu, try '#b' to return to barcode input, in ListMode@$LOCATION_FIELD, 'e' does the same{Style.reset}''')
                ...
                with db.Session(db.ENGINE) as session:
                    ...
                    location_fields=["Shelf","BackRoom","Display_1","Display_2","Display_3","Display_4","Display_5","Display_6","ListQty","SBX_WTR_DSPLY","SBX_CHP_DSPLY","SBX_WTR_KLR","FLRL_CHP_DSPLY","FLRL_WTR_DSPLY","WD_DSPLY","CHKSTND_SPLY",]
                    tmp=[]
