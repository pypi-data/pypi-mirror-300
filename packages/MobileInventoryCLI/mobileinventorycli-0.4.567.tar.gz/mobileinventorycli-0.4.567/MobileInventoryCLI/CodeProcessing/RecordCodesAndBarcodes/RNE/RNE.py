		...
		with Session(ENGINE) as session:
			while True:
				...
				if ct == 0:
					...
					for num,entry in enumerate(results):
						msg=f'''{Fore.light_yellow}{num}/{Fore.dark_goldenrod}{num+1}/{Fore.light_red}{ct}{Fore.light_magenta} -> {Fore.light_green}{entry.Barcode}|{Fore.green_yellow}{entry.Name}|{entry.Note}|{Fore.orange_red_1}{Style.bold}{entry.BB_Expiry}{Style.reset}'''
						print(msg)

	def rm_expo(self):
		toRm=self.search_expo(returnable=True,short=True)
		ct=len(toRm)
		if ct == 0:
