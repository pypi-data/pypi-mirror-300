...
int main(int argc, char *argv[]) {
    ...
    /* Check usage */
    if (argc < 6) {
        printf("Usage: encode   <0(packed)/1(itu)/2(au)> <input-audio-file> <output-bitstream-file> <bit-rate> <bandwidth>\n\n");
    ...
    /* add AU format header if needed */
    if (control.syntax == 2) {
        // Calculate number of audio frames
        fseek(control.fpin, 0L, SEEK_END);
        UWord32 pcmDataSize = ftell(control.fpin);
        fseek(control.fpin, 0L, SEEK_SET);
        UWord16 totalAudioFrames = (UWord16)ceil((float)pcmDataSize / (control.frame_size * 2));

        // Write header
        struct AudioHeader audioHeader = au_header_init(control.bandwidth, control.bit_rate, totalAudioFrames, totalAudioFrames * number_of_16bit_words_per_frame);
        fwrite(&audioHeader, AUDIOHEADER_DEFAULTSIZE, 2, control.fp_bitstream);
        // --   Add here MarkTable if needed
    }
    ...
    while (samples == control.frame_size) {
        ...
        //memset(input, 0, samples * 2);      // antonio debug: all input data is ZERO
        ...
        // printf("mag_shift: %ld\n", mag_shift);
        //printf("data[0] = 0x%x   data[19] = 0x%x\n", out_words[0], out_words[19]);

        /* Write output bitstream to the output file */
        if (control.syntax == 0 || control.syntax == 2)
...
void parse_command_line(char *argv[], ENCODER_CONTROL *control) {
    ...
    if ((control->syntax != 0) && (control->syntax != 1) && (control->syntax != 2)) {
        printf("syntax must be 0 for packed, 1 for ITU format or 2 for AU format\n");
        exit(1);
    }
    ...
    if (control->bandwidth == 7000) {
        ...
        else if (control->syntax == 2)
            printf(" AU-format bitstream\n");
