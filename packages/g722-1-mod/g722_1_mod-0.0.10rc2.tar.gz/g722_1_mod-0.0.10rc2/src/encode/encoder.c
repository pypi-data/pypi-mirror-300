...
void bits_to_words(UWord32 *region_mlt_bits,
    ...
    for (region = 0; region <= number_of_regions; region++) {
        ...
        if (j >= 0) {
            ...
            out_words[out_word_index++] = endianessT(out_word);         // antonio: call to endianessT to reverse bits
            //printf("A- %d - %x\n", out_word_index-1, out_words[out_word_index-1]&0xffff);
            move16();
        ...
        if (accb < 0) {
            ...
            while ((region_bit_count > 0) && (acca < 0)) {
                ...
                if (temp >= 0) {
                    ...
                    out_words[out_word_index++] = extract_l(endianessT(out_word));      // antonio: call to endianessT to reverse bits
                    //printf("B- %d - %x\n", out_word_index-1, out_words[out_word_index-1]&0xffff);
                    move16();
...
/*
Word16 compute_region_powers
        (short *mlt_coefs,short mag_shift,short *drp_num_bits,unsigned short *drp_code_bits,
         short *absolute_region_power_index,short number_of_regions)

{
    ...
    iVar14 = (int)number_of_regions;
    if (0 < iVar14) {
        psVar13 = absolute_region_power_index;
        psVar15 = mlt_coefs;
        do {
            psVar1 = psVar15 + 0x14;
            uVar8 = L_deposit_l(0);
            ...
            do {
                ...
                sVar2 = *psVar15;
                move16();
                uVar9 = L_mac0(uVar8,(int)sVar2,(int)sVar2);
            ...
            iVar5 = L_sub(uVar8,0x7fff);
            sVar2 = add(iVar12,0xf);
            test();
            test();
            logic16();
            if (-1 < sVar2) {
                do {
                    ...
                    uVar9 = L_shl(uVar8,1);
                    uVar8 = uVar9 & 0xffffffff;
                    iVar5 = L_sub(uVar9,0x7fff);
                    uVar6 = add(iVar12,0xf);
                } while ((uVar6 >> 0xf & 1) == 0);
            }
            uVar10 = L_shr(uVar8,1);
            iVar5 = L_sub(uVar10,0x7123);
            test();
            if (-1 < iVar5) {
                sVar2 = add(iVar12,1);
                iVar12 = (int)sVar2;
            }
            uVar10 = L_deposit_l((int)mag_shift);
            uVar7 = L_shl(uVar10,1);
            uVar7 = L_sub(iVar12,uVar7);
            uVar10 = L_add(0x23,uVar7);
            sVar2 = extract_l(L_sub(uVar10,0x18));
            *psVar13 = sVar2;
            psVar13 = psVar13 + 1;
            psVar15 = psVar1;
        } while (psVar1 != mlt_coefs + ((unsigned long)(unsigned short)(number_of_regions - 1) + 1) * 0x14);
    }
    sVar2 = sub(iVar14,2);
    iVar12 = (int)sVar2;
    psVar13 = absolute_region_power_index + iVar12;
    if (-1 < iVar12) {
        do {
            while( 1 ) {
                uVar7 = sub(psVar13[1],0xb);
                sVar2 = sub(*psVar13,uVar7);
                test();
                if (-1 < sVar2) break;
                *psVar13 = (short)uVar7;
                psVar13 = psVar13 + -1;
    ...
    LAB_00105724:
    uVar7 = sub(1,7);
    sVar2 = sub(*absolute_region_power_index,uVar7);
    test();
    if (sVar2 < 0) {
        *absolute_region_power_index = (short)uVar7;
        move16();
    }
    uVar7 = sub(0x1f,7);
    sVar2 = sub(*absolute_region_power_index,uVar7);
    ...
    *drp_num_bits = 5;
    move16();
    uVar3 = add(*absolute_region_power_index,7);
    *drp_code_bits = uVar3;
    move16();
    if (iVar14 < 2) {
        iVar14 = 5;
        ...
        psVar13 = absolute_region_power_index + 1;
        do {
            uVar7 = sub(0xfffffff8,7);
            sVar2 = sub(*psVar13,uVar7);
            test();
            if (sVar2 < 0) {
                *psVar13 = (short)uVar7;
            ...
            psVar13 = psVar13 + 1;
        } while (psVar13 != absolute_region_power_index + (unsigned long)(unsigned short)(number_of_regions - 2) + 2);
        iVar12 = 1;
        iVar14 = 5;
        psVar13 = absolute_region_power_index + (unsigned long)(unsigned short)(number_of_regions - 2) + 1;
        do {
            drp_num_bits = drp_num_bits + 1;
            drp_code_bits = drp_code_bits + 1;
            uVar10 = sub(absolute_region_power_index[1],*absolute_region_power_index);
            iVar5 = (int)(short)uVar10;
            sVar2 = sub(uVar10,0xfffffff4);
            test();
            if (sVar2 < 0) {
                iVar5 = -0xc;
            }
            sVar2 = sub(iVar5,0xfffffff4);
            move16();
            move16();
            //uVar10 = add(*absolute_region_power_index);
            uVar10 = add(*absolute_region_power_index, absolute_region_power_index[1]);
            sVar4 = add(uVar10,0xfffffff4);
            absolute_region_power_index = absolute_region_power_index + 1;
            *absolute_region_power_index = sVar4;
            move16();
            lVar11 = (long)iVar12;
            iVar12 = iVar12 + 1;
            lVar11 = lVar11 * 0x18 + (long)(int)sVar2;
            sVar2 = add(iVar14,*(short *)(differential_region_power_bits + lVar11 * 2));
            iVar14 = (int)sVar2;
            *drp_num_bits = *(short *)(differential_region_power_bits + lVar11 * 2);
            move16();
            *drp_code_bits = *(unsigned short *)(differential_region_power_codes + lVar11 * 2);
            move16();
        } while (absolute_region_power_index != psVar13);
    }
    return iVar14;
}
*/
