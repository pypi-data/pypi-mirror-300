...
void categorize(Word16 number_of_available_bits,
    ...
    Word16 iVar2 = 0;
    Word16 sVar4 = 0;
    while (iVar2 < number_of_regions) {
        Word16 psVar1 = power_categories[iVar2];
        iVar2 = iVar2 + 1;
        if (psVar1 == 7) {
            sVar4 = sVar4 + 1;
        }
    }
    if (sVar4 == number_of_regions) {
        power_categories[0] = 6;
    }
    // antonio: wiggly-custom-code end */

    /* adjust the category assignments */
    /* compute the new power categories and category balances */
    comp_powercat_and_catbalance(power_categories, category_balances, rms_index, number_of_available_bits,
                                 number_of_regions, num_categorization_control_possibilities, offset);

    /* // antonio: wiggly-custom-code start
    for (Word16 i = 0; i < number_of_regions; i = i + 1) {
        if (power_categories[i] < 0) {
